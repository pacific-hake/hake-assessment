...
# Reference points with subscripts
f_35 <- "*F*~35%~"
f_40 <- "*F*~40%~"
f_45 <- "*F*~45%~"
f_40_40_10 <- "*F*~40%~-40:10"
f_45_40_10 <- "*F*~45%~-40:10"
fspr_35 <- "*F*~SPR=35%~"
fspr_40 <- "*F*~SPR=40%~"
fspr_40_for_latex_table <- "\\textit{F}\\textsubscript{SPR=40\\%}"
fspr_40_bold_for_latex_table <- "\\textbf{\\textit{F}}\\textsubscript{\\textbf{SPR=40\\%}}"
fspr_45 <- "*F*~SPR=45%~"
fspr_50 <- "*F*~SPR=50%~"
fspr_40_10 <- "*F*~SPR=40%~-40:10"
fspr_40_10_noit <- "F~SPR=40%~-40:10"
fspr_40_bold <- "***F***~**SPR=40%**~"

b_10 <- "*B*~10%~"
b_25 <- "*B*~25%~"
b_40 <- "*B*~40%~"
b_40_for_latex_table <- "\\textit{B}\\textsubscript{40\\%}"
b_40_bold_for_latex_table <- "\\textbf{\\textit{B}}\\textsubscript{\\textbf{40\\%}}"
bspr_40 <- "*B*~SPR=40%~"
bspr_40_for_latex_table <- "\\textit{B}\\textsubscript{SPR=40\\%}"
bspr_40_10_noit <- "B~SPR=40%~-40:10"
b_40_bold <- "***B***~**40%**~"

spr_40 <- "SPR~40%~"
fishing_rate <- "(1 - SPR(*F*))/(1 - SPR~40%~)"
rel_fishing_intensity_for_latex_table <- "(1 - SPR)/(1 - SPR\\textsubscript{40\\%})"
b_0 <- "*B*~0~"
b_0_for_latex_table <- "\\textit{B}\\textsubscript{0}"
b_0_bold_for_latex_table <- "\\textbf{\\textit{B}}\\textsubscript{\\textbf{0}}"
r_0 <- "*B*~0~"
r_0_for_latex_table <- "\\textit{R}\\textsubscript{0}"
f_msy <- "*F*~MSY~"
b_msy <- "*B*~MSY~"
b_msy_for_latex_table <- "\\textit{B}\\textsubscript{MSY}"
f_spr <- "*F*~SPR~"
...
med_est_2021_rec <-
  f(((recruitment_med_to_compare - prev_assess_recruitment_med)["20218"]), 1)
med_est_2020_rec <-
...
dfo_probs_fore <-
  base_model$risks[[2]][,(ncol(base_model$risks[[2]]) - 2):ncol(base_model$risks[[2]])]
...
m_prior <- param_details |> 
  filter(grepl("Natural mortality", V1)) |> 
  pull(4) |> 
...
fore_catch_prop <-
  as.data.frame(t(as.numeric(f(apply(extramc$natsel_prop, 2, median) * 100))))
...
# start with age-0).
fore_catch_prop_age2_lower <- quantile(extramc$natsel_prop |> pull(3),
                                       probs[1]) * 100
fore_catch_prop_age2_upper <- quantile(extramc$natsel_prop |> pull(3),
                                       probs[3]) * 100
fore_catch_prop_age3_lower <- quantile(extramc$natsel_prop |> pull(4),
                                       probs[1]) * 100
fore_catch_prop_age3_upper <- quantile(extramc$natsel_prop |> pull(4),
                                       probs[3]) * 100
fore_catch_prop_age6_lower <- quantile(extramc$natsel_prop |> pull(7),
                                       probs[1]) * 100
fore_catch_prop_age6_upper <- quantile(extramc$natsel_prop |> pull(7),
                                       probs[3]) * 100
fore_catch_prop_age7_lower <- quantile(extramc$natsel_prop |> pull(8),
                                       probs[1]) * 100
fore_catch_prop_age7_upper <- quantile(extramc$natsel_prop |> pull(8),
                                       probs[3]) * 100

# Estimated prop at age (catch) of catch in first forecast year ---------------
fore_catch_prop_wt_age2_median <- median(extramc$natselwt_prop |> pull(3)) * 100
fore_catch_prop_wt_age3_median <- median(extramc$natselwt_prop |> pull(4)) * 100
fore_catch_prop_wt_age4_median <- median(extramc$natselwt_prop |> pull(5)) * 100
fore_catch_prop_wt_age5_median <- median(extramc$natselwt_prop |> pull(6)) * 100
fore_catch_prop_wt_age6_median <- median(extramc$natselwt_prop |> pull(7)) * 100
fore_catch_prop_wt_age7_median <- median(extramc$natselwt_prop |> pull(8)) * 100
fore_catch_prop_wt_age10_median <- median(extramc$natselwt_prop |> pull(11)) * 100
fore_catch_prop_wt_age11_median <- median(extramc$natselwt_prop |> pull(12)) * 100
...
log_theta_fishery <- base_model$mcmcparams$log_theta_fishery
log_theta_survey <- base_model$mcmcparams$log_theta_survey
theta_fishery <- base_model$mcmcparams$theta_fishery
theta_survey <- base_model$mcmcparams$theta_survey
...
# in `end_yr - 1` and below the $\Bforty$ (40% of $B_0$) reference point at
# the start of `end_yr`
