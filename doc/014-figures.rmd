...
(ref:main-maturity-ogive-cap) Annual estimates of the fraction of fish that
are mature at each age north of 34.44° N. The thicker solid line represents
equilibrium maturity and the dashed line represents forecast maturity.

(ref:main-maturity-ogive-alt) Maturity increases with age. Fish appear to reach
full maturity between the age of 3 and 4. Maturity varies annually with 2016
having the lowest overall maturity at older ages.
...
ggplot(
  maturity_estimates_df |>
    group_by(model) |>
    mutate(
      yr_forecast = year %in% ((max(year) - 4):max(year))
    ) |>
    group_by(model, yr_forecast, age) |>
    group_modify(
      ~ .x |>
        add_row(
          year = max(.x$year) + 1,
          age = unique(.x$age),
          p_mature = mean(.x$p_mature, na.rm = TRUE),
    ...
    group_modify(
      ~ .x |>
        add_row(
          year = min(.x$year) - 1,
          age = unique(.x$age),
          p_mature = mean(.x$p_mature[.x$year != 9999], na.rm = TRUE),
          yr_forecast = FALSE,
          model = unique(.x$model)
        )
    ) |>
  ...
  # hake_theme(ax_title_font_size = 10) +
  guides(linewidth = "none", linetype = "none") +
  labs(color = "Year") +
  theme(legend.position = c(0.65, 0.45)) +
  scale_linetype_manual(values=c(1, 2, 1)) +
  scale_linewidth_manual(values=c(0.8, 0.2, 0.2))
```
