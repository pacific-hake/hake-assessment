...
![(#fig:main-acoustic-age2-bs-fig) (ref:main-acoustic-age2-bs-cap)](`r here::here("doc", "main-figures", "hake_survey_1995-21_NASCTimeSeries_BiomassAtAgeHistograms_grayblue.png")`)
...
![(#fig:main-acoustic-age1-bs-fig) (ref:main-acoustic-age1-bs-cap)](`r here::here("doc", "main-figures", "age1hake_03-21_sA_squareroot_bin_narrow-panel_grayblue.png")`)
...
![(#fig:main-us-catch-rates-fig) (ref:main-us-catch-rates-cap)](`r here::here("doc", "main-figures", "fishCatchRatesUS.png")`)
...
plot_stock_recruitment(base_model, probs = hake::probs)
```
...
credibility intervals. Dotted lines do not have any particular significance
and are included only to aid in reading the plot.
...
(\Ffortyten). The median is `r ct_limit_quantiles["median"]`~t
...
policy catches are
`r `f(base_model$catch_default_policy[1])`~t in `r `end_yr`,
`r (base_model$catch_default_policy[2])`~t in `end_yr + 1`, and
`r (base_model$catch_default_policy[3])`~t in `end_yr + 2`.
...
```{r main-age-comp-forecast-fig, fig.cap = "(ref:main-age-comp-forecast-cap)", fig.height = 4.5, fig.width = 6.5, is.fig = TRUE, alt.text = "(ref:main-age-comp-forecast-alt)"}
...
plist <- list()
plist[[1]] <- plot_biomass(d_obj = base_model$retros$biomass_df,
                           leg_font_size = 9,
                           x_expansion = 4,
                           leg_pos = c(0.6, 0.89),
                           leg_ncol = 2,
                           alpha = 0.1,
                           rev_colors = TRUE)
# This extra panel with no plot in it is to allow clipping of the huge
# recruitment from the plot below it to happen without cutting off the
# biomass panel above. It's acting as a spacer
plist[[2]] <- NA
plist[[3]] <- plot_recruitment(d_obj = base_model$retros$recr_df,
                               leg_pos = "none",
                               rev_colors = TRUE,
                               clip_cover = 8)
