...
Canada freezer trawlers fished at an approximate bottom depth of 250 to 400 meters
...
through time. The weight of older fish appears to have decreased overall from
1975 to present.
...
through time. See fleet -2 in the weight-at-age.ss file for raw numbers.
...
(ref:main-bridge-summary-cap-1) Bridging models showing some of the
...
(ref:main-bridge-summary-cap-2) Bridging models showing some of the
sequential steps made towards the base model from the `r last_assess_yr` base
...
to only minor changes in estimated spawning biomass.
...
parameter posterior for the acoustic survey index is shown with a median around
0.84.
...
under-fitting in the early part of the time series from 1975 to 1995.
...
biomass has been increasing since 2022. The value for 2023 is around B zero.
...
characterised by large uncertainty due to it being so young in the data.
...
exploitation fraction of the female spawning biomass is approximately 0.07.
This is a drop from the previous five years which have been above 0.1.
...
biomass time series when it is calculated using B zero based on the standard (static) equilibrium
unfished biomass estimate versus a time-varying (dynamic) estimate of B zero based on the unfished
...
stock declines in 2025 is 50 percent or greater for all catch levels
above 350 kilotonnes. The probability that the stock falls under B40 is almost
...
stock declines in 2026 is over 50 percent for all catch levels
...
cohort is predicted to be the largest cohort in the catch in 2024. That
...
estimate decreases in recent years with the removal of the age-1 index from the
model.
...
the absolute biomass figure the relative biomass estimate decreases with the
...
changes in recruitment deviations for some of the sensitivity runs. Removing
the age 1 index decreases the estimate of 2022 recruitment.
...
95% posterior credibility intervals for a select low, moderate,
...
line is the 50% value. For each year except 2017 and 2023, both
...
with 0~t being the lowest probability, shown in pink.
