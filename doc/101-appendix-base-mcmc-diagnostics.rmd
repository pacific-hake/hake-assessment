...
diagnostics for the base model for natural mortality and log R0 parameters.
...
plot_grid(plotlist = plist, nrow = 2)
```
(ref:app-mcmc-diag-extra-sd-cap) Summary of Markov chain Monte Carlo diagnostics
for the additional standard deviation (SD) in the biomass index (upper panels)
and for the age-1 index (lower panels) in the base model. Sub-panel descriptions
as in Figure~\@ref(fig:app-mcmc-diag-m-r0-fig).

(ref:app-mcmc-diag-extra-sd-alt) This figure shows good Markov chain Monte Carlo
diagnostics for the base model for the survey and age-1 index additional standard
...
fishery ($\theta_{\text{fish}}$, upper panels) and the survey ($\theta_{\text{surv}}$,
