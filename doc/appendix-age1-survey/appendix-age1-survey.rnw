...
The figures and tables show results from this sensitivity run (plus
Figure~\ref{fig:age-one-index-age1-base} for the base model, as noted in its caption).

**Need to update the next paragraph depending on final results. Current results (with
diagnostics issue) give high 2020 recruitment without age-1, so it's not just
age-1 index driving that. That might help counter our previous warning below.
...
**Mention Figures~\ref{fig:age-one-index-age1-base} and~-ref{fig:age-one-index-age1}
...
A retrospective plot of historical performance is given in
Figure~{fig:historical-retro-all-age1}, and discussed with respect to
Figure~\ref{fig:historical-retro-all-age1} in
...
\caption{Acoustic survey age-1 index overlaid on estimated numbers
  ...
  for the base model is shown in Figure~\ref{fig:age-one-index-base}.}
