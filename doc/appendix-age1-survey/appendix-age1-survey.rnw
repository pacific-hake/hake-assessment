...
This appendix contains Bayesian MCMC results for the sensitivity run in which the age-1
survey index is excluded as an index of recruitment, as described in
...
compared to the base model, and the exclusion of the age-1 index was deemed
important enough to warrant further consideration, especially in the context of characterizing
forecast uncertainty, and given it was excluded in the base model for previous assessments.
Nonetheless, this appendix is meant to provide supplemental information,
and should not be viewed as an alternative base model.
The figures and tables show results from this sensitivity run (except where stated
in the caption when the base model is shown for comparison).

In general, the pattern of the estimated median age-1 index is similar whether
or not the age-1 index is excluded (the sensitivity run, Figure~\ref{fig:age-one-index-age1}) or
included (the base model,
Figure~\ref{fig:age-one-index-age1-base}). The only appreciative changes (on the
log scale) are for the 2018 and 2020 cohorts. The exclusion of the age-1 index
decreases the estimate of age-1 fish in 2019 (2018 cohort) away from the survey age-1 index value, and
the 2020 cohort is also somewhat decreased. In absolute
...
Due to the age-1 index generally only being available in odd years, the sensitivity run shows little difference in the retrospectives
for odd-year cohorts
(Figures~\ref{fig:main-retrospective-recruitment} and~\ref{fig:retrospective-recruitment-age1}),
except for the 2011 cohort (due to the extra survey in 2012) which appears to
settle down more slowly to its final estimated size than in the base model.
The estimated time series of spawning biomass in the sensitivy run is similar to the base model's
...
The largest recent cohort is 2014, and its estimated size
...
behaviour using data up to age-2, with the age-1 index suggesting it is above
...
\begin{figure}[H]
\begin{center}
<<age-one-index-age1-sens, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the age-1 index has dropped from 2017 to 2019">>=
...
\begin{figure}[H]
\begin{center}
<<age-one-index-age1-base, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the age-1 index has dropped from 2017 to 2019">>=
...
\begin{figure}[tbp]
\begin{center}
<<forecast-risk-comparison-plot-year-1-age1, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that there is a high probability that the stock will decline next year regardless of catch level using the age-1 sensitivity model">>=
...
\begin{figure}[tbp]
\begin{center}
<<forecast-risk-comparison-plot-year-2-age1, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that there is a high probability that the stock will decline next year regardless of catch level using the age-1 sensitivity model">>=
make.forecast.risk.comparison.plot(sens.models.2[[1]],
                                   forecast_yrs = forecast_yrs,
...
\begin{figure}[tbp]
\begin{center}
<<forecast-risk-comparison-plot-year-3-age1, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that there is a high probability that the stock will decline in the next three years regardless of catch level using the age-1 sensitivity model">>=
make.forecast.risk.comparison.plot(sens.models.2[[1]],
                                   forecast_yrs = forecast_yrs,
