...
make.SPR.illustration(last_yr_base_model, yrs = start_yr_age_comps:last_data_yr)
@
\end{center}
\caption{**[Update to MCMC if time. Issue 923.] Illustration of the spawning potential ratio (SPR) calculation based on
  ...
  in the final year of the base model from the 2021 assessment \citep{JTC2021}.}
%  **Need to make terminology the same as the fishing intensity definition.....
