...
Acceptable biological catch (ABC): The acceptable biological catch is a scientific calculation of the sustainable harvest level of a fishery used historically to set the upper limit for fishery removals by the Pacific Fishery Management Council. It is calculated by applying the estimated (or proxy) harvest rate that produces maximum sustainable yield (MSY, see below) to the estimated exploitable biomass (the portion of the fish population that can be harvested). For \fishname, the calculation of the acceptable biological catch and application of the 40:10 adjustment is now replaced with the default harvest rate and the Total Allowable Catch.
...
Benchmark spawning potential ratio ($\mbox{SPR}_{40\%}$): The spawning potential ratio of 40\%, where the 40\% relates to the default harvest rate of $\Fforty$ specified in the Agreement. Even under equilibrium conditions, $\Fforty$ would not necessarily result in a female spawning biomass of $B_{40\%}$ because $\Fforty$ is defined in terms of the spawning potential ratio which depends on the female spawning biomass \emph{per recruit}.
...
Female spawning biomass: The biomass of mature female fish at the beginning of the year. Sometimes abbreviated to spawning biomass.
...
\ee
where SPR$(F)$ is the spawning potential ratio for the value of $F$ accumulated over the entire year. It is often given as a percentage. Relative fishing intensity is the fishing intensity relative to that at the SPR fishing rate $\Fforty$, where $\Fforty$ is the $F$ that gives an SPR of 40\% such that, by definition, SPR$(\Fforty) = 40\%$ (the benchmark spawning ratio). Therefore
\eb
...
Management Strategy Evaluation (MSE):  A formal process for evaluating Harvest Strategies (see above).
...
NUTS: No-U-Turn Sampler is an advanced Hamiltonian Bayesian MCMC sampling algorithm used to efficiently create posterior distributions and used in \fishname\ Bayesian stock assessments beginning in 2021.
...
\fishname: Common name for \emph{\sciencename}, the species whose offshore stock in the waters of the United States and Canada is subject of this assessment.

\commonname: an alternative name for \fishname\ commonly used in the United States.
...
rwMH: Random walk Metropolis Hastings Bayesian MCMC sampling algorithm used to create posterior distributions used in \fishname\ Bayesian stock assessment models prior to 2021.
...
Simulation: A model evaluation under a particular state of nature, including combinations of parameters controlling stock productivity, stock status, and the time series of recruitment deviations. In this assessment, there are \Sexpr{f(num_mcmc_samples)} simulations used to characterize alternative states of nature, each of which are based on a sample from the posterior distribution of the parameters, as calculated using MCMC, for a particular model (e.g., the base model).
...
$\mbox{SPR}_{40\%}$: See benchmark spawning potential ratio.

SS: Stock Synthesis (see below).

Steepness ($h$): A stock-recruit relationship parameter representing the proportion of $R_0$ expected (on average) when the female spawning biomass is reduced to 20\% of $B_0$ (i.e.,~when relative spawning biomass is equal to 20\%).
...
U.S./Canadian allocation: The division of the total allowable catch of \Sexpr{us_allotment_percent}\% as for the U.S. share and \Sexpr{can_allotment_percent}\% for the Canadian share.
...
\begin{figure}[t]
\begin{center}
<<fishing-intensity-illustration, fig.height=5, fig.width=5, is.fig = TRUE, alt.text = "This figure shows how fishing intensity linearly increases with SPR.">>=
...
@
\end{center}
\caption{Fishing intensity as a function of SPR (top axis) and 1-SPR (bottom axis); given the benchmark SPR of 40\%, the bold line is simply $1/0.6$, as shown in equation (\ref{appendix-glossary-spr}).}
...
\begin{figure}[t]
\begin{center}
<<SPR-illustration, fig.height=7, fig.width=6, is.fig = TRUE, alt.text = "This figure shows how SPR is calculated.">>=
plot_spr_illustration(base_model, yrs = start_yr_age_comps:last_data_yr)
@
\end{center}
\caption{Illustration of the spawning potential ratio (SPR) calculation based on
  the combination of maturity and fecundity used in the model, using the maximum
  likelihood estimates of natural mortality, selectivity, and fishing mortality
  in the final year of the base model from this year.}
