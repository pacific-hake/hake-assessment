...
Simulation: A model evaluation under a particular state of nature, including combinations of parameters controlling stock productivity, stock status, and the time series of recruitment deviations. In this assessment, there are 999 simulations used to characterize alternative states of nature, each of which are based on a sample from the posterior distribution of the parameters, as calculated using MCMC, for a particular model (e.g., the base model).
