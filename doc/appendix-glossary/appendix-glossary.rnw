...
$\Bforty$: The level of female spawning biomass corresponding to 40\% of unfished equilibrium female spawning biomass, i.e.~$\Bforty = 0.4B_0$ ($B_0$). This is the level below which the calculated TAC is decreased from the value associated with $\Fforty$, based on the default harvest policy (see below).
