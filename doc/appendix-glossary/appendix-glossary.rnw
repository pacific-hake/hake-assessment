...
$\Fforty$ (F-40 Percent): The rate of fishing mortality estimated to reduce the spawning potential ratio (SPR, see below) to 40\%. Therefore, by definition this satisfies
\eb
...
Fishing intensity: A measure of the magnitude of fishing relative to the SPR target (see SPR below). In this assessment it is defined as:
\eb
\mbox {fishing intensity} = \frac{1-\mbox{SPR}}{ 1-40\%},
\label{appendix-glossary-spr}
\ee
where SPR = 40\% is Spawning Potential Ratio associated with the target harvest rate. See Figure~\ref{fig:glossary-fishing-intensity-illustration}.
...
@
\end{center}
\caption{Fishing intensity as a function of SPR (top axis) and 1-SPR (bottom axis) as defined in equation (\ref{appendix-glossary-spr}).}
%  **Need to make terminology the same as the fishing intensity definition.....
\label{fig:glossary-fishing-intensity-illustration}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
