...
$\Bten$: The level of female spawning biomass (output) corresponding to 10\% of average unfished equilibrium female spawning biomass ($B_0$, size of fish stock without fishing; see above). This is the level at which the calculated TAC based on the 40:10 adjustment (see above) would reduce the catch to 0.

$\Bforty$: The level of female spawning biomass (output) corresponding to 40\% of average unfished equilibrium female spawning biomass ($B_0$, size of fish stock without fishing; see above). This is the level below which the calculated TAC based on the default harvest policy (see below) is decreased from the value associated with $\Fforty$.
...
Spawning potential ratio (SPR): A metric of fishing intensity. The ratio of the spawning output per recruit under a given level of fishing to the estimated spawning output per recruit in the absence of fishing; i.e.~for fishing mortality rate $F$
\eb
