...
Fishing intensity (relative fishing intensity): A measure of the magnitude of fishing in relation to the SPR target (see SPR below). In this assessment the SPR target fishing rate is 40\% (i.e., $\Fforty$), and fishing intensity for a fishing rate $F$ is defined as:
\eb
\mbox {fishing intensity for }F & = & 1-\mbox{SPR}(F),\\
\mbox {and relative fishing intensity for }F & = & \frac{1-\mbox{SPR}(F)}{1-\mbox{SPR}(\Fforty)}\\
...
% SPR$_{40\%}$: The target spawning potential ratio, which is associated with the spawning biomass per recruit equal to 40\% of the spawning biomass per recruit that would occur in the absence of fishing. Even under equilibrium conditions, SPR$_{40\%}$ would not necessarily result in a relative spawning biomass of B$_{40\%}$.
