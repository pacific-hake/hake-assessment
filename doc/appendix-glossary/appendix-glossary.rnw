...
40:10 adjustment: a reduction in the overall total allowable catch that is triggered when the biomass falls below 40\% of its equilibrium level in the absence of fishing. This adjustment reduces the total allowable catch on a straight-line basis from the 40\% level such that the total allowable catch would equal zero when the stock is at 10\% of its equilibrium level in the absence of fishing. This is one component of the default harvest policy (see below).
...
$\Bten$: The level of female spawning biomass (output) corresponding to 10\% of unfished equilibrium female spawning biomass ($B_0$). This is the level below which the calculated TAC based on the 40:10 adjustment (see above) would reduce the catch to 0.

$\Bforty$: The level of female spawning biomass (output) corresponding to 40\% of unfished equilibrium female spawning biomass ($B_0$). This is the level below which the calculated TAC based on the default harvest policy (see below) is decreased from the value associated with $\Fforty$.

$\Bmsy$: The estimated female spawning biomass (output) which theoretically would produce the maximum sustainable yield (MSY) under equilibrium fishing conditions (constant fishing and average recruitment in every year). Also see $\Bforty$ (above).

Backscatter: The scattering by a target back in the direction of an acoustic source. Specifically, the Nautical Area Scattering Coefficient (a measure of scattering per area, denoted by $s_a$) is frequently referred to as backscatter.
...
\eb
0.4 = \frac{\mbox{spawning output per recruit with }\Fforty}{\mbox{spawning output per recruit with no fishing}}.
\ee
The 40\% value is specified in the Agreement.
...
Fishing intensity: A measure of the magnitude of fishing relative to the SPR target (see SPR below). In this assessment the SPR target is 40\%, and fishing intensity for a fishing rate $F$ is defined as:
\eb
\mbox {fishing intensity for }F & = & \frac{1-\mbox{SPR}(F)}{1-\mbox{SPR}(\Fforty)}\\
 & = & \frac{1-\mbox{SPR}(F)}{1-0.4}\\
 & = & \frac{1-\mbox{SPR}(F)}{0.6},
\label{appendix-glossary-spr}
\ee
where SPR$(F)$ is the spawning potential ratio associated with fishing rate $F$, and $\Fforty$ is the $F$ that gives an SPR of 40\%. See Figure~\ref{fig:glossary-fishing-intensity-illustration}.
...
A numerical method used to estimate a single value for each of the parameters and derived quantities. It is less computationally intensive than MCMC methods (see below), but parameter uncertainty is less well characterized.
...
Spawning potential ratio (SPR): The ratio of the spawning output per recruit under a given level of fishing to the estimated spawning output per recruit in the absence of fishing; i.e.~for fishing mortality rate $F$
\eb
\mbox{SPR}(F) = \frac{\mbox{spawning output per recruit with }F}{\mbox{spawning output per recruit with no fishing}}.
\ee
Often expressed as a percentage, it achieves a value of 100\% in the absence of fishing and declines toward zero as fishing intensity increases.
...
% Not found in text in 2017, but now defining 'target spawning potential ratio' below.
% SPR$_{40\%}$: The target spawning potential ratio, which is associated with the spawning output per recruit equal to 40\% of the spawning output per recruit that would occur in the absence of fishing. Even under equilibrium conditions, SPR$_{40\%}$ would not necessarily result in a relative spawning biomass of B$_{40\%}$.
...
\begin{figure}[t]
\begin{center}
<<fishing_intensity_illustration, fig.height=5, fig.width=5>>=
# Do as a function so that variables remain local
...
\begin{figure}[t]
\begin{center}
<<SPR_illustration, fig.height=7, fig.width=6>>=
make.SPR.illustration(base.model)
@
\end{center}
\caption{Illustration of the spawning potential ratio (SPR) calculation based on the combination of maturity and fecundity used in the model, as well as the maximum likelihood estimates of natural mortality, selectivity, and fishing mortality in the final year of the base model.}
%  **Need to make terminology the same as the fishing intensity definition.....
