...
survey and commercial fisheries.
  ...
  \item Coast-wide catch in \Sexpr{end.yr-1} was the fourth largest on record at
    ...
    bilateral TAC agreement.  The U.S. caught \Sexpr{last.year.us.attained}\%
    of their quota (\Sexpr{last.year.us.landings}~t), and Canada caught
    \Sexpr{last.year.can.attained}\% (\Sexpr{last.year.can.landings}~t) of their quota.
