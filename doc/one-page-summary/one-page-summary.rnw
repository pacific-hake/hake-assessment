...
\begin{itemize}
  \item The stock assessment model for \Sexpr{assess.yr} is similar in structure to the \Sexpr{last.assess.yr} model with the addition
    of fishery age compositions from \Sexpr{end.yr-1}, reanalyzed acoustic survey biomass and age compositions for 1995 (completing the reanalyzed acoustic survey time series initiated in the \Sexpr{last.assess.yr} model), and minor refinements to data including catch
  ...
  \item Projections setting the \Sexpr{forecast.yrs[1]} and \Sexpr{forecast.yrs[2]} catch equal to the \Sexpr{forecast.yrs[1]-1} TAC of
    \Sexpr{last.year.tac}~t show the estimated median relative spawning biomass increasing from 89\%
    in \Sexpr{end.yr} to 85\% in \Sexpr{end.yr+1} and again in \Sexpr{end.yr+2} to 79\%. 
    However, due to uncertainty there is an estimated 16\% chance of the spawning biomass falling below 40\%
     of $B_0$ in two years (2019). There is an estimated 63\% chance of the spawning biomass declining from 2017 to 2018, and a 80\% chance of it declining from 2018 to 2019 under this constant catch level.
