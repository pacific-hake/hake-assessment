...
weight-at-age data, addition of fishery catch and age-composition data
from \Sexpr{end.yr-1}, and addition of weight-at-age data for \Sexpr{last.data.yr}.

  \item The model was updated to include year-specific fecundity from 1975-\Sexpr{last.data.yr}.
  However, limited exploratory and supplementary analyses could be completed for 
...
the U.S. was \Sexpr{last.year.us.attained}\% of its quota (\Sexpr{last.2year.us.attained.diff}
point change from last year); in Canada it was
\Sexpr{last.year.can.attained}\% (\Sexpr{last.2year.can.attained.diff}
point change from last year).
...
\Sexpr{prev.bio.lower}--\Sexpr{prev.bio.upper}~million~t).
  ...
  \item The model estimates that the joint probability of being both
    ...
    at the start of \Sexpr{end.yr} is less than 10\%.
...
decreasing from 64\% in \Sexpr{end.yr} to 59\% in \Sexpr{end.yr+1} and 49\% in
\Sexpr{end.yr+2}. However, due to uncertainty there is an estimated 40\% chance
of the spawning biomass falling below 40\% of $B_0$ in \Sexpr{end.yr+2}. There is an
estimated 72\% chance of the spawning biomass declining from \Sexpr{end.yr} to
\Sexpr{end.yr+1}, and a
86\% chance of it declining from \Sexpr{end.yr+1} to
