...
survey and commercial fisheries.
  ...
  \item The main technical change from \Sexpr{last.assess.yr} is the use of a new
    ...
    all model results, including sensitivity and retrospective model runs, are
...
out of a total allowable catch (TAC), adjusted for carryovers, of \Sexpr{last.year.tac}~t. Attainment in
...
from last year). Quotas were specified unilaterally in 2020 due to the lack of a bilateral TAC agreement.
  ...
  \item The median estimate of female spawning biomass at the start
    ...
    million~t). This is less than the median estimate of \Sexpr{end.yr-1} female spawning biomass\Sexpr{prev.bio.median}~million~t
    (with 95\% credible interval
    \Sexpr{prev.bio.lower}--\Sexpr{prev.bio.upper}~million~t) from the current assessment.
