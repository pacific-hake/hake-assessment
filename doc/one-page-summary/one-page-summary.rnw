...
%  \item The default predicted catch level for 2016, based on the default harvest rule, is 839,476~t.  **DOES NOT MENTION uncertainty; suggest replacing with (copied and adapted from Exec Summary):
  ...
  \item Based on the default harvest rule, the median catch limit for \Sexpr{min(forecast.yrs)} is \Sexpr{catch.limit.quantiles["median"]}~t (with 95\% interval from \Sexpr{catch.limit.quantiles["lower"]} to \Sexpr{catch.limit.quantiles["upper"]}~t).
  ...
  \item Projections setting \Sexpr{forecast.yrs[1]} and \Sexpr{forecast.yrs[2]} catch equal to the \Sexpr{forecast.yrs[1]-1} TAC of
    \Sexpr{last.year.tac}~t show the median spawning relative biomass estimates increasing from \Sexpr{curr.depl.median}\%
    ...
    in \Sexpr{end.yr+2}. However, this projection is highly uncertain and shows a 2\% chance of the spawning biomass falling below 25\%
     of B0 in 2 years (by 2018) and a 50\% chance of declining from 2017 to 2018.
\end{itemize}
