...
\begin{itemize}
  \item The stock assessment model for \Sexpr{assess.yr} is the same in structure
...
survey and commercial fisheries.
  ...
  \item Major technical changes from \Sexpr{last.assess.yr} includes the use of a new,
    more efficient algorithm (No-U-Turn Sampler) for obtaining posterior samples.  This
    change has enabled all model results (including sensitivity and retrospective model runs)
    to be based on posterior distributions. Therefore, previous comparisons between base model
    MCMC results and sensitivity or retrospective MLE-based results is no longer needed.
  ...
  \item Updates to the data include:
    ...
    \Sexpr{end.yr-1}, and minor changes to pre-\Sexpr{end.yr-1} data. Due to the Covid-19 pandemic,
    age data were not available from the Canadian Freezer Trawler fleet in \Sexpr{end.yr-1}.
...
from last year). Quotas were specified unilaterally in 2020 due to the lack of a bilateral TAC agreement.
  ...
  \item The median estimate of the \Sexpr{end.yr} relative spawning biomass
    ...
    \Sexpr{curr.depl.upper}\%). The median relative spawning biomass reached a recent
    high of \Sexpr{median.relative.bio.2017}\% in 2017 but has since gradually declined due to
    aging above average cohorts (2010, 2014, and 2016) and the recent period of record catches.
...
\Sexpr{forecast_yrs[2]} catch equal to the \Sexpr{forecast_yrs[1]-1}
coast-wide (unilaterally summed) Total Allowable Catch of
