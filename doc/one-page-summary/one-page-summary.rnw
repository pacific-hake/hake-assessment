...
\begin{itemize}
  ...
  \item The stock assessment is fit to an acoustic survey index of abundance as well as age compositions from the survey and commercial fisheries.
  ...
  \item The stock is estimated to be near its highest biomass level since 1990 as a result of
    ...
    thus its estimate is highly uncertain.  The survey observed high numbers of age-1 hake in 2015, but those data
...
%  \item The default predicted catch level for 2016, based on the default harvest rule, is 839,476~t.  **DOES NOT MENTION uncertainty; suggest replacing with (copied and adapted from Exec Summary):
  ...
  \item Projections setting \Sexpr{forecast.yrs[1]} and \Sexpr{forecast.yrs[2]} catch equal to the \Sexpr{forecast.yrs[1]-1} TAC of
    \Sexpr{last.year.tac}~t show the estimated median relative spawning biomass increasing from \Sexpr{curr.depl.median}\%
    in \Sexpr{end.yr} to \Sexpr{next.depl.median.tac.based}\% in \Sexpr{end.yr+1} and remaining stable at \Sexpr{next2.depl.median.tac.based}\%
    in \Sexpr{end.yr+2}. 
    However, due to uncertainty there is an estimated 2\% chance of the spawning biomass falling below 25\%
     of $B_0$ in two years (by 2018). There is an estimated 38\% chance of the spawning biomass declining from 2016 to 2017, and a 50\% chance of it declining from 2017 to 2018 under this constant catch level.
    % Manual to do ('remaining stable')
    % 2% from Table e. 
    % Last sentences were just: 
    % However, this projection is highly uncertain and shows a 2\% chance of the spawning biomass falling below 25\%
    % of $B_0$ in two years (by 2018) and a 50\% chance of declining from 2017 to 2018.
     % Last bullet ends up with two values of 83.6%. These seem to be 
     %  correctly referenced and are not identical in the output, but
     %  are the same due to rounding.
\end{itemize}
