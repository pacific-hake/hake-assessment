...
\begin{itemize}
  \item The stock assessment model for \Sexpr{assess.yr} is similar in structure to the \Sexpr{last.assess.yr} model with the addition
    of fishery catch and age compositions from \Sexpr{end.yr-1}, reanalyzed acoustic survey biomass and age compositions for 1995 (completing the reanalyzed acoustic survey time series initiated in the \Sexpr{last.assess.yr} model), and minor refinements to data including catch
