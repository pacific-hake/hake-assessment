...
\begin{enumerate}[noitemsep,nolistsep]
  ...
  \item Coastwide catch in \Sexpr{end.yr-1} was \Sexpr{last.year.landings} t, out of a
    TAC (adjusted for carryovers) of \Sexpr{last.year.tac} t. Attainment of the quota in the U.S. was \textbf{NOT YET CORRECT}
    \Sexpr{last.year.attained}\% of its quota; in Canada it was \textbf{NOT YET CORRECT, need to put together the country-disaggregated catch file}
    \Sexpr{last.year.attained}\%. A variety of factors influenced the attainment of the quota.
  \item The stock is estimated to be near its highest biomass level since the early 1990s as a result of an above average 2008 cohort
    ...
    -- \Sexpr{end.yr-1} have not been observed long enough in the data to estimate their size or even if they are likely to be above or below average.
  \item The \Sexpr{end.yr} median relative spawning biomass (current spawning biomass divided by unfished equilibrium, B\subscr{0}) is
  ...
  %%   \Sexpr{last.year.landings} t show the median spawning relative biomass estimates declining from \Sexpr{curr.depl.median}\%
  %%   of B\subscr{0} in \Sexpr{end.yr} to \Sexpr{next.depl.median}\% in \Sexpr{end.yr+1} and \Sexpr{next2.depl.median} in \Sexpr{end.yr+2}.
  %%   However, this projection is highly uncertain and shows a \textbf{FROM here on is not correct} 5\% chance of falling below 25\%
