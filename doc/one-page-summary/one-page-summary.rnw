...
\begin{itemize}
  ...
  \item The spawning biomass in \Sexpr{end.yr} is estimated to have increased from \Sexpr{end.yr-1} due to the 2014 year-class estimate being well above average size and the estimate of the 2012 year-class being slightly above average. % Manual to do.
...
%  \item The default predicted catch level for 2016, based on the default harvest rule, is 839,476~t.  **DOES NOT MENTION uncertainty; suggest replacing with (copied and adapted from Exec Summary):
  ...
  \item Projections setting the \Sexpr{forecast.yrs[1]} and \Sexpr{forecast.yrs[2]} catch equal to the \Sexpr{forecast.yrs[1]-1} TAC of
    ...
    in \Sexpr{end.yr} to \Sexpr{next.depl.median.tac.based}\% in \Sexpr{end.yr+1} and remaining somewhat stable near 80\%
    in \Sexpr{end.yr+2}. 
    However, due to uncertainty there is an estimated 3\% chance of the spawning biomass falling below 25\%
