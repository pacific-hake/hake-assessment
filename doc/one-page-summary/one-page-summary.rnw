...
\begin{itemize}
  \item The stock assessment model for \Sexpr{assess_yr} has the same population
    dynamics structure as the \Sexpr{last_assess_yr} model. For \Sexpr{assess_yr},
    ...
    The addition of the age-1 index is the main change in data streams from \Sexpr{last_assess_yr}.

%  \item The main change from \Sexpr{last_assess_yr} is the use of the age-1 index
...
%    now based on posterior distributions rather than maximum likelihood estimates.
  ...
  \item Updates to the data include:
    ...
    \Sexpr{end_yr-1}, the addition of the age-1 index time series
    (\Sexpr{survey_start_yr}--\Sexpr{survey_end_yr}), and minor changes to pre-\Sexpr{end_yr-1} data. Due to
    staffing issues, age data from \Sexpr{end_yr-1} were unavailable from the Canadian freezer-trawler fleet and minimally
...
\Sexpr{last.yr.tac.fore.2.biomass}\% in \Sexpr{end_yr+1} (due to the
growth of the expected large 2020 cohort) then decreasing to
\Sexpr{last.yr.tac.fore.3.biomass}\% in
\Sexpr{end_yr+2}, with a \Sexpr{last.yr.tac.risk.2.bforty}\% chance
...
for these constant catches.
    % Always good to check those numbers manually from the tables. Done 2022.
\end{itemize}
