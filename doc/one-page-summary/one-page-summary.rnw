...
\begin{itemize}
  \item The stock assessment model for \Sexpr{assess.yr} has the same population
    ...
    survey index of abundance, an index of age-1 fish, annual commercial catch data,
...
%    now based on posterior distributions rather than maximum likelihood estimates.
  ...
  \item Updates to the data include:
    ...
    (\sexp{survey.start.yr}--\sexpr{survey.end.yr}), and minor changes to pre-\Sexpr{end.yr-1} data. Due to
    staffing issues, age data were unavailable from the Canadian freezer-trawler fleet and minimal
    for the Shoreside fleet in \Sexpr{end.yr-1}.

  \item Coast-wide catch in \Sexpr{end.yr-1} was about average over the past decade at
...
%\Sexpr{us.allotment.percent}\% and \Sexpr{can.allotment.percent}\%.
  ...
  \item The median estimate of the \Sexpr{end.yr} relative spawning biomass
    ...
    has declined since 2017 but remains well above levels of conservation concern.
...
staying relatively stable from
