...
survey and commercial fisheries.
  ...
  \item Structurally, the assessment was changed from \Sexpr{last.assess.yr} in three ways. First, a nearly uniform prior (in transformed space) was applied to the Dirichlet-Multinomial parameters used to weight the age-composition data. Second, the constraint that estimated recruitment deviations sum to zero was removed. Lastly, weight-at-age data for the forecast period was calculated using the average of the five most recent years rather than all years.

  \item Updates to the data include:
recent catches, biomass estimate and age-composition data from the acoustic survey conducted in \Sexpr{survey.end.yr},
minor changes to historical (pre-\Sexpr{end.yr-1}) catch, biomass estimates, age-composition data, and
