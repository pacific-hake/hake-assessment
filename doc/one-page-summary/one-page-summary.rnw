...
survey and commercial fisheries.
  ...
  \item **Structural changes from \Sexpr{last.assess.yr} involve a new prior
    ...
    using the average of the most recent five years (rather than all years) of
  ...
  \item **Coast-wide catch in \Sexpr{end.yr-1} was the **third largest on record at
...
\Sexpr{f(base.model$mcmccalcs$dmed[names(base.model$mcmccalcs$dmed) == "2010"] * 100, 0)}\%
    in 2010,
  ...
  \item **(Making an Issue as 2020 numbers are different to last year's Exec Summary) The median estimate of female spawning biomass at the start
