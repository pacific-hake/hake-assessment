...
Figures \@ref(fig:app-maturity-analysis-sampling-platform-fig)-
...
(Malick et al. 2020) and co-occurrence of hake with prey (Phillips et al. 2003);
...
expected temperature generally decreases with depth (Figure \@ref(fig:app-maturity-analysis-temp-depth-fig)) and unlike
surface temperature that is warmest in summer months, the warmest temperatures
at depth often occur in winter (Figure \@ref(fig:app-maturity-analysis-raw-feb-temp-fig)).
...
including samples south of Point Conception (previous maturity modeling has imposed
...
the relative support of models using Akaike information criterion (AIC; Akaike
1973). Similarly, alternative temperature indices were also compared using AIC,
and the model(s) with the lowest AIC values were deemed to have the most support.
...
as a driver (February temperature at 130.67 m depth north of Point Conception)
...
Point Conception, and also include the ASHOP samples.
...
indicating high abilities to discriminate between whether or not hake are
...
(Figure \@ref(fig:app-maturity-analysis-temp-depth-fig) -
...
Estimate of the spatial variance for the weighting model based on estimated
biomass of `r sp` was higher than the estimate of the spatiotemporal variance
suggesting that differences in locations are more prominent than differences
in locations between years (Table \@ref(tab:app-maturity-analysis-biomass-tab)).
...
(ref:app-maturity-analysis-sampling-platform-years-alt) There are more samples for younger rather than older ages.
...
(ref:app-maturity-analysis-sampling-platform-age-years-alt) The first samples, in 2009, come from the WCGBTS.
...
(ref:app-maturity-analysis-temp-depth-alt) All sampling platforms sampled fish
...
(ref:app-maturity-analysis-raw-feb-temp-alt) Temperatures in all months between
...
averages of temperature; for this illustration data from a depth of 130.67 m in
January, north of Point Conception is used.
...
Tables \@ref(tab:app-maturity-analysis-aic-tab) and
\@ref(tab:app-maturity-analysis-estimates-tab)). Year effects are included as an autocorrelated random walk
...
a covariate (At-Sea Hake Observer Program (ASHOP) samples included,
only data north of Point Conception), for 2020 as a baseline year;
slight variation between ages is driven by the spatially
...
(lower AIC corresponds to more data support).
...
February temperature (Celsius) at a depth of 130.67 m (using samples from the
At-Sea Hake Observer Program (ASHOP) and data north of Point Conception).
Predictions are made for age-3 fish, and using coefficients corresponding
to the non-ASHOP surveys.
...
depth of 130.67m (using samples from the At-Sea Hake Observer Program (ASHOP)
and data north of Point Conception) and
