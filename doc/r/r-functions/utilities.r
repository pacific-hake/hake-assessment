...
##     }
##     i <- 1
##     mylist$benchmarks <- allnums[i]
##     i <- i + 1
##     mylist$MSY <- allnums[i]
##     i <- i + 1
##     mylist$SPRtarget <- allnums[i]
##     i <- i + 1
##     mylist$Btarget <- allnums[i]
##     i <- i + 1
##     if (version == 3.24) {
##         mylist$Bmark_years <- allnums[i:(i + 5)]
##         i <- i + 6
##     }
##     else {
##         mylist$Bmark_years <- allnums[i:(i + 9)]
##         i <- i + 10
##     }
##     if (verbose) {
##         cat("Benchmark years: ", mylist$Bmark_years, "\n")
##     }
##     mylist$Bmark_relF_Basis <- allnums[i]
##     i <- i + 1
##     mylist$Forecast <- Forecast <- allnums[i]
##     i <- i + 1
...
##         }
##         mylist$Nforecastyrs <- allnums[i]
##         i <- i + 1
##         mylist$F_scalar <- allnums[i]
##         i <- i + 1
##         if (version == 3.24) {
##             mylist$Fcast_years <- allnums[i:(i + 3)]
##             i <- i + 4
##         }
##         else {
##             mylist$Fcast_years <- allnums[i:(i + 5)]
##             i <- i + 6
##         }
##         if (verbose) {
##             cat("Forecast years: ", mylist$Fcast_years, "\n")
##         }
##         mylist$ControlRuleMethod <- allnums[i]
##         i <- i + 1
##         mylist$BforconstantF <- allnums[i]
##         i <- i + 1
##         mylist$BfornoF <- allnums[i]
##         i <- i + 1
##         mylist$Flimitfraction <- allnums[i]
##         i <- i + 1
##         mylist$N_forecast_loops <- allnums[i]
##         i <- i + 1
##         mylist$First_forecast_loop_with_stochastic_recruitment <- allnums[i]
##         i <- i + 1
##         mylist$Forecast_loop_control_3 <- allnums[i]
##         i <- i + 1
##         mylist$Forecast_loop_control_4 <- allnums[i]
##         i <- i + 1
##         mylist$Forecast_loop_control_5 <- allnums[i]
##         i <- i + 1
##         mylist$FirstYear_for_caps_and_allocations <- allnums[i]
##         i <- i + 1
##         mylist$stddev_of_log_catch_ratio <- allnums[i]
##         i <- i + 1
##         mylist$Do_West_Coast_gfish_rebuilder_output <- allnums[i]
##         i <- i + 1
##         mylist$Ydecl <- allnums[i]
##         i <- i + 1
##         mylist$Yinit <- allnums[i]
##         i <- i + 1
##         mylist$fleet_relative_F <- allnums[i]
##         i <- i + 1
##         if (mylist$fleet_relative_F == 2) {
##             stop("SS_readforecast doesn't yet support option 2 for 'fleet relative F'")
##         }
##         mylist$basis_for_fcast_catch_tuning <- allnums[i]
...
##             }
##             mylist$Ncatch <- Ncatch <- allnums[i]
##             i <- i + 1
##             mylist$InputBasis <- allnums[i]
##             i <- i + 1
##             if (Ncatch == 0) {
##                 ForeCatch <- NULL
##             }
