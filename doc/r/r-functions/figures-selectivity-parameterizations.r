...
mtext("Status-quo selectivity parameterization", side=3, line=1,
      outer=TRUE, adj=.2)

# second plot showing cumulative sum of parameters
plot(ages[sub], S_prime_base[sub], type='l', lwd=4, yaxs='i', axes=FALSE, 
     xlim=xlim, ylim=c(0,1.1*max(S_prime_base[sub], S_prime_adj[sub])),
...
plot(ages, S_base, type='l', lwd=4, xlim=xlim, ylim=c(0,1.15), yaxs='i', axes=FALSE,
...
plot(ages[sub], selpar_base_alt[sub], type='l', lwd=4,
     ylim=c(-1,3), xlim=xlim, xlab="", ylab="", axes=FALSE)
add_label("(c)")
axis(1, at=0:8); axis(2); box()
abline(h=0, col='grey', lty=3)
...
plot(ages[sub], S_prime_base_alt[sub], type='l', lwd=4, yaxs='i', axes=FALSE,
     xlim=xlim, xlab="", ylab="",
     ylim=c(0,1.1*max(S_prime_base_alt[sub], S_prime_adj[sub])))
add_label("(d)")
axis(1, at=0:8); axis(2); box()

# third plot showing resulting selectivity
plot(ages, S_base_alt_B, type='l', lwd=4, xlim=xlim, yaxs='i',  axes=FALSE,
     ylim=c(0,1.15), xlab="", ylab="")
add_label("(e)")
axis(1, at=0:8); axis(2); box()
