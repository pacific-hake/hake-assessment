## MCMC variability in SR relationship

make.mcmc.sr.variability.plot <- function(model,   ## model is an mcmc run and is the output of the r4ss package's function SSgetMCMC
                                          start.yr,
                                          end.yr,
  ...
  for(iyr in 1:length(yrs)){
    y <- yrs[iyr]
    Bs <- b.ratio[,iyr + 1] # +1 to account for equilbrium in first column
    Rs <- r.ratio[,iyr + 1]
  ...
  ## axis(4, at = 1:7, label = round((1:7) * mn.r.virg / 1e6,1), las = 1, cex.axis = 0.8)
