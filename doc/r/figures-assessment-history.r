make.assessment.history.plot <- function(base,
  ...
  xx[nrow(xx), paste0("X", years[years %in% latestYrs])] <- apply(latestAssess, 2, median)[paste0("SPB_", years[years %in% latestYrs])] / 2e6
  slower <- apply(latestAssess, 2, quantile, probs = 0.025)[paste0("SPB_", years[years %in% latestYrs])] / 2e6
  supper <- apply(latestAssess, 2, quantile, probs = 0.975)[paste0("SPB_", years[years %in% latestYrs])] / 2e6
  ...
  plot(range(years), range(xx[,yearInd], na.rm = TRUE), type = "n", xlab = "Year", ylab = "Spawning Biomass (million t)",
       las = 1, xlim = c(min(years), max(years)), cex.axis = 0.9, cex.lab = 1)
  ...
  legend(end.yr + 0.5, 7.15, paste(xx$Year, xx$Model), col = legCol, lwd = lwds + 1, pch = legPch, cex = 0.65, bty = "n", xpd = NA)
}
