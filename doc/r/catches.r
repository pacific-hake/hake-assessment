...
make.catches.plot <- function(catches,
  ...
  ##catches <- catches[,c("Year","US_foreign","US_JV","atSea_US_MS","atSea_US_CP","US_shore",
  ##                      "CAN_forgn","CAN_JV","CAN_Shoreside","CAN_FreezeTrawl")]
  ...
  tmp <- barplot(t(as.matrix(catches))/1000,beside=F,names=catches[,1],
  ...
  barplot(t(as.matrix(catches))/1000,beside=F,names=catches[,1],
          col=cols,
