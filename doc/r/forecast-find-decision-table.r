...
out <- read.table(file.path(SSdir,baseModel,"DecisionTable/defaultHR/derived_posteriors.sso"),header=T)
#no catches input into forecast.ss file to determine median 2017 catch
median(out$ForeCatch_2017)   #969840 1/23/17
#now fix this catch for 2017 in the forecast.ss file, run mceval, and find median 2018 catch
out <- read.table(file.path(SSdir,baseModel,"DecisionTable/defaultHR/derived_posteriors.sso"),header=T)
median(out$ForeCatch_2018)   #843566  1/23/17
#now fix this catch for 2018 in the forecast.ss file, run mceval, and find median 2019 catch
out <- read.table(file.path(SSdir,baseModel,"DecisionTable/defaultHR/derived_posteriors.sso"),header=T)
...
#median SPR=100
#set the catch for 2017, run mceval, until the median SPRratio_2017 is really close to 1
out <- read.table(file.path(SSdir,baseModel,"DecisionTable/SPR100/derived_posteriors.sso"),header=T)
head(out[,c("ForeCatch_2017","ForeCatch_2018","ForeCatch_2019")])
median(out$SPRratio_2017)   #934000  =  0.9996     1/23/17
#set the catch for 2018, run mceval, until the median SPRratio_2018 is really close to 1
out <- read.table(file.path(SSdir,baseModel,"DecisionTable/SPR100/derived_posteriors.sso"),header=T)
head(out[,c("ForeCatch_2017","ForeCatch_2018","ForeCatch_2019")])
median(out$SPRratio_2018)   #848000  = 0.9999     1/23/17
#set the catch for 2019, run mceval, until the median SPRratio_2019 is really close to 1
out <- read.table(file.path(SSdir,baseModel,"DecisionTable/SPR100/derived_posteriors.sso"),header=T)
head(out[,c("ForeCatch_2017","ForeCatch_2018","ForeCatch_2019")])
...
out <- read.table(file.path(SSdir,baseModel,"DecisionTable/constantCatch/derived_posteriors.sso"),header=T)
head(out[,c("ForeCatch_2017","ForeCatch_2018","ForeCatch_2019")])
