...
`r str_comma_sep(forecast_yrs[2:length(forecast_yrs)])` projections
...
is `r f(fore_catch_prop_wt_age4_median)`% for the age-3
`r forecast_yrs[1] - 4` cohort (compared to `r fore_catch_prop[["11"]]`% by
numbers) and `r f(fore_catch_prop_wt_age11_median)`%
for the age-4 `r forecast_yrs[1] - 11` cohort
...
recruitment deviations from the base model was intermediate with a median of
`r sigma_r_this_year_main` These
...
female spawning biomass are relatively insensitive. The method that
...
age-1 index (which only starts in 1995) but diverge near the end of the time series
...
recruitment in 2022 is estimated to be somewhat larger for the base model than
when removing the age-1 index, though both median recruitment deviations are
negative (unlike last year's assessment for which the base model's was positive).
Removing the relative age-1 index led to changes in fit to the
age-2$+$ survey biomass index. Both models underestimate the 2021 age-2$+$ index (the
base model less so), and both overestimate the 2023 age-2$+$ (the base model
more so); (Figure~\@ref(fig:main-index-base-alternative-2-fig)). So the
inclusion of the age-1 index in the base model estimates a higher biomass in
recent years than excluding it.
...
the years 2019, 2021, and 2023. The index was specified as having
...
of fish age-1$+$), with a median catchability estimate of YYY. The eDNA index
declined from 2019 to 2023, though more steeply between 2019 to 2021 then between
2021 and 2023 (see Figure~\@ref(fig:app-eDNA-index-1-fig)). The eDNA index is
fit well in this sensitivity model, somewhat at the cost of reduced fits in the age
composition data. While the inclusion of the eDNA index did not significantly alter most
...
to produce more accurate and robust survey estimates, before it should be used directly
...
recruitment estimates for 2020, 2021, and particularly 2022 are linked to the choice of $\Phi$,
where the smallest investigated value of $\Phi$ (0.21) led to the highest
estimates of these recruitment deviations
(Figure~\@ref(fig:main-recruitment-devs-base-alternative-4-fig)).
For 2021 and 2022 the second highest deviations were for the largest value of
$\Phi$ (2.10), the next highest for $\Phi = 1.4$, and the smallest for
$\Phi = 0.7$ (rather than going from lowest to highest $\Phi$).
The higher estimates of recruitment for $\Phi = 0.21$ led to a large increase in female spawning
biomass in recent years compared to the base model, with, due to the above recruitment
pattern, the biomass decreasing as $\Phi$ changes from 0.21 to 2.1 to 1.4 to 0.7.
