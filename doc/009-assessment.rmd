...
time series high in 2017. While there is large uncertainty in the `r end_yr-1` estimates of relative fishing
...
`r f_40` relative fishing intensity in `r end_yr - 1` and below the `r b_40`
...
P$(\mathrm{B}_{`r assess_yr`} > `r b_10`) = `r probs_curr_b10`$% and
P$(\mathrm{B}_{`r assess_yr`} > `r b_40`) = `r probs_curr_b40`$%
...
P$(\mathrm{B}_{`r assess_yr - 1`} > `r b_40`) = 90.5$%. Note that a probability
...
P$(\mathrm{B}_{`r assess_yr`} > 0.4`r b_msy`) = `r f(dfo_probs_curr[1, 2] |> pull(), 0)`$%
and
P$(\mathrm{B}_{`r assess_yr`} > 0.8`r b_msy`) = `r f(dfo_probs_curr[1, 3] |> pull(), 1)`$%
such that the stock is estimated to be in the provisional `healthy zone'.
For completeness, we note that
P$(\mathrm{B}_{`r assess_yr`} > `r b_msy`) = `r f(dfo_probs_curr[1, 1] |> pull(), 1)`$%.
...
P$(\mathrm{B}_{`r assess_yr`} > `r b_40`) = `r probs_curr_b40`$% (as noted
above), and of being above `r b_25` is
P$(\mathrm{B}_{`r assess_yr`} > `r b_25`) = `r probs_curr_b25`$%.
...
P$(\mathrm{B}_{`r assess_yr + 1`} > `r b_10`) = `r prob_next_over_b10`$%
and in the healthy zone with a probability of
P$(\mathrm{B}_{`r assess_yr + 1`} > `r b_40`) = `r prob_next_over_b40`$%.
...
of
P$(\mathrm{B}_{`r assess_yr + 1`} > 0.4`r b_msy`) = `r dfo_prob_next_over_40bmsy`$%,
in the healthy zone with a probability of
P$(\mathrm{B}_{`r assess_yr + 1`} > 0.8`r b_msy`) = `r dfo_prob_next_over_80bmsy`$%,
and above `r b_msy` with a probability of
P$(\mathrm{B}_{`r assess_yr + 1`} > `r b_msy`) = `r dfo_prob_next_over_bmsy`$% for
this catch.
...
i.e.,~P$(\mathrm{B}_{t+1} < `r b_40`)$; Figure~\@ref(fig:main-historical-2-fig).
...
$>`r b_40`$ may not be a particular high bar to attain.
...
calculations for P$(\mathrm{B}_{t+1} < `r b_40`)$ show little change as more
