...
$M$ increased with age due to a lack of data data to inform the estimates.
...
estimates of baseline $M$ and estimates of $M$ at age for ages 0--1, 2, 3, 4,
and 5$^+$. Where the $M$ at-age estimates are strictly natural mortality due to
...
1. Adding a relative index of abundance based on environmental-DNA;

1. Using the middle of the year to predict maturity-at-age data; and

1. Assuming $M$ is specific to ages 0--1, 2, 3, 4, and 5$^+$.
...
Estimates of mortality at age since 1993 due to cannibalism were estimated
using CEATTLE [@HolsmanEtAl2016] and input into the assessment as fixed values.
These deviations were available for five groups of fish, age-1 fish and younger,
age-2 fish, age-3 fish, age-4 fish, and fish ages five and older. The same
deviations were used for each sensitivity but the first sensitivity
...
estimate of baseline mortality from CEATTLE. Given the age-specific
...
age-2 fish in some years (Figure~\@ref(fig:main-mortality-sens-fig)). These
values were estimated by fitting the CEATTLE model to diet data up to 2019.
Diet data since 2019 exists but has yet to be analyzed and included in regional
database for its use in CEATTLE. Age-0 and age-1 fish were found in over 7000
stomachs of `r sp`. Additionally, CEATTLE is informed by temperature at 130 m
from the same spatial footprint of the acoustic survey taken from GLORYS
output. More details about fitting data on `r sp` to CEATTLE can be found
...
fixed at 0.13 for all ages. Additionally, the uncertainty in almost all
...
In the future, an additional sensitivity should be conducted that adds another
age group, i.e., age six and above, where the deviations are fixed at zero
instead of assumed to be the same as age-5 fish.
