...
> and sensitivty to data and structural decisions.*
...
> spawning stock biomass time series (see Figure
...
> These can be used to visually compare equilibirum versus non-equilibrium
...
> and outreach opportunties related to dynamic reference point methods and
> approaches.*
...
> as the Woods Hole Assessment Method have more flexibility in modeling natural
...
> survey biomass estimate, associated age compositions, and the 2023 age-1 index
> estimate in a timely manner for the stock assessment. Age-1 index CVs were not
> available for inclusion in the 2024 assessment; so, the fixed values of 0.5 were
> retained but a student-t distribution was used to fit the survey instead of a
> lognormal (see the above response).*
...
> freezer they are currently using for the purpose and organizational
...
> determined not to be feasible for foodsafe and storage space reasons.
...
> basis from which wholescale re-weighting of data sources (fishery or survey)
> is done with the Dirichlet data weighting model parameters. The JTC has
...
> acoustic-trawl sampling net. Yet, selectivity for ages 2 and older with acoustics
...
> difficulties with permits to send biological specimens across the border.
...
> *Response -- The JTC agrees that investiagions into alternative selectivity functions
...
(Figure~\@ref(fig:main-bridge-summary-fig-1)). While including the `r last_data_yr` survey
biomass age compositions had a negligible effect on the stock trajectory, they did
adjust estimates of recent recruitment strength, in particular lowering the 2020
recruitment somewhat. The addition of the `r last_data_yr` age-1
index led to an estimated increase in stock biomass from 2023 to 2024 as a result of small shifts in expected recruitment, particularly for the 2020 and 2022 year classes.
...
(see Section~\@ref(sec:data-maturity) and
Appendix~\@ref(sec:app-maturity-analysis) for details). This is
...
biomass pulls down the last few years of estimated biomass, such that 2019 fit is very good
...
The new 2023 biomass index is the third lowest in the series
(Table~\@ref(tab:main-survey-history-tab)), and is well below the estimated index
from the model, similar to the 2001 index that has always been below
estimated values [@JTC2023]. While no direct cause for the
...
increase from 2021 to 2023 is driven by the addition of the age-1 index
(Figure~\@ref(fig:main-bridge-summary-fig-1)), which suggested an above-average
2022 cohort (and also increased the 2021 age-1 index compared to the previous assessment;
...
young, include a large amount of uncertainty.

TODO: This next paragraph is specific for 2021 survey so needs updating, including checking
and updating references, Issue \#1102.

The high age-1 survey index from 2021 is
...
acoustic survey (Figure~\@ref(fig:main-age-comp-fits-fig)), with the survey's inclusion
decreasing its estimated size (Figure~\@ref(fig:main-bridge-summary-fig-1)).
...
The 2016 cohort, which has been observed three times by the survey, still appears
to be smaller than the 2014 cohort.

The `r survey_end_yr` survey was the first
to sample the `r survey_end_yr - 2` cohort, suggesting that it was large
...
2014 cohort at age-5 was slightly more than that of the 2010 cohort at age-5
...
logistic pattern
(Figure~\@ref(fig:main-tv-selex-uncertainty-fig)),
likely as a
...
female spawning biomass since 2021 is due to the expected above-average 2020 and
...
(`r prev_bio_lower`--`r prev_bio_upper`) million~t. This is less than half of the
median, and a much narrower credibility interval, compared to the
...
assessment; in particular the tail of the distribution is greatly curtailed
...
methods for modeling temporal weight-at-age and spatio-temporal maturity have
also changed some historical estimated recruitments.
For example, the 1980 median recruitment (the highest in the time series) of
`r f(recruitment_med_to_compare["1980"], 1)`~billion individuals is now estimated to
be somewhat larger than the 2010 recruitment of
`r f(recruitment_med_to_compare["2010"], 1)`~billion, whereas in last year's
assessment the 1980 median was
`r f(prev_assess_recruitment_med["1980"], 1)`~billion, very close to the 2010
median of
`r f(prev_assess_recruitment_med["2010"], 1)`~billion.
This change appears to be caused by the penultimate bridging step of using the modelled
temporal weight-at-age inputs (compare the red to blue steps in
Figure~\@ref(fig:main-bridge-summary-fig-1) noting the increase, on the log
scale, for 1980 and the slight decrease for 2010).

Regarding more recent recruitments, this assessment's median estimate of the 2020 recruitment is
<!-- check values carefully; use - sign and lower and decrease, or no - sign and -->
<!-- higher and increase; could automate with abs() and ifelse() -->
`r f(-((recruitment_med_to_compare - prev_assess_recruitment_med)["2020"]), 1)`
~billion fish lower than in the last assessment (a
`r f(-((((recruitment_med_to_compare - prev_assess_recruitment_med)["2020"]) /
(prev_assess_recruitment_med)["2020"]) * 100), 0)`% decrease), due to the
age-2+ biomass index and survey age composition data (Figure~\@ref(fig:main-bridge-summary-fig-1)).
Similarly, estimates of 2019 recruitment have changed by
...
only information was the proportions of age-1 fish caught in the 2022 commerical
fishery). The median has increased by
...
years (Figure~\@ref(fig:main-age-comp-fits-fig)).
...
suggesting it is probably not large (Figures~\@ref(fig:main-survey-age1-fig)
...
only the 1980 and 1984 recruitments are probably larger than 2010 (median
relative values $>1$), and the 1970
...
1999, 2014, 2020, and 2021 could also possibly be larger than in 2010, giving an
...
Tables~\@ref(tab:main-median-posterior-tab) and
\@ref(tab:main-ci-posterior-tab)). It was close to 1.0 in 1999 and 2006--2008,
but catches in 2006--2008 did not exceed the catch limits that were specified,
...
estimated to have declined from `r median_intensity_2011`% in 2011 to
`r median_intensity_2015`% in 2015, and then leveled off around 78-90% from
...
below `r f_40` from 2012-- 2015 and again in 2020, 2021, and 2023 (Figure~\@ref(fig:main-fishing-intensity-fig)).
...
also a fall in the U.S. utlization rate to 59.7% in 2023.
...
reference point in all years except 1999--2000 and 2007--2012
...
remained below `r f_40`, and total catch has been declining since the
...
reduction of the 2020 cohort size by more than half. Medians of some of the derived quantities
also change somewhat; in particular, recruitment in 2016 and 2014 both decreased
by 10% from those estimated in the `r assess_yr - 1` assessment.
...
further observations of these year classes will improve estimates. The addition of
...
algorithm with the same number of posterior samples as the base model.
...
\@ref(tab:main-parameter-estimates-sens-3-tab). None of the sensitivities
resulted in any substantial departure from the main population dynamics of
the base model (Figures~\@ref(fig:main-biomass-base-alternative-1-fig) --
\@ref(fig:main-index-base-alternative-4-fig)). All sensitivity
models showed large estimated increases in
...
estimated relative fishing intensity in `r end_yr - 1`, and more than doubled
...
`r sigma_r_sens[2]`, respectively. The base model calculated
standard deviation of recruitment deviations was intermediate at 1.70.
Where, in theory the calculated standard
...
base model with the age-1 index indicates slightly larger 2020 and 2021 year classes
then the model removing the age-1 index
(Figure~\@ref(fig:main-recruitment-base-alternative-2-fig)).  Similarly, recruitment
in 2022 is estimated to be slighlty above average with the
age-1 index compared to slightly below average without the index. Removing the age-1
index led to minor changes in fit to the age-2+ survey biomass index, with
2021 showing a slight improvement and 2023 a deterioration compared to the
...
associated with recruitment in 2014 or 2016, but it did impact more recent
recruitments (Figure~\@ref(fig:main-recruitment-base-alternative-4-fig)). In
particular, recruitment estimates for 2020 and 2021 are linked to the choice
of $\Phi$, where the model with the smallest $\Phi$ (0.21) estimates the 2020
and 2021 recruitment deviation as the highest of the $\Phi$ sensitivity
