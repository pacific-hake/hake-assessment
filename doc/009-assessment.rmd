...
- Turn off estimation of the `r last_data_yr - 1` and `r last_data_yr` recruitment deviations (i.e., recruitment comes from the stock--recruitment relationship) due to limited data to inform the estimation of `r last_data_yr - 1` year class strength in the absence of survey data.
...
trajectory were relatively unchanged through 2023, but the female spawning biomass increased in the final year of the model due to well above average recruitment being estimated for `r last_data_yr-1` (Figure~\@ref(fig:main-bridge-summary-2-fig)). With the lack of a survey in 2024, the `r last_data_yr-1` recruitment deviation is being informed solely by age-1 observations in the `r last_data_yr` fishery age-compositions.
...
The final step in the model bridging turned off the estimation of the `r last_data_yr - 1` and `r last_data_yr` recruitment deviations (i.e., recruitment comes from the stock--recruitment relationship, Figure~\@ref(fig:main-bridge-summary-2-fig)). Given the unique seasonal
fishing conditions during the year, this data point was considered potentially suspect
as a standalone observation (i.e., without the addition of survey information). This change in the model decreased the female spawning biomass at the start of `r assess_yr` relative to the previous model bridging steps. The next assessment will include data from the `r assess_yr` survey that will provide an age-2+ index with age compositions that will provide additional information on the `r last_data_yr-1` year class strength. Additionally, the `r assess_yr+1` assessment will explore turning back on the estimation of recruitment deviations in the final two model years (e.g., `r last_data_yr` and `r assess_yr`) since there will be both a relative age-1 index and fishery age composition data to inform those estimates. 
...
most recent data year (`r last_data_yr`), the quantity of available data determines whether it is the case
for the year previous (`r last_data_yr - 1`). The only information on the size of the 2023 cohort
...
by the age-1 index in 2023 and in recent years by the fishery.  To date, this information
...
female spawning biomass in the early- to mid-2010s
driven by the 2010, 2014, and 2016 cohorts, followed by a period of general
decline (2018--2023) during a period of high catches, before increasing again due to the above average 2020 and
2021 cohorts. All sensitivity models indicate that `r assess_yr` relative spawning
biomass is essentially above `r b_40` (the lowest median relative spawning biomass is
0.65$B_0$ and a few of the credible intervals dip slightly below `r b_40`). The overall scale of the population was impacted
...
the median of the prior was also changed for the latter sensitivity that uses the @HamelCope2022 prior. Each of these
...
`r f(nat_m[1], 3)`--`r f(nat_m[3], 3)`, and for the senstivity runs were,
respectively,
`r f_nat_m_02[1]`--`r f_nat_m_02[3]`,
`r f_nat_m_03[1]`--`r f_nat_m_03[3]` , and
`r f_nat_m_hamel[1]`--`r f_nat_m_hamel[3]` (Table~\@ref(tab:main-parameter-estimates-sens-1-tab)).
...
scales. They also showed roughly a halving of the estimated relative fishing intensity in `r end_yr - 1`
and roughly a doubling of equilibrium yield at `r bspr_40`
...
the years 2019, 2021, and 2023. The relative eDNA index was specified as having
a Student’s t-distribution for the error structure (similar to the age-1 index)
with the number of degrees of freedom equal to one less than the
number of available data points (i.e., 2). An additional constant and additive
standard deviation on the log-scale was freely estimated to accommodate
unaccounted-for sources of process and observation error (median estimate
is XXX 0.09).
The index is representative of all fish age-1 and older (i.e., fully selective
of fish age-1$+$), with a catchability estimated at (median) YYY. The eDNA index
declined from 2019 to 2023, though more steeply between 2019 to 2021 then between
2021 and 2023 (see Figure~\@ref(fig:app-eDNA-index-1-fig)). This models fit to the
eDNA index is adequate. While the inclusion of the eDNA index did not significantly alter most
key parameters and management quantities (Table~\@ref(tab:main-parameter-estimates-sens-2-tab)),
it did reduce spawning stock biomass estimates from 2021 onward, similar in size and trajectory
to the removal of the age-1 index sensitivity
(Figure~\@ref(fig:main-biomass-base-alternative-2-fig)). The eDNA index was included in
the stock assessment as an initial exploration and proof-of-concept. Further explorations
are needed, such as integrating eDNA data with acoustic data outside the assessment model
to produce more accurate and robust survey estiamtes, before it should be used directly
for management advice. The construction of a longer time-series, through continued
environmental DNA sampling, will be advantageous for understanding `r sp` population
...
year data for ten years and estimating the posterior distribution of
parameters under the assumptions of the base model. Medians of various
quantities of interest are given in
...
most cohorts. The 2022 cohort, age 3 in `r assess_yr`, is estimated to
be a below-average cohort, whereas in the `r assess_yr - 1` assessment it
was estimated as possibly being an above-average cohort at the age of 2.
Uncertainties are shown for select cohorts in
Figures~\@ref(fig:main-retrospective-recruitment-fig)
and~\@ref(fig:main-retrospective-recruitment-2-fig). The uncertainty is
represented as credible intervals from `r f(probs[1] * 100, 1)`% to
`r f(probs[3] * 100, 1)`%. For cohorts that have positive recruitment
deviations such as the 2014 cohort, the uncertainty is a narrower band
around the median due to a higher sampling rate over the years than the
...
than those from previous assessments; the last two years of the
time series plus the assessment year have their recruitment deviations fixed
at zero for this assessment. This represents zero deviations from the
stock-recruit relationship (Figure~\@ref(fig:main-stock-recruitment-fig))
for the last two data years (`r last_data_yr - 1` and `r last_data_yr`) and
the assessment year (`r assess_yr`). These zeroes can be seen for ages 0, 1,
and 2 in the retrospective plots
(Figures~\@ref(fig:main-retrospective-recruitment-fig)
and~\@ref(fig:main-retrospective-recruitment-2-fig)).

The latest data confirm that 2020 is not likely to be a large cohort
(Figures~\@ref(fig:main-retrospective-recruitment-fig)
and~\@ref(fig:main-retrospective-recruitment-2-fig)). The 2021 cohort at
age-3 has a similar median to the 2020 cohort at age-3, but with less uncertainty
...
survey until it was another year older). Addition of the `r last_data_yr` data
show that the recruitment deviation for the 2021 cohort is almost the same as
it was in the `r assess_yr - 1` assessment, indicating that the 2021 cohort
is likely to be a large cohort.

Cohort strength is further informed when at least one year of age-2$+$
