...
stock assessments are the addition of another year of fishery data and a change
in the estimation of recruitment in 2023 (age-1 fish in 2024). Given the unique
seasonal fishing conditions during the year, this data point was considered
...
(no additional recruitment deviate estimate).
...
*Response -- Hake have several natural predators, including themselves,
...
AFWwaea2_PScUeInVLp6dXD_hYL6&html=&pos=0&folt=kw-top) identified hake as the largest
...
Stock Synthesis allows for the inclusion of predators so predation mortality can be
estimated on top of baseline natural mortality. Data for predation mortality can be
...
*A potential better path forward is to use Dynamic Structural Equation Modeling (DSEM)
to include indices for multiple predators and allow for two-way interactions between
them and the process that estimates $M$. This capability is not, and will not be,
available within Stock Synthesis but is possible using models written in Template Model
Builder. A proof of concept has been implemented for the Alaska Fisheries Science Center
and discussions are ongoing to implement this in the Fisheries Integrated Modeling System
(FIMS). Thus, for the 2025 assessment a simple version of FIMS will be fit, where the
simple version does not have the capacity to estimate ageing error or facilitate DSEMs.
A more complex FIMS model is planned to be presented to the SRG in 2026.*

*Predation in SS3 does not allow for feedback between hake and the predator, and thus,
we used the Climate- Enhanced, Age-based model with Temperature-specific Trophic Linkages
and Energetics (CEATTLE; Holsman et al., 2016) to estimate cannibalism. Results have been
presented before and have recently been updated to provide estimates of baseline $M$ and
$M$ at age due to cannibalism. These values can be input into SS3 as fixed $M$ at age. In
2023 the JTC was unsuccessful in estimating age-specific $M$ because of a lack of data to
inform the estimates. Additionally, fitting Lorenzen $M$ is not possible without
simultaneously estimating growth. Thus, the JTC sees using fixed age-specific $M$ from
CEATTLE as a way forward. This work is included in the sensitivity model section in this
assessment (Section~\@ref(sec:assessment-sensitivity-analyses)). Additionally,
the JTC will investigate using the results from CEATTLE to inform age-specific priors on $M$
for the 2026 SRG meeting.*
...
species distribution models for hake. This work will include incorporating environmental
factors across the full transboundary range of hake and will evaluate prediction skill
...
countries are concurrently being developed so that the JTC can explore coastwide fishing location
...
tools (e.g., PacFEM) that are currently under development (but available for beta
explorations).*
...
reference point figures and summary metrics in this stock assessment (e.g., see
Figure~\@ref(fig:main-fished-unfished-biomass-cap) and
Figure~\@ref(fig:main-fished-unfished-rel-biomass-cap)).*
...
*Response -- The estimates of maturity have been updated with recent data for the
...
*Response -- This comment seems to arise from Figure~\@ref(fig:main-historical-assessments-cap)
...
estimatess are shown for historical estimates). In particular, with median estimates presented:*
 ...
 *An age-0 recruitment index could help, and the JTC plans to continue analyses
 of age-0 hake data presented at previous SRG meetings when time allows.*
...
when haul information is not available (e.g., shore-based), and further work is needed to
determine the effective sample sizes at the haul and trip level and how to calculate a fleetwide
...
size could vary requiring consideration on how to model the survey. The JTC does not plan
on investigating this issue for this year's assessment but supports the prioritization of this
...
*Response -- Future improvements in the stock assessment software used to assess hake
...
*Other frameworks (e.g., Woods Hole Assesment Model, or WHAM, and bespoke models) can
...
stock assessment models (occurring at AFSC and University of Washington) and
...
a 6 December JMC meeting. Refinements of the number of catch scenarios were discussed
...
of catch scenarios and in the number of forecast years (from 3 to 2).*
...
new data pulls need to be thourougly investigated.*
...
and bias tradeoffs associated with new parameterizations (e.g., doubling the number of
...
*Additonally, model complexity in the spatial domain needs to be addressed relative to
...
Current IEA results, and `r sp` distribution modeling research informed by environmental data.*
...
(Section~\@ref(sec:data-maturity)). This is
...
`r last_assess_yr` base model. The low `r last_survey_yr` survey
...
`r last_data_yr` was characterised by higher than average for age-1 fish
...
information about it during the 2023 survey. The biennial age-1 index does not provide
...
(`r prev_bio_lower`--`r prev_bio_upper`) million~t, giving a slightly narrower range of
...
the 2022 year class with updated data, and the setting of the 2023 year class using only
...
assumptions about recent recruitments.  In particular, the 2023 and 2024 year classes
were set based solely on the stock--recruitment relationship for this assessment, such that
no recruitment deviations were estimated for these years.  While this is standard for the
most recent data year (2024), the quantity of available data determines whether it is the case
for the year previous (2023). The only information on the size of the 2023 cohort
(age-1 fish in 2024) in this assessment comes from the age composition of fishery catches.
The catch of age-1 fish was particularly high in 2024, potentially due to the unique
...
By restricting 2023 recruitment to the stock--recruitment relationship, the assessment model
was not allowed to predict a very high 2023 year class based solely on the age-1
catch in the fishery. Another year of data, particularly form the age-2$+$ acoustic--trawl
survey in 2025 will provide additional information on the size of the 2023 year class.
...
a 95% credible interval of `r recruitment_lower_in_2021`--
`r recruitment_upper_in_2021`~billion fish). Last year's assessment
...
slightly reduced in the current assessment to
...
In general, `r sp` have low average recruitment with occasional large year classes
...
`r top_coh(base_model, 2017, ret_cohort = 2014)[[2]]`% of the 2017 catch,
`r top_coh(base_model, 2018, ret_cohort = 2014)`% of the 2018 catch,
`r top_coh(base_model, 2019, ret_cohort = 2014)[[2]]`% of the 2019 catch,
`r top_coh(base_model, 2020, ret_cohort = 2014)`% of the 2020 catch,
`r top_coh(base_model, 2021, ret_cohort = 2014)[[2]]`% of the 2021 catch,
`r top_coh(base_model, 2022, ret_cohort = 2014)`% of the 2022 catch,
`r top_coh(base_model, 2023, ret_cohort = 2014)[[2]]`% of the 2023 catch, and
...
`r top_coh(base_model, 2019, ret_cohort = 2016)[[2]]`% of the 2019 catch,
`r top_coh(base_model, 2020, ret_cohort = 2016)`% of the 2020 catch,
`r top_coh(base_model, 2021, ret_cohort = 2016)[[2]]`% of the 2021 catch,
`r top_coh(base_model, 2022, ret_cohort = 2016)`% of the 2022 catch,
`r top_coh(base_model, 2023, ret_cohort = 2016)[[2]]`% of the 2023 catch, and
`r top_coh(base_model, 2024, ret_cohort = 2016)`% of the 2024 catch.
The strong 2021 cohort comprises
`r top_coh(base_model, 2023, ret_cohort = 2021)[[2]]`% of the 2023 catch, and
...
by the age-1 index in 2023 and in recent years by the fishery.  To date, this infomration
suggests that it is well below average (Figures~\@ref(fig:main-survey-age1-fig)
and~\@ref(fig:main-recruitment-fig)). It will be observed as part of the age-2$+$
...
process have an intuition that the 2010 is a very large recruitment event. As a result,
...
2016 to 2019. It then declined to around 63% from 2020 to 2022 before declining further
to `r median_intensity_2024`% in 2024. The
median exploitation fraction has increased from a recent low of
`r exploitation_med_2012` in 2012 to `r exploitation_med_2021`. It has since declined
...
below 100% from 2012--2016 and again from 2020--2024 (Figure~\@ref(fig:main-fishing-intensity-fig)).
...
was above 40% (the `r b_40` reference point) in all years except 2007--2011
...
index and age-1 index catchabilities (*q*~b~ and *q*~1~, respectively) and
...
(Table~\@ref(tab:main-parameter-estimates-tab)). Parameter differences were generally less
than 10%. For derived management quantities, the largest change was a reduction in the
estimated relative spawning biomass at the start of 2024 from
`r f(last_yr_base_model$mcmccalcs$dmed["2024"], 1)`% in last year's model to
`r f(base_model$mcmccalcs$dmed["2024"], 1)`% this year as a result
of updated data and using average recruitment for 2023 and 2024.
Medians of the 2014 and 2016 recruitment increased slightly, while 2021 recruitment
decreased slightly from those estimated in the `r assess_yr - 1` assessment.
...
further observations of these year classes will improve estimates. The continued inclusion of
the age-1 index in this assessment will, in some cases, also help to reduce this
uncertainty during survey years (see
...
1. Downweighting the fishery age-composition data;

1. Consideration of alternative standard deviations for time-varying
selectivity;

1. Adding a relative index of abundance based on environmental-DNA; and

1. Using the middle of the year to predict maturity-at-age data.
...
led to a decrease in estimates of spawning biomass for the majority of the time
series, including unfished spawning biomass
(Table~\@ref(tab:main-parameter-estimates-sens-2-tab),
Figure~\@ref(fig:main-biomass-base-alternative-2-fig)). Because this model
leads to a lower estimates of unfished biomass and similar terminal female
spawning biomass, the estimates of current depletion are higher than the base
model (Figure~\@ref(fig:main-status-base-alternative-2-fig)).
