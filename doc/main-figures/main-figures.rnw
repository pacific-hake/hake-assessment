...
\caption{Bridging models showing the sequential addition of updating
  the Stock Synthesis model code version, adding \Sexpr{last_data_yr} catch,
  adding \Sexpr{last_data_yr} weight-at-age, and adding \Sexpr{last_data_yr}
  fishery age compositions. Panels are spawning biomass (upper panel),
  relative spawning biomass (spawning biomass in each year
  relative to the unfished equilibrium spawning biomass, middle left),
  recruitment deviation (middle right), age 2+ survey biomass index
  (lower left), and age 1 index (lower right).}
...
@
\end{center}
\caption{Medians (solid circles) of the posterior distribution
  of `recruitments divided by the 2010 recruitment',
  with
  95\% posterior credibility intervals (red lines);
  the division is done for each MCMC sample.
  This scales out the uncertainty due to uncertainty in unfished equilibrium
  recruitment ($R_0$), and shows that, for example, the 2014 recruitment
  is definitely smaller than the 2010 recruitment
  (2010 was chosen as it is estimated to be one of the largest
  recruitment events). However, intuition from
  plots of absolute recruitment (Figure~\ref{fig:main-recruitment})
  is that the 2014
  is possibly (even probably) larger than the 2010 recruitment \citep{JTC2022}.
  This figure gives a somewhat more intuitive y-axis
  (people familiar with
  \fishname\ know 2010 to be a very large recruitment year) than the
  `billions of age-0 fish' necessarily used in Figure~\ref{fig:main-recruitment}.
}
...
@
\end{center}
\caption{**[if sens runs change] MCMC estimates of spawning biomass for the base
...
#TODO: Run extra mcmc for these models
# plot_survey_index_fits(d_obj = d_obj_sens_index[[2]],
#                        x_expansion = 2,
#                        rev_colors = TRUE)
# make.comparison.plot(sens_models[[2]],
#                      subplots = 13,
#                      model.names = sens_models_names[[2]],
#                      legend = TRUE,
#                      legend_loc = "topleft",
#                      indexfleets= c(2),
#                      end_yr = end_yr)
@
\end{center}
\caption{** TODO: Run extra mcmc for these models...** MCMC estimates of the fit to the biomass index for the base
...
# TODO: run extra mcmc for sensitivity models
# plot_survey_index_fits(d_obj = d_obj_sens_index[[3]],
#                        x_expansion = 2,
#                        rev_colors = TRUE)
@
\end{center}
...
% temp <- grep("tvSelect_sig", sapply(sens_models_4, "[[", "path"))[1]
% make.selectivity.illustration(mod.old = base_model,
%                               mod.alt = sens_models_4[[temp]],
