...
\vspace{-5mm}
\begin{center}
\includegraphics[max size={\paperwidth}{\paperheight}]{main-figures/age1hake_2003-17_timeseries.eps} % width=\maxwidth,
\end{center}
\vspace{-5mm}
\caption{Spatial distribution of acoustic backscatter attributable to age-1 \fishname\
  from joint US-Canada acoustic surveys
  \Sexpr{survey.start.yr}--\Sexpr{survey.end.yr}. Age-1 \fishname\ are not fully sampled 
