...
\caption{Retrospective analysis of recruitment deviations from MCMC models over the last
  ...
  each cohort became a given age (so the 2014 cohort is age-8 at the
  start of 2022, and so that retrospective calculation
  includes data to 2021).} %Sorry, no time to
...
\caption{Retrospective versions of Figure~\ref{fig:main-historical-1}.
  ...
  and~\ref{fig:main-historical-retro-3}, and
