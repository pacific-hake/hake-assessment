...
\caption{Bridging models showing the difference between the
  ...
  shown in Figure~\ref{fig:main-bridge-5-panel} above, the result
  ...
  the update of the maturity ogive. That last step is the final
  change to arrive at the \Sexpr{assess.yr} base model (shown by the red line in the plots).
