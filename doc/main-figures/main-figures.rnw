...
\caption{Bubble plot of maximum likelihood (MLE) estimates of population numbers at age at the beginning of each year, where diagonals follow each year-class through time.
  ...
  \Sexpr{round(max(base.model$natage[base.model$natage$"Beg/Mid"=="B",paste(0:20)])/1e6,1)}
  billion age-0 recruits in 1980).
...
\caption{Summary of historical \fishname\ assessment estimates of spawning
  biomass. Shading represents the approximate 95\% confidence range from the
  \Sexpr{end.yr} base model.}
