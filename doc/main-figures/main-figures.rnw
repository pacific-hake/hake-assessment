...
@
\end{center}
\caption{Forecast age compositions in numbers and in weight for the 2017 fishery catch 
  ...
  selectivity averaged across the most recent 5 years and the distribution for expected 
  numbers at age at the start of 2017 (see Table~\ref{tab:main-est-numbers-at-age} for 
  the MLEs for numbers-at-age for all years). The panel on the right is scaled based on 
  the weight at each age averaged across all years.}
