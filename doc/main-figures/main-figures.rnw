...
make_age_comp_bubble_plot(base.model, subplot = 2, legend.position = "top")
make_age_comp_bubble_plot(base.model, subplot = 1, legend.position = "none")
@
\end{center}
...
\caption{Bridging models showing the \Sexpr{last.assess.yr} base model
  ...
  recruitment period (see Section~\ref{sec:assessment-base-model} for further
  details about recruitment deviation assumptions). Panels are spawning biomass
...
\caption{Bridging models starting with the addition of simple recruitment deviations (shown
  ...
  adding a prior on the survey Dirichlet Multinomial parameter (see 
  Section~\ref{sec:assessment-base-model} for further details about model priors)
  and model tuning (= 2020 base model). Panels are spawning biomass
  (upper panel), relative spawning biomass (spawning biomass in each year
  relative to the unfished equilibrium spawning biomass, middle left),
  absolute recruitment (middle right), recruitment deviations (lower left),
  and survey index (lower right).}
