...
\caption{Spatial distribution of acoustic backscatter attributable to age-2 and older \fishname\
  ...
  Figure produced by Julia Clemons (NOAA).**NEED NEW FIGURE.}
...
\caption{Unstandardized (raw) catch-rates (t/hr) of \fishname\ catches by tow in
  the U.S. at-sea fleet from 2014--\Sexpr{last.data.yr}.**NEED NEW FIGURE}
...
@
\end{center}
\caption{Distribution of fishing depths (left) and bottom depths (right), in meters, of \fishname\ catches in the U.S. at-sea fleet from 2014--\Sexpr{last.data.yr}.**NEED NEW FIGURE}
...
\caption{Fraction of fish that are mature at each age north and south of
  ...
  averaged across 1975 to \Sexpr{last.data.yr}.**Caption automatically says last
year, but figure not updated, so maybe hardwire it to say 2018.}
...
\caption{Posterior correlations among key base-model parameters and derived
  ...
  size proportional to the square root of the coefficient.**2014 probably still
  best as the latest cohort to highlight, just think about it.}
...
\caption{Pearson residuals for base model MLE fits to the age-composition data. Closed bubbles are positive residuals (observed > expected)
  ...
  negative residuals (observed < expected).**Year labels look offset, should make
  neater.}
...
\caption{Fishery selectivity sampled from posterior probability distribution by year for the base model.
  ...
  \Sexpr{tv.selex.start.yr}.**Balance out the numbers of years in each column
...
\caption{Retrospective analysis of recruitment deviations from MLE models over the last
  ...
  each cohort was a given age.**See Issue \#616.}
...
\caption{Bubble plot of MLE estimates of population numbers at age at the beginning of each year, where diagonals follow each year-class through time.
  ...
  See Table~\ref{tab:main-est-numbers-at-age} for values.**Double check number
  with table, especially due to changes in SS output. Expect okay.}
...
\caption{Estimated historical path followed by medians of relative fishing
  ...
  and relative spawning biomass (horizontal).**Update to match Figure h caption.}
...
\caption{Forecast age compositions in numbers and in weight for the \Sexpr{end.yr} fishery catch
  ...
  the weight at each age averaged across 1975 to \Sexpr{last.data.yr}.**check
  last sentence.}
...
\caption{Maximum likelihood estimates of spawning biomass for the base
  ...
  0.1 to 0.2 or 0.3.**check these are all correct.}
...
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative
  ...
  Francis approach to weight composition data.**check these are all correct.}
...
@
\end{center}
\caption{**Not sure yet if we're doing any of these. Empirical weight-at-age (kg) values used for sensitivity
  run~53 and the base
...
@
\end{center}
\caption{**Keep as template for now, in case needed.
  Maximum likelihood estimates of spawning biomass for the base
