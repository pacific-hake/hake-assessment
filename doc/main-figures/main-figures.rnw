...
<<main-overview-map, fig.show='hide', fig.height=10>>=
make.overview.map.plot(path = rootd.map.data)
@
\hspace*{-8.5cm}\includegraphics[width=2\linewidth]{knitr-figs/main-overview-map-1}
\end{center}
...
\includegraphics[max size={1.15\paperwidth}{1.15\paperheight}]
  {main-figures/hake_survey_1995-19_NASCTimeSeries_BiomassAtAgeHistograms_grayblue.png}
...
\includegraphics[max size={1.10\paperwidth}{1.10\paperheight}]
   {main-figures/age1hake_03-19_sA_squareroot_bin_narrow-panel_bluergray.png}
   % Download from Google Drive
\end{center}
%\vspace{-5mm}
...
\includegraphics[width=\maxwidth, height=3.5in]{main-figures/fishCatchRatesUS.png}
\end{center}
...
\begin{figure}[H]
\begin{center}
<<main-weight-at-age-numbers, fig.height=9, fig.width=8>>=
weight.at.age.heatmap(base.model,
                      proj.line.yr = base.model$endyr,
                      extrap.mask = weight_age_extrapolation_mask,
