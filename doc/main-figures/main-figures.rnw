...
**Check this shows 2017 when all data uploaded.}
...
@
\end{center}
\caption{Acoustic survey biomass index (millions of metric tons).
...
@
\end{center}
\caption{**Updated acoustic survey biomass indices with and without extrapolation
  (millions of metric tons) relative to the index used \Sexpr{last.assess.yr}.
  Approximate 95\% confidence intervals are based on only sampling variability
  (and squid/hake apportionment uncertainty in 2009). See
  Table~\ref{tab:main-survey-comparison} for values used in the base model
...
@
\end{center}
\caption{**Bridging models comparison showing the \Sexpr{last.assess.yr} base model
  and the terminal model from sequentially updating all pre-\Sexpr{last.assess.yr}
  data.  This included updating fishery catch and age-compositions as well as
  weight-at-age information.  The points disconnected from the time-series on the
  left side show the unfished equilibrium spawning biomass estimates.}
...
@
\end{center}
\caption{**Bridging models showing the difference between the
  \Sexpr{last.assess.yr} base model and the sequential addition of the new
  acoustic survey time-series (\Sexpr{survey.start.yr}--\Sexpr{last.data.yr})
  and then the new \Sexpr{last.data.yr} fishery data.  Spawning biomass
  ...
  and survey index (lower right) are shown.}
...
@
\end{center}
\caption{**Bridging models showing the difference between the \Sexpr{end.yr}
  pre-tuned base model, the sequential addition of the main base model tuning
  runs (adjusting time periods and levels for recruitment bias and reweighting
  the survey and fishery compositional data), and increased flexibility
  associated with time-varying selectivity (larger standard deviation
  associated with temporal deviates). The red line is equivalent to the
  \Sexpr{end.yr} base model. Spawning biomass (upper left panel), relative
  spawning biomass (spawning biomass in each year relative to the unfished
  equilibrium spawning biomass, upper right), absolute recruitment (lower left),
  and recruitment deviations (lower right) are shown.}
...
\caption{Summary histograms of MCMC diagnostics for all base model parameters
  ...
  together with the derived time series of
...
@
\end{center}
\caption{**[Not building, but no error shows up?! May get fixed automatically].
  Pearson residuals for base model MLE fits to the fishery age
...
make.selex.uncertainty.lines.plot(base.model,
                                  ages = tv.selex.ages,
                                  year = last.data.yr - 1,   # think this should be end.yr-1
                                  type = 1,
                                  selex.list = tv.selex,
                                  probs = c(0.025, 0.975))
par <- oldpar
...
\caption{Medians (solid circles) of the posterior distribution for log-scale recruitment deviations with
  ...
  the years **[hardwired - check if correct]
  1970--2012 are constrained to sum to zero while deviations outside this range do not have a constraint.}
