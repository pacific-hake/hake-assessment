...
\caption{**Retrospective versions of Figure~\ref{fig:main-historical-1} for second
  ...
  2021 assessment results - interesting if changes with new model). Only one for
  which newer model with data to year $t$
  performs 'worse' than original assessment from year $t$. Check the years are
  lined up right **not off by a year?.
  For each panel, the current base model is run using only data up to the year
  shown (i.e. a retrospective analysis). Results are shown in a single panel in
  Figure~\ref{fig:main-historical-retro-all}.}
...
\caption{**Retrospective versions of Figure~\ref{fig:main-historical-1} for final
  retrospectives **bump select.retros to 9 when have updated base model results.
  For each panel, the current base model is run using only data up to the year
  shown (i.e. a retrospective analysis). Results are shown in a single panel in
  Figure~\ref{fig:main-historical-retro-all}.}
