...
@
\end{center}
\caption{Bridging models showing the difference between the \Sexpr{last.assess.yr} base model (blue) and the sequential addition of the new acoustic survey time-series (\Sexpr{survey.start.yr}--\Sexpr{last.data.yr}; red) and then the new \Sexpr{last.data.yr} fishery data (green).  Spawning biomass (upper panel), relative spawning biomass (spawning biomass in each year relative to the unfished equilibrium spawning biomass, middle left), absolute recruitment (middle right), recruitment deviations (lower left), and survey index (lower right) are shown.}
...
@
\end{center}
\caption{Bridging models showing the difference between the \Sexpr{end.yr} pre-tuned base model (blue) and the sequential addition of the main base model tuning runs (adjusting time periods and levels for recruitment bias [green] and reweighting the survey [yellow] and fishery [red] compositional data).  The red line is equivalent to the \Sexpr{end.yr} base model.  Spawning biomass (upper left panel), relative spawning biomass (spawning biomass in each year relative to the unfished equilibrium spawning biomass, upper right), absolute recruitment (lower left), and recruitment deviations (lower right) are shown.}
...
\begin{figure}[H]
\begin{center}
<<biomass.base.alternative, fig.height=6, fig.width=8>>=
make.comparison.plot(models[c(base.model.ind, sens.model.inds)],
                     subplots = 2,
                     model.names = c(paste0("Base model ", end.yr), sens.model.names),
                     end.yr = end.yr)

@
\end{center}
%\vspace{0mm}
\caption{Posterior MCMC predictions of spawning biomass at the start of each year for the base model and alternative sensitivity runs.}
\label{fig:main-biomass-base-alternative}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\begin{center}
<<recruitment.base.alternative, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
\caption{Posterior MCMC predictions of recruitment for the base model (black) and alternative sensitivity run (red) which ***AS PREVIOUS.}
\label{fig:main-recruitment-base-alternative}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
