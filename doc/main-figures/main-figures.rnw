...
\begin{figure}[H]
\begin{center}
<<main.overview.map, fig.height=8, fig.width=8>>=
make.overview.map.plot(r.loc = file.path("..", "r"))
@
\end{center}
\caption{Overview map of the area in the Northeast Pacific Ocean occupied by Pacific Hake. Common areas referred to in this document are shown.}
...
\begin{figure}[H]
\begin{center}
<<main.backscatter.map, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
\caption{Spatial distribution of acoustic backscatter attributable to Pacific Hake from joint US-Canada acoustic surveys \Sexpr{survey.start.yr}--\Sexpr{survey.end.yr}. Area of the circle is roughly proportional to observed backscatter.}
...
\end{center}
\caption{Distribution of bottom depths (left) and fishing depths (right), in fathoms, of \fishname\ catches in the U.S. at-sea fleet from 2008--\Sexpr{last.data.yr}.}
...
\begin{figure}[H]
\begin{center}
<<age.one.index, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
\caption{Preliminary acoustic survey age-1 index overlaid on estimated numbers of age-1 fish (posterior medians from the base model). **The y-axis is on a log scale with labels in real space. **AE suggests no need for log axis, given the scale. This figure represents a comparison with, not a fit to, the preliminary age-1 index data.}
...
\begin{figure}[H]
\begin{center}
<<prop.mature.by.source, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
...
\label{fig:main-proportion-mature-by-source}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\begin{center}
<<prop.mature.by.latitude, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
\caption{As for Figure~\ref{fig:proportion-mature-by-source} but for two-degree latitude bins. Observed proportion mature-at-length (bubbles with circle size relative to number of samples at length), fitted proportion mature-at-length with an estimated asymptote (lines), and number of samples at length (barplots beneath each panel).}
\label{fig:main-proportion-mature-by-latitude}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\begin{center}
<<empirical.weight.at.age, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
\caption{Empiricial weight-at-age (kg) used in the assessment (numbers, with colors given by the scale at the bottom). Numbers shown in bold were interpolated or extrapolated from adjacent areas.}
...
@
\end{center}
\caption{Bridging models showing the difference between the \Sexpr{end.yr} pre-tuned base model and the sequential addition of the main base model tuning runs (adjusting time periods and levels for recruitment bias and reweighting the survey and fishery compositional data). The red line is equivalent to the \Sexpr{end.yr} base model. Spawning biomass (upper left panel), relative spawning biomass (spawning biomass in each year relative to the unfished equilibrium spawning biomass, upper right), absolute recruitment (lower left), and recruitment deviations (lower right) are shown.}
...
\end{center}
%\vspace{0mm}
\caption{Prior (black lines) and posterior (gray histograms) probability distributions for key parameters in the base model. From the top left, the parameters are: steepness ($h$), natural mortality ($M$), equilibrium log recruitment $\log(R_0)$, and the additional process-error standard deviation for the acoustic survey. The maximum likelihood estimates and associated symmetric uncertainty intervals are also shown (blue lines).}
...
@
\end{center}
\caption{Bubble plot of maximum likelihood (MLE) estimates of population numbers at age at the beginning of each year.
...
\caption{A comparison of maximum likelihood estimates with 95\% confidence intervals determined from asymptotic variance estimates (red)
  to the median of the posterior distribution with 95\% credibility intervals (black).}
...
\caption{Time series of relative spawning biomass at the start of each year until \Sexpr{end.yr} as estimated from the base model, and forecast trajectories
  ...
  credibility intervals. The \Sexpr{end.yr} catch of \Sexpr{fmt0(catch.default.policy[1])}~t was calculated using the default
...
\begin{figure}[H]
\begin{center}
<<mle.survey.base.alternative, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
\caption{Maximum likelihood estimates of expected survey biomass for the base model (black, same as red line in Figure~\ref{fig:main-mcmc-survey-fit}) and the alternative sensitivity run (red) which ***ONCE WE DECIDE ON ONE.}
\label{fig:main-mle-survey-base-alternative}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
<<<<<<< HEAD
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative sensitivity runs representing changes to selectivity, $\sigma_r$, and steepness parameters.}
\label{fig:main-biomass-base-alternative-1}
=======
\caption{MLE estimates of spawning biomass for the base model and alternative sensitivity runs representing no extrapolation on the acoustic survey estimate and inclusion of an age-1 index.}
...
%\vspace{0mm}
<<<<<<< HEAD
\caption{Maximum likelihood estimates of recruitment for the base model and alternative sensitivity runs representing changes to selectivity, $\sigma_r$, and steepness parameters.}
\label{fig:main-recruitment-base-alternative-1}
=======
\caption{Density plot showing the MLE recruitment deviate estimates for the 2014 cohort for the base model and alternative sensitivity runs representing no extrapolation on the acoustic survey estimate and inclusion of an age-1 index.}
...
\begin{figure}[p]
\begin{center}
<<main.biomass.base.alternative.3, fig.height=4.5, fig.width=8>>=
...
\end{center}
<<<<<<< HEAD
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative sensitivity runs representing no extrapolation on the acoustic survey estimate and inclusion of an age-1 index.}
\label{fig:main-biomass-base-alternative-2}
=======
\caption{MLE estimates of spawning biomass for the base model and alternative sensitivity runs representing changes in the natural mortality parameter and inclusion of an ageing error matrix.}
\label{fig:main-biomass-base-alternative-3}
>>>>>>> aberger/master
%% \end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%% JOINED ON PAGE
...
%% \begin{figure}[H]
\begin{center}
<<main.recruitment.base.alternative.3, fig.height=4.5, fig.width=8>>=
make.comparison.plot(models[c(base.model.ind, sens.model.inds.3)],
                     subplots = 8,
                     model.names = c(paste0("Base model ", end.yr), sens.model.names.3),
                     legendloc = "topleft",
...
%\vspace{0mm}
<<<<<<< HEAD
\caption{Maximum likelihood estimates of recruitment for the base model and alternative sensitivity runs representing no extrapolation on the acoustic survey estimate and inclusion of an age-1 index.}
\label{fig:main-recruitment-base-alternative-2}
=======
\caption{MLE estimates of recruitment for the base model and alternative sensitivity runs representing changes in the natural mortality parameter and inclusion of an ageing error matrix.}
\label{fig:main-recruitment-base-alternative-3}
>>>>>>> aberger/master
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
<<<<<<< HEAD
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative sensitivity runs representing changes in the natural mortality parameter and inclusion of an ageing error matrix.}
\label{fig:main-biomass-base-alternative-3}
=======
\caption{MLE estimates of spawning biomass for the base model and alternative sensitivity runs representing changes to $\sigma_r$, steepness, and natural mortality parameters.}
\label{fig:main-biomass-base-alternative-1}
>>>>>>> aberger/master
%% \end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
%\vspace{0mm}
<<<<<<< HEAD
\caption{Maximum likelihood estimates of recruitment for the base model and alternative sensitivity runs representing changes in the natural mortality parameter and inclusion of an ageing error matrix.}
\label{fig:main-recruitment-base-alternative-3}
=======
\caption{MLE estimates of stock status (relative spawning biomass) for the base model and alternative sensitivity runs representing changes to $\sigma_r$, steepness, and natural mortality parameters.}
...
\begin{figure}[H]
\begin{center}
<<catch.limit.base.alternative, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
\caption{The posterior distribution of the default \Sexpr{end.yr} catch limit calculated using the default harvest policy (\Ffortyten). The dark shaded area ranges from the 2.5\% to 97.5\% quantiles. The base model is shown in black (the same distribution as in Figure~\ref{fig:main-projected-catch-density}) with a median at **GET FROM EARLIER, and the alternative sensitivity run (red) which ***AS PREVIOUS has a median of ***.}
\label{fig:main-catch-limit-base-alternative}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\begin{center}
<<natural.mortality.sensitivities, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
\caption{Natural mortality as a function of age (left) and year (right) for two sensitivity analyses compared to the base model.}
\label{fig:main-natural-mortality-sensitivities}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\begin{center}
<<biomass.sensitivities, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
\caption{Maximum likelihood estimates (MLEs) of spawning biomass at the beginning of each year for **two sensitivity analyses (red, blue) compared to the base model (black, **mostly obscured by the red lines). **Shaded areas are what?}
\label{fig:main-biomass-sensitivities}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
%\vspace{0mm}
\caption{Retrospective recruitment estimates shown Figure \ref{fig:main-retrospective.recruitment} scaled relative to the most recent estimate of the strength of each cohort.}
...
\begin{figure}[H]
\begin{center}
<<historical.assessments, fig.height=6, fig.width=8>>=
plot(1:10, main="Placeholder dummy figure")
@
\end{center}
%\vspace{0mm}
\caption{Summary of historical Pacific Hake assessment estimates of spawning biomass. **The 2013 and 2014 assessment estimated trajectories (red and blue lines) are almost completely covered by the 2015 estimated trajectory. **What is the shaded region? **The 95\% range from the current model?.}
