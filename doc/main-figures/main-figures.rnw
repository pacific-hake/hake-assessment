...
@
\end{center}
\caption{Medians (solid circles) of the posterior distribution of recruitment
  relative to that in 2010 (recruitment divided by the 2010 recruitment for
  every MCMC sample), with 95\% posterior credibility intervals (red
  lines). This procedure scales out the uncertainty due to uncertainty in mean
  unfished equilibrium recruitment ($R_0$), and better elicits comparisons of
  relative cohort size while retaining individual time series pairs within
  aggregated posterior distributions (for example, recruitment in 2014 is
