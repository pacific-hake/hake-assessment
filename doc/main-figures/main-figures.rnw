...
@
\end{center}
\caption{MCMC estimates of recruitment for the base model and alternative sensitivity runs with maximum age-based selectivity decreased (age-5) or increased (age-7 and age-8) relative to the base model (age-6), and constraining recruitment in the model such that recruitment deviations sum to zero.}
