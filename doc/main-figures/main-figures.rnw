...
\end{center}
%\vspace{0mm}
\caption{Empiricial weight-at-age (kg) used in the assessment (numbers, with colours given by the scale at the bottom). Numbers shown in bold were interpolated or extrapolated from adjacent areas.}
...
@
\end{center}
\caption{Graphical representation of the base model results presented in Table \ref{tab:es-risk-year-1} for catch in
...
\end{center}
%\vspace{0mm}
\caption{Posterior MCMC predictions of spawning biomass at the start of each year for the base model (black) and alternative sensititivy run (red) which ***AS PREVIOUS.}
...
\end{center}
%\vspace{0mm}
\caption{Posterior MCMC predictions of recruitment for the base model (black) and alternative sensititivy run (red) which ***AS PREVIOUS.}
