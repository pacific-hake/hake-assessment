...
@
\end{center}
\caption{Pearson residuals for base model fits to the age-composition data for the medians of the MCMC posteriors. Closed bubbles are positive residuals (observed > expected) and open bubbles are negative residuals (observed < expected).}
