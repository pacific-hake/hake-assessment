...
\caption{Bridging models comparison showing the \Sexpr{last.assess.yr} base model
  and the results of converting that model to Stock Synthesis version 3.30 and then
  updating the pre-\Sexpr{last.assess.yr} data.  
...
\caption{Bridging models showing the difference between the
  ...
  shown in Figure~\ref{fig:main-bridge-biomass} above, 
  and the sequential addition of the new
  survey and fishery data. Spawning biomass
...
\end{center}
\caption{Bridging models showing the difference between the
  \Sexpr{last.assess.yr} base model, the result of the second set of changes
  shown in Figure~\ref{fig:main-bridge-5-panel} above, and the result
  of tuning the model using the McAllister-Ianelli data-weighting method
  the change to the new Dirichlet-Multinomial data-weighting method, and
  the update of the maturity ogive, where that last step is the final
  change to make the base model (shown by the red line in the plots).
  Spawning biomass (upper left panel), relative
...
\caption{Illustration of parameterization of time-varying selectivity as
  ...
  selectivity after exponential transforamtion and rescaling to have maximum 1.0 (c),
  ...
  applied as exponential offsets to the resulting selectivity (e).}
