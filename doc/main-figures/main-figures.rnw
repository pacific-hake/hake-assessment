...
\pdftooltip{\includegraphics[max size={1.15\paperwidth}{1.15\paperheight}]
  {main-figures/hake_survey_1995-21_NASCTimeSeries_BiomassAtAgeHistograms_grayblue_update}}{This
...
\begin{figure}[H]
\begin{center}
<<age-one-index, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the age 1 index has dropped from 2017 to 2019">>=
make.survey.age1.plot(base.model,
                      age.1.index)
@
\end{center}
%\vspace{0mm}
\caption{**[maybe no longer call it preliminary if we use it]Preliminary acoustic survey age-1 index overlaid on estimated numbers
