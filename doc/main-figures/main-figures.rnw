...
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative
  ...
  using the harmonic mean approach to weight composition data, and using the
...
\caption{Maximum likelihood estimates of spawning biomass for the base model
  ...
  semi-parametric approach for implementing time-varying selectivity (sigma-S).}
...
\caption{Empirical weight-at-age (kg) used for sensitivity
  ...
  Numbers shown in
  bold were interpolated or extrapolated from adjacent ages or years. Data are
  only available from 1975--\Sexpr{max.fishery.age.prop[2]}.
  ...
  with the sample-weighted mean shown at the bottom.}
...
weight.at.age.heatmap(sens.models.5[[4]], proj.line.yr=2017)
@
\end{center}
\caption{Empirical weight-at-age (kg) used for sensitivity
...
# Run 58 (verified same heatmap as 56)
weight.at.age.heatmap(sens.models.5[[7]])
@
\end{center}
\caption{Empirical weight-at-age (kg) used for sensitivity
  runs~56 and~**58. Numbers shown in
  bold were interpolated or extrapolated from adjacent ages or years. Data are
  only available from 1975--\Sexpr{max.fishery.age.prop[2]} (between the two blue lines),
  with the sample-weighted mean shown at the bottom.
  Remaining values are used for the early and late years.}
