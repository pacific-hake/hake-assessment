...
\caption{Spatial distribution of acoustic backscatter attributable to age-2 and older \fishname\
  ...
  \Sexpr{survey.start_yr}--\Sexpr{survey.end.yr}. Area of the circle is roughly
...
\caption{Fraction of fish that are mature at each age north and south of
  ...
  averaged across \Sexpr{start_yr.age.comps} to \Sexpr{last.data.yr}.}
...
\caption{Empirical weight-at-age (kg) values used for the base model. Colors
  ...
  Data are only available from \Sexpr{start_yr.age.comps}--\Sexpr{last.data.yr}.
  Values based on assumptions for the pre-\Sexpr{start_yr.age.comps} and forecast years are shown
  outside the blue lines.
  Bold values between \Sexpr{start_yr.age.comps}--\Sexpr{max.fishery.age.prop[2]} represent
...
\caption{Sample sizes of empirical weight-at-age measurements
  ...
  pre \Sexpr{start_yr.age.comps} and post \Sexpr{last.data.yr}.
...
\caption{Estimated selectivities for the biomass index (top -- for all years)
  and fishery (bottom -- for \Sexpr{end.yr-1} only)
