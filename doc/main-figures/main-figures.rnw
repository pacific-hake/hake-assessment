...
\end{center}
%\vspace{0mm}
\caption{Density plot showing the MLE recruitment deviate estimates for the \Sexpr{sens.density.yr} cohort for the base model and alternative sensitivity run that sets the full bias ramp adjust period to 2014 instead of 2013.}
...
\end{center}
%\vspace{0mm}
\caption{Maximum likelihood estimates of stock status (relative spawning biomass) for the base model and alternative sensitivity run representing changes in the standard deviation of the selectivity parameters from the base model's value of $\phi=0.20$. See Figure~\ref{fig:main-biomass-base-tv-increase-1} for legend.} % Manual done 2017.
\label{fig:main-status-base-tv-increase-1}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
