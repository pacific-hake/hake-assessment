...
@
\end{center}
\caption{Estimated selectivities for the acoustic survey (top -- for all years)
  and fishery (bottom -- for \Sexpr{end_yr-1} only)
  from the posterior distribution for the base model.
  Selectivity for the acoustic survey is 0 at age-1 and
...
@
\end{center}
\caption{Medians (solid circles) of the posterior distribution of recruitment relative to that in 2010 (recruitment divided by the 2010 recruitment for every MCMC sample), with 95\% posterior credibility intervals (red lines). This procedure scales out the uncertainty due to uncertainty in mean unfished equilibrium recruitment ($R_0$), and better elicits comparisons of relative cohort size while retaining individual time series pairs within aggregated posterior distributions (for example, recruitment in 2014 is clearly smaller than in 2010). The year 2010 was chosen as the basis for comparison due to its well recognized size and the stability of cohort strength estimates over time.}
...
\caption{Time series of relative spawning biomass at the start of each year until \Sexpr{end_yr} as estimated from the base model, and forecast trajectories
  to the start of \Sexpr{forecast_yrs[length(forecast_yrs)]} for several management options from the decision table (grey rectangle), with 95\% posterior
...
@
\end{center}
\caption{MCMC estimates of spawning biomass for the base model and alternative sensitivity runs with maximum age-based selectivity decreased (age-5) or increased (age-7 and age-8) relative to the base model (age-6), and constraining recruitment in the model such that recruitment deviations sum to zero.}
...
\end{center}
%\vspace{0mm}
\caption{MCMC estimates of stock status for the base model and alternative sensitivity runs with maximum age-based selectivity decreased (age-5) or increased (age-7 and age-8) relative to the base model (age-6), and constraining recruitment in the model such that recruitment deviations sum to zero.}
...
@
\end{center}
\caption{MCMC estimates of recruitment for the base model and alternative sensitivity runs with maximum age-based selectivity decreased (age-5) or increased (age-7 and age-8) relative to the base model (age-6), and constraMCMC estimates of recruitment forining recruitment in the model such that recruitment deviations sum to zero.}
...
@
\end{center}
\caption{MCMC estimates of recruitment deviations for the base model and alternative sensitivity runs with maximum age-based selectivity decreased (age-5) or increased (age-7 and age-8) relative to the base model (age-6), and constraining recruitment in the model such that recruitment deviations sum to zero.}
\label{fig:main-recruit-devs-base-alternative-6}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Retrospective models}
%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
\begin{center}
...
of variation but less susceptable to outliers) and is calculated as $\protect{(Q3-Q1) / (Q1+Q3)}$.}
...
The red circles and blue squares represent probabilities calculated analagously
