...
across data types).
Additionally, mean weight-at-age data (not depicted here) are
...
\end{center}
%\vspace{0mm}
\caption{**[check key ones have not changed] Prior (black lines) and posterior (gray histograms) distributions for
