...
make.overview.map.plot(r.loc = file.path("..", "r"))
@
\end{center}
\caption{Overview map of the area in the Northeast Pacific Ocean occupied by \fishname. Common areas referred to in this document are shown.}
...
\vspace{-5mm}
\begin{center}
\includegraphics[max size={\paperwidth}{\paperheight}]{main-figures/hake_survey_1995-15_sA_squareroot_bin_narrow-panel2.eps} % width=\maxwidth,
\end{center}
\vspace{-5mm}
\caption{Spatial distribution of acoustic backscatter attributable to \fishname\ from joint US-Canada acoustic surveys \Sexpr{survey.start.yr}--\Sexpr{survey.end.yr}. Area of the circle is roughly proportional to observed backscatter.}
...
@
\end{center}
\caption{Overview of data used in this assessment, \Sexpr{start.yr}--\Sexpr{last.data.yr}.}
...
make.catches.plot(catches, leg.y.loc = 475)
@
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%
%% Call the figure directly instead of create it because the data are confidential
\begin{figure}[H]
\begin{center}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}
\includegraphics[width=\maxwidth, height=3.5in]{main-figures/fishDepthsUS}
\end{center}
\caption{Distribution of fishing depths (left) and bottom depths (right), in fathoms, of \fishname\ catches in the U.S. at-sea fleet from 2008--\Sexpr{last.data.yr}.}
...
\end{center}
\caption{Unstandardized (raw) catch-rates (t/hr) of \fishname\ catches by tow in the U.S. at-sea fleet in \Sexpr{last.data.yr}.}
...
\caption{Age compositions for the acoustic survey (top) and the aggregate fishery (bottom, all sectors combined)
  ...
  The largest bubble in the survey data is \Sexpr{f(max.survey.age.prop[3] / 100, 2)} for age \Sexpr{max.survey.age.prop[5]}
  in \Sexpr{max.survey.age.prop[4]} and in the fishery is \Sexpr{f(max.fishery.age.prop[3] / 100, 2)} for age \Sexpr{max.survey.age.prop[5]}
...
\begin{figure}[H]
\begin{center}
<<main.survey.biomass, fig.height=6, fig.width=8>>=
...
\begin{figure}[H]
\begin{center}
<<main.survey.extrap.biomass, fig.height=6, fig.width=8>>=
make.survey.biomass.extrap.plot(survey.comparison)
@
\end{center}
\caption{Updated acoustic survey biomass indices with and without extrapolation (millions of metric tons) relative to the index used \Sexpr{last.assess.yr}. Approximate 95\% confidence intervals are based on only sampling variability (and squid/hake apportionment uncertainty in 2009). See Table~\ref{tab:main-survey-comparison} for values used in the base model and the 2016 assessment.}
...
\begin{figure}[p]
\begin{center}
<<age.one.index, fig.height=3.5, fig.width=8>>=
...
\begin{center}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}
\includegraphics[width=6.5in, height=4in]{main-figures/maturityNS}
\end{center}
\caption{Observed proportion mature-at-length (bubbles with circle size relative to number of samples at length), fitted proportion mature-at-length with an estimated asymptote (lines), and number of samples at length (barplots beneath each panel). Panels show categories of source (top left), year (top right), source and year (bottom left), and month (for the trawl survey only, bottom right).}
\label{fig:main-proportion-mature-by-NS}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
%\vspace{0mm}
\caption{Empirical weight-at-age (kg) used in the assessment (numbers, with colors given by the scale at the bottom). Numbers shown in bold were interpolated or extrapolated from adjacent areas.}
...
\begin{figure}[H]
\begin{center}
<<main.bridge.biomass, fig.height=6, fig.width=8>>=
bridge.density.names <- c("SPB_Virgin",
                          "R0",
                          "Main_RecrDev_2010",
                          "Late_RecrDev_2012",
                          "Late_RecrDev_2014",
...
@
\end{center}
\caption{Bridging models comparison showing the \Sexpr{last.assess.yr} base model and the terminal model from sequentially updating all pre-\Sexpr{last.assess.yr} data.  This included updating fishery catch and age-compositions as well as weight-at-age information.  The points disconnected from the time-series on the left side show the unfished equilibrium spawning biomass estimates.}
...
\begin{figure}[H]
\begin{center}
<<main.bridge.5.panel, fig.height=8, fig.width=8>>=
...
@
\end{center}
\caption{Bridging models showing the difference between the \Sexpr{last.assess.yr} base model and the sequential addition of the new acoustic survey time-series (\Sexpr{survey.start.yr}--\Sexpr{last.data.yr}) and then the new \Sexpr{last.data.yr} fishery data.  Spawning biomass (upper panel), relative spawning biomass (spawning biomass in each year relative to the unfished equilibrium spawning biomass, middle left), absolute recruitment (middle right), recruitment deviations (lower left), and survey index (lower right) are shown.}
\label{fig:main-bridge-5-panel}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
% \clearpage
...
\begin{figure}[H]
\begin{center}
<<main.bridge.4.panel, fig.height=8, fig.width=10>>=
oldpar <- par(no.readonly=TRUE)
par(mar = c(2.1, 4.1, 1.1, 1.1), oma = c(2.1, 0, 0, 0))
...
## subplot 10 is recruitment devs with CI
make.comparison.plot(bridge.models.3,
                     subplots = 10,
                     model.names = bridge.model.names.3,
                     densitynames = bridge.density.names,
...
@
\end{center}
\caption{Bridging models showing the difference between the \Sexpr{end.yr} pre-tuned base model, the sequential addition of the main base model tuning runs (adjusting time periods and levels for recruitment bias and reweighting the survey and fishery compositional data), and increased flexibility associated with time-varying selectivity (larger standard deviation associated with temporal deviates). The red line is equivalent to the \Sexpr{end.yr} base model. Spawning biomass (upper left panel), relative spawning biomass (spawning biomass in each year relative to the unfished equilibrium spawning biomass, upper right), absolute recruitment (lower left), and recruitment deviations (lower right) are shown.}
\label{fig:main-bridge-4-panel}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
% \clearpage
...
\begin{figure}[H]
\begin{center}
<<main.mcmc.diag.m.r0, fig.height=4.8, fig.width=8>>=
oldpar <- par(no.readonly=TRUE)
...
\begin{figure}[H]
\begin{center}
<<main.mcmc.diag.h.extra.sd, fig.height=4.8, fig.width=8>>=
oldpar <- par(no.readonly=TRUE)
par(mar=c(4.1, 4.1, 3.1, 0.1))
layout(matrix(c(1,2), nrow = 2, ncol = 1, byrow=TRUE))
...
\caption{Summary of MCMC diagnostics for steepness (upper panels) and the additional standard deviation (SD)
  in the survey index (lower panels) in the base model. Top sub-panels show the trace of the sampled values across iterations (absolute values, top left; cummulative running mean with 5th and 95th percentiles, top right). The lower left sub-panel indicates the autocorrelation present in the chain at different lag times (i.e., distance between samples in the chain), and the lower right sub-panel shows the distribution of the values in the chain (i.e., the marginal density from a smoothed histogram of values in the trace plot).}
...
\begin{figure}[H]
\begin{center}
<<main.mcmc.diag.hists, fig.height=6, fig.width=8>>=
...
\caption{Summary histograms of MCMC diagnostics for all base model parameters along with derived
  quantities for the time-series of spawning biomass, and relative spawning biomass. The level of autocorrelation in the chain (distribution across lag times, i.e. distance between samples in the chain, shown in the top right panel) influences the effective sample size (top left panel) used to estimate posterior distributions. The Geweke statistic (lower left panel) tests for equality between means located in the first part of the chain against means in the last part of the chain. The Heidelberger and Welch statistic (lower right panel) tests if the sampled values come from a stationarity distribution by comparing different sections of the chain.}
...
@
\end{center}
\caption{Posterior correlations among key base-model parameters and derived quantities. Numbers refer to the absolute correlation coefficients, with font size proportional to the square root of the coefficient.}
...
@
\end{center}
\caption{Posterior correlations among recruitment deviations from recent years. Numbers refer to the absolute correlation coefficients, with font size proportional to the square root of the coefficient.}
...
make.age.comp.pearson.plot(base.model)
@
\end{center}
...
\end{center}
%\vspace{0mm}
\caption{Prior (black lines) and posterior (gray histograms) probability distributions for key parameters in the base model. The parameters are: natural mortality ($M$), equilibrium log recruitment $\log(R_0)$, steepness ($h$), and the additional process-error standard deviation for the acoustic survey. The maximum likelihood estimates and associated symmetric uncertainty intervals are also shown (blue lines).}
...
\caption{Estimated acoustic (top - for all years) and fishery selectivity (bottom - for \Sexpr{end.yr-1} only)
  ogives from the posterior distribution for the base model.}
...
\caption{Medians (solid circles) of the posterior distribution for log-scale recruitment deviations with
  ...
  the years 1970--2012 are constrained to sum to zero while deviations outside this range do not have a constraint.}
...
\caption{Estimated stock-recruit relationship for the base model with median predicted recruitments
  ...
  tendency (mean) and the red line the central tendency after bias correcting for the log-normal
...
@
\end{center}
\caption{Bubble plot of maximum likelihood (MLE) estimates of population numbers at age at the beginning of each year, where diagonals follow each year-class through time.
  The red line represents the mean age. The scale of the bubbles is represented in the key where the units are
  billions of fish (with the largest bubble representing about 14 billion age-0 recruits in 1980). See Table~\ref{tab:main-est-numbers-at-age} for values.}
...
@
\end{center}
\caption{Trend in median exploitation fraction (catch divided by biomass of fish of age-3 and above) through \Sexpr{end.yr-1} with 95\% posterior credibility intervals.}
...
\caption{A comparison of maximum likelihood estimates with 95\% confidence intervals determined from asymptotic variance estimates (red)
  ...
  while the posterior mean recruitment is displayed in the lower panel to be more comparable to the MLE value.}
...
#models.names <- sapply(catch.levels, "[[", 2) ## pretty catch level name
...
\caption{Time series of relative spawning biomass at the start of each year until \Sexpr{end.yr} as estimated from the base model, and forecast trajectories
  to the start of \Sexpr{forecast.yrs[length(forecast.yrs)]} for several management options from the decision table (grey region), with 95\% posterior
  credibility intervals. The \Sexpr{end.yr} catch of \Sexpr{f(catch.default.policy[1])}~t was calculated using the default
...
\caption{Graphical representation of the base model results presented in Table \ref{tab:main-risk-year-1} for various catches in
  \Sexpr{forecast.yrs[1]}. The symbols indicate points that were computed directly from model output and lines interpolate between the points.}
...
\caption{Graphical representation of the base model results presented in Table \ref{tab:main-risk-year-2} for catch in
  \Sexpr{forecast.yrs[2]}, given the \Sexpr{forecast.yrs[1]} catch level shown in Table \ref{tab:main-risk-year-1}. The symbols indicate points that were computed directly from model output and lines interpolate between the points.}
...
%% %%%%%%%%%%%%%%%%%%%%%%%
%%%%% old code to include .eps file for non-dynamic version of figure
%% \begin{figure}[H]
%% \begin{center}
%% \includegraphics[width=6.5in, height=4.5in]{main-figures/main_age_comp_forecast.eps}
%% \end{center}
%% \caption{Forecast age compositions in numbers and in weight for the 2017 fishery catch (combined across all sectors in both countries). Gray bars show median estimates. Thick black lines show 50\% credibility intervals and thin black lines show 95\% credibility intervals. These estimates are based on the posterior distribution for selectivity averaged across the most recent 5 years and the distribution for expected numbers at age at the start of 2017 see Table~\ref{tab:main-est-numbers-at-age} for the MLEs for numbers-at-age for all years). The panel on the right is scaled based on the weight at each age averaged across all years.}
...
@
\end{center}
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative sensitivity run that sets the full bias ramp adjust period to 2014 instead of 2013.}
...
%% \begin{figure}[H]
\begin{center}
<<main.recruitment.base.alternative.2, fig.height=4.5, fig.width=8>>=
make.comparison.plot(c(list(base.model), sens.models.2),
                     subplots = 4,
                     model.names = c(paste0("Base model ", end.yr), sens.model.names.2),
                     legend = FALSE,
...
\end{center}
%\vspace{0mm}
\caption{Maximum likelihood estimates of stock status (relative spawning biomass) for the base model and alternative sensitivity run that sets the full bias ramp adjust period to 2014 instead of 2013.}
\label{fig:main-recruitment-base-alternative-2}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
% \clearpage
...
@
\end{center}
\caption{Maximum likelihood estimates of spawning biomass and selectivity for the base model and alternative sensitivity runs representing changes in the age of maximum selectivity from the value of 6 in the base model. Selectivity panels are a) Base model, b) \Sexpr{sens.model.names.3[1]}, c) \Sexpr{sens.model.names.3[2]}, and d) \Sexpr{sens.model.names.3[3]}.}
...
\begin{figure}[H]
\begin{center}
<<main.biomass.base.tv.increase.1, fig.height=4.5, fig.width=8>>=
...
@
\end{center}
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative sensitivity run representing changes in the standard deviation of the selectivity parameters from the base model's value of $\phi=0.20$.} % Manual done 2017.
\label{fig:main-biomass-base-tv-increase-1}
%% \end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%% JOINED ON PAGE
%%%%%%%%%%%%%%%%%%%%%%%
%% \begin{figure}[H]
\begin{center}
<<main.status.base.tv.increase.6, fig.height=4.5, fig.width=8>>=
##sens.density.names <- c(paste0("Late_RecrDev_", sens.density.yr))
oldpar <- par()
...
@
\end{center}
\caption{Density plot showing the MLE recruitment estimates for the 2014 cohort for the base model and alternative sensitivity run representing changes in the standard deviation of the selectivity parameters from the base model's value of $\phi=0.20$.} % Manual done 2017.
\label{fig:main-status-base-tv-increase-6}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
@
\end{center}
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative sensitivity runs representing changes to the standard deviation of recruitment variability from the base model's $\sigma_r=$\Sexpr{sigma.r}.}
...
\end{center}
%\vspace{0mm}
\caption{Maximum likelihood estimates of stock status (relative spawning biomass) for the base model and alternative sensitivity runs representing changes to the standard deviation of recruitment variability from the base model's $\sigma_r=$\Sexpr{sigma.r}. See Figure~\ref{fig:main-biomass-base-alternative-1} for legend.}
...
@
\end{center}
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative sensitivity runs representing changing the mean of the prior for steepness from 1.0 to 0.5, fixing steepness at 1.0, and changing the standard deviation of the prior for natural mortality from 0.1 to 0.2 or 0.3.}
...
\end{center}
%\vspace{0mm}
\caption{Maximum likelihood estimates of stock status (relative spawning biomass) for the base model and alternative sensitivity runs representing changing the mean of the prior for steepness from 1.0 to 0.5, fixing steepness at 1.0, and changing the standard deviation of the prior for natural mortality from 0.1 to 0.2 or 0.3. See Figure~\ref{fig:main-biomass-base-alternative-4} for legend.}
...
\begin{figure}[H]
\begin{center}
<<main.biomass.base.alternative.5, fig.height=4.5, fig.width=8>>=
make.comparison.plot(c(list(base.model), sens.models.5),
                     subplots = 2,
                     model.names = c(paste0("Base model ", end.yr), sens.model.names.5),
                     end.yr = end.yr)
@
\end{center}
\caption{Maximum likelihood estimates of spawning biomass for the base model and alternative sensitivity runs representing time/cohort-invariant ageing error (no downward adjustments for large labelled cohorts), not adjusting the 2014 cohort ageing error downwards (i.e., labelling it a not-very-large cohort at this point) and adding in the age-1 index as a separate data source on recruitment strength.}
\label{fig:main-biomass-base-alternative-5}
%% \end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%% JOINED ON PAGE
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
%\vspace{0mm}
\caption{Maximum likelihood estimates of stock status (relative spawning biomass) for the base model and alternative sensitivity runs representing time/cohort-invariant ageing error (no downward adjustments for large labelled cohorts), not adjusting the 2014 cohort ageing error downwards (i.e., labelling it a not-very-large cohort at this point) and adding in the age-1 index as a separate data source on recruitment strength. See Figure~\ref{fig:main-biomass-base-alternative-5} for legend.}
...
@
\end{center}
\caption{Estimates of spawning biomass at the start of each year (top) and recruitment (bottom) for the base model and retrospective runs (based on MLE model runs).}
...
\end{center}
%\vspace{0mm}
\caption{Retrospective analysis of recruitment deviations from maximum likelihood estimate (MLE) models over the last \Sexpr{length(plot.retro.yrs)+1} years. Recruitment deviations are the log-scale differences between recruitment estimated by the model and expected recruitment from the spawner-recruit relationship. Lines represent estimated recruitment deviations for cohorts from \Sexpr{min(retro.cohorts)} to \Sexpr{max(retro.cohorts)}, with cohort birth year marked at the right of each color-coded line. Values are estimated by models using data available only up to the year in which each cohort was a given age.}
...
\begin{figure}[H]
\begin{center}
<<historical.assessments, fig.height=4, fig.width=6.5>>=
...
\end{center}
%\vspace{0mm}
\caption{Summary of historical \fishname\ assessment estimates of spawning biomass. Shading represents the approximate 95\% confidence range from the \Sexpr{end.yr} base model.}
