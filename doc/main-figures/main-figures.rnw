...
\end{center}
%\vspace{0mm}
\caption{**update (and move blue line) Empirical weight-at-age (kg) used for the base model. Numbers shown in
  bold were interpolated or extrapolated from adjacent ages or years. Data are
  only available from 1975-**2017 (between the two blue lines). Remaining values
  are used for the early and late years, with alternative assumptions tested in
  sensitivity runs.}
...
\caption{**Bridging models showing the \Sexpr{last.assess.yr} base model
  and the results of updating the historic catch, adding the 2018 catch
  (the only data available for 2018), adding the 2018 weights-at-age as the
  average from 2015-2017 (and running the model to 2018), changing the
  initial value used for steepness, and using the 2018 weights-at-age (rather
  than the overall mean) for projections.
  The points disconnected from the time-series on the
...
\caption{**Bridging models showing the difference between the
  \Sexpr{last.assess.yr} base model, the result of the first set of changes
  shown in Figure~\ref{fig:main-bridge-biomass}
  and the incorporation of time-varying fecundity.
...
\caption{**Maximum likelihood estimates of spawning biomass for the base
  ...
  (1.8) levels of sigma-R, and changing the standard deviation of the prior for natural mortality from
  0.1 to 0.2 or 0.3.}
...
\caption{**Maximum likelihood estimates of stock status (relative
  ...
  fixing steepness at 1.0, lower (1.0) and higher
  (1.8) levels of sigma-R, and changing the standard deviation of the prior
  for natural mortality from 0.1 to 0.2 or 0.3. See
  Figure~\ref{fig:main-biomass-base-alternative-1} for legend.}
...
\caption{**Maximum likelihood estimates of spawning biomass for the base model and alternative
  sensitivity runs that represent changes in data: adding an age-1 index of abundance,
  incorporating ageing error that is cohort invariant, using the harmonic mean approach to
  weight composition data, accounting for seasonal age sampling in the aggregation of age
  composition information, and removing catch which was double-counted for the
  Canadian JV fishery and adding U.S. tribal catch which was missed for 2016 and 2017.}
...
\caption{**Maximum likelihood estimates of spawning biomass for the base model
  and alternative sensitivity runs representing the use of a semi-parametric approach
  for implementing time-varying selectivity (sigma-S).}
...
%\vspace{0mm}
\caption{**Maximum likelihood estimates of stock status (relative
  spawning biomass) for the base model and alternative sensitivity runs representing the use of a semi-parametric approach
  for implementing time-varying selectivity (sigma-S).
...
\end{center}
\caption{**Maximum likelihood estimates of recruitment deviations
for the base model and alternative sensitivity runs representing the use of a semi-parametric approach
  for implementing time-varying selectivity (sigma-S).
  See Figure~\ref{fig:main-biomass-base-alternative-4} for legend.}
...
# Run 32 (same as base)
weight.at.age.heatmap(sens.models.5[[1]])
@
\end{center}
%\vspace{0mm}
\caption{**Andy to update run numbers and blue line. Empirical weight-at-age (kg) used for sensitivity run 32 and the base
  model (Figure~\ref{fig:main-empirical.weight.at.age}).
  Numbers shown in
  bold were interpolated or extrapolated from adjacent ages or years. Data are
  only available from 1975-2017 (between the two blue lines). Remaining values
  are used for the early and late years.}
...
# Run 33 (verified same heatmap as 34)
weight.at.age.heatmap(sens.models.5[[2]])
@
\end{center}
%\vspace{0mm}
\caption{**Andy to update run numbers and blue line. Empirical weight-at-age (kg) used for sensitivity runs 33 and 34.
  Numbers shown in
  bold were interpolated or extrapolated from adjacent ages or years. Data are
  only available from 1975-2017 (between the two blue lines). Remaining values
  are used for the early and late years.}
...
# Run 36 (verified same heatmap as 38)
weight.at.age.heatmap(sens.models.5[[7]])
@
\end{center}
%\vspace{0mm}
\caption{**Andy to update run numbers and blue line. Empirical weight-at-age (kg) used for sensitivity runs
  36 and 38. (**confirm they match once 36 runs okay - wtatage.ss files are good). Numbers shown in
  bold were interpolated or extrapolated from adjacent ages or years. Data are
  only available from 1975-2017 (between the two blue lines). Remaining values
  are used for the early and late years.}
...
# Run 37 (verified same heatmap as 35)
weight.at.age.heatmap(sens.models.5[[4]])
@
\end{center}
%\vspace{0mm}
\caption{**Andy to update run numbers and blue line. Empirical weight-at-age (kg) used for sensitivity runs
  35 and 37. Numbers shown in
  bold were interpolated or extrapolated from adjacent ages or years. Data are
  only available from 1975-2017 (between the two blue lines). Remaining values
  are used for the early and late years.}
...
\end{center}
\caption{**Maximum likelihood estimates of spawning biomass for the base
  model and alternative sensitivity run that sets ....(use run numbers and refer
  to table) the unfished and
  pre-1975 weights-at-age (and therefore fecundity) to the mean from 1975-1979.}
...
\caption{**Maximum likelihood estimates of stock status (relative
  ...
  and alternative sensitivity run that sets the unfished and
  pre-1975 weights-at-age (and therefore fecundity) to the mean from 1975-1979.
...
\end{center}
\caption{**Maximum likelihood estimates of spawning biomass for the base
  model and alternative sensitivity runs that set the **TO UPDATEunfished and
  pre-1975 weights-at-age (and therefore fecundity) to the mean from 1975-1979.}
...
\caption{**Maximum likelihood estimates of stock status (relative
  spawning biomass) for the base model
  and alternative sensitivity runs that set **TO UPDATE the unfished and
  pre-1975 weights-at-age (and therefore fecundity) to the mean from 1975-1979.
  Figure~\ref{fig:main-biomass-base-alternative-6} for legend.}
