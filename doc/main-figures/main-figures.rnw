...
% \begin{figure}[H]
% \begin{center}
% <<main-bridge-5-panel-3, fig.height=8, fig.width=8, is.fig = TRUE>>=
% oldpar <- par(no.readonly=TRUE)
% par(mar = c(2.1, 4.1, 1.1, 1.1), oma = c(2.1, 0, 0, 0))
% layout(matrix(c(1,1,2,3,4,5), nrow = 3, ncol = 2, byrow=TRUE))
% bridge.density.names <- c("SSB_Virgin",
...
% @
% \end{center}
% \caption{Bridging models starting with the addition of 2019 fishery compositions
%   and weights (shown
...
%   relative to the unfished equilibrium spawning biomass, middle left),
%   absolute recruitment (middle right), recruitment deviations (lower left),
%   and survey index (lower right). For models without \Sexpr{assess.yr -1} data,
%   estimates shown for \Sexpr{assess.yr} are forecasts.}
