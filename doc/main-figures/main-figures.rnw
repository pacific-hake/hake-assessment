...
\end{center}
%\vspace{0mm}
\caption{Unstandardized (raw) catch-rates (t/hr) of Pacific Hake catches by tow in the U.S. at-sea fleet in 2015.}
\label{fig:main-us-catch-rates}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\begin{center}
<<main.bridge.5.panel, fig.height=6, fig.width=8>>=
...
@
\end{center}
\caption{Bridge models showing the difference between the  \Sexpr{last.assess.yr} base model (blue) and the sequential addition of the new acoustic survey time-series (\Sexpr{survey.start.yr}-\Sexpr{last.assess.yr}; red) and then the new \Sexpr{last.assess.yr} fishery data (green).  Spawning biomass (upper panel), relative spawning biomass (spawning biomass in each year relative to the unfished equilibrium spawning biomass, middle left), absolute recruitment (middle right), recruitment deviations (lower left), and survey index (lower right) are shown.}
...
\begin{figure}[H]
\begin{center}
<<main.bridge.4.panel, fig.height=6, fig.width=10>>=
oldpar <- par(no.readonly=TRUE)
par(mar = c(2.1, 4.1, 1.1, 1.1), oma = c(2.1, 0, 0, 0))
...
@
\end{center}
\caption{Bridge models showing the difference between the \Sexpr{end.yr} pre-tuned base model (blue) and the sequential addition of the main base model tuning runs (adjusting time periods and levels for recruitment bias [red] and reweighting the survey [yellow] and fishery [red] compositional data).  The red line is equivalent to the \Sexpr{end.yr} base model.  Spawning biomass (upper left panel), relative spawning biomass (spawning biomass in each year relative to the unfished equilibrium spawning biomass, upper right), absolute recruitment (lower left), and recruitment deviations (lower right) are shown.}
