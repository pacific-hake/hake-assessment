...
\end{center}
%\vspace{0mm}
\caption{Empirical weight-at-age (kg) values (colors) used for the base model.
  Transparency, which is age-specific, highlights years with the heaviest fish for a given age.
...
\caption{Sample sizes for developing empirical weight-at-age values
  (colors, as shown in Figure ~\ref{fig:main-empirical-weight-at-age})
