...
plist[[1]] <-
  plot_rel_biomass(d_obj = d_obj_bridge_rel_biomass[[1]],
                   x_breaks = c(seq(1970,
                                    ...
                                    by = 10)),
                   ylim = c(0, 2.5),
                   y_labels = expression("0", "0.1B"[0], "0.4B"[0], "0.5",
                                         "B"[0], "1.5", "2", "2.5"),
                   leg_pos = "none")
plist[[2]] <-
...
# TODO: Error in model$retros[[i]] : subscript out of bounds
# par(mfrow = c(3,1))
...
\begin{figure}[H]
\begin{center}
<<historical-retro-4, fig.height=3, fig.width=8, is.fig = TRUE, alt.text = "This figure compares historical estimates of probabilities of the stock declining with those from the current base model and for retrospective analyses.">>=
make.historical.probs.retro.plot(base_model,
                                 select.retros = 10,
                                 make.one.figure = FALSE,
                                 legend.cex = 0.6,
...
>>>>>>> 9a09910d (Add a bunch of new plots)
%%%%%%%%%%%%%%%%%%%%%%%
% \begin{figure}[H]
~ \\
\begin{center}
