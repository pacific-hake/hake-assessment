...
sens.density.year <- last.data.yr - 1
...
\end{center}
%\vspace{0mm}
\caption{Density plot showing the MLE recruitment deviate estimates for the \Sexpr{sens.density.year} cohort for the base model and alternative sensitivity runs representing no extrapolation on the acoustic survey estimate and inclusion of an age-1 index.}
...
make.selex.plot(models[[sens.model.inds.3[1]]], pan.letter = "b")
make.selex.plot(models[[sens.model.inds.3[2]]], pan.letter = "c")
make.selex.plot(models[[sens.model.inds.3[3]]], pan.letter = "d")
par <- oldpar
mtext("Age", side = 1, line = 1, outer = TRUE)
@
\end{center}
\caption{Maximum likelihood estimates of spawning biomass and selectivity for the base model and alternative sensitivity runs representing changes age of maximum selectivity from the value of 6 in the base model. Selectivity panels are a) Base model, b) \Sexpr{sens.model.names.3[1]}, c) \Sexpr{sens.model.names.3[2]}, d) \Sexpr{sens.model.names.3[3]}}
