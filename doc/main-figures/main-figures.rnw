...
\end{center}
\caption{Bridging models showing the \Sexpr{last.assess.yr} base model
  and the results of sequentially building upon the base model with updates shown in
  Figure~\ref{fig:main-bridge-biomass} (i.e., adding the 2018 catch data and 
  2018 age-composition and weight-at-age data) and then finally changing to
...
\caption{Forecast age compositions in numbers and in weight for the \Sexpr{end.yr} fishery catch
  ...
  the weight at each age averaged across 1975 to \Sexpr{last.data.yr}.}
...
\caption{Empirical weight-at-age (kg) values used for sensitivity
  ...
  Values based on assumptions for the early and late years are shown
  outside the blue lines (see Table~\ref{tab:main-wtatage-runs}).
  Bold values between the blue lines represent unavailable data and 
...
\caption{Empirical weight-at-age (kg) values used for sensitivity
  ...
  the 2018 values have to be assumed (the location of the top blue line is different to
