...
\pdftooltip{\includegraphics[max size={1.15\paperwidth}{1.15\paperheight}]
  {main-figures/hake_survey_1995-19_NASCTimeSeries_BiomassAtAgeHistograms_grayblue}}{This
  figure shows that \fishname\ migrate further north in some years}
...
\begin{center}
\hspace{-10mm}
\pdftooltip{\includegraphics[max size={1.10\paperwidth}{1.10\paperheight}]
   {main-figures/age1hake_03-19_sA_squareroot_bin_narrow-panel_bluergray}}{This
   figure shows the assumed varying locations of age-1 \fishname}
   % Download from Google Drive
\end{center}
%\vspace{-5mm}
...
\begin{figure}[H]
\begin{center}
<<main-total-catches, fig.height=5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that catches in the last 5 years have been the largest in the time series">>=
...
\pdftooltip{\includegraphics[width=0.75\paperwidth]{main-figures/fishCatchRatesUS}}{This
  figure shows changes in catch rates for the U.S. fleet}
\end{center}
...
\begin{figure}[H]
\begin{center}
<<main-data-overview-map, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that catches span the whole time series, ages from 1975 on, and the survey from 1995 on">>=
...
\begin{figure}[H]
\begin{center}
<<main-survey-extrap-biomass, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the survey estimate has increased from 2017 to 2019">>=
...
\begin{figure}[H]
\begin{center}
<<main-age-one-index, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the age-1 index has dropped from 2017 to 2019">>=
...
\caption{**Sample sizes of empirical weight-at-age measurements
  ...
  pre \Sexpr{start.yr.age.comps} and post \Sexpr{last.data.yr}.
...
@
\end{center}
\caption{**[Mention in text, standardize wording and axis label with preceding two figures,
  (empirical instead of 'annual mean'?),
  maybe add tick marks]. Annual mean weight-at-age by age (colors for ages one through ten) through time. Blue lines are for the youngest ages and green lines are for the oldest ages shown.}
...
\begin{figure}[H]
\begin{center}
<<main-bridge-5-panel-1, fig.height=8, fig.width=8, is.fig = TRUE, alt.text = "This figure shows the minimal differences between the first bridging models for various model outputs">>=
...
box()
# TODO: This should be MCMC but they all need to be extra-mcmc also
# make.comparison.plot(bridge.models.1,
#                      subplots = 14,
#                      model.names = bridge.model.names.1,
#                      legend = TRUE,
...
\caption{**Bridging models showing the \Sexpr{last.assess.yr} base model
  ...
  and changing the estimation of the Dirichlet Multinomial parameters to the final
  estimation phase. Moving the data weighting parameters to the final phase
  more accurately reflects the timing used in other methods that manually
  tune data weights. Panels are spawning biomass
...
\begin{figure}[H]
\begin{center}
<<main-bridge-5-panel-2, fig.height=8, fig.width=8, is.fig = TRUE, alt.text = "This figure shows the minimal differences between the first bridging models for various model outputs">>=
oldpar <- par(no.readonly=TRUE)
par(mar = c(2.1, 4.1, 1.1, 1.1), oma = c(2.1, 0, 0, 0))
#layout(matrix(c(1,1,2,3,4,5), nrow = 3, ncol = 2, byrow=TRUE))
layout(matrix(c(1,1,2,3,4,4), nrow = 3, ncol = 2, byrow=TRUE))
...
box()
# TODO: This should be bridge.models.2, but that fails due to age1-index having more than one index.
#       Also this should be MCMC but that breaks
# make.comparison.plot(bridge.models.1,
#                      subplots = 13,
#                      model.names = bridge.model.names.2,
#                      densitynames = bridge.density.names,
...
@
\end{center}
\caption{**Bridging models showing the sequential addition of updating pre-2020
  fishery data, adding 2020 catch data, and adding 2020 weight-at-age information, and
  adding 2020 fishery composition data starting from the final bridge model
  in Figure \ref{fig:main-bridge-5-panel-1}. Panels are spawning biomass
  (upper panel), relative spawning biomass (spawning biomass in each year
  relative to the unfished equilibrium spawning biomass, middle left),
  absolute recruitment (middle right), recruitment deviations (lower left),
  and survey index (lower right).}
...
@
\end{center}
\caption{**Fits (colored lines) to the age-1 index estimated from the
  acoustic survey (points) with input 95\% intervals around the observations.
  The thin blue lines are the results of a random subset of individual MCMC samples.
  Thicker uncertainty intervals around observed survey points indicate 95\%
  log-normal uncertainty intervals estimated by the [**still correct to say for
  age-1??]
  kriging method and are used as input to the assessment model.
  Thinner uncertainty intervals indicate estimated 95\% uncertainty intervals that account for the
  model estimate of additional uncertainty.}
...
make_key_posteriors_mcmc_priors_vs_posts_plot(base.model,
                                              key.posteriors,
                                              hist_breaks = c(50, 50, 50, 50, 500, 500),
                                              ncol = 2,
                                              nrow = 3,
                                              show_legend = TRUE,
                                              legend_panel = 3,
...
\caption{**Prior (black lines) and posterior (gray histograms) distributions for
  ...
  the survey ($\theta_{\text{surv}}$). The maximum likelihood estimates and
  associated symmetric uncertainty intervals are also shown (blue lines). There
...
\begin{figure}[H]
\begin{center}
<<main-spawning-biomass, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the stock has been decreasing in size since 2017">>=
...
%% \begin{figure}[H]
\begin{center}
<<main-relative-spawning-biomass, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the relative biomass has been decreasing in size since 2017">>=
...
\begin{figure}[H]
\begin{center}
<<main-recruitment, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows the last three large cohorts' recruitment: 2010, 2014, and 2016">>=
...
%% \begin{figure}[H]
\begin{center}
<<main-recruitment-devs, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows the last three large recruitment deviations: 2010, 2014, and 2016">>=
...
\caption{**Medians (solid circles) of the posterior distribution for log-scale recruitment deviations with
  ...
  are used to calculate the numbers at age in \Sexpr{start.yr}, the initial year of the model.}
...
%% \begin{figure}[H]
\begin{center}
<<main-forecast-depletion-comparison-plot, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the median relative spawning biomass is projected to decline for all non-zero catch levels evaluated">>=
...
\begin{figure}[H]
\begin{center}
<<main-forecast-risk-comparison-plot-year-1, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that there is a high probability that the stock will decline next year regardless of catch level">>=
...
%% \begin{figure}[H]
\begin{center}
<<main-forecast-risk-comparison-plot-year-2, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that there is a high probability that the stock will decline in the next two years regardless of catch level">>=
make.forecast.risk.comparison.plot(base.model,
                                   forecast_yrs = forecast_yrs,
...
\begin{figure}[H]
\begin{center}
<<main-forecast-risk-comparison-plot-year-3, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that there is a high probability that the stock will decline in the next three years regardless of catch level">>=
make.forecast.risk.comparison.plot(base.model,
                                   forecast_yrs = forecast_yrs,
...
\begin{figure}[H]
\begin{center}
<<main-index-base-alternative-2, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows the similarity between some sensitivity runs of the estimates of the fit to the survey index">>=
make.comparison.plot(c(list(base.model), sens.models.2),
                     subplots = 14,
                     model.names = c(base.model.name, sens.model.names.2),
                     legend = TRUE,
                     legendloc = "bottomright",  # seems to get ignored
                     indexfleets= c(2),
                     end.yr = end.yr)
@
\end{center}
\caption{**MCMC estimates of the fit to the survey index of abundance for the base
  model and alternative sensitivity runs that represent changes in data.
  See Figure~\ref{fig:main-biomass-base-alternative-2} for sensitivity descriptions.}
...
\begin{figure}[H]
\begin{center}
<<main-index-base-alternative-4, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows differences in the fit to the survey index for certain sensitivity runs">>=
make.comparison.plot(c(list(base.model), sens.models.4),
                     subplots = 14,
                     model.names = c(base.model.name, sens.model.names.4),
                     legend = TRUE,
...
\end{center}
%\vspace{0mm}
\caption{**MCMC estimates of the fit to the survey index of abundance
...
\begin{figure}[H]
\begin{center}
<<main-biomass-base-alternative-5, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows how spawning biomass changes between ageing-error related sensitivity runs">>=
...
\end{center}
\caption{**MCMC estimates of spawning biomass for the base
  model and alternative sensitivity run with cohort-based ageing error replaced
  with a time-invariant ageing error vector.}
...
%% \begin{figure}[H]%%%%%%%%%%%%%%%%%%%%%%%
\begin{center}
<<main-status-base-alternative-5, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows how relative spawning biomass changes between ageing-error related sensitivity runs">>=
make.comparison.plot(c(list(base.model), list(sens.models.5)),
                     subplots = 4,
                     model.names = c(base.model.name, sens.model.names.5),
                     legend = TRUE,
...
retro_cohorts <- (end.yr - 11):(end.yr - 2)
...
\begin{figure}[H]
\begin{center}
<<main-relative-retrospective-recruitment, fig.height=6, fig.width=8, is.fig = TRUE, alt.text = "The estimates of recruitment deviations scaled to the most recent estimate almost all trend towards zero by age three, except for the 2015 cohort, which was a strong negative recruitment deviation. This figure provides support for not estimating recruitment of fish until they are seen in the fishery and the survey given current selectivity.">>=
make_squid_plot(base.model,
                subplot = 2,
                cohorts = retro_cohorts,
                plot_mcmc = TRUE)
...
@
\end{center}
\caption{**Retrospective versions of Figure~\ref{fig:main-historical-1} for first
  three retrospectives (rephrase). **May not include all these, just wanted
  to get them constructed and add options. Will take a bit of explaining
  but they seem interesting.
  For each panel, the current base model is run using only data up to the year
  shown (i.e. a retrospective analysis). Results are shown in a single panel in
  Figure~\ref{fig:main-historical-retro-all}. Final retro point should in theory
...
@
\end{center}
\caption{**Retrospective versions of Figure~\ref{fig:main-historical-1} for second
  three retrospectives. The pink 2015 value for 2016 retro is interesting (for
  2021 assessment results - interesting if changes with new model -- check).
  Only one for
  which newer model with data to year $t$
  performs 'worse' than original assessment from year $t$. Looking at
  squid plot I think it may possibly be the slight drop in 2010 retro from
  age 5 to 6 (a big abs decrease in estimated 2010 rec
  when going from 2015 data to 2016); maybe think about more, may
  be too hard to tease out (and those thoughts may be wrong).
  For each panel, the current base model is run using only data up to the year
  shown (i.e. a retrospective analysis). Results are shown in a single panel in
...
@
\end{center}
\caption{**Retrospective versions of Figure~\ref{fig:main-historical-1} for final
  retrospectives **can also bump select.retros to 10 when have updated base
  model results, maybe just add to next plot.
  For each panel, the current base model is run using only data up to the year
  shown (i.e. a retrospective analysis). Results are shown in a single panel in
  Figure~\ref{fig:main-historical-retro-all}.}
...
\caption{**Retrospective results of
  Figures~\ref{fig:main-historical-retro-1}--\ref{fig:main-historical-retro-3}
...
@
\end{center}
\caption{**Retrospective results of **Figure~\ref{fig:main-historical-2}, just
  showing all in a single panel. Intriguing that the retro estimate
  with data up to 2011 (first
  black square) is not close to that from the 2012 assessment (that uses the
  same data). Does this tell us that it's model changes not data changes?}
