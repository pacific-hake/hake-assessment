...
\begin{figure}[p]
\begin{center}
\includegraphics[max size={1.08\paperwidth}{1.08\paperheight}]
  {main-figures/age2plushake_03-17_NASC_BiomassAtAge_color.eps}
\end{center}
\vspace{-3mm}
\caption{Spatial distribution of acoustic backscatter attributable to age-2 and older \fishname\
  from joint US-Canada acoustic surveys
...
\vspace{-5mm}
\begin{center}
\includegraphics[max size={\paperwidth}{\paperheight}]
%  {main-figures/age1hake_03-17_NASC_narrow-panel.eps}
   {main-figures/age1-use-me.eps}     % Download from Google Drive
\end{center}
\vspace{-5mm}
\caption{Spatial distribution of acoustic backscatter attributable to age-1 \fishname\
  from joint US-Canada acoustic surveys 2003 --\Sexpr{survey.end.yr}. Age-1 \fishname\ are not fully sampled
  during the acoustic survey and were not explicitly considered during establishment of the
  survey sampling design. The area of the circle is roughly
...
\end{center}
\caption{Distribution of fishing depths (left) and bottom depths (right), in fathoms, of \fishname\ catches in the U.S. at-sea fleet from 2014--\Sexpr{last.data.yr}.}
...
make.age.comp.bubble.plot(base.model,
                          subplot = 2,
                          show.key = TRUE,
                          key.yrs = c(1985, 1991, 1997, 2003),
                          start.yr = min(base.model$dat$agecomp$"Yr"),
                          end.yr = max(base.model$dat$agecomp$"Yr"))
make.age.comp.bubble.plot(base.model, subplot = 1,
                          start.yr = min(base.model$dat$agecomp$"Yr"),
                          end.yr = max(base.model$dat$agecomp$"Yr"))
@
\end{center}
\caption{**[check numbers make sense]Age compositions for the acoustic survey (top) and the aggregate fishery (bottom, all sectors combined)
...
\begin{figure}[H]
\begin{center}
<<main.survey.biomass, fig.height=4.5, fig.width=8>>=
...
\caption{**Fraction of fish that are mature at each age north and south of
  34.44$^\circ$N
  (upper panel) and comparison of old and new fecundity relationships
  (lower panel). The old fecundity relationship (black line in lower plot)
  was a product of the
  maturity-at-length reported by \citet{DornSaunders1997} and the
  weight-at-length estimated in 2011, converted to age using a parametric
  growth curve estimated in 2011.
  The new fecundity relationship (purple line) is the product
  of the mean weight-at-age
  averaged across all years (green line) and the maturity-at-age for the samples
...
\caption{Empirical weight-at-age (kg) values used for the base model
  (the values match the colours).
  Data are only available from 1975--\Sexpr{max.fishery.age.prop[2]}.
  Values based on assumptions for the early and late years are shown
  outside the blue lines (see Table~\ref{tab:main-wtatage-runs});
  alternative assumptions were tested in sensitivity runs.
  Bold values between 1975--\Sexpr{max.fishery.age.prop[2]} represent unavailable
  data such that weights were interpolated or extrapolated from adjacent ages or years.
...
\caption{Sample sizes for developing empirical weight-at-age values used in the
  base model (the values match the colours).
  Data are available from 1975--\Sexpr{max.fishery.age.prop[2]}.}
...
\caption{Fit (red lines) to yearly weight-at-age data (standard box plots) by
   age group (age-2 to 15; where age-15 is an accumulator group) using weighted
   least squares.  Statistical weights in the regression were based on the
   inverse of the annual sample variance.}
...
\caption{Bridging models showing the \Sexpr{last.assess.yr} base model
  and the results of sequentially updating historic catch data, historic age composition data,
  and then historic weight-at-age data.  The points disconnected from the time-series on the
...
\end{center}
\caption{Bridging models showing the \Sexpr{last.assess.yr} base model
  and the results of sequentially building upon updates shown in
  Figure~\ref{fig:main-bridge-biomass} by adding the 2018 catch data, adding
  the 2018 age composition and weight-at-age data, and then finally changing to
  time-varying fecundity which became the base model. See
  Section~\ref{sec:assessment-weights} for further details about modeling
  time-varying fecundity.  Panels are spawning biomass
...
\label{fig:main-mcmc.diag.gelrub}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
@
\end{center}
\caption{Pearson residuals for base model MLE fits to the age
...
@
\end{center}
\caption{Medians (solid circles) and means ($\times$) of the posterior distribution for recruitment (billions of age-0) with
...
\caption{Estimated historical path followed by medians of relative fishing
  ...
  labeled, as is the year with the highest relative fishing intensity.
...
\caption{Illustration of parameterization of time-varying selectivity as
  ...
  as described by equations (1) through (3).
...
\begin{figure}[H]
\begin{center}
<<main.recruit.base.alternative.4, fig.height=4.5, fig.width=8>>=
make.comparison.plot(c(list(base.model), sens.models.4),
                     legend = FALSE,,
                     subplots = 10,
                     model.names = c(base.model.name, sens.model.names.4),
...
%% heatmaps for sensitivity runs - need to re-order once decided on an order
%% (see GitHub Issue).
%% 53 = base, 55 = 57, 54 = 52, 58 = 56
%% Do 32 first to verify that it's same as base - be handy to have here to
%% compare to the next ones. Need the MCMC ones also. And do short names to
%% read okay in captions.

%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
\begin{center}
% Using .a since doesn't correspond to a sens group
<<main.weight.at.age.sens.a, fig.height=9, fig.width=8>>=
# Run 53 (same as base). Not worth automating caption.
weight.at.age.heatmap(sens.models.5[[2]])
@
\end{center}
\caption{Empirical weight-at-age (kg) values used for sensitivity
  run~53 and the base
  model (Figure~\ref{fig:main-empirical.weight.at.age}).
  Data are only available from 1975--\Sexpr{max.fishery.age.prop[2]}.
  Values based on assumptions for the early and late years are shown
  outside the blue lines (see Table~\ref{tab:main-wtatage-runs}).
  Bold values represent unavailable data -- between the blue lines
  such weights were interpolated or extrapolated from adjacent ages or years.
  The sample-weighted mean weight-at-age is shown at the bottom.}
\label{fig:main-empirical.weight.at.age.sens.a}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\begin{center}
<<main.weight.at.age.sens.b, fig.height=9, fig.width=8>>=
# Run 55 (verified same heatmap as 57)
weight.at.age.heatmap(sens.models.6[[1]], proj.line.yr=2017)
@
\end{center}
\caption{Empirical weight-at-age (kg) values used for sensitivity
  runs~55 and~57. Details are as in
  Figure~\ref{fig:main-empirical.weight.at.age.sens.a}. Note
  the 2018 values have to be assumed (the top blue line is different to
  Figure~\ref{fig:main-empirical.weight.at.age.sens.a}), as described
  in Table~\ref{tab:main-wtatage-runs}.}
\label{fig:main-empirical.weight.at.age.sens.b}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\begin{center}
<<main.weight.at.age.sens.c, fig.height=9, fig.width=8>>=
# Run 54 (verified same heatmap as 52)
weight.at.age.heatmap(sens.models.5[[3]], proj.line.yr=2017)
@
\end{center}
\caption{Empirical weight-at-age (kg) values used for sensitivity
  runs~52 and~54. Details are as in
  Figure~\ref{fig:main-empirical.weight.at.age.sens.a}. Note
  the 2018 values have to be assumed (the top blue line is different to
  Figure~\ref{fig:main-empirical.weight.at.age.sens.a}), as described
  in Table~\ref{tab:main-wtatage-runs}.}
\label{fig:main-empirical.weight.at.age.sens.c}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\begin{center}
<<main.weight.at.age.sens.d, fig.height=9, fig.width=8>>=
# Run 56 (verified same heatmap as 58)
weight.at.age.heatmap(sens.models.6[[2]], proj.line.yr=2017)
@
\end{center}
\caption{Empirical weight-at-age (kg) used for sensitivity
  runs~56 and~58. Details are as in
  Figure~\ref{fig:main-empirical.weight.at.age.sens.a}. Note
  the 2018 values have to be assumed (the top blue line is different to
  Figure~\ref{fig:main-empirical.weight.at.age.sens.a}), as described
  in Table~\ref{tab:main-wtatage-runs}.}
\label{fig:main-empirical.weight.at.age.sens.d}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
\caption{Maximum likelihood estimates of spawning biomass for the base
  model and alternative sensitivity runs 52, 53 (base 2018), and 54 that all set
  the pre-1975 (1966-1974) weights-at-age to the long term mean (1975--2018).
  Run 52 has non-time-varying fecundity and the short term mean (2016--2018)
  as the post-2017 weights-at-age. Run 53 is the base model from 2018 without
  time-varying fecundity and with long term (1975--2018) mean as the post-2018
  weights-at-age. Run 54 has time-varying fecundity and the short term mean
  (2016--2018) as the post-2017 weights-at-age.
  See Table~\ref{tab:main-wtatage-runs} for descriptions of runs.}
...
%\begin{figure}[H]
\begin{center}
<<main.biomass.base.alternative.5.mcmc, fig.height=4.5, fig.width=8>>=
...
@
\end{center}
\caption{MCMC median posterior estimates with 95\% credible interval of spawning
  biomass for the base model and alternative sensitivity runs 52, 53 (base 2018),
  and 54. See Figure~\ref{fig:main-biomass-base-alternative-5} and
  Table~\ref{tab:main-wtatage-runs} for descriptions of runs.}
\label{fig:main-biomass-base-alternative-5-mcmc}
\end{figure}
...
\caption{Maximum likelihood estimates of stock status for the base
  model and alternative sensitivity runs 52, 53 (base 2018), and 54.
  See Figure~\ref{fig:main-biomass-base-alternative-5} and
  Table~\ref{tab:main-wtatage-runs} for descriptions of runs.}
\label{fig:main-status-base-alternative-5}

\begin{center}
<<main.status.base.alternative.5.mcmc, fig.height=4.5, fig.width=8>>=
make.comparison.plot.mcmc(c(list(base.model), sens.models.5),
                     subplots = 4,
                     model.names = c(base.model.name, sens.model.names.5),
                     legend = FALSE,
...
%\vspace{0mm}
\caption{MCMC median posterior estimates with 95\% credible interval of spawning
  biomass for the base model and alternative sensitivity models 52, 53 (base 2018),
  and 54. See Figure~\ref{fig:main-biomass-base-alternative-5} and
  Table~\ref{tab:main-wtatage-runs} for descriptions of runs.}
\label{fig:main-status-base-alternative-5-mcmc}
\end{figure}
...
\begin{figure}[H]
\begin{center}
<<main.biomass.base.alternative.6, fig.height=4.5, fig.width=8>>=
...
\end{center}
\caption{Maximum likelihood estimates of spawning biomass for the base
  model and alternative sensitivity runs 55, 56, 57, and 58 that all set
  the pre-1975 (1966-1974) weights-at-age to the short term mean (1975--1979).
  Run 55 has time-varying fecundity and the long term mean (1975--2018)
  as the post-2017 weights-at-age. Run 56 has non-time-varying fecundity
  and with short term (2016--2018) mean as the post-2017 weights-at-age.
  Run 57 has non-time-varying fecundity and the long term mean
  (1975--2018) as the post-2018 weights-at-age. Run 58 has time-varying
  fecundity and the short term (2016--2018) mean as the post-2017 weights-at-age.
  See Table~\ref{tab:main-wtatage-runs} for descriptions of runs.}
\label{fig:main-biomass-base-alternative-6}

\begin{center}
<<main.biomass.base.alternative.6.mcmc, fig.height=4.5, fig.width=8>>=
...
\end{center}
\caption{MCMC median posterior estimates with 95\% credible interval of spawning
  biomass for the base model and alternative sensitivity runs 55, 56, 57, and 58.
  See Figure~\ref{fig:main-biomass-base-alternative-6} and
  Table~\ref{tab:main-wtatage-runs} for descriptions of models.}
\label{fig:main-biomass-base-alternative-6-mcmc}
\end{figure}
