...
\item there have only been two past recruitment events (1980 and 1984) that might be as large as 2010.
  ...
  could also possibly be that large, giving an
...
estimated parameters). Since the random draws are independent from year to year,
...
Given this analysis, we can include the resulting Figure~\ref{fig:recruitment-scale-all}
