...
**Wasn't time in 2022 virtual meeting to discuss this. Can easily be updated for
this year (though first figure won't be from 2021 assessment, can leave out). JTC to discuss.
...
make.recruitment.plot(last_yr_base_model,
...
@
\end{center}
\caption{Recruitment Figure~e (or Figure~26) from the \Sexpr{assess_yr - 1} assessment \citep{JTC2021}.
...
figure from the \Sexpr{assess_yr - 1} assessment \citep{JTC2021}, reproduced
...
% In issue I did: mean(base_model$mcmc$"Recr_2014" > base_model$mcmc$"Recr_2010")
...
\begin{figure}[tbp]
\begin{center}
<<recruitment-scale, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows recent distributions of recruitment estimates relative to those in 2010">>=
make.mcmc.recruitment.plot(base_model,
                           start_yr = 2006,
                           equil.yr = unfished_eq_yr,
                           samples = NULL,
...
\begin{figure}[tbp]
\begin{center}
<<recruitment-scale-all, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows distributions of recruitment estimates relative to those in 2010 for all years">>=
make.mcmc.recruitment.plot(base_model,
                           start_yr = 1966,
                           equil.yr = unfished_eq_yr,
                           samples = NULL,
...
@
\end{center}
\caption{Estimated recruitments divided by the estimated 2010 recruitment. The same as Figure~\ref{fig:recruitment-scale}, but for all years.}
