...
setwd("r")
source("r-functions/utilities.r") ## Contains remove.all.objects.except function
remove.all.objects.except(c("models"))

## Load all R code
source("all.r")

## If you want to reload all model outputs, uncomment next line
## rm(models)

## Load the model outputs if they haven't been loaded yet.
if(!exists("models")){
  models <- create.models.list(model.dir, model.dir.names)
}
## See model-setup.r for the order these should be in
base.model <- models[[1]]
last.yr.base.model <- models[[2]]
bridge.models.1 <- models[[3]]
bridge.models.2 <- models[[4]]
bridge.models.3 <- models[[5]]
sens.models.1 <- models[[6]]
sens.models.2 <- models[[7]]
sens.models.3 <- models[[8]]
sens.models.4 <- models[[9]]
sens.models.5 <- models[[10]]
## Load in variables used in \Sexpr{} expressions in text.
## Needs to come after the models have been defined
source("custom-knitr-variables.r")

## Load the model-partest which was previously run for the base model.
if(!exists("model.partest")){
...
\pagenumbering{roman}
%% \lfoot{\fishname\ stock assessment \Sexpr{assess.yr} \today} %% Species common name for left footer
\lfoot{\fishname\ assessment \today} %% Species common name for left footer
\rfoot{DRAFT - DO NOT CITE} %% The area of the assessment for right footer

%% Authors page
This document reports the collaborative efforts of the official U.S. and Canadian JTC members, and others that contributed significantly.

Authors of this document are (in no particular order): \\
\hspace{4ex}Ian G. Taylor\supscr{2} \\
\hspace{4ex}Aaron Berger\supscr{3} \\
\hspace{4ex}Andrew M. Edwards\supscr{1} \\
\hspace{4ex}Chris J. Grandin\supscr{1} \\
\hspace{4ex}Sean Cox\supscr{4} \\
\bigskip

\emph{\supscr{1}Fisheries and Oceans Canada, Pacific Biological Station,
  3190 Hammond Bay Road, Nanaimo, BC V9T 6N7, Canada}

\emph{\supscr{2}Northwest Fisheries Science Center, National Marine Fisheries Service, National Oceanic and Atmospheric Administration, U.S. Department of Commerce.
...
% Once finalised, uncomment these and update:
% This document should be cited as follows: Grandin, C.J., A.C. Hicks, A. Berger, A.M. Edwards, N. Taylor, S. Cox, and I.G. Taylor. 2015.
