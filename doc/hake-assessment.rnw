...
** - THIS INDCIATES a paragraph, or sentence that needs to be thought about this year
and double checked (i.e. isn't fully automated, may just need someone to have a
quick think). Should end up with no ** by the
end (including here!). We'll still read through everything, but the ** indicates
what definitely needs some thinking.
