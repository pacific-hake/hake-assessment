...
\usepackage{hake}
\usepackage[titletoc,title]{appendix}   % title gives 'Appendix A', not just 'A', but doesn't seem to work.
...
\lstset{breakatwhitespace=TRUE,numberstyle=\tiny\color{gray},title=\lstname}
   % ,language=C++}  % ,font=pcr}   no font option???
...
\lhead{}
\rhead{}
\lfoot{Pacific Hake stock assessment 2016} %% Species common name for left footer
...
##  on its own.
load("./r/model-partest.RData")
@

%% Title page and author references page
\title{\TitleFont DRAFT - Status of the Pacific Hake (whiting) stock in U.S. and Canadian waters in \Sexpr{assess.yr}}
\author{Joint Technical Committee of the Pacific Hake/Whiting Agreement \\ Between the Governments of the United States and Canada}
\maketitle
\newpage

%% Authors page
This document reports the collaborative efforts of the official U.S. and Canadian JTC members.

Authors of this document are (In no particular order): \\
\hspace{4ex}Allan C. Hicks\supscr{1} \\
\hspace{4ex}Aaron Berger\supscr{1} \\
\hspace{4ex}Ian G. Taylor\supscr{1} \\
\hspace{4ex}Nathan Taylor\supscr{2} \\
\hspace{4ex}Chris Grandin\supscr{2} \\
\hspace{4ex}Andrew Edwards\supscr{2} \\
\hspace{4ex}Sean Cox\supscr{3} \\
\bigskip
...
\emph{\supscr{3}School of Resource and Environmental Management, Simon Fraser University, TASC I -- Room \#8405, 8888 University Drive, Burnaby, B.C. V5A-1S6, Canada}

This document should be cited as follows: Taylor, I.G., C. Grandin, A.C. Hicks, N. Taylor, and S. Cox. 2015.
Status of the Pacific Hake (whiting) stock in U.S. and Canadian waters in \Sexpr{assess.yr}.
Prepared by the Joint Technical Committee of the U.S. and Canada Pacific Hake/Whiting Agreement;
National Marine Fishery Service; Canada Department of Fisheries and Oceans. 159~p.
\newpage
...
% To get the References labelled as a numbered section
\renewcommand{\bibsection}{\section{REFERENCES}}

%%\subtitle{Joint Technical Committee of the Pacific Hake/Whiting Agreement Between the Governments of the United States and Canada}
...
% Keeping track of missing references here; this section will get
%  delted once its empty.
<<missing-refs, child='missing-references/missing-references.rnw'>>=
@

\clearpage

%% \addcontentsline{toc}{section}{REFERENCES}   % May get page number wrong
% \phantomsection \addcontentsline{toc}{subsection}{REFERENCES} %sorted above
\bibliographystyle{resDoc}
\bibliography{all}                             %  if starting a new page
...
% Tried using appendix package, so may not need these (AME's manual fix),
%  but either way treats appendices as chapters and formats them differently
%  to sections in the Table of Contents (bold and no dots). Could try and fix
...
<<appendix-SS-data-file, child='appendix-SS-data-file/appendix-SS-data-file.rnw'>>=
@ 

% <<appendix-SS-control-file, child='appendix-SS-control-file/appendix-SS-control-file.rnw'>>=
% @ 

%<<appendix-SS-starter-file, child='appendix-SS-starter-file/appendix-SS-starter-file.rnw'>>=
%@ 

%<<appendix-SS-forecast-file, child='appendix-SS-forecast-file/appendix-SS-forecast-file.rnw'>>=
%@ 
