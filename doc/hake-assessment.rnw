...
%% Allows the numbering of figures/tables to be unique within a section
...
%% For figures in chapter subdi rectories
\usepackage{import}
...
\begin{document}
\emergencystretch 3em    % https://tex.stackexchange.com/questions/9107/how-can-i-make-my-text-never-go-over-the-right-margin-by-always-hyphenating-or-b
\counterwithin{figure}{section}
\counterwithin{table}{section}

<<load-everything, echo=FALSE,  message=FALSE, results='hide', warning=FALSE>>=
# Load all `hake` package data
...
opts_chunk$set(dev = "png",
               ...
               comment = "#>",
               results = "hide",
               fig.path = here::here("doc", "knitr-cache", "/"),
               cache.path = here::here("doc", "knitr-cache", "/"),
               cache = TRUE)
...
knit_hooks$set(alt.text = function(before, options, envir){
  if(before && options$is.fig){
    # Add first line of non-comment code and the alt text to a data frame
    code_chunk <- options$code
    # Remove leading whitespace from all lines
    code_chunk <- gsub("^[[:space:]]+", "\\1", code_chunk)
    # Remove all comment lines
    code_chunk <- code_chunk[!grepl("^#", code_chunk)]
    # Select the first line that is a function call (may have leading
    # package name and double-colon)
    code_chunk <- code_chunk[grepl("^[:a-zA-Z0-9_\\.]+\\(", code_chunk)]
    # Remove all instances of par() and layout() as they always comes before
    # the function call
    code_chunk <- code_chunk[!grepl("^par\\(", code_chunk)]
    code_chunk <- code_chunk[!grepl("^layout\\(", code_chunk)]
    if(length(code_chunk)){
      alt_text <- ifelse(is.null(options$alt.text),
                         "Alt text not provided",
                         options$alt.text)
      alt_fig_text <<- bind_rows(alt_fig_text,
                                 c(code = code_chunk[1],
                                   text = alt_text))
    }
  }
...
# Default chunk options needed for the alt-text routine to work right
opts_chunk$set(is.fig = FALSE)
opts_chunk$set(alt.text = "")
@

% Load packages needed by the document code
% <<load-globals, child='load-document-variables/00-load-packages.rnw'>>=
% @

% Load important starting globals required by other scripts
<<load-globals, child='load-document-variables/01-load-globals.rnw'>>=
@

% Load all data tables into global variables
<<load-data-tables, child='load-document-variables/02-load-data-tables.rnw'>>=
@

% Load all models here
<<load-models, child='load-document-variables/03-load-models.rnw'>>=
@

% Load/create all R variables used in the document text
% any of these are based on the base model, sensitivity models, and bridge models
% so those must be loaded first (see previous chunk)
<<load-knitr-variables, child='load-document-variables/04-load-knitr-variables.rnw'>>=
@

% Title page and author references page
\thispagestyle{empty}

\title{\TitleFont \mypdftitle~in \Sexpr{assess_yr}
%% Uncomment for DRAFT:
% \\ ~\\ DRAFT for submission to Scientific Review Group\thanks{\noindent \disclaimer}
\\ ~\\ ~\\ \pdftooltip{\includegraphics[width=4in]{main-figures/hake-picture}}{Drawing of a \fishname}}
...
% ** For figures and tables, only flagging captions that will need
% weaking. Assume we will go through the figures and tables carefully anyway when
% new results are incorporated.

%% Final version (with standard author rotation)
This document should be cited as follows:\\
Berger, A.M., C.J. Grandin, K.F. Johnson and A.M. Edwards.
\Sexpr{assess_yr}. Status of the \fishname\ (whiting) stock in
U.S. and Canadian waters in \Sexpr{assess_yr}. Prepared by the Joint Technical
...
\lfoot{
       %% DRAFT -- \fishname\ assessment \Sexpr{assess_yr}
       \fishname\ assessment \Sexpr{assess_yr}
       }
