...
\title{\TitleFont Status of the \fishname\ (whiting) stock in U.S. and Canadian
  waters in \Sexpr{assess_yr}
%% Uncomment for DRAFT:
\\ ~\\ DRAFT for submission to Scientific Review Group\thanks{\noindent \disclaimer}
\\ ~\\ ~\\ \pdftooltip{\includegraphics[width=4in]{hake-picture}}{Drawing of a \fishname}}
...
% \rfoot{Appendix~\ref{chap:add-forecast} -- Additional forecast catch levels}
% <<appendix-add-forecast, child='appendix-add-forecast/appendix-add-forecast.rnw'>>=
% @

% Can include these listings for the draft if we want to but keep them commented out for
% the final submission. There are over 630 unannotated tags created by this appendix
% listing so we fix the issue by just not listing them in the final version.
