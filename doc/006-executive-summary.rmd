...
to better inform fecundity. The only data point that can directly inform the size of
...
recruitment deviations (i.e., deviations fixed at zero, giving average recruitment). For consistency, this was also done for 2024 and the
...
Also, the relative index of age-1 fish from the survey was not used to fit the
model this year
...
from the model (which in particularly further reduced the estimated 2022 year class).
...
recruitments, and removal of the age-1 index.
...
year and a modelled age of 2 (observed as age-1 fish, though the age-1 index is
now not fit to) with a survey, giving
further motivation for fixing recruitment deviations to 0 for `r end_yr - 2` onwards.
...
below 1.0 from 2012--2015 and again in 2020, 2021, and 2023
(Figure~\@ref(fig:es-fishing-intensity-fig)).
...
above) peaked in 1999 and 2006 (Figure~\@ref(fig:es-exploitation-fraction-fig)).
...
TODO check first statement here when have full build -- cannot tell from figure:

The median relative spawning biomass was above the `r b_40`
reference level in all years except 2007--2011
...
fishery and survey, typically at a minimum age of three).
However, the changes in assumptions of recent recruitments and removal of the
age-1 index have reduced overall uncertainty in this assessment.
In particular, while the model estimates the 2020 and 2021 cohorts as
above average in size and their absolute size remains highly uncertain, the
...
spawning biomass. The 2023 acoustic survey provided additional
information on the size of the 2020 year-class (as well as informed the 2021 year
class).
Collectively, these lessened uncertainty around estimates of female
spawning biomass, as did the changes in model assumptions. Information on the size of the 2023 year class from the 2024
fishery was considered suspect given the unique fishing conditions during the
year and so, as mentioned earlier, recruitment deviations from 2023 onwards were
set to zero. Further, the impact of interactions among variance parameters that
...
uncertainty for projections appears to be less than for previous assessments.
...
in `r assess_yr + 1` or `r assess_yr + 2` because it is conditional on previous year(s) catch
levels.
...
With zero catch for the next three years and average recruitment from 2023 onward, the biomass has a
...
Figure~\@ref(fig:es-risk-year-1-fig)), a
...
Figure~\@ref(fig:es-risk-year-1-fig)). As the above average 2014 and 2016
