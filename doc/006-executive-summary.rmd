...
and minor changes to pre-`r end_yr-1` data. In addition, a new model-based
approach was used to develop the input weight-at-age matrix, and time-varying
temperature-dependent maturity was introduced to better inform fecundity.
...
growth-mortality transition during a period of high catches. The increases in
female spawning biomass from 2023--2025 are due to the expected above-average
2021 cohort entering maturity and the recent declining trend in catch.
...
(`r prev_bio_lower`--`r prev_bio_upper`)~Mt, giving less uncertainty than
...
median is reduced from that in the `r end_yr - 1` assessment due to the tail of the
distribution being greatly curtailed and a
slight lowering of the lower end of the interval.
...
years of data and thus was highly uncertain. It suggested the 2020 cohort could
...
but now with information from the
age-2$+$ biomass index and survey age-composition data the 2020 cohort looks to be
less but still above average (95% interval:
...
The 2021 recruitment is estimated to be potentially large, whereas
it was estimated to be below average in last year's assessment (with very limited data);
...
`r sp` recruitment is highly uncertain, and estimates for recent years (based on
