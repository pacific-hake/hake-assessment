...
most even years (odd year classes). There is no data to inform the size of
...
and minor changes to pre-`r end_yr-1` data. In addition, a model-based approach was
used to develop the input weight-at-age matrix, and time-varying maturity was
added to better inform fecundity.
...
(`r prev_bio_lower_last_assess`--`r prev_bio_upper_last_assess`)~Mt. The current median is reduced from last
year, partly due to the tail of the distribution being greatly curtailed, and a
...
(prev_assess_recruitment_med)["2020"]) * 100), 0)`% decrease; though last year's
estimate was particularly uncertain), and is now informed by the
age-2$+$ biomass index and survey age-composition data.
...
recruitment to be unreliable prior to at least a modelled age of three
...
From 2020 the Canadian rate steadily declined to a time-series low of 16.5% in 2023, and
the U.S. rate fell to 59.7% in 2023.
...
The median relative spawning biomass was above the 0.4
...
HR) in 2024 results in a median relative fishing intensity of 0.95 (Table~\@ref(tab:es-decisions-spr-tab)).
...
1% for a `r end_yr` catch level similar to that for `r end_yr - 1`
(scenario f: `r end_yr - 1` catch). For all explored catches, the maximum
probability of female spawning biomass at the start of `r end_yr + 1` dropping
below `r b_10` is 0% and of dropping below `r b_40` is 6%
(Table~\@ref(tab:es-risk-year-1-tab) and
Figure~\@ref(fig:es-risk-year-1-fig)).
As the above average 2014 and 2016 cohorts continue to age, total biomass of these
cohorts even without fishing mortality is expected to decrease as losses
from mortality outweigh increases from growth.
The estimated above-average (yet still highly uncertain) 2020 and 2021 cohorts will
