...
(`r survey_start_yr`--`r survey_end_yr`), a relative index of age-1 fish
(Figure~\@ref(fig:es-survey-age1-fig); `r survey_start_yr`--`r survey_end_yr`),
...
particularly large numbers of age-1 fish in 2009, 2011, and 2015 (2008,
2010, and 2014 year classes, respectively), and is not available for
most even years (odd year classes, Figure~\@ref(fig:es-survey-age1-fig)).
There are no survey data to inform the size of the `r last_data_yr-1` and
`r last_data_yr` year classes.
...
recruitments in such years also included a random component (extra variability
around the stock-recruitment relationship), yielding a large range of
recruitments.
...
was followed by a brief increase to a peak in 2003 as the very large 1999 year
...
The estimated biomass mostly declined
...
(`r prev_bio_lower`--`r prev_bio_upper`)~Mt, giving a slightly narrower range of
uncertainty compared to the estimate from the `r end_yr - 1` assessment of `r prev_bio_median_last_assess`
(`r prev_bio_lower_last_assess`--`r prev_bio_upper_last_assess`)~Mt. This
median is reduced from last year's assessment, partly due to the influence of the age-2$+$ biomass index
pulling down the estimated biomass for recent years, the reduction in the estimate of
the 2022 year class with updated data, and the aforementioned fixed zero
recruitment deviation for the 2023 recruitment.
...
have occurred since then in 2014, 2016 and 2021, which have been sustaining the fishery over the
last decade, with small year classes for all other years from 2011--2019
...
However, estimates of absolute recruitment for the most recent years can change
with new data and updated assumptions about recent recruitments. The estimate of 2020 recruitment is above average, whereas in the previous
...
by the age-1 index in 2023, suggesting it is average to below average in size.
There are no survey data available to inform the estimates of
the `r end_yr - 2`--`r end_yr` year class sizes, for which recruitment
...
year and a modelled age of 2 (observed as age-1 fish) with a survey, giving
...
However, interpolation is not appropriate for the Default HR scenario
