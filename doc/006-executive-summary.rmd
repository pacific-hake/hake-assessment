...
that cohort of
`r f(cohort_catch_1999 / 1e6, 2)` Mt [1~Mt = 1~megatonne = 1~million metric
tonnes]. Through `r last_data_yr`, the
cumulative catches of the 2010, 2014, and 2016 year classes were
...
near the time series average in 2017. The survey shows a decline from 2019
(the fourth highest of the series) to 2023 (the third lowest of the series).
...
Over the last five years, the median relative fishing intensity was the
highest in 2019 at `r median_intensity_2019`% of the target level of
`r fspr_40` (1.0), dropping by
`r abs(as.numeric(median_intensity_2020) - as.numeric(median_intensity_2019))`%
to `r median_intensity_2020`% in 2020, then remaining stable for 2021 and 2022
...
fishing intensity dropped again by
`r abs(as.numeric(median_intensity_2022) - as.numeric(median_intensity_2023))`%
to `r median_intensity_2023`% of the target level of `r fspr_40`
...
below `r fspr_40` from 2012--2016 and again in 2020--2023
(Figure~\@ref(fig:es-fishing-intensity-fig)).
...
(scenario n: default HR) in 2024 results in a median relative fishing
intensity of `r fi_at_hcr` (Table~\@ref(tab:es-decisions-spr-tab)).
...
table_decision(
  ...
  caption = paste0(
      ...
      "and are a constant value across all forecasted years unless otherwise ",
      "defined in the first column. Acronyms are defined in the glossary ",
...
`r end_yr + 1` is `r prob_decl_last_yr_catch`% for a `r end_yr` catch level
similar to that for `r end_yr - 1` (scenario e: `r end_yr - 1` catch). For
all explored catches, the maximum probability of female spawning biomass at
...
from growth. The estimated above-average (yet still highly uncertain) 2020
and 2021 cohorts will continue to play a large role in determining female
