...
fishing at the `r fspr_40` default harvest-rate catch limit (scenario n: default
...
`r end_yr + 1` is below 76% for all catch levels examined
(Table~\@ref(tab:es-risk-year-1-tab) and
Figure~\@ref(fig:es-risk-year-1-fig)). This probability is
36% for a `r end_yr` catch level similar to that for `r end_yr - 1`
(scenario e: `r end_yr - 1` catch). For all explored catches, the maximum
probability of female spawning biomass at the start of `r end_yr + 1` dropping
below `r b_10` is 0% and of dropping below `r b_40` is 7%
(Table~\@ref(tab:es-risk-year-1-tab) and
Figure~\@ref(fig:es-risk-year-1-fig)).
