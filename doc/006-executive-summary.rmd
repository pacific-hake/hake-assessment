...
retained and improved upon with time-varying estimates of maturity.
...
`r f(base_model$mcmccalcs$rmed["2020"], 1)`~billion fish based on
...
In 2023, the relative fishing intensity dropped further
to `r median_intensity_2023`%
...
`r exploitation_med_2021` to `r exploitation_med_2024` in 2024, which is a
...
year and was therefore set according to the stock--recruitment relationship
(no estimated recruitment deviate). Further, the interactions among variance parameters that
...
However, interpolation is only appropriate for the constant catch scenarios in `r assess_yr + 1`
or `r assess_yr + 2` (i.e., not the Default HR scenario) because it is conditional on previous year(s) catch
levels. This explains why probabilities can sometimes decline (rather than rise) with
...
`r forecast_yrs[length(forecast_yrs)]` for several management actions, which
...
`r forecast_yrs[1]` to `r forecast_yrs[2]` (Table~\@ref(tab:es-risk-year-1-tab);
Figure~\@ref(fig:es-depletion-comparison-fig)), a
