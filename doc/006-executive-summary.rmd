...
model <- base_model
# Filter out the forecast catch streams that are not used in the
# decision table
...
table_decision(
  ...
  type = "biomass",
  forecast_inds =
    seq_along(model$forecasts[[length(model$forecasts)]]),
  caption = paste0(
...
```{r es-decisions-biomass-inc-fi-and-stable-catch-tab, results = "asis", echo = FALSE, eval = FALSE}

table_decision(
  base_model,
  type = "biomass",
  inc_fi_stable_catch = TRUE,
  forecast_inds =
...
```{r es-decisions-spr-inc-fi-and-stable-catch-tab, results = "asis", echo = FALSE, eval = FALSE}
...
model$ct_levels <- model$ct_levels[c(1:10, 12)]
...
plot_depl_fore_comparison(model,
