...
\Sexpr{number_to_word(assess_yr - 2011, th = TRUE)} annual stock assessment
...
age-\Sexpr{us_age_1_prop_age_shore} (\Sexpr{us_age_1_prop_shore}\%),
age-\Sexpr{us_age_2_prop_age_shore} (\Sexpr{us_age_2_prop_shore}\%),
age-\Sexpr{us_age_3_prop_age_shore} (\Sexpr{us_age_3_prop_shore}\%), and
age-\Sexpr{us_age_4_prop_age_shore} (\Sexpr{us_age_4_prop_shore}\%);
...
landed \Sexpr{last_yr_can_shore}~t, the lowest on record since 1990.
The freezer-trawler component,
which freezes headed and gutted product while at sea, landed
\Sexpr{last_yr_can_freezer}~t. There was no Joint-Venture fishery in
\Sexpr{assess_yr - 1}.
