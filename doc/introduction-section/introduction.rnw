...
of this \Sexpr{assess_yr} base model,
...
\Sexpr{last.assess_yr} base model \citep{JTC2020}.  The exception is that the
\Sexpr{assess_yr} base model includes an age-1 index from the acoustic survey.
...
Responses to \Sexpr{last.assess_yr} SRG requests are in
...
agreement on the coast-wide TAC could not be reached in 2020 or 2021, and so catch targets were set unilaterally
...
Starting in 1996, the Makah Tribe has conducted a fishery with a specified
...
The coast-wide TAC of \Sexpr{last.year.tac}~t for \Sexpr{last_data_yr}
was specified as the sum of unilateral TAC decisions due to the lack of
a bilateral agreement in \Sexpr{last_data_yr}. The U.S. catch target
...
The historical
catch of \fishname\ for \Sexpr{start_yr}--\Sexpr{last_data_yr} by nation and
...
A review of the \Sexpr{last_data_yr} fishery now
follows by nation.
...
In both U.S. at-sea sectors, age-5 and age-7 fish, associated with the 2016 and 2014 year classes,
were the most common ages.
Age-2 fish were again not seen in appreciable numbers in the catch this year.
Age sampling was conducted on \Sexpr{us.age.n.cp} catcher-processor hauls and \Sexpr{us.age.n.ms}
mothership hauls (Table~\ref{tab:main-sampling-history}).
...
Age-samples from \Sexpr{us.shore.age[us.shore.age$year == last_data_yr, "n.trips"]} shoreside trips
...
in \Sexpr{last_data_yr} (Table~\ref{tab:main-us-shore-age-data}).
...
\Sexpr{third.shoreside.age.prop}\%, and age-\Sexpr{fourth.shoreside.age.prop.age}
at \Sexpr{fourth.shoreside.age.prop}\%.
