...
First, a nearly uniform prior (in transformed space) was applied to the Dirichlet-Multinomial parameters used to weight the age-composition data.
Second, the constraint that estimated recruitment deviations sum to zero was removed.
...
%some developed for
Responses to \Sexpr{last.assess.yr} SRG requests appear in
(see Section~\ref{sec:assessment-response-review}) and a
...
**Usually paraphrase from Appendix from Canadian AP, so wait for that.
...
**[currently all come out as NA] The most abundant year classes (by numbers) in the Canadian Freezer trawler
catch were age
