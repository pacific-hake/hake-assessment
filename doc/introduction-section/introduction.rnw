...
the previous year and catch rates were considerably higher throughout the year
(Figure~\ref{fig:main-us-catch-rates}). Initial database extractions reported minor amounts
of hake catch by tribal fisheries in \Sexpr{last.data.yr}.  However, the
U.S. advisory panel report on the \Sexpr{last.data.yr} fishery
(Appendix~\ref{chap:us-fishery-report}) indicated a tribal catch of 6,012 mt.
...
in the regional (PacFIN) database. These discrepencies were not resolved until late in the
assessment preparation process, thereby precluding an update to the overall
catch this year (see section~\ref{sec:data-total-catch}).  However, this amount of catch 
is negligible relative to the total catch, and a sensitivity to the inclusion of this 
catch in the \Sexpr{assess.yr} base model resulted in negligible influence on model 
results and subsequent management forecasts. The catcher-processor, mothership, and
...
fishery was age-7 and age-3 fish associated with the 2010 and 2014 year-classes, 
...
rates.  The at-sea fleets sometimes fished in shallower water than observed in the past 
