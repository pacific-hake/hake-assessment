...
are the same as the \Sexpr{last_assess_yr} base model \citep{JTC2020}.
...
the northern boundary related to fluctuations in annual migration \citep{JTC2013}
...
across thier distribution \citep{MalickHunsickerEtAl2020}. The distribution of age-1 fish also changes
...
predator \citep{JTC2013}. Previous research developed an index of abundance for
Humboldt Squid (\emph{Dosidicus gigas}) and suggested that the abundance of
\fishname\ decreased with increasing squid abundance \citep{StewartHazenEtAl2014},
and has also evaluated distribution, recruitment, and growth patterns of \fishname\
in relation to oceanographic conditions for assessment and
...
further research on this topic is still needed. New research has developed a
krill (i.e., euphausiids; \emph{Euphausiacea}) abundance time
series \citep{Phillips2022} and suggests that temperature dynamically
influences hake--krill co-occurrence \citep{Phillips2023}, which can influence
annual /fishname/ growth and recruitment as the availability of key prey species shifts.
Ongoing research investigating abiotic (environmental conditions) and biotic
(e.g., maturity and diet) drivers of the distribution,
recruitment, growth, and survival of \fishname\ could provide insight into how
the population is linked with broader ecosystem considerations. For example,
...
quotas were an average of \Sexpr{tot.9399.attainment}\% of the limit. The Agreement
...
Agreement. Catch shares distributions as specified in the Agreement were once
...
were allocated a TAC of \Sexpr{last.year.can.tac}~t, which did not include any carryover
quota.
%of which included
...
workers on mothership vessels. The exported value of \fishname\ was U.S.
\$127~million in 2020, including to Ukraine, Nigeria, and Italy, which make up
...
2020 was U.S. \$289 million in income and 3,950 U.S. jobs.

The Joint Management Committee (JMC) determined an adjusted (for carryovers)
coast-wide TAC of \Sexpr{last.year.tac}~t for \Sexpr{last_data_yr}. The U.S.
catch target was set at \Sexpr{last.year.us.tac}~t and the Canadian catch target
at \Sexpr{last.year.can.tac}~t. The historical catch of \fishname\ for
...
reports (Appendicies~\ref{chap:us-fishery-report}--\ref{chap:canada-fishery-report}).
...
(\Sexpr{last.year.us.attained}\%) continued to be maintained close to what it
has been in recent years (see Appendix~\ref{chap:us-fishery-report} for more
details). There was \Sexpr{last.year.us.not.attained}\% of the total U.S.
adjusted TAC that was not caught. The catcher-processor, mothership, and
...
shoreside sector totals were \Sexpr{f(last.year.us.ti.catch)}~t. Monthly catch
...
\Sexpr{us.age.n.cp} catcher-processor hauls and \Sexpr{us.age.n.ms} mothership
...
(Table~\ref{tab:main-us-shore-age-data}). Age-composition differences between
at-sea and shoreside fleets were larger than usual due, in part, to
spatiotemporal fleet dynamics during the 2022 fishing year. For example, the
timing of the at-sea \fishname\ fishery shifted due to low Bering Sea pollock
quotas, and the fleet stayed south along the
...
\Sexpr{last.year.can.landings}~t from Canadian waters
(Table~\ref{tab:intro-catches-Can}), which was \Sexpr{last.year.can.attained}\%
of the Canadian TAC of \Sexpr{last.year.can.tac}~t. The attainment for Canada
...
the Canadian fleet is that abundance in Candian-waters was down in 2022, including
areas outside of Southwest Vancouver Island. The freezer trawlers fished in
considerably shallower areas than last year and at shallower depths, where the shoreside
...
Canadian shoreside catch were
age-\Sexpr{max.shoreside.age.prop.age} (\Sexpr{max.shoreside.age.prop}\%),
age-\Sexpr{second.shoreside.age.prop.age} (\Sexpr{second.shoreside.age.prop}\%),
age-\Sexpr{third.shoreside.age.prop.age} (\Sexpr{third.shoreside.age.prop}\%), and
age-\Sexpr{fourth.shoreside.age.prop.age} (\Sexpr{fourth.shoreside.age.prop}\%)
...
(\Sexpr{second.freezer.trawler.age.prop}\%), age
\Sexpr{third.freezer.trawler.age.prop.age}
(\Sexpr{third.freezer.trawler.age.prop}\%), and age
\Sexpr{fourth.freezer.trawler.age.prop.age}
