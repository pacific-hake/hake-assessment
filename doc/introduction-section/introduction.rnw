...
primarily upon an acoustic survey index of biomass time series for information on
the scale of the current population. Age-composition data from the aggregated
...
information regarding changes in abundance and places a lower bound
...
are largely the same as the \Sexpr{last_assess_yr} base model \citep{JTC2020}.
...
conditions (colder water, such as in 2001) result in a southward shift in their
...
between years (Figure~\ref{fig:main-backscatter-map-age1}). **Maybe add any new refs?
...
and has evaluated distribution, recruitment, and growth patterns of \fishname\
...
\Sexpr{last_data_yr - 1}. Canadian priority lies with the domestic fishery, but
...
component. However, due to staffing issues, there were no observers available to
...
Dockside Monitoring Program, but because of staffing
issues only four samples were taken in 2021.
...
**[newer numbers may not be available] In 2020, \fishname\ was Canada's largest commercial wild fishery (species with
the largest catch),
...
% Manual updated 2022; Email Erin Steiner
...
\subsubsection{Overview of the fisheries in \Sexpr{last_data_yr}}\label{sec:intro-fisheries-overview}
...
Section~\ref{sec:assessment-model-results}). A review of the
\Sexpr{last_data_yr} fishery now follows by nation.
...
which was slightly higher, and August, which typically has a catch rate of zero
...
(Table~\ref{tab:main-us-shore-age-data}).
...
appears much lower than usual, due to the fishing vessels having a difficult
...
Fishing started in early April and ended in mid-October. The general view from
the Canadian fleet is that general abundance was down in 2022, especially in
areas outside of Southwest Vancouver Island. The freezer trawlers fished in
shallower areas than last year and at shallower depths, where the shoreside
vessels fished their gear at deeper depths than in 2021
(Figure~\ref{fig:main-can-at-sea-depths}). Finding fish was difficult in 2022.
The fish caught in Canada appeared to be mostly from three age classes (ages 5,
6, and 8), with very few smaller fish (less than 500 grams) caught.
...
age-\Sexpr{max.shoreside.age.prop.age} at \Sexpr{max.shoreside.age.prop}\%,
age-\Sexpr{second.shoreside.age.prop.age} at \Sexpr{second.shoreside.age.prop}\%,
age-\Sexpr{third.shoreside.age.prop.age} at \Sexpr{third.shoreside.age.prop}\%, and
age-\Sexpr{fourth.shoreside.age.prop.age} at \Sexpr{fourth.shoreside.age.prop}\%.
