...
are thoroughly described in this assessment document. The structural assumptions
of this \Sexpr{assess.yr} base model are effectively the same as the
\Sexpr{last.assess.yr} base model, including time-varying fishery selectivity.
...
\citep{AlheitPitcher1995, LlorisMatallanasEtAl2005}. The coastal stock of
...
(Figure~\ref{fig:main-backscatter-map}). The research on links between migration
...
Act in the United States and the declaration of a 200~mile fishery conservation
zone in both countries in the late 1970s, annual quotas (or catch targets) have
been used to limit the catch of \fishname\ in both zones. Scientists from both
...
targets reasonably well (Table~\ref{tab:main-landings-tac}). Since 1999, catch
...
allocations in the form of IFQs to the shore-based sector and group shares to
...
managed with Individual Fishing Quotas (IFQ).
...
codends from Canadian catcher vessels while at sea. The last joint-venture
program was conducted in \Sexpr{latest.year.can.jv}.
...
A more detailed description of the history of the fishery is provided in the
2013 \fishname\ stock assessment \citep{JTC2013}.
...
and~\ref{tab:intro-catches-total}. A review of the \Sexpr{last.data.yr} fishery
follows.
...
respectively.
% Manual done 2017. Is all good for 2017 providing the %ages have not changed.
...
the previous year and catch rates were more stable throughout the year
(Figure~\ref{fig:main-us-catch-rates}). Initial database extractions reported no
landings of hake by tribal fisheries in \Sexpr{last.data.yr}.  However, the
U.S. advisory panel report on the \Sexpr{last.data.yr} fishery
(Appendix~\ref{chap:us-fishery-report}) indicated a tribal catch of 2,470 mt.
The Joint Technical Committee was not made aware of this catch until late in the
assessment preparation process, thereby precluding an update to the overall
catch this year.  However, this amount of catch is negligible relative to the
total catch and thus would have negligible influence on model results and
subsequent management forecasts. The catcher-processor, mothership, and
...
% Manual done 2017, best I can tell the below is still correct.

In both U.S. at-sea sectors (CP and MS) the most common cohort in the spring
fishery was age-6 fish associated with the 2010 year-class, but by the fall,
both sectors were catching a majority of age-2 fish from the 2014 cohort. In
total, 47\% of the CP catch was age-2 and 36\% was age-6
(Table~\ref{tab:main-us-cp-age-data}). For the MS sector, the total for the year
was 59\% age-2 and 29\% age-6 (Table~\ref{tab:main-us-ms-age-data}). These
totals were based on samples from over 500 hauls in each sector
(Table~\ref{tab:main-sampling-history}).  Age-samples from 59 shoreside trips
showed an even higher proportion of age-2 fish than the at-sea sectors, at 62\%,
with 24\% of the shoreside samples coming from the 2010 year class
(Table~\ref{tab:main-us-shore-age-data}). Age-4 fish from the 2012 year-class
were the third largest proportion in all three U.S. fishery sectors, but made up
only 3--7\% of the age samples in each case.
% Above paragraph manually done in 2017 but in the future could have values 
...
The at-sea fishery maintained relatively consistent catch rates throughout the
year (Figure~\ref{fig:main-us-catch-rates}), averaging around 15 t/hr. Relative
to last year, the spring (May--June) fishery saw lower catch rates, whereas the
fall (September--November) fishery fared substantially better.  The at-sea
fleets sometimes fished in deeper water than observed in past years
(Figure~\ref{fig:main-us-at-sea-depths}). During July and August, some operators
...
catches during July and August.  Due to these moderate but consistent
catch-rates throughout the year (for all U.S. fleets), the U.S. utilization rate
went up to 71\% from 47\% last year.
% Manual done 2017.
...
landed \Sexpr{last.year.can.shore}~t. The freezer trawler component, made up of
four vessels which freeze headed and gutted product while at sea, landed
\Sexpr{last.year.can.freezer}~t.

% Paraphrased from Appendix B, no point in repeating lots of it:
The Canadian fishery began in early March, two months earlier than in
\Sexpr{last.data.yr-1}, and the last delivery for the freezer trawler vessels
was in mid-November. Fish were present continuously along the shelf break and on
the shelf off the West Coast of Vancouver Island throughout the season. Similar
to 2015, there appeared to be a larger hake biomass in Canada than in 2013 and
2014. Bycatch was seldom a problem throughout the year. For further details see
the report from the Canadian Advisory Panel
(Appendix~\ref{chap:canada-fishery-report}).

% Shoreside deliveries continued to the end of December. In late May, the
% vessels made a move westward, further offshore, to avoid large aggregations of
% age-1 hake that were appearing on the shelf. Many fishermen reported that these
% aggregations were the largest, acoustically, that they had seen in
% years. Gradually, these aggregations covered more and more of the southwest of
% the fishing grounds off the West coast of Vancouver Island, so fishing vessels
% moved North, into deeper water, to avoid the small fish. Industry reported an
% overall larger hake biomass in Canada compared to the last two years.

% In mid-August, at the request of industry, DFO permitted the harvest of
% offshore hake for the production of fish meal. This required special permission
% from the Minister of Fisheries and Oceans, as the production of fish meal is
% usually disallowed according to the Fisheries and Oceans act. This request was
% made in response to poor market conditions in \Sexpr{last.data.yr-1}, which were
% expected to continue into \Sexpr{last.data.yr}. However, the markets were better
% than expected, and with poor prices for landed fish meal, the fleet processed
% only 68~t for this fishery.
...
% The distribution of catch by month remained similar to other years, with the 
%  summer months showing the greatest catch.
...
and
\Sexpr{years.Can.JV.catch.eq.0.recent[length(years.Can.JV.catch.eq.0.recent)]}
there was no Joint-Venture fishery operating in Canada and this is reflected as
zero catch in that sector for those years in Table~\ref{tab:intro-catches-Can}.

% New for 2016 assessment, since Table was not actually referenced in 2015 
%  assessment. 2017 - moved table reference to earlier.
% The total historical U.S, Canadian and coastwide catches of \fishname\ are 
%  shown in Table~\ref{tab:intro-catches-total}, together with the percentage of 
%  the total catch that came from each country.
