...
(e.g., euphausid distribution and abundance) drivers of hake distribution and
...
The overall catch of \fishname\ in U.S. waters was slighly less than in 2017,
...
catch levels (Figure~\ref{fig:main-us-catch-rates}). Through discussions with the
Washington Department of Fish and Wildlife and a U.S. advisory panel member,
tribal landings were confirmed to be 5,706 t.  Landed catch is brought to
shore-based processing facilities and through fish tickets is coded in PacFIN using
the attribute PARTICIPANT_CODE and/or FLEET_CODE. As in recent years, careful
...
but by the fall, both sectors were catching a majority of age-2 and 4 fish. In
...
was 25\% age-2, 28\% age-4 and 18\% age-8 (Table~\ref{tab:main-us-ms-age-data}). These
totals were based on samples from 492 and 357 hauls, respectively, in each sector
(Table~\ref{tab:main-sampling-history}).  Age-samples from 80 shoreside trips
...
catches during June, July, and August.  Due to moderatley high catch-rates throughout the year
...
delivering 2,439.63~t to the Dutch vessel \emph{Margiris}.
