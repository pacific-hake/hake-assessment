...
KingMcFarlaneEtAl2012, GarciaEtAl2018}. Genetic differences have also been found between the
coastal population and hake off the west coast of Baja California
\citep{VroomanPaloma1977, GarciaEtAl2018}. The coastal stock is also distinguished from the
inshore populations by larger size-at-age and seasonal migratory behavior
and from fish off west of coast of Baja California by smaller size-at-age and
...
Dockside Monitoring Program (DMP), but these were also impacted by staffing
issues and only one sample in was taken.
...
% The \fishname\ stock is of huge commercial value.
In 2020, \fishname\ was Canada's largest commercial wild fishery (species
with the largest catch), representing 14\% of Canada's total landings of all species
\href{https://www.dfo-mpo.gc.ca/stats/commercial/land-debarq/sea-maritimes/s2020pq-eng.htm}{(click
  for reference)}. % It's too long to fit well on page
...
The total economic impact of the whiting fishery on the U.S. West Coast in 2019 was
...
Canadian report from the Advisory panel explains this in more detail (\ref{chap:canada-fishery-report}).
...
but due staffing issues with observers there were no observers on board in 2021, so there were no age samples taken
