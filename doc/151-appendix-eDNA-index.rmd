...
deviations of a single observed water sample from the mean location
(i.e. year-station-depth) DNA concentration. We find that individual water
...
\begin{align}
  G_{jr} &\sim \textrm{Bernoulli}\left(1-\exp(-2 K_j \phi_{j})\right),\\
  C_{jr} &\sim \textrm{Normal}(\beta_{0j}+\beta_{1j}\log{K_{j}},\sigma_C({K_j})) \quad  \textrm{if} \: G_{jr} =1.
\end{align}
