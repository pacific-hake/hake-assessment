...
make.catches.table.US(catches, start.yr=start.yr, end.yr=end.yr,
                   weight.factor=1000,
                   xcaption = paste("Annual catches of \\fishname\\ (t) in U.S. waters by sector, ", start.yr, "-", last.data.yr, ". Tribal catches are included in the sector totals. Research Catch includes landed catch associated with research-related activities. Catch associated with surveys and discarded bycatch in fisheries not targeting hake is not currently included in the model.", sep=""),
...
make.us.age.data.table(us.cp.age,
                       ...
                       xcaption = paste0("Recent age proportion data used in the assessment for ",
                                         "the U.S. Catcher-processor fleet. Proportions are ",
...
make.us.age.data.table(us.ms.age,
                       ...
                       xcaption = paste0("Recent age proportion data used in the assessment for ",
                                         "the U.S. Mothership fleet. Proportions are ",
                                         "calculated from numbers of individuals in each age group. ",
                                         "Age 15 is an accumulator group."),
...
make.survey.summary.table(survey.summary,
                          digits = 3,
                          xcaption = paste0("Biomass indices from the acoustic survey (million~t) used in the 2016 and 2017 assessments."),
...
make.maturity.ogives.table(maturity.ogives,
                           interval = 2,
                           xcaption = "Estimated fraction mature at length as shown in Figure~\\ref{fig:main-proportion-mature-by-NS}.",
...
make.maturity.samples.table(ovary.samples,
                            ...
                            xcaption = "Number of \\fishname\\ ovaries collected for histological analysis with maturity determined from different years and different sources. Numbers for 2016 are preliminary and may be adjusted when preparation of the samples is completed.",
...
make.median.posterior.table(
  ...
  xcaption = paste0("Time-series of median posterior population estimates from the base model. Relative spawning biomass is spawning biomass relative to the unfished equilibrium ($\\Bzero$). Total biomass includes females and males of all ages. Exploitation fraction is total catch divided by total age-3+ biomass. Relative fishing intensity is (1-SPR)/(1-SPR\\subscr{40\\%})."),
...
make.est.numbers.at.age.table(
  ...
  table.type = 2,
  csv.dir = output.csv.dir,
  xcaption = paste0("Estimated exploitation-rate-at-age at the middle ",
                    "of the year from the base model (MLE; percentage of age class removed by fishing)."),
...
make.est.numbers.at.age.table(
  ...
  table.type = 3,
  csv.dir = output.csv.dir,
  xcaption = paste0("Estimated catch-at-age in numbers at the ",
                    "beginning of the year from the base model (MLE; thousands)."),
...
make.est.numbers.at.age.table(
  ...
  table.type = 4,
  csv.dir = output.csv.dir,
  xcaption = paste0("Estimated catch-at-age in biomass at the beginning ",
                    "of the year from the base model (MLE; metric tons)."),
...
make.cohort.table(base.model,
                  ...
                  last.data.yr,
                  xcaption = paste0(
                      # "Age-dependent Key weights for strong cohorts by age. ",
                                    "For the strong cohorts, calculations of ",
                                    ...
                                    "M is the biomass attributed to natural mortality, ",
                                    "and Surviving Biomass is what survives to the ",
                                    "end of the year. Surviving Biomass does not ",
                                    "equal the Start Biomass in the following year ",
                                    "because the empirical weights-at-age change between years."),                      
#                                    "and Surviving Biomass is the weight of the cohort ",
#                                    "at the beginning of the year based on the weight-",
#                                    "at-age from the previous year."),
...
make.short.parameter.estimates.sens.table(sens.models.2.for.table,
                                           ...
                                           digits = 3,
                                           xcaption <- paste0("Maximum likelihood estimates (MLE) of select parameters, derived quantities, and reference points for the base model and key sensitivity runs (described in Section~\\ref{sec:assessment-sensitivity-analyses})."),
