...
make.decision.table(base.model,
                    xcaption <- paste0("Decision table of forecast quantiles of
                     ...
                     TAC from ",min(forecast.yrs)-1," (row d), the catch values
                     ...
                     Row~e uses 600,000~t rather than the 500,000~t from last
                     year's assessment, because 500,000~t is essentially row~d.
