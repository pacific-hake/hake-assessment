...
make.maturity.samples.table(ovary.samples,
                            xcaption = "Number of \\fishname\\ ovaries collected for histological analysis with maturity determined from different years and different sources. **No need to add 2018 since not updating maturity?",
...
make.maturity.ogives.table(maturity.ogives,
                           xcaption = "Information on maturity and fecundity used in this assessment as shown in Figure~\\ref{fig:main-maturity}. The sample sizes refer to the subset of samples in Table~\\ref{tab:main-ovaries} for which age readings and histological estimates of maturity have been completed. The mean weight (kg) is based on a much larger set of samples. **[clarify now that time-varying] The new fecundity estimate is the product of the maturity and mean weight values while the old fecundity was the result of an older length-based maturity estimate converted to ages. The values reported for ages 15 and above (with the exception of the old fecundity) represent the average across all samples in this range.",
...
make.short.parameter.estimates.table(
  ...
  xcaption <- paste0("Select parameters, derived quantities, ",
                     ...
                     "estimates from the previous (",end.yr-1,") base model. **The 2018 column is correctly the same as last year's, except for $q$?!?! 0.940 here, 0.961 last year (but 0.940 for 2017!)."),
...
make.reference.points.table(base.model,
                            xcaption = paste0("Summary of median and 95\\% credibility intervals of equilibrium reference points for the \\fishname\\ base assessment model. ** [surely not 1966, and check what we end up using]Equilibrium reference points were computed using ", start.yr, "--", end.yr-1, " averages for mean weight-at-age and baseline selectivity."),
...
% Ugly, but wanted to get it in quickly. Should be a function like all the other tables.
...
\begin{table}
\caption{Summary of models runs that consider alternative assumptions concerning
  weight-at-age and time-vary fecundity. Results from the MCMC runs 32, 36 and 38
  are given in Appendices **, ** and **. $^*$Note that this run contained the
  inconsistency noted on page~23 of \citet{JTC2018}, in that the time-varying fecundity
  was based on the weight-at-age assumptions described here, but the spawning biomass
  calculations were still based on the 1975-2017 mean (this made little difference
  to results and is fixed for all 2019 runs, as described in
  Section~**bridging-when-written) **Make headers neater. Andy to update based when
  updating run numbers for figures.
  \label{tab:main-wtatage-runs}}
\centering
\begin{tabular}{ccccc}
\hline
Model run when  & Model run when & Time-varying & Pre-1975 & Pre-1975 \\
2018 onwards       & 2018 onwards &  fecundity? & weight-at-age & weight-at-age\\
weight-at-age is   &  weight-at-age (and & & is 1975-1979? & is 1975-2017?\\
mean from 2015-2017; &  fecundity when varying) & & & \\
full MCMC          & is mean from 1975-2017; & & & \\
                   & MLE only & & & \\
  \hline
Base model & 34	& Y &   & Y\\
38         & 35 & Y & Y &  \\
