...
make_catch_table(ct,
                 ...
                 xcaption = paste("Annual catches of \\fishname\\ (t) in
                        ...
                        "-",
                        last_data_yr,
                        ". Tribal catches are included in the fleet totals.
...
make.sampling.history.table(sampling_history_df,
                            xcaption =
                              "Annual summary of U.S. and Canadian
                              fishery sampling included in this stock
                              assessment by fleet. Majority of values are
                              reported as number of hauls but U.S. shore-based
                              and Canadian shoreside fleets are reported as the
                              number of trips. A dash (--) indicates there was
                              no sampled catch. The number of
...
make.us.age.data.table(us_cp_age_df,
                       fleet = 1,
                       start_yr = end_yr - num.recent.yrs,
                       end_yr = last_data_yr,
...
make.us.age.data.table(us_ms_age_df,
                       fleet = 2,
                       start_yr = end_yr - num.recent.yrs,
                       end_yr = last_data_yr,
                       xcaption = paste0("Recent age-proportion data used in
...
make.us.age.data.table(us_ss_age_df,
                       fleet = 3,
                       start_yr = end_yr - num.recent.yrs,
                       end_yr = last_data_yr,
...
make.can.age.data.table(can_ages_lst,
                        can.ss.num.fish,
                        fleet = 1,
                        start_yr = end_yr - num.recent.yrs,
                        end_yr = last_data_yr,
...
make.input.age.data.table(base_model,
                          fleet = 1,
                          ...
                          xcaption = paste0("Aggregated fishery age-proportion data used in the base model. Proportions are ",
                                            ...
                                            "contributions from each fleet are weighted by the catch in that fleet. ",
                                            "Sample sizes are sum of hauls and trips from individual fleets ",
...
make.survey.history.table(survey_history_df,
                          base_model[["dat"]][["CPUE"]],
                          digits = 3,
                          xcaption = paste0("Summary of the acoustic surveys from ", survey_start_yr, " to ", survey_end_yr, "."),
                          xlabel = "tab:main-survey-history",
                          font.size = 10,
                          space.size = 11)
@
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
<<main-survey-by-country-table, results='asis', echo=FALSE>>=
make.survey.by.country.table(survey_by_country_df,
                             digits = 3,
                             xcaption = paste0("Summary of the acoustic survey age-2+ biomass attributed to each country."),
...
make.assessment.changes.table(assess_changes_df,
                              xcaption = paste0("Annual changes in the modeling framework used to assess ",
                                                ...
                                                "the main text. The MCMC column gives the number of Markov chain Monte Carlo samples used to describe ",
...
atage_table(
  ...
  end_yr = end_yr,
  # csv_dir = output_csv_dir,
  xcaption = paste0("Estimated total biomass-at-age at the beginning ",
...
atage_table(
  ...
  digits = 2,
  # csv_dir = output_csv_dir,
  xcaption = paste0("Estimated exploitation-fraction-at-age ",
...
atage_table(
  ...
  end_yr = end_yr - 1,
  # csv_dir = output_csv_dir,
  xcaption = paste0("Estimated catch-at-age in total biomass ",
...
cohort_table(base_model,
             ...
             xcaption = "Calculations showing
                         ...
                         Estimated quantities are posterior medians.",
...
make.median.posterior.table(
  ...
  xcaption = paste0("Time series of median posterior population estimates from the base
                     ...
                     Relative fishing intensity is (1-SPR)/(1-SPR\\subscr{40\\%}) such that
                     values below 100\\% represent fishing below $\\Fforty$. A dash (--)
                     indicates a quantity requiring ", end_yr, " catch which has not taken place yet."),
...
make.reference.points.table.old(base_model,
                            xcaption = paste0("Summary of median and 95\\% credibility
                                               ...
                                               the base assessment model. Equilibrium reference
                                               points were computed using 1975--",
...
<<main-biomass-table, results='asis', echo=FALSE>>=
...
decision_table(base_model,
               xcaption = paste0("Forecast quantiles of \\fishname\\ ",
                                 ...
                                 "Table~\\ref{tab:main-decisions-biomass}. Values greater than ",
...
make.risk.table(base_model,
                ...
                xcaption = paste0("Probabilities related to spawning biomass,
                                 relative fishing intensity, and the ",
                                  ...
                                  Tables~\\ref{tab:main-risk-year-1}
                                  and~\\ref{tab:main-risk-year-2}  (catch options
                                  explained in Table
...
table_param_est(retro.list,
                model_nms = retro.model.names,
                end_yr = end_yr,
                digits = 3,
                rec_yrs = c(2010, 2014, 2016, 2020),
                xcaption = "Posterior medians from the base model for
                            ...
                            derived quantity was not output by the model.",
