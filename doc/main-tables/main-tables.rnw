...
\caption{Summary of models runs that consider alternative assumptions concerning
  ...
  Results from the MCMC runs **32, 36 and 38
  are given in Appendices **, ** and **. $^a$Due to the configuration of Stock Synthesis
  ...
  **Refer to Section~**bridging-when-written **Make headers neater (use the
  other table commands, I forget for now). 
