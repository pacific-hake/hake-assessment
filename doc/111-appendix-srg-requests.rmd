...
in the survey age composition fit for age-3 fish in 2021. Other than that there are
no substantial differences in the age composition fits.

(ref:mcmc-survey-fit-srg-app-cap) Fit to the acoustic survey for the model that excludes the age-1 index (new base model).

(ref:mcmc-survey-fit-srg-app-alt) The fit is similar to the model that includes the age-1 survey, except for small differences in 2019, 2021, and 2023.

```{r mcmc-survey-fit-srg-app, fig.cap = "(ref:mcmc-survey-fit-srg-app-cap)", fig.height = 4, out.height = "80%"}

plot_survey_fit_mcmc(sens_models[[2]][[1]],
                     type = "acoustic",
                     n_posts = 1000,
                     y_lim = c(0, 6),
                     glow = TRUE,
...
all years and the posterior selectivity for the survey and the fishery selectivity in 2024.
The latter included 1000 draws from the posterior distribution. The set of figures based
on the model that removes that age-1 index are not shown here. This model is now the base model and is in the main text.  Shown below are the requested figures for the other four models.
...
variance (phi) set to 0.21. See Figure~\@ref(fig:main-tv-selex-uncertainty-fig) in the main text for futher description.
...
```{r main-tv-selex-uncertainty-fig-srg-app, fig.cap = "(ref:main-tv-selex-uncertainty-srg-app-cap)", fig.height = 9}

plot_selex_uncertainty(sens_models[[3]][[1]],
                       n_col = 2,
                       pad_top = TRUE,
                       pad_bottom = TRUE)
```
...
```{r main-tv-selex-posteriors-fig-srg-app, fig.cap = "(ref:main-tv-selex-posteriors-srg-app-cap)", fig.height = 8}

p <- list()
p[[1]] <- plot_selex_posteriors(sens_models[[3]][[1]],
                                type = "survey",
                                age_range = c(1, 8),
...
<!-- phi=0.70-->
(ref:main-tv-selex-uncertainty2-srg-app-cap) Fishery selectivity sampled from
posterior probability distribution by year for the sensitivity model with time-varying
variance (phi) set to 0.70. See Figure~\@ref(fig:main-tv-selex-uncertainty-fig) in the main text for futher description.

(ref:main-tv-selex-uncertainty2-srg-app-alt) This figure shows individual panels for
each year with selectivity medians and 0.95 credible intervals, with less variability in
selectivity among years.

```{r main-tv-selex-uncertainty-fig2-srg-app, fig.cap = "(ref:main-tv-selex-uncertainty2-srg-app-cap)", fig.height = 9}
...
(ref:main-tv-selex-posteriors2-srg-app-cap) Estimated selectivities for the acoustic
survey age-2$+$ biomass index (top, with selectivity of zero for age-1 fish)
and fishery (bottom -- shown for `r end_yr - 1` only, age-1 and older) from
a subsample of 1,000 draws from the posterior distribution for the sensitivity
model with time-varying variance (phi) set to 0.70.

(ref:main-tv-selex-posteriors2-srg-app-alt) This plot shows individual median
selectivity trajectories for a subset of samples from the posteriors of
fishery and survey selectivity parameters with 95 percent credible intervals.
Fishery selectivity is slowly increasing from ages 1 to 5 before a large jump to
fully selective at age 6.

```{r main-tv-selex-posteriors-fig2-srg-app, fig.cap = "(ref:main-tv-selex-posteriors2-srg-app-cap)", fig.height = 8}
...
<!-- phi=2.10-->
(ref:main-tv-selex-uncertainty3-srg-app-cap) Fishery selectivity sampled from
posterior probability distribution by year for the sensitivity model with time-varying
variance (phi) set to 2.10. See Figure~\@ref(fig:main-tv-selex-uncertainty-fig) in the main text for futher description.

(ref:main-tv-selex-uncertainty3-srg-app-alt) This figure shows individual panels for
each year with selectivity medians and 0.95 credible intervals, with more variability in
selectivity among years.

```{r main-tv-selex-uncertainty-fig3-srg-app, fig.cap = "(ref:main-tv-selex-uncertainty3-srg-app-cap)", fig.height = 9}
...
(ref:main-tv-selex-posteriors3-srg-app-cap) Estimated selectivities for the acoustic
survey age-2$+$ biomass index (top, with selectivity of zero for age-1 fish)
and fishery (bottom -- shown for `r end_yr - 1` only, age-1 and older) from
a subsample of 1,000 draws from the posterior distribution for the sensitivity
model with time-varying variance (phi) set to 2.10.

(ref:main-tv-selex-posteriors3-srg-app-alt) This plot shows individual median
selectivity trajectories for a subset of samples from the posteriors of
fishery and survey selectivity parameters with 95 percent credible intervals.
...
```{r main-tv-selex-posteriors-fig3-srg-app, fig.cap = "(ref:main-tv-selex-posteriors3-srg-app-cap)", fig.height = 8}
...
<!-- eDNA-->
(ref:main-tv-selex-uncertainty4-srg-app-cap) Fishery selectivity sampled from
...
(ref:main-tv-selex-uncertainty4-srg-app-alt) This figure shows individual panels for
...
```{r main-tv-selex-uncertainty-fig4-srg-app, fig.cap = "(ref:main-tv-selex-uncertainty4-srg-app-cap)", fig.height = 9}

plot_selex_uncertainty(sens_models[[2]][[3]],
                       n_col = 2,
                       pad_top = TRUE,
                       pad_bottom = TRUE)
```

(ref:main-tv-selex-posteriors4-srg-app-cap) Estimated selectivities for the acoustic
survey age-2$+$ biomass index (top, with selectivity of zero for age-1 fish)
and fishery (bottom -- shown for `r end_yr - 1` only, age-1 and older) from
a subsample of 1,000 draws from the posterior distribution for the sensitivity
model that included the eDNA index.

(ref:main-tv-selex-posteriors4-srg-app-alt) This plot shows individual median
selectivity trajectories for a subset of samples from the posteriors of
fishery and survey selectivity parameters with 95 percent credible intervals.
Fishery selectivity is similar to the base model.

```{r main-tv-selex-posteriors-fig4-srg-app, fig.cap = "(ref:main-tv-selex-posteriors4-srg-app-cap)", fig.height = 8}

p <- list()
p[[1]] <- plot_selex_posteriors(sens_models[[2]][[3]],
                                type = "survey",
                                age_range = c(1, 8),
                                n_posts = 1000,  # if change then change below
                                        # and in caption
                                ...
                                unc_line_color = "red",
                                glow = FALSE)
p[[2]] <- plot_selex_posteriors(sens_models[[2]][[3]],
                                type = "fishery",
                                age_range = c(1, 8),
                                n_posts = 1000,
                                glow = FALSE)
...
(Figure~\@ref(fig:srg-app-btsi-fig)) and the relative proportion by in
latitudinal sub-areas (Figure~\@ref(fig:srg-app-btsi2-fig)) provided
...
At-Sea (Figure) and Shoreside (Figure) revenue over recent years indicated a tendency for
...
by Owen Liu, Rebecca Thomas, Kristin Marshall, Mary Hunsicker, and Andy Edwards.
...
```{r srg-app-btsi-fig, fig.cap = "(ref:srg-app-btsi-cap)", fig.height = 9}
include_graphics(file.path(figures_dir,"Estimated_indices_2018_2023.png"), error = FALSE)
```

(ref:srg-app-btsi-cap) Bottom trawl survey index proportions by area from the combined
DFO and NWFSC data. Courtesy of Eric Ward.

(ref:srg-app-btsi-alt) This plot shows an increasing proportion of fish being South of
42 latitude and a decreasing proportion north of 42 latitude.

```{r srg-app-btsi2-fig, fig.cap = "(ref:srg-app-btsi-cap)", fig.height = 9}
...
```{r srg-app-edna-fig, fig.cap = "(ref:srg-app-edna-cap)", fig.height = 9}
include_graphics(file.path(figures_dir,"eDNA_index_1.png"), error = FALSE)
```

(ref:srg-app-pacfem-atsea-cap) Distribution of fishing revenue from 2016 to 2023 for the United
States At-Sea fleet. Courtesy of Lisa Pfeiffer.
...
```{r srg-app-pacfem-atsea-fig, fig.cap = "(ref:srg-app-pacfem-atsea-cap)", fig.height = 9}
...
```{r srg-app-pacfem-ss-fig, fig.cap = "(ref:srg-app-pacfem-ss-cap)", fig.height = 9}
...
For completeness, the bottom trawl index was shown with the uncertainty fixed.  Not shown here.
...
The following bottom trawl survey figures were produced to show trends from both north
...
```{r srg-app-btsi3-fig, fig.cap = "(ref:srg-app-btsi3-cap)", fig.height = 9}
...
```{r srg-app-btsi4-fig, fig.cap = "(ref:srg-app-btsi4-cap)", fig.height = 9}
...
(base; 262 parameters) for time-varying selectivity compared to a maximum of age of 8
(333 parameters), 10 (403 parameters), and 12 (473 parameters). The following figures
show some key results comparing the models that increase the maximum selectivity age.
...
```{r main-tv-selex-unc-tv8-srg-app, fig.cap = "(ref:main-tv-selex-unc-tv8-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
```{r main-tv-selex-unc-tv10-srg-app, fig.cap = "(ref:main-tv-selex-unc-tv10-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
for ages 1 through 7 and then a jump to 50% selective for ages 8 to 12 and above.

```{r main-tv-selex-unc-tv12-srg-app, fig.cap = "(ref:main-tv-selex-unc-tv12-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
```{r main-tv-selex-8-srg-app, fig.cap = "(ref:main-tv-selex-8-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
posterior probability distribution by year for the sensitivity model with maximum age
of 10 for selectivity. See Figure~\@ref(fig:main-tv-selex-uncertainty-fig) in the main text for futher description.

(ref:main-tv-selex-10-srg-app-alt) This figure shows a large increase in variability around ages
7 and above.

```{r main-tv-selex-10-srg-app, fig.cap = "(ref:main-tv-selex-10-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
posterior probability distribution by year for the sensitivity model with maximum age
of 12 for selectivity. See Figure~\@ref(fig:main-tv-selex-uncertainty-fig) in the main text for futher description.

(ref:main-tv-selex-12-srg-app-alt) This figure shows a large increase in variability around ages
7 and above.

```{r main-tv-selex-12-srg-app, fig.cap = "(ref:main-tv-selex-12-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
```{r main-biomass-base-a-srg-app, fig.cap = "(ref:main-biomass-base-a-srg-app-cap)", fig.height = 4, out.height = "80%"}

plot_biomass(list(sens_models[[2]][[1]], model_age_8, model_age_10, model_age_12),
...
```{r main-relative-biomass-base-a-srg-app, fig.cap = "(ref:main-relative-biomass-base-a-srg-app-cap)", fig.height = 4, out.height = "80%"}

plot_rel_biomass(list(sens_models[[2]][[1]], model_age_8, model_age_10, model_age_12),
             c("pre-SRG base model", request_models_names[[1]]),
                 ylim = c(0, 2.5),
                 alpha = 0.2,
                 leg_pos = c(0.75, 0.80)) |>
...
```{r main-survey-index-fit-mcmc-8-srg-app, fig.cap = "(ref:main-survey-index-fit-mcmc-8-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
```{r main-survey-index-fit-mcmc-10-srg-app, fig.cap = "(ref:main-survey-index-fit-mcmc-10-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
```{r main-survey-index-fit-mcmc-12-srg-app, fig.cap = "(ref:main-survey-index-fit-mcmc-12-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
(ref:main-recruitment-devs-base-a-srg-app-alt) Only minor differences in recruitment deviations
were detected.

```{r main-recruitment-devs-base-a-srg-app, fig.cap = "(ref:main-recruitment-devs-base-a-srg-app-cap)", fig.height = 4, out.height = "80%"}

plot_recdevs(list(sens_models[[2]][[1]], model_age_8, model_age_10, model_age_12),
             c("pre-SRG base model", request_models_names[[1]]),
...
The figures requested are presented next and include each of the four sequential age-1
index data point removal models.  The base model shown in the following figures is the
...
```{r main-biomass-base-srg-app, fig.cap = "(ref:main-biomass-base-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
plot_biomass(model_lst = list(sens_models[[2]][[1]], model_age1_minus_1, model_age1_minus_2, model_age1_minus_3, model_age1_minus_5),
...
(ref:main-relative-biomass-base-srg-app-alt) The removal of the 2023 and 2019 data points were most
influencial, with the relative spawning biomass being lower than otherwise.

```{r main-relative-biomass-base-srg-app, fig.cap = "(ref:main-relative-biomass-base-srg-app-cap)", fig.height = 4, out.height = "80%"}

plot_rel_biomass(model_lst = list(sens_models[[2]][[1]], model_age1_minus_1, model_age1_minus_2, model_age1_minus_3, model_age1_minus_5),
             model_names = c("pre-SRG base model", request_models_desc[[2]]),
                 ylim = c(0, 2.5),
                 alpha = 0.2,
                 leg_ncol = 2,
...
were most influencialon 2019 and 2023 recruitment deviations.

```{r main-recruitment-devs-base-srg-app, fig.cap = "(ref:main-recruitment-devs-base-srg-app-cap)", fig.height = 4, out.height = "80%"}

plot_recdevs(model_lst = list(sens_models[[2]][[1]], model_age1_minus_1, model_age1_minus_2, model_age1_minus_3, model_age1_minus_5),
             model_names = c("pre-SRG base model", request_models_desc[[2]]),
             line_color = ts_single_line_color,
             leg_ncol = 2,
             leg_pos = c(0.52, 0.87))
...
```{r main-survey-index-fit-mcmc-base-srg-app, fig.cap = "(ref:main-survey-index-fit-mcmc-base-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
```{r main-survey-index-fit-mcmc-rm1-srg-app, fig.cap = "(ref:main-survey-index-fit-mcmc-rm1-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
```{r main-survey-index-fit-mcmc-rm2-srg-app, fig.cap = "(ref:main-survey-index-fit-mcmc-rm2-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
```{r main-survey-index-fit-mcmc-rm3-srg-app, fig.cap = "(ref:main-survey-index-fit-mcmc-rm3-srg-app-cap)", fig.height = 4, out.height = "80%"}
...
```{r main-survey-index-fit-mcmc-rm5-srg-app, fig.cap = "(ref:main-survey-index-fit-mcmc-rm5-srg-app-cap)", fig.height = 4, out.height = "80%"}
