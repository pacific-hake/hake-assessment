...
Unsexed = U; Tables~\@ref(tab:srg-weight-at-age-sample-size-can)
...
fixed and Figures~\@ref(fig:srg-plot-base-v-tv-maturity-ssb-fig) and
\@ref(fig:srg-plot-base-v-tv-maturity-rel-fig) show assessment model results.
...
Of the un-Kriged biomass (3.1%), 22.9 kt out of the 735 kt came from
hake mixes in 2023.
...
j <- read.csv(
  "https://raw.githubusercontent.com/pacific-hake/hake-assessment/2443c4f807f3cd32e33d0390bbc4df0f7b0430d8/data-tables/maturity-ogives.csv",
  header = TRUE
) |>
...
were provided to the SRG to fulfill the request thoroughly. While substantially downweighting age-composition data results in an improved survey fit, the model
loses critical information on recruitment and thus overall population scale.
...
for ages 19 and 20, that there were no data. Figure produced by Rebecca Thomas
(NOAA).
...
\resizebox{\textwidth}{!}{$maturity_{functional} \sim -1 + s(doy) + age + age^2 + temp + temp^2$}

where there is **NO** interaction with the smoother on the day of the year.
Thus, the shape of the relationship
(\@ref(fig:app-maturity-analysis-day-of-year-fig)) does not change with time
...
The updated figure was presented at the SRG meeting and
Figure~\@ref(fig:main-maturity-ogive-fig) was also updated.
...
(ref:srg-new-base-biomass-cap) Female spawning biomass across time for the terminal
bridging steps to the new base model, as well as the pre-SRG base model. The bridge
model is the step in which the student-t distribution was used for the relative age-1
index of numbers of fish. Weight-at-age data were then updated, followed by fecundity
using the new maturity ogives to create the new base model.

(ref:srg-new-base-biomass-alt) The estimated female spawning biomass of the new base
model is more similar to the bridging models than the pre-SRG base model that has a
lower estimated female spawning biomass.
...
(ref:srg-new-base-rel-biomass-cap) Female spawning biomass relative to
unfished female spawning biomass across time for the terminal bridging steps
to the new base model, as well as the pre-SRG base model. The bridge model
is the step in which the student-t distribution was used for the relative
age-1 index of numbers of fish. Weight-at-age data were then updated,
followed by fecundity using the new maturity ogives to create the new
base model.

(ref:srg-new-base-rel-biomass-alt) The current status of the new base model
is near unfished levels where the pre SRG model was between unfished and
40 percent of unfished.

```{r srg-new-base-rel-biomass-fig, out.height = "85%", fig.cap = "(ref:srg-new-base-rel-biomass-cap)"}

plot_rel_biomass(list(step_before,
                      first_step,
                      new_base,
                      old_base),
                 model_names = m_names,
...
The JTC completed the new Executive Summary and submitted it to the SRG
in an unchecked, draft form. It has been since completed and is the new
...
* The One-page summary has been modified, in particular the penultimate bullet point.
