...
for conducting efficient Bayesian MCMC algorithms (AD No-U-Turn Sampler, or
adnuts, software; \citet{Monnahan2018}, \citet{Monnahan2019}). 
This version of the base model uses the more efficient
adNuts software for conducting MCMC which has the potential to improve the
...
The following figures and tables follow an executive summary format, with the addition
