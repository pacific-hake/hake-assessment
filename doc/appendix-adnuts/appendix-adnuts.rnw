...
It took approximately 1.5~hours to run, compared to over 40~hours for the usual
MCMC algorithm.
...
and \ref{fig:mcmc-pairs-adnuts}--\ref{fig:mcmc-pairs-recruit-devs-adnuts}) are very similar, with most
of the difference being the density of the scatterplots which is attributed to the number of posterior
samples.

Considering these diagnostics, the similarity in estimated values in the posteriors for key parameters,
...
the sensitivities to be run in an MCMC context as opposed to MLE, and all results would be more comparable.
