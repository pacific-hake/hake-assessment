...
differs slightly between the two, with the base model median being 2.505~billion and the adnuts
...
Figures~\ref{fig:mcmc-diag-m-r0-adnuts}--\ref{fig:mcmc-diag-dm-adnuts}). There are only slight differences
in the densities of the posteriors (bottom-right panels). Parameter autocorrelation remains low
...
base model and Figure~\ref{fig:mcmc-diag-hists-adnuts} for the adnuts model. These diagnostics
are comparable between the two models, with the adnuts having more
parameters that have a low effective sample size. Bearing in mind that there are almost four times
the number of posterior samples for the adnuts model when compared to the base model, some of these
effects may be minimized with further thinning of the posterior samples.
...
main difference being the density of the scatterplots due to differing number of posterior
samples.

Considering these diagnostics, the similarity in estimated posteriors for key parameters,
and the increased efficiency of the algorithm, the JTC recommends that the adnuts approach be
adopted for next year's base model and sensitivities. The increased efficiency would allow
the sensitivities to be run in an MCMC context as opposed to MLE, making all results more comparable.
