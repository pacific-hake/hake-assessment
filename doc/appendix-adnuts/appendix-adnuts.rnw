...
are comparable between the two models, with only small differences a number
of parameters with a lower effective sample size in the adnuts model. Bearing in mind that there are almost 4 times
...
and \ref{fig:mcmc-pairs-adnuts}--\ref{fig:mcmc-pairs-recruit-devs-adnuts}) is very similar, with most
...
and the increased effficiency of the algorithm, the JTC recommends that the adnuts approach be
