...
\chapter{Sensitivity run using NUTS algorithm -- MCMC results}
\label{chap:adnuts}

This appendix contains base model Bayesian MCMC results using a new analytical tool
for conducting efficient Bayesian MCMC sampling, the Automatic Differentiation
No-U-Turn Sampler (NUTS; \citep{Hoffman2014}), implemented using the {\tt adnuts} R package
...
MCMC algorithm. The adnuts run resulted in 7,850 samples -- reducing this would
...
although the adnuts model suggests slightly higher uncertainty. The main difference is with the estimate
of initial recruitment, $R_0$, with the base model median being 2.505~billion and the adnuts
...
Diagnostics for the adnuts model are comparable to the base model for all key posteriors
...
for the adnuts model (bottom-left panels). The traceplots (top-left panels) appear denser for the
adnuts model because the number of samples from the posterior is 7,850 compared to
...
base model and Figure~\ref{fig:mcmc-diag-hists-adnuts} for the adnuts model. The adnuts model
...
and the considerable increase in efficiency of the algorithm, the JTC recommends that the adnuts approach be
...
\caption{MCMC median posterior estimates with 95\% credible intervals of relative
  spawning biomass for the base model and alternative sensitivity run using the adnuts MCMC algorithm.}
