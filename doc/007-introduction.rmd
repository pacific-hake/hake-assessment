...
for which the process defined by the Agreement was followed, which includes the
stock assessment. This is the
...
age-composition data. The assessment depends primarily upon an in index of
biomass from the acoustic survey for information on the scale of the current
...
for the age-1 survey is now a student-t distribution rather than a log-normal
distribution [@JTC2022].
All model runs reported in this document are performed in a Bayesian context.
Responses to `r last_assess_yr` SRG requests are in
Section~\@ref(sec:assessment-response-review) and a Glossary of terms appears
in Appendix~\@ref(sec:app-glossary).
...
maternal preconditioning, as well as with northward long-shore transport
...
@HamelEtAl2015; @MalickHunsickerEtAl2020; @MalickSiedleckiEtAl2020] but further
...
In 2021 (the latest data available), the `r sp` fishery was Canada's largest
commercial wild fishery (species with the largest catch), representing 10% of
[Canada's total landings](https://www.dfo-mpo.gc.ca/stats/commercial/land-debarq/sea-maritimes/s2021pq-eng.htm) of all species.
...
In the US, over US\$75.1 million in wages is estimated to have been paid to
employees in 2021
...
workers on mothership vessels. The exported value of Pacific Hake was US\$163
...
was US\$335 million in income and 4,450 jobs.
...
(Appendices~\@ref(sec:app-us-fishery-report)--\@ref(sec:app-canada-fishery-report)).
...
Age-samples from
`r pull(us_ss_age_df[us_ss_age_df$year == last_data_yr, "n.trips"])`
...
Table~\@ref(tab:main-us-sb-age-data-tab). Age-composition differences
