...
output:
 hake::hake_pdf:
   ...
   # NOTE: cannot use backticks inside R-chunk which already uises backticks
   # here, so need to use [['hake::hake_pdf']] instead of $`hake::hake_pdf`
...
home <- ifelse(sys_info[["sysname"]] != "Windows",
               system("echo $HOME", intern = TRUE),
               shell("echo $HOME", intern = TRUE))
