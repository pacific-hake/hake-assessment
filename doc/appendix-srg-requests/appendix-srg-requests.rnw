...
the model already estimates a very low abundance of age-1 fish in 2001 (i.e., the inclusion of the 2001 value will
...
joined <- dplyr::left_join(
  ...
  base_model$extra_mcmc$index.med %>%
...
plot_selex_uncertainty(sens_models[[4]][[2]], n_col = 2)
@
\end{center}
...
plot_selex_uncertainty(sens_models[[4]][[3]], n_col = 2)
@
\end{center}
...
plot_selex_uncertainty(sens_models[[4]][[4]], n_col = 2)
@
\end{center}
\caption{As for Figure~\ref{fig:srg-requests-tv-selex-uncertainty-1} but with
