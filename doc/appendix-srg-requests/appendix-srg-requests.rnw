...
{\bf Show the `squid plot' for the base-case model with and without Age-1
  index.}
...
{\bf Colour code weight-at-age matrix by column rather than global colouring.}
...
{\bf Investigate changes in weight-at-age for 1987, when 1977 was expected to change but did not occur (compare 2019 to 2020).}
...
{\bf Calculate average of recruitment deviations over the main period
...
{\bf Redo ADNUTs plots thinning down to about 2,000 samples from samples available, and redo convergence diagnostic plots.}

**

\section{Request 6}

{\bf Redo recruitment retrospective analysis including Age-1 index, focusing on
...
Input sample sizes and effective sample sizes have increased over time, where the latter increase appears to be positively autocorrelated with periods of highs and lows (Figure~\ref{fig:srg-inputsamplesize}).

\section{Day 2 -- Request 1}
...
The number of estimated ratios that were close to one did not drastically improve when the age-1 index was included in the base model (Figure~\ref{fig:srg-squidhist}).
...
Correlations were similar between the base model and the adnuts model (Figure~\ref{fig:srg-reccorr}). Correlations decreased when the full time period was analyzed as compared to just the main recruitment period.
...
\section{Day 3 -- Request 1}

** Do we include the informal request?
