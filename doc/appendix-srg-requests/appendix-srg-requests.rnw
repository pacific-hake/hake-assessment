...
The JTC made the a density plot, and included additional plots to show the other key parameters and large cohort recruitment parameters. Figures~\ref{fig:srg-day1-req1-ro}--\ref{fig:srg-day1-req1-recr2016} show that, for most parameters the medians are comparable, but for $ln(R_0)$ and $h$, the median value for the NUTS model is slightly less. The median is slightly higher for the Dirichlet-multinomial fishery parameter for the rwMH model. In all cases, the parameter space appears to be better explored with the NUTS model due to the presence of more samples in the tails of the distributions (Blue hash marks).

\begin{figure}[tbp]
\begin{center}
<<srg-day1-req1-ro, fig.height = 3.5>>=
...
\begin{figure}[tbp]
\begin{center}
<<srg-day1-req1-m, fig.height = 3.5>>=
...
\begin{figure}[tbp]
\begin{center}
<<srg-day1-req1-h, fig.height = 3.5>>=
...
\begin{figure}[tbp]
\begin{center}
<<srg-day1-req1-sd, fig.height = 3.5>>=
...
\begin{figure}[tbp]
\begin{center}
<<srg-day1-req1-dm1, fig.height = 3.5>>=
...
\begin{figure}[tbp]
\begin{center}
<<srg-day1-req1-dm2, fig.height = 3.5>>=
...
\begin{figure}[tbp]
\begin{center}
<<srg-day1-req1-recr2010, fig.height = 3.5>>=
...
\begin{figure}[tbp]
\begin{center}
<<srg-day1-req1-recr2014, fig.height = 3.5>>=
...
\begin{figure}[tbp]
\begin{center}
<<srg-day1-req1-recr2016, fig.height = 3.5>>=
...
\begin{figure}[H]
\begin{center}
<<srg-wt-at-age, fig.height=9,fig.width=8,is.fig=TRUE,alt.text="Annual mean weight-at-age is lower for most ages in the most recent years then for years prior to 1980.">>=
...
The JTC followed the request of the SRG and calculated a three-year average weight-at-age to produce projections using the forecast parameters in Stock Synthesis. Since the forecast parameters are estimated in the base model run, the entire NUTS run had to be re-run for this new model prior to running the 2-year forecasting with the new weights-at-age. This run took approximately 4.5 hours for the NUTS portion, and another 3 hours for the forecasting and model loading steps. Table~\ref{tab:srg-day2-request1-decision-table} shows the relative biomass decision table for this case which is identical in format to the decision table (Table~\ref{tab:es-decisions-biomass}) found in the Executive summary and can be compared directly.

When compared to the base model decision table, Table~\ref{tab:srg-day2-request1-decision-table} shows an increase in median relative spawning biomass for all constant catch streams (rows a--g). The credible interval is also shifted upwards be several percent relative to the base model.
...
- $B_{2022}$ is less than $B_{40\%}$ is lower for all constant catch levels, and within 4\% of the base model.
...
The JTC ran the 3-year projections required to make the table, and re-coded the decision table to display the results in a way similar to that shown in a mock-up table provided by the SRG. Due to there being one more forecast parameter, the entire NUTS run had to be re-run for this new model prior to running the 3-year forecasting. This run took approximately 4.5 hours for the NUTS portion, and another 4 hours for the forecasting and model loading steps.

Table~\ref{tab:srg-day2-request2-decision-table} shows the new table, which can be compared with the base model decision table (Table~\ref{tab:es-decisions-biomass}) found in the Executive summary.

Differences between the the new table and the current one in the Executive summary include:

- 2021 biomass is shown in a single row at the top ({\emph Start 2021}) in the new table. In the current one, it is the first row in every row chunk (a--j).

- Values shown as proportions instead of as percentages. This allows the removal of all the percentage signs shown on on the relative biomass values which can be distracting.

- Removal of the 25\% and 75\% columns.

- Addition of a new column, {\emph Biomass year}, which gives the timing that the biomass values in the table represent.
...
- The values shown in the {\emph Resulting relative spawning biomass} columns now represent the biomass at the beginning of the year which result due to the catch taken in the previous year, which is found in the ({\emph Catch year}) column.
...
more proportions-at-age for earlier cohorts). But this comment led us
...
\caption{For each year $t$, the probability that the spawning biomass at the
  ...
  Red: the probability is taken from year $t$'s stock assessment document's
...
We have extracted similar probabilities from past assessment documents, currently going back to
2012, and calculated the analogous probabilities from the current assessment model.
Specifically, for each assessment year $t$, we take
P$(B_{t+1} < B_{t} | \mbox{catch we now know to have occurred in year}~t)$ from
the year $t$ assessment document, and also calculate
...
decrease the following year. Also, for all years (except 2018) the assesment
year's probabilities are closer to 50\% than those from the current base
model. Such behaviour is desirable and sensible. The probabilities being
calculated are for binary events that either happen or do not (the stock either
declines or it does not decline, similar to a tossed coin only being a head or a
tail). The current assessment model has more information and so gives a more
...
documents are not too definitive (too close to 0\% or to 100\%), since they are
...
(Figure~\ref{fig:main-female-spawning-biomass}), and may or may not get resolved
in the future with further data.

From this current \Sexpr{assess.yr} assessment, we show the probabilities for
all catch alternatives in Figure~\ref{fig:appendix-srg-historical-1}, since
obviously we do not yet know which will correspond to the \Sexpr{assess.yr}
...
In Figure~\ref{fig:appendix-srg-historical-2} we do similar calculations for the
%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[tbp]
\begin{center}
...
$\Bforty$ in the subsequent year, P$(B_{t+1} < \Bforty)$. The 2012 assessment's probability
is the poorest performing, and gave a $>50\%$ chance of the biomass falling
below $\Bforty$ in 2013; but again, this did not occur thanks to the very large
2010 year class. The 2013-2017 assessments had information on the 2010 class
and so estimated a low probability of falling below $\Bforty$. Again, these
estimates are closer to 50\% than those from the current base model (blue dots),
which is desirable behaviour as mentioned above -- the assessments gave low
probabilities of an unlikely event occurring, that we now believe to have been even more
unlikely to have occurred. For the 2018-2020 assessments the probability becomes $>10\%$ and
continues rising, and the current model's estimate of P$(B_{t+1} < \Bforty)$
also rises.

The past assessments' probabilities lie below those estimated from the
...
\Bforty)$, are mostly in the 30\%-50\% range, which is `uncharted territory'
in the sense that no previous probabilities have been in this range. Also, the
...
Overall, these results suggest that good confidence in the
...
incorrectly expecting the biomass to fall below projecting (due to
the unknown large 2010 year class that did ), projections `correctly' estimated the
