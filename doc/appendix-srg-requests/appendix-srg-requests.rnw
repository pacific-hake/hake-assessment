...
The JTC made the density plot, and included additional plots to show the other
key parameters and large cohort recruitment
parameters. Figures~\ref{fig:srg-day1-req1-ro}--\ref{fig:srg-day1-req1-recr2016}
...
The following two excerpts from \citet{Betancourt2018} were presented to the SRG regarding differences between rwMH and NUTS and their appropriateness to high-dimensional models such as the \fishname\ assessment model:
...
When compared to the base model decision table, Table~\ref{tab:srg-day2-request1-decision-table} shows an increase in median relative spawning biomass for all constant catch streams (rows a--g). The credible interval is also shifted upwards by several percent relative to the base model.
...
Table~\ref{tab:srg-day2-request2-decision-table} shows the new table, which can
...
2012, and calculated the analogous probabilities from the current assessment
model (Figure~\ref{fig:appendix-srg-historical-1}).
Specifically, for each assessment year $t$, we take the value of
...
(Figure~\ref{fig:main-female-spawning-biomass}); this may or may not get resolved
...
$\Bforty$ in the subsequent year, i.e.~P$(B_{t+1} < \Bforty)$. The 2012 assessment's probability
is the poorest performing, and gave a $>50\%$ chance of the biomass falling
below $\Bforty$ in 2013; but this did not occur thanks again to the very large
