...
probabilities do indeed change between assessments. For example, values projected for \Sexpr{assess.yr} in the \Sexpr{assess.yr - 1} assessment changed compared to values estimated for \Sexpr{assess.yr} in this assessment because the \Sexpr{assess.yr} assessment model depends on the catch
...
that we now know occurred in 2019 (Figure~\ref{fig:appendix-srg-historical-1}).
...
model we get P$(B_{2020} < B_{2019}) = 98\%$ (Figure~\ref{fig:appendix-srg-historical-1}).
%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[tbp]
\begin{center}
...
i.e.,~P$(B_{t+1} < \Bforty)$.
The probability of $>50\%$ listed in the the 2012 assessment
was the poorest performing probability of being
below $\Bforty$; but, this did not occur thanks again to the very large
