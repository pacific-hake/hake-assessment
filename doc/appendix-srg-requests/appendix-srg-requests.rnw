...
probabilities change because a
decision table in, say, the \Sexpr{assess.yr} assessment already accounts for
the catch in \Sexpr{assess.yr - 1} and has updated data. But this comment led us
to investigate the general question of how much confidence can we have in the
probabilities, based on those from previous assessments.
...
assessment to give P($B_{2020} < B_{2019}$) given the catch that we now know
occurred in 2019, as estimated in the 2019 assessment.

Then, we can caculate P($B_{2020} < B_{2019}$) using the current
...
implicitly includes the 411,574~t catch from 2019. Then we can compare these two
probabilities to ask how much confidence we can put in such one-year-ahead probabilities
