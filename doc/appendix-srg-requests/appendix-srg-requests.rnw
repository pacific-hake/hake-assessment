...
The JTC confirmed that the age-1 index for 2001 is not included in the base
model. A run was tried with a low value of 100, since a zero cannot be included
(due to the assumption of a lognormal distribution), but there were convergence
issues. This will be looked into further for next year, perhaps trying
alternative distributions that can account for zeros.
...
\subsection*{Request 3 -- max age of constant selectivity}
...
The model was re-run with the maximum ages set to ages 7, 8, and also 5 (to
bracket the base model value of 6). The estimates of historical spawning biomass
before 1990 change somewhat, but later estimates are somewhat similar (Figure~\ref{fig:srg-max-age-select}). Associated
estimates of key parameters and derived quantities are shown in
...
\begin{figure}[tbp]
\begin{center}
<<srg-requests-age1, fig.height=4, is.fig = TRUE, alt.text = "This figure that there is no trend in the residuals of the fits to the age-1 index">>=
gg
@
\end{center}
...
\caption{Estimates of female spawning biomass for the base model and alternative
  ...
  selectivity from the base model value of 6 to 5, 7, and 8.}
...
These are for changing the maximum age of estimated selectivity to 5, 7, and 8
(Figures~\ref{fig:srg-requests-tv-selex-uncertainty-1}-\ref{fig:srg-requests-tv-selex-uncertainty-3}),
compared to the base model value of 6 (Figure~\ref{fig:main-tv-selex-uncertainty}).
