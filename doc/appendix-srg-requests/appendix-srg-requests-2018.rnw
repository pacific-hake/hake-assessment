...
\subsection*{\altlong\ -- **name to be agreed upon**}  % \altlong
                                        % defined in hake-assessment.rnw
...
\end{center}
\caption{A comparison of MCMC posterior median estimates and 95\% credibility intervals of relative spawning biomass for the base model (blue line) and the \altlong\ (red line)}
...
\end{center}
\caption{Posterior estimates of 2018 forecast catch for the base model and the \altlong. The boxplot central tendency represents the posterior median (value reported at the top). Probability of 2018 stock status (relative spawning biomass) going below B40\%, B20\%, and B10\% are also shown.}
\label{fig:main-sens-alt-status}
\end{figure}
