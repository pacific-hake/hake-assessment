...
are the equivalent to those for the based model run presented in the 
Executive Summary.

For the \altshort\, there is an estimated 
16.4\% joint probability of being both above the target relative fishing intensity
...
Figures~\ref{fig:main-sens-alt-trend}-\ref{fig:main-sens-alt-status} compare
some results from the \altshort\ to those of the based model run.
...
make.median.posterior.table(
  ...
  xcaption = paste0("Time-series of median posterior population estimates from the time-varying fecundity model. Relative spawning biomass is spawning biomass relative to the unfished equilibrium ($\\Bzero$). Total biomass includes females and males of all ages ages 0 and above. Age-2+ biomass includes females and males ages 2 and above. Exploitation fraction is total catch divided by total age-2+ biomass. Relative fishing intensity is (1-SPR)/(1-SPR\\subscr{40\\%})."),
...
\begin{figure}[H]
\begin{center}
<<phase.plot.alt, fig.height=5, fig.width=8>>=
...
<<reference.points.table.alt, results='asis', echo=FALSE>>=
make.reference.points.table(alt.base.model,
                            xcaption <- paste0("Summary of median and 95\\%
                             ...
                             for the \\fishname\\ time-varying fecundity assessment model.
...
<<decisions.biomass.table.alt, results='asis', echo=FALSE>>=
make.decision.table(alt.base.model,
                    xcaption <- paste0("Forecast quantiles of \\fishname\\
                     ...
                     (\\Ffortyten) for the time-varying fecundity model (row g), and the fishing
...
<<decisions.spr.table.alt, results='asis', echo=FALSE>>=
make.decision.table(alt.base.model,
                    xcaption <- paste0("Forecast quantiles of \\fishname\\
...
\begin{figure}[tbp]
\begin{center}
<<forecast.depletion.comparison.plot.alt, fig.height=5, fig.width=8>>=
...
\caption{Time series of estimated relative spawning biomass to \Sexpr{end.yr}
         from the time-varying fecundity model, and forecast trajectories to
...
\begin{figure}[tbp]
\begin{center}
<<forecast.risk.comparison.plot.year.1.alt, fig.height=5, fig.width=8>>=
...
<<risk.forecast.year.1.table.alt, results='asis', echo=FALSE>>=
...
\begin{figure}[tbp]
\begin{center}
<<forecast.risk.comparison.plot.year.2.alt, fig.height=5, fig.width=8>>=
make.forecast.risk.comparison.plot(alt.base.model,
                                   forecast.yrs = forecast.yrs,
...
<<risk.forecast.year.2.table.alt, results='asis', echo=FALSE>>=
make.risk.table(alt.base.model,
                forecast.yrs,
...
\end{center}
\caption{A comparison of MCMC posterior median estimates and 95\% credibility intervals of spawning biomass for the base model (blue line) and the alternative time-varying fecundity model (red line).}
\label{fig:main-sens-alt-trend}
\end{figure}
...
\end{center}
\caption{A comparison of MCMC posterior median estimates and 95\% credibility intervals of relative spawning biomass for the base model (blue line) and the alternative time-varying fecundity model (red line). Relative spawning biomass (depletion) at the start of 2018 is estimated to be 67\% for the base model and 41\% for the alternative model.}
\label{fig:main-sens-alt-status}
\end{figure}
...
\end{center}
\caption{A comparison of MCMC posterior median estimates and 95\% credibility intervals of recruitment deviations for the base model (blue line) and the alternative time-varying fecundity model (red line). }
\label{fig:main-sens-alt-status}
\end{figure}
...
\caption{Posterior estimates of 2018 forecast catch for the base model and the \altlong. The boxplot central tendency represents the posterior median (value reported at the top). Probability of 2018 stock status (relative spawning biomass) going below 
  B40\%, B20\%, and B10\% are also shown.}
\label{fig:main-sens-alt-status}
\end{figure}
