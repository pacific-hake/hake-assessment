...
This appendix contains results from additional analyses requested at the 
Scientific Review Group meeting held from 26th February to 1st
March 2018.
% Use appendix-srg-requests.rnw as a template, which was for 2017.

\clearpage

\subsection*{Residuals for model maximum age of selectivity sensitivities}
...
\subsection*{Alternate time-varying Fecundity model}
...
\end{center}
\caption{Posterior estimates of 2018 forecast catch for the base model and the alternative time-varying fecundity model.  The boxplot central tendency represents the posterior median (value reported at the top). Probability of 2018 stock status (relative spawning biomass) going below B40\%, B20\%, adn B10\% are also shown.}
\label{fig:main-sens-alt-status}
\end{figure}
