...
added by the JTC during the meetinng, and is included in all tables and figures in the
final assessment document.

\section{Run retrospectives on the sensitivity that includes the age-1 index}

The SRG requested the retrospectives be run on the sensivity that includes the age-1 index
in the model. The resulting retrospective cohort plots and associated uncertainty in the
recruitment deviations were viewed and discussed.
...
decision tables, and all other relevant MCMC outputs.
...
make.recr.dev.uncertainty.table(list(base.model,
                                ...
                                xcaption = paste0("Estimated recruitment deviations ",
                                                  "and log standard deviations for age-2 cohorts for the ",
