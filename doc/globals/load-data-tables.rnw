...
load_dir <- system.file("data", package = "hake")
if(load_dir == ""){
...
pkg <- "hake"
assessment.history <-
  read.csv(file.path(load_dir, "assessment-history.csv"))
assessment.changes <-
...
further.tac <-
  further.tac.details(file.path(load_dir, "further-tac-details.csv"))
# * Canadian catch ----
...
# Sampling data ----
sampling.history <-
  load.sampling.history(file.path(load_dir, "fishery-sampling-history.csv"))
# * Canada sampling ----
can.ages <-
  load.can.age.data(file.path(load_dir, "can-age-data.csv"))
can.ft.num.fish <-
