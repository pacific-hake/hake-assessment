...
knit_hooks$set(alt.text = function(before, options, envir){
  if(before && options$is.fig){
    ...
    # Select the first line that is a function call
    code_chunk <- code_chunk[grepl("^[:a-zA-Z0-9_\\.]+\\(", code_chunk)]
    # Remove all instances of par() as they always comes before the function call
...
tweaking. Assume we will go through the figures and tables carefully anyway when
