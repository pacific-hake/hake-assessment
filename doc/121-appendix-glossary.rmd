...
associated with `r f_40`, based on the 40:10 adjustment.
...
of 40%, where 40% relates to the default harvest rate of `r f_40` specified in
the Agreement. Even under equilibrium conditions, `r f_40` would not
necessarily result in a female spawning biomass of `r b_40` because `r f_40` is
...
Default harvest policy (rate): The application of `r f_40` with the 40:10
...
that at the SPR fishing rate `r f_40`, where `r f_40` is the *F* that gives an
...
Metric tonne (t): A unit of mass (often referred to as weight) equal to
...
Pacific Coast Fisheries Information Network (PacFIN): A database that provides
...
priors can be constructed which allow the data to dominate the posterior
...
function of the spawing potential ratio (SPR; top axis) and 1-SPR (bottom
...
model used in this year's assessment.
