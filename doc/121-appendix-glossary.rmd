...
$\Bten$: The level of female spawning biomass corresponding to 10% of
unfished equilibrium female spawning biomass, i.e., $\Bten = 0.1B_0$. This
is the level below which the calculated TAC is set to 0, based on the 40:10
adjustment (see above).

$\Bforty$: The level of female spawning biomass corresponding to 40% of
unfished equilibrium female spawning biomass, i.e., ~$\Bforty = 0.4B_0$.
This is the level below which the calculated TAC is decreased from the value
associated with $\Fforty$, based on the 40:10 adjustment (see above).

$\Bmsy$: The estimated female spawning biomass which theoretically would
...
Benchmark spawning potential ratio ($\mbox{SPR}_{40\%}$): The spawning
potential ratio of 40%, where the 40% relates to the default harvest rate of
$\Fforty$ specified in the Agreement. Even under equilibrium conditions,
$\Fforty$ would not necessarily result in a female spawning biomass of
$B_{40\%}$ because $\Fforty$ is defined in terms of the spawning potential
...
Default harvest policy (rate): The application of $\Fforty$ (see below)
...
$\FSPRforty$: The rate of fishing mortality estimated to give a spawning
potential ratio (SPR, see below) of 40%. Therefore, by definition this
satisfies
\begin{align} 
  0.4
    &= \frac{\mbox{spawning biomass per recruit with }\Fforty}{\mbox{spawning biomass per recruit with no fishing}},
\end{align} 
and SPR$(\Fforty) = 40%$. The 40% value is specified in the Agreement.

\FSPRfortyten harvest policy: The default harvest policy (see above).
...
the SPR fishing rate $\Fforty$, where $\Fforty$ is the $F$ that gives an
SPR of 40% such that, by definition, SPR$(\Fforty) = 40\%$ (the benchmark
spawning ratio). Therefore
\begin{align}
  (\#eq:appendix-glossary-spr)
  \mbox{relative fishing intensity for }F
    &= \frac{1-\mbox{SPR}(F)}{1-\mbox{SPR}(\Fforty)} \\
    &= \frac{1-\mbox{SPR}(F)}{1-0.4} \\
    &= \frac{1-\mbox{SPR}(F)}{0.6},
\end{align}
as shown in Figure~\@ref(fig:glossary-fishing-intensity-illustration-fig).
For brevity we use $\mbox{SPR}_{40\%} = \mbox{SPR}(\Fforty)$ in the text.
...
$\Fmsy$: The rate of fishing mortality estimated to produce the maximum
...
t: Metric ton(s). A unit of mass (often referred to as weight) equal to
1,000 kilograms or 2,204.62 pounds. Some previous stock assessments used
the abbreviation "mt" (metric tons).
...
spawning biomass to the unfished equilibrium female spawning biomass ($B_0$,
...
proportion of $R_0$ expected (on average) when the female spawning biomass
is reduced to 20% of $B_0$ (i.e.,~when relative spawning biomass is equal
to 20%).
...
(ref:fishing-intensity-illustration-cap) Fishing intensity as a function
...
(ref:fishing-intensity-illustration-alt) This figure shows how fishing
intensity linearly increases with SPR.

```{r glossary-fishing-intensity-illustration-fig, fig.cap = "(ref:fishing-intensity-illustration-cap)", fig.height = 5, fig.width = 5, is.fig = TRUE, alt.text = "(ref:fishing-intensity-illustration-alt)"}
# Do as a function so that variables remain local
plot_intensity <- function(target = 0.4){
...
plot_intensity()
```
(ref:spr-illustration-cap) Illustration of the spawning potential ratio
(SPR) calculation based on the combination of maturity and fecundity used
in the model, using the maximum likelihood estimates of natural mortality,
selectivity, and fishing mortality in the final year of the base model from
this year.

(ref:spr-illustration-alt) This figure shows how SPR is calculated.

```{r glossary-spr-illustration-fig, fig.cap = "(ref:spr-illustration-cap)", fig.height = 7, fig.width = 6, is.fig = TRUE, alt.text = "(ref:spr-illustration-alt)"}
