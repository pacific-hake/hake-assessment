...
No additional analysis of maturity samples collected in 2015 have been done, but results reported in the \fishname\ assessment from 2015 \citep{JTC2015} indicated that maturity-at-age and length observations show differences across years (Figure~\ref{fig:main-proportion-mature-by-source}). It has been difficult to determine if these difference are due to the source (bottom trawl, acoustic survey, or ASHOP) or the year. Investigating data through **2014 showed that \fishname\ south of 34.5 degrees latitude (approximately Point Conception) mature at a smaller size (Figure~\ref{fig:main-proportion-mature-by-latitude}). The trawl survey is the only source of the three analyzed here that samples in that area, and genetic samples were collected in 2015 to determine if there is any stock structure that could help to explain this. Another interesting observation from the maturity data is that there are large, old fish classified as immature \citep{JTC2015}. It is believed that these fish may be mature, but are ``skip spawners'' and will not be spawning in the upcoming year.
