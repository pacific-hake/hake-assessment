...
Testing that \citet{JTC2016} is okay.


From 2014:

Appendix C. Details on non-parametric selectivity
...
This formulation has the properties that the maximum selectivity equals 1, positive values are associated with increasing selectivity between ages $a-1$ and $a$, and negative values are associated with decreasing selectivity between those ages. Beyond the maximum age for which selectivity is estimated (**6 in the base model), $p_a = 0$, resulting in constant selectivity beyond the last estimated value. The condition that maximum selectivity equals 1 results in one fewer degree of freedom than the number of estimated $p_a$. Therefore, $p_{A_{\text{min}}}=0$ can be set for the fishery and for the survey.

%corresponding to the first age of estimated selectivity ($p_1$ for the fishery and $p_2$ for the survey), is fixed at 0. **Add issue - don't understand last two.
...
are additional parameters estimated in the model. The values of 
