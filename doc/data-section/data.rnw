...
For this assessment, the data from all surveys since 1998 were scrutinized and reanalyzed using the same geostatistical techniques, but with more robust assumptions and some corrections. These include:

\begin{itemize}
  
\item the minimum ($k$\subscr{min}) and maximum ($k$\subscr{max}) number of points used to calculate the value in a cell;
  
\item standardizing the search radius to be three times the length scale that is estimated from the variogram; 

\item a decay of estimated biomass the farther predictions are made from the end of a transect;
  
\item correcting spurious off-transect zeros that were erroneously generated in previous export of data;
  
\item all data from 1998 onwards were re-analyzed using an updated version of the EchoPro software with consistent data input files.  

\end{itemize}

The net result of these changes is a consistent approach applied to all survey years (Table~\ref{tab:main-kriging-parameters}). Therefore, the biomass indices (Table~\ref{tab:main-survey-history} and Figure~\ref{fig:main-survey-biomass}) and age compositions (Figure~\ref{fig:main-age-comp-bubbles}, top) are new for this assessment and differ from the 2015 assessment \citep{JTC2015}.

Table caption: Summary of key kriging parameters for the acoustic survey as used in the 2015 and 2016 assessments. Search radius is the distance in the transformed space from which observations are drawn to calculate weights in the kriging. Parameters $k$\subscr{min} and $k$\subscr{max} are the minimum and maximum number of data points used to calculate a kriged value. Length scale is a parameter estimated from the variogram for each year.
