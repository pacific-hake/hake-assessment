...
However, the data from the 1995 survey was not in a format that could be processed, so the biomass index and associated age composition for that year were excluded from the model. Those 1995 data have now been processed in the same manner as the subsequent years and included in the time series.
