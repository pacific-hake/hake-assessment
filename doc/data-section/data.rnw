...
\item Age compositions composed of data from the U.S. fishery (\Sexpr{max.fishery.age.prop[1]}--\Sexpr{max.fishery.age.prop[2]}) and the Canadian fishery (1990--\Sexpr{max.fishery.age.prop[2]}). The last 9 years of these data are shown in Tables \ref{tab:main-us-cp-age-data}--\ref{tab:main-can-ft-age-data}.

\item Biomass indices and age compositions from the Joint U.S.~and Canadian integrated acoustic and trawl survey (\Sexpr{survey.age.years[-length(survey.age.years)]} and \Sexpr{survey.age.years[length(survey.age.years)]}; Table \ref{tab:main-survey-history}).
