...
Time-varying fishery selectivity was introduced in the 2014 assessment \citep{JTC2014} and is modelled with yearly deviations applied to the selectivity-at-age parameters. A penalty function in the form of a normal distribution is applied to each deviation to keep the deviation from straying far from zero, unless the data are overwhelming. The amount of deviation from zero is controlled by a fixed standard deviation, $\phi$ (explained further below).

Recent assessments \citep{JTC2014, JTC2015, JTC2016} used $\phi=0.03$, which was estimated externally by treating the deviations as random effects and integrating over them using the Laplace method, as described by \citet{ThorsonHicksEtAl2014}. This parameterization allowed for the estimation of time-varying selectivity without allowing large year-to-year changes. However, for this assessment, $\phi=0.03$ led to a record-high 2014 year class, resulting in a huge estimated current biomass (Figures~\ref{fig:main-biomass-base-tv-increase-1} and \ref{fig:main-status-base-tv-increase-6}). The biomass estimates for 2014 and earlier are relatively insensitive to the tested values of $\phi$.

**Ian to add text explaining $\phi=0.20$, partly based on \citet{ThorsonHicksEtAl2014} and Figures~\ref{fig:main-biomass-base-tv-increase-1} and \ref{fig:main-status-base-tv-increase-6} ($\phi=0.30$ not too different from 0.20).
