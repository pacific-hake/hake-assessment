...
last_2yr_can_attained_diff <- ((ct_last1 |>
                                 ...
                                 (ct_secondlast |>
                                    pull(can_attain))) |>
  f(1)
last.year.can.landings <- ct_last1 |>
  pull(can_catch) |>
  f(0)
last.year.can.tac <- ct_last1 |>
  pull(can_tac) f(0)
last.year.can.tac.jv <- ft |>
  filter(Year == last_data_yr) |>
  pull(can.jv.tac) |>
  f(0)
last.year.can.shoreside.tac <- ((ct_last1 |>
                                   pull(can_tac)) -
                                  (ft |>
                                     filter(Year == last_data_yr) |>
                                     pull(can.jv.tac))) |>
  f(0)
latest.year.can.jv <- ct |>
  filter(can_jv_xx > 0) |>
  pull(year) |>
  max()
...
fore.tac.mcmc.yr1 <- base_model$forecasts[[1]][[ct_tac_ind]]$mcmccalcs
...
fore.tac.mcmc.yr2 <- base_model$forecasts[[2]][[ct_tac_ind]]$mcmccalcs
...
next.treaty.catch <- f(base_model$catch.levels[[ct_default_policy_ind]][[1]][1], 0)
pfmc.prob.next.year.below.b40 <- f(base_model$risks[[1]][ct_default_policy_ind, paste0("Bratio_", assess_yr + 1, "<0.40")], 0)
pfmc.prob.next.year.below.b25 <- f(base_model$risks[[1]][ct_default_policy_ind, paste0("Bratio_", assess_yr + 1, "<0.25")], 0)
same.catch.as.last.year <- f(base_model$catch.levels[[ct_actual_ind]][[1]][1], 0)
same.catch.prob.next.year.below.b40 <- f(base_model$risks[[1]][ct_actual_ind, paste0("Bratio_", assess_yr + 1, "<0.40")], 0)
same.catch.prob.year.after.next.below.b40 <- f(base_model$risks[[2]][ct_actual_ind, paste0("Bratio_", assess_yr + 2, "<0.40")], 0)
...
last_year_can_ages_ft <- can_ages_lst[[2]][rownames(can_ages_lst[[2]]) == last_data_yr, ]
...
last.year.can.ages.ss <- can_ages_lst[[1]][rownames(can_ages_lst[[1]]) == last_data_yr, ]
