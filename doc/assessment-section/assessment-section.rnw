...
It is difficult to operationally assess the accuracy of projections in fisheries
assessments because the truth is never known with 100\% certainty. However,
starting in 2021 \citep{JTC2021}, based on a question from an industry representative, the JTC set out to assess the performance of
the projections provided in decision tables
...
As an example, the 2019 assessment \citep{JTC2019} gave the estimated probability of the spawning stock
...
Figure~\ref{fig:main-historical-1}). Also
the estimates from the historical assessments
...
This is because there is no definitive trend in biomass around 2018
(Figure~\ref{fig:main-female-spawning-biomass}),
and probabilities of annual decline are
expected to be sensitive to slight changes in results in such cases (and whether
...
assessment estimates that the 2020 cohort was particularly large and that its total biomass increased by
...
From this current \Sexpr{assess_yr} assessment's projections, we show the probabilities for
all catch alternatives in Figure~\ref{fig:main-historical-1} because
we do not yet know which will correspond to the \Sexpr{assess_yr}
catch. Catching zero fish in \Sexpr{assess_yr} (colored in pink) obviously gives
...
likely decline in spawning biomass.
...
below $\Bforty$ due to the very large
...
and~\ref{fig:main-historical-2}, which calculate the probabilities using the
...
The retrospective probabilities of decline
(colored squares in
