...
likehood approach (\citet{ThorsonEtAl2017Dirichlet}) was used to estimate the weights
associated with age compostion data, rather than iteratively tuning the sample size multiplier
...
observation error. A Dirichlet-Multinomial (D-M) likehood was applied to age-composition data,
...
extended to 2018, but the estimates for 2018 remaining hightly uncertain 
...
relatively weak link between spawning bimoass and recruitment.
...
convergence was adquately achieved (Figure~\ref{fig:main-mcmc.diag.gelrub}). 
...
1999 cohort occuring in the survey at ages 2 and 4 but not 3 
...
\item Use of an alternative method for aggregating the age compostitions from
...
uncertain size of the 2016 cohort was more variable across sensitivity analyes
...
resuled in a model where the standard deviation of the MLE estimates of 
...
Use of an alternative method for aggregating the age compostitions from
the U.S. fishery (weighting by the catch within each month)
...
mortalty, which aren't represented in the model, would lead to a shift
...
at 0.21 (equilvalent to $\phi=0.03$ in the previous setup) estimates 
...
the model structure has remained relatively consistent in recent years, the 2017 acousitc 
