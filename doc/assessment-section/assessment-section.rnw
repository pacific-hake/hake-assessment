...
\item The next priority for 2018 is processing the archived ovary collections of
  ...
  \emph{Response -- } We estimated a new maturity ogive based on
    1,947 samples with ages from north of 34.44$^\circ$~N, as discussed in
    **section ??. **Confirm which further points of the SRG request this
    did or did not address. **Maybe also see notes in Issue \#325. 
...
\item The SRG notes that MCMC convergence diagnostics for the 2017 base model
  ...
  \emph{Response -- } **We ran the MCMC algorithm for ** iterations
  and retained \Sexpr{num.mcmc.samples} for the base model. **..
...
\item The following two recommendations are to fulfil requests for specific
 ...
 \begin{itemize}
  ...
  \emph{Response -- The exploitation rates by age and year 
    ...
    We included the requested estimated fate of cohorts for strong cohorts
    only (Table~25).}
    ...
    respectively. **Clarify if cohort table was only requested for strong
    cohorts -- I kind of think it was but the SRG report doesn't specify; will 
    make an Issue to discuss.}
...
**From 2017 assessment: have deleted some but keeping some here for reference before
deleting....
