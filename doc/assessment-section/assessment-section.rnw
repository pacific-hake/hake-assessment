...
** Andy to shorten this whole section
...
% gives the probs, maybe worth making a table

As an example, the 2019 assessment \citep{JTC2019} provides the estimated probability of the spawning stock
biomass declining in the subsequent year, i.e.,~P($B_{2020} < B_{2019}$),
for several possible catches in 2019, such as 0~t, 180,000~t,
350,000~t, 410,000~t etc. Now, in \Sexpr{assess_yr}, we `know' that the catch in 2019 was
...
Interestingly, in the last assessment (Figure~B.3.1 of \citealt{JTC2021}) we found that
P${\protect (B_{2019} < B_{2018})}$
from the
...
the lowest probability that the stock will decline from \Sexpr{assess_yr} to \Sexpr{assess_yr+1}.
...
for P$(B_{2023} < \Bforty)$, presumably again due to the incoming 2020 cohort.
...
% from projections in this year's assessment, P$(B_{\Sexpr{assess_yr+1}} <
...
framework (Table~\ref{tab:main-assessment-changes}) beyond recent inclusion of the age-1 index are improving
understanding of stock dynamics.

% % Kelli's versions in 2022, can use some of next year to make a shorter version. Kept longer
% %  version in 2022 (but used some of these edits) to explain more clearly.
