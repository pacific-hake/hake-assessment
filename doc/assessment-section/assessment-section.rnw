...
the software version (3.24U) used in the previous assessment \citep{JTC2016}.  In addition
to the new version of Stock Synthesis, the largest changes between the
\Sexpr{assess.yr} and \Sexpr{last.assess.yr} stock assessments is the addition of another
year of acoustic survey data, a new maturity ogive, a new approach for weighting composition
data, and a new approach for parameterizing time-varying selectivity in the base
model. The \Sexpr{assess.yr} base model includes an acoustic data time series from 1995 to 2017,
where estimates of abundance from the survey followed the same procedures as in 
\Sexpr{last.assess.yr}.  An updated maturity ogive was estimated and incorporated into the
\Sexpr{assess.yr} base model (see Section~\ref{sec:data-maturity}). The Dirichlet-Multinomial (D-M)
likelihood approach (\citet{ThorsonEtAl2017Dirichlet}) was used to estimate the weights
...
unchanged from 0.20 used in the \Sexpr{last.assess.yr} base model to a value of 1.4.
In Stock Synthesis version \Sexpr{ss.version}, deviations associated with time-varying
selectivity are no longer logistically transformed such that 1.4 is effectively the same
as 0.2 (see Section~\ref{sec:data-variability-selectivity}). Otherwise, the general parameterization of
...
deviation estimates, and is the same as assumed in 2013, 2014, 2015, 2016, and
...
fishing fleets, and the number of trawl sets in the research surveys. A weighting parameter for the fishery
...
For this assessment document, we have further refined the Glossary
...
The Dirichlet-Multinomial method represents an replacement of the status-quo
...
biomass. The new fecundity estimates have a larger increase from age 2 to age 3
...
as the updated fecundity estimates show smaller increase from age 4 to 5 
than the previous estimates, such that the loss due to mortality is greater
...
relatively weak link between spawning biomass and recruitment.
