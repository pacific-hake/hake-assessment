...
$B_{10/%}$ ($0.1B_0$) in 2019 is
less than 1\% and the probability of dropping below $B_{40\%}$ is less than 24\%
...
which considers a combination of the variability among the
...
$\mbox{SE}(\hat{r}_y)$ is the standard error of each estimate 
...
parameters which automatically weight each of the fishery and survey age 
...
previous assessments, where the input samples sizes are iteratively 
...
composition was also explored through sensitivity model evaluations. In most years, 
...
was minor differences in the overall fishery age composition: less than a 0.4\% change for 
...
**Aaron, can you add something about the following:
Use of an alternative method for aggregating the age compositions from
the U.S. fishery (weighting by the catch within each month)
...
differences in the dynamics of these cohorts, such as difference in natural
mortality, which aren't represented in the model, would lead to a shift
