...
algorithm with a similar number of posterior samples as the base model. Comparisons of the
...
\Sexpr{assess_yr} base model. Here we focus on the main sensitivities which are:
...
\item Consideration of higher standard deviations on the prior distribution
  used in the base model for natural mortality;
...
\item Consideration of an alternative prior distribution and a fixed value for steepness;
...
driven by the 2010, 2014, and 2016 cohorts followed by severl years of steady decline
(2019--2021) before increasing again due to a strong 2020 cohort.
...
base-model value of 1.4 than the 1.77 estimated in the 2021 assessment.
...
associated with recent age-1 indicies (i.e., 2018 and 2020 cohorts detected in the 2019 and 2021 age-1 index).
In particular, the base model with the age-1 index suggests that the 2020 year class is estimated to be large,
but not as large as the 2021 and 2022 fishery data alone (i.e., removal of the age-1 index) would otherwise
...
**It also provides the worst fit to the most recent survey biomass estimate
...
Table~\ref{tab:main-assessment-changes}; \citet{JTC2020}), lead to a very similar spawning
...
were ubiquitously more negative than in the base model
