...
part of the same survey, on the same vessels (but no value is available for 2001). Selectivity was set to one for age-1 for this index
...
\emph{The Management Strategy Evaluation (MSE) framework for \fishname\ creates
  ...
  Research projects using the MSE framework are underway to evalute the robustness
...
\emph{We again include Appendix~\ref{chap:rec-plot} to promote continued discussion on ideas
...
\emph{Response -- The JTC agrees that future reference point discussions stemming
  ...
  the JTC continues to have interest in exploring the utility of dynmaic reference points.
  ...
  initiate simulations that explore alterantive dynamic F-based target reference points coupled
  with static (equilibrium-based) biomass limite reference points in exploratory harvest control
  rule scenarios.}
...
\emph{Over the past year, the JTC has continued to engage in research activities and outreach related to
  ...
  Commission science provider (Secretariate of the Pacific Community), to learn from thier experience developing
  and applying dynmaic reference points to operational higly migratory species assessments and management plans.
  Additionally, the JTC is contributing to, and following, updates to Canadian and United States national guidance
  documentation on the incorporation of prevailing ennvironmetnal conditions into stock assessments and
...
\emph{Response -- \fishname\ ovary samples were not collected over the past year, but plans include more
  ...
  remains a goal, but barriers (lab training, capacity, and moving bio-samples across the borader) still
...
\emph{Response -- It is important to capture key, mechanistic and/or stochastic population processes
  ...
  on a newly funded research project aimed at incorporting environmentally-driven growth into the
...
\emph{Empirical weight-at-age data indicate inter-annual variation in \fishname\ growth, and this variation could
  ...
  by a graduate student looking at variation in \fishname\ weight-at-age.
...
\Sexpr{round(median(bridge_models_2[[1]]$mcmc$Bratio_2022),3)*100}\%. to
\Sexpr{round(median(base_model$mcmc$Bratio_2022),3)*100}\%.
