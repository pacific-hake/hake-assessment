...
Dirichlet-Multinomial parameters used to weight the age-composition data.
...
genetic analyses. That team provided the following update.}
...
\emph{Otolith samples of approximately 50 fish are requested of at sea observers working on foreign joint venture hake vessels. This target sample size has not changed over time. Dockside observers are asked to collect approximately 50 fish for their length-sex-maturity-age-weight samples. This target sample size has not changed over time.}
...
joint probility of being both above the target relative fishing intensity in
