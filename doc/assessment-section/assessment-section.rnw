...
\item Correction to catch to account for double counting of the Canadian JV
  catch from 2007 to present, and missing US tribal catch for 2016 and 2017.
...
\item *****Catch corrections, as discussed in Section ****.
...
An error in the code which extracts and calculates the catch for Canadian fisheries was found
late in the assessment process. The JV catch was erroneously being added to the Shoreside
catch, resulting in the values being added to the total catch twice. This error was for
each year the JV fishery took place since 2007 including 2007, 2008, 2010, 2011, and 2017.
In addition, some U.S. tribal catch was not reported in time to be added to the base model.
There was some fish ticket information which was missing at all levels. The values which
changed were for the 2016 and 2017 seasons and were 2,522.75 t and 6405.86 t respectively.
These values were added to the base model catch values, and the JV values were subtracted to
create one sensitivity which is included in
Figures~\ref{fig:main-biomass-base-alternative-2} and \ref{fig:main-status-base-alternative-2}.
The sensitivity shows that running the base model with the corrected values shows no
difference in the model outcome, with the trajectories overlapping
for the entirety of the time series.
