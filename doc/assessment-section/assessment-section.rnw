...
year of
% acoustic survey and 
fishery data,** a new maturity ogive, a new approach for weighting composition
data, and a modification of the parameterization for time-varying selectivity.

**mention time-varying fecundity, basedon the SRG-requested alternative model in
2018. Refer to later section.

**The \Sexpr{assess.yr} base model includes an acoustic data time series from 1995 to 2017,
where estimates of abundance from the survey followed the same procedures as in 
2017 \citep{JTC2017}. The updated maturity ogive from 2018 was retained
...
Section~\ref{sec:data-age-comp-likelihood}). **[presumably no need to mention
$\phi$, just $\Phi$] Time-varying fishery
selectivity is retained in the \Sexpr{assess.yr} base model with the magnitude of the allowable deviations effectively
unchanged from the $\phi=0.20$ used in the \Sexpr{last.assess.yr} base model to
an equivalent value of $\Phi = 1.4$ (see Section~\ref{sec:data-variability-selectivity}).
Otherwise, the general parameterization of
...
estimate calculated analytically as per \citet{LudwigWalters1981}. Maturity
and fecundity relationships are assumed to be time-invariant and fixed values
were updated using the new maturity ogive discussed earlier.

**Add any new differences to the above. Time-varying fecundity if not mentioned earlier
...
composition data. Tuning quantities changed again this year with the addition of the
new D-M data weighting (tuning) approach.
...
\emph{Response -- **Use Ian's Issue \#464 text and refs.}
...
**These are all from the 2018 assessment (based on 2017 SRG); keep for now for
...
\item In the current assessment, the change from $\phi = 0.03$ to $\phi = 0.2$
  ...
  \emph{Response -- The JTC has conducted several analyses in response to this 
    request including evaluating alternative $\phi$ values in sensitivity trials 
    and exploring an alternative ``semi-parametric'' parameterization of the 
    approach to specifying time-varying selectivity, all described in 
    Section~\ref{sec:assessment-sensitivity-analyses}. Although the 
    semi-parametric option shows promise, no considered alternative was a clear
    improvement over the status-quo parameterization with the larger $\phi$ 
    value (previously $\phi = 0.2$ but now converted to $\Phi = 1.4$ after
    removing a logistic transformation as described in 
    Section~\ref{sec:assessment-changes}). 
    The data from 2017 suggest that the 2014 cohort was more 
    in line with the estimates from the model with more (rather than less) flexible
    fishery selectivity (larger $\phi$), providing support for maintaining that 
...
\item The next priority for 2018 is processing the archived ovary collections of
  ...
  \emph{Response -- The JTC worked with the Northwest Fisheries Science Center
    (NWFSC) to estimate a new functional maturity ogive included in the base model
    which is based on histological examination of 1,947 ovary samples with ages 
    from north of 34.44$^\circ$~N, as discussed in 
    sections~\ref{sec:data-maturity},~\ref{sec:assessment-modeling-results}, 
    and~\ref{sec:assessment-sensitivity-analyses}. 
    A thorough analysis of whether maturity is more dependent upon age or 
    upon weight has not yet been completed although the greatest variability
    in weight occurs at older ages which are mostly mature.
    Models were explored that assumed fecundity-at-age varied over 
    time in proportion to mean weight-at-age, but the impact of that 
    alternative assumption was small due to the 
    weak link between recruitment and spawning biomass and the 
    lack of information about weight-at-age in the unfished equilibrium.
    Further analysis of the relationships 
    between fecundity, age, and weight would depend upon collecting whole ovaries
    rather than ovary samples. The difference in 
    maturity between fish sampled north and south of Point Conception is strongly
    evident when considering maturity-at-age as well as maturity-at-length, and
    NWFSC is seeking to conduct genetic analyses that could test whether
    the southern samples are from a distinct stock.}
...
\item The SRG notes that MCMC convergence diagnostics for the 2017 base model
  ...
  diagnostics, we also evaluated multi-chain diagnostics (Gelman-Rubin) that compare
  within-chain and among-chain variances to further diagnose convergence.  The R package
...
\item The list of sensitivity tests presented in the 2017 assessment covers the
  ...
  \emph{Response -- The 2018 assessment includes all of the key sensitivities that
...
\item The SRG supports continued development of an age-1 index from the acoustic
  ...
  \emph{Response -- The 2018 assessment includes a sensitivity model run that 
  incorporates the acoustic survey age-1 index (see Figures~\ref{fig:main-backscatter-map-age1},~\ref{fig:main-biomass-base-alternative-2}, and~\ref{fig:main-status-base-alternative-2}).  The age-1 index time series was 
...
\item The following two recommendations are to fulfill requests for specific
 ...
 \begin{itemize}
  ...
  \emph{Response -- The exploitation rates by age and year 
    ...
    include the fate of other cohorts can be accommodated upon request. These tables 
    are included in this \Sexpr{assess.yr} assessment as 
...
**Update once bridging finalised - done 1/31/19, AMB
...
assessment were now included. These changes were small enough that they had
...
(Figure~\ref{fig:main-bridge-5-panel}) in the absence of additional information 
...
estimates without those data (**Figure~\ref{fig:main-bridge-5-panel}).  However, 
the addition of these data did slightly alter recent recruitment
estimates, including an increase in the estimated size of the 2016 year class.
...
any data for the forecast years. The addition of the 2018 fishery data, which
included a relatively higher number of age-1 fish from the 2017 cohort and
...
The final bridging step (Time-varying fecundity (= base model), as discussed
in Section~\ref{sec:data-maturity})
resulted in the biggest change to current biomass and became the 2019 base model.  Fecundity
(calcualted as weight-at-age multiplied by maturity) changed from being time-invariant
(i.e., using a single catch-weighted average of weight-at-age over all years)
to time-varying. In this case, fecundity was based on year specific weight-at-age
over years with data (1975-2018). For periods without data (pre-1975 and post-2018), 
catch-weighted average weight-at-age was used (see Section~\ref{sec:data-maturity}
and Section~\ref{sec:data-weight-at-age}for more details).  The results of this
step caused a visible change in the time series
of spawning biomass (Figure~\ref{fig:main-bridge-4-panel}) 
...
each 10,000th value thereafter was retained, resulting in 2,000
samples from the posterior distributions for model parameters and derived
quantities. **Initial MCMC explorations indicated that $\log
...
\Sexpr{end.yr} with both the catch from default harvest rule in \Sexpr{end.yr} 
and the recruitment in 2014 being highly correlated, as to be expected given the dependencies
...
remains similar to the \Sexpr{last.assess.yr} base model up through 2013. The addition of the 2017
survey biomass estimate resulted in a downward shift in the fit to the 2015 survey data
point and a leveling off of the biomass trend over recent years (Figure~\ref{fig:main-bridge-5-panel}). 
...
dominated by age-3 fish from the 2014 year-class (33\% of the catch in the
fishery) and age-7 fish from the 2010 year-class (40\% of the catch in the
fishery).  Age composition from the 2015 acoustic survey also indicated that the
2010 year-class was large (59\% of the catch for that year).  The 2017 acoustic survey
was the first time the survey was able to sample the 2014 year-class, and it confirmed
that the 2014 year-class (53\% of the catch for that year) was indicated above average. The
...
model. Combined, the 2015, 2016, and 2017 fishery age composition data and the 2017
...
**The \Sexpr{assess.yr} base model specified a similar level of variation (standard
deviation of $\Phi = 1.4$) associated with time-varying fishery selectivity to that of
$\phi = 0.20$ in the 
\Sexpr{last.assess.yr} base model, effectively
...
in 2016.  The selectivity of age-3 fish by the fishery in 2017 (2014 cohort) 
...
recruitment in the time series, and a slight decrease from 2017 to 2018 due, in part, to
the model fit of the acoustic survey index (Figure~\ref{fig:main-bridge-5-panel}).
...
**In the 2017 assessment, the equivalent estimate of 2017 female spawning biomass was
2.129~(0.763-7.445)~million~t. Thus the 2018-estimated median is smaller than the 2017-estimated
median, while the 2018 credible interval remains within the 2017 interval. The additional
...
33\% of the 2016 catch, and
40\% of the 2017 catch.
The current assessment also estimates a strong 2014 year class
(Figure~\ref{fig:main-numbers-at-age}) comprising 50\% of the 2016 catch and 33\% of the 2017
catch. Although the absolute size of the 2014 year class remains highly uncertain, at least more so than
cohorts that have been observed for more years, two years of fishery data and one year of survey data
suggest that it is one of the higher estimates in the time series.

**The extra data in the 2018 assessment has reduced the median estimate of the
2014 year class to 
\Sexpr{recruitment.med.in.2014}~billion fish (Table~\ref{tab:main-median-posterior}),
from the 12.105~billion estimated in the 2017 assessment (Table~18 of \citealt{JTC2017}).
...
**Probably mention 2016 recruitment, as first year we've seen it and it was
highly uncertain last year (and obvoiusly remained so in our runs without 2018 age data).
...
series of median recruitment deviation estimates for the years 1970-2015, which
are informed by the age compositions, is
\Sexpr{f(sd(base.model$mcmccalcs$devmed[names(base.model$mcmccalcs$devmed) >=
1970 & names(base.model$mcmccalcs$devmed) <= 2015]), 2)}. This value is consistent
...
in 2015, after which it increased to \Sexpr{median.intensity.2017}\% in 2017.
The exploitation fraction has decreased from
\Sexpr{exploitation.med.2011} in 2011 to \Sexpr{exploitation.med.2015} in
2015 and then increased to \Sexpr{exploitation.med.2017} in 2017.
...
for 2016 and 2017 are below the management target, however the 95\% posterior
...
increase.  The utilization rate in 2017 continued to increase from the 2015 low in
...
intensity and relative spawning biomass, the model predicts a less than 6\%
...
\item Use of an alternative method for aggregating the age compositions from
  the U.S. fishery (weighting by the catch within each month)

\item Consideration of an alternative catch stream which addresses a 
  few issues related to the annual catch in recent years.
...
uncertain size of the 2016 cohort was more variable across sensitivity analyses
...
% No change needed in 2018 (so far)
...
However, the high $\sigma_r$ model had a larger difference between the unfished
equilibrium and the initial year of the model 
...
(Figure~\ref{fig:main-age-one-index}). These changes are subtle, however
since the base model generally tracks the trends in the age-1 index well.

% Manual 2018: didn't change much other than shrinking this paragraph a bit from 2017
...
As noted in the description of assessment changes in 
Section~\ref{sec:assessment-changes}, the base model used a new
Dirichlet-Multinomial likelihood which includes two additional estimated 
parameters that automatically weight each of the fishery and survey age 
compositions. This produced higher weights on the age compositions but 
generally very similar results to the McAllister-Ianelli method used in 
previous assessments, whereby the input samples sizes are iteratively 
adjusted by a multiplier to make the arithmetic mean of the input 
...
% Manual 2018
Three types of alternative setups for selectivity were explored in sensitivity
analyses: alternative assumptions about the age beyond which selectivity 
is assumed constant, alternative values of the $\Phi$ parameter 
...
% Manual 2018: lots of changes to paragraphs below
...
% Manual 2018: parameter counts below aren't dynamic
...
$\Phi = 7 \phi$
resulting in
approximately equivalent dynamics. Thus, the base model value is $\Phi = 1.40$, and
...
which is only informed by the 2017 fishery age composition, is strongly
linked to the choice of $\Phi$, where the model with the smallest $\Phi$ 
at 0.21 (equivalent to $\phi=0.03$ in the previous setup) estimates 
the 2016 recruitment deviation as the highest in the time series 
...
2 pairs of parameters with correlations above 0.7 or below -0.7 in the 
semi-parametric model that most closely matched the based model 
($M$ and $log(R_0)$ were positively correlated, and the deviation for 
selectivity at age 1 in 2017 was negatively correlated with 2016 recruitment).
This is in contrast to the base model, which had 36 parameter pairs with
correlations above 0.7 or below -0.7, of which 34 of the pairs were 
between two of the selectivity deviations. The Laplace method of 
...
of this cohort that has only been observed as age-1 in the 2017 fishery.
...
ahead for potential inclusion in a future hake assessment.
