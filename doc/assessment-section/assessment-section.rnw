...
(\Sexpr{start_yr_age_comps}--\Sexpr{last.data.yr}; see Section~\ref{sec:data-weight-at-age}).
The D-M
...
The addition of \Sexpr{last.data.yr} catch and fishery age compositions extends the model
...
The \Sexpr{survey.end_yr} survey biomass estimate resulted in a slight upward shift
...
%trend that has been relatively stable from 2013 to \Sexpr{survey.end_yr}.
...
Age compositions from the \Sexpr{survey.end_yr} acoustic survey suggest a similar age
structure, i.e., \Sexpr{top.coh(base.model, survey.end_yr, 3, cap = FALSE, fleet = 2)}.
Combined, the 2015--\Sexpr{last.data.yr} fishery age-composition data and the
2017--\Sexpr{survey.end_yr} acoustic survey age-composition data
...
%\Sexpr{ceiling((survey.end_yr-2016)/2)} times
still appears to be smaller than
the 2014 cohort.
The \Sexpr{survey.end_yr} survey was the first to sample the
\Sexpr{survey.end_yr-2} cohort, confirming that
it
was not extremely large (\Sexpr{survey.a2.prop}\% of the \Sexpr{survey.end_yr} survey catch).
Residual
...
age. This level
of variation led to results that were consistent with the \Sexpr{survey.end_yr} acoustic survey
