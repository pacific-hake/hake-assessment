...
quantities. Initial MCMC explorations indicated that the $\log(\theta)$
Dirichlet-Multinomial (D-M) weighting parameter for the survey
was not being sampled efficiently due to many samples occurring in a part of
parameter space where the effective sample size multiplier, $\theta/(1+\theta)$,
...
Therefore, to improve MCMC convergence, $\log(\theta_{\mbox{surv}})$ 
was fixed at the MLE estimate of $\log(\theta_{\mbox{surv}})$ = \Sexpr{log.theta.survey}, 
corresponding to a weight of 
$\theta_{\mbox{surv}}/(1+\theta_{\mbox{surv}})$ = \Sexpr{DM.weight.survey}. 
The D-M parameter for the fishery weights was well sampled by the MCMC, 
with a median estimate of 
$\log(\theta_{\mbox{fish}})$ = \Sexpr{log.theta.fishery.median} and an associated
median weight of 
$\theta_{\mbox{fish}}/(1+\theta_{\mbox{fish}})$ = \Sexpr{DM.weight.fishery.median}.
...
~\ref{fig:main-mcmc.diag.h.extra.sd}). The Gelman-Rubin multi-chain diagnostic test, 
...
(Figure~\ref{fig:main-mcmc-survey-fit}, Figure~\ref{fig:main-survey-extrap-biomass})
...
and no direct cause for this is known.  The survey did began earlier that year than all other surveys
...
from the 2012 fishery composition data onward, and thus are fit well by the
...
distribution and the prior may be constraining the posterior distribution. Broadening
the prior distribution by increasing the prior standard deviation for the natural mortality  
parameter is examined as sensitivity runs (see Section~\ref{sec:assessment-sensitivity-analyses}).
...
deviation) associated with time-varying fishery selectivity to that in the 
...
age. This value is 1.4 in Stock Synthesis version \Sexpr{ss.version}). This level
of variation led to results that were more consistent with the 2017 acoustic survey
biomass estimate and reasonable fits to the fishery age composition data, 
...
Fishery selectivity on age-2 fish was the highest throughout the time
series in 2016.  The selectivity of age-3 fish by the fishery in 2017 (2014 cohort) 
