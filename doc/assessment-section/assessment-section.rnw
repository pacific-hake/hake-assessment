...
\citep{MethotWetzel2013} version \Sexpr{ss.version} was used 
for the first time this year, which is a significant update to
the software version (3.24U) used in the previous assessment \citep{JTC2017}.  In addition
to the new version of Stock Synthesis, the largest changes between the
\Sexpr{last.assess.yr} and \Sexpr{assess.yr} stock assessments are the addition of another
year of acoustic survey and fishery data, a new maturity ogive, a new approach for weighting composition
...
\Sexpr{last.assess.yr} \citep{JTC2017}.  An updated maturity ogive was estimated and incorporated into the
\Sexpr{assess.yr} base model (see Section~\ref{sec:data-maturity}). The Dirichlet-Multinomial (D-M)
likelihood approach \citep{ThorsonEtAl2017Dirichlet} was used to estimate the weights
associated with age-composition data, rather than iteratively tuning the sample size multiplier
as in previous assessments (see Section~\ref{sec:data-age-comp-likelihood}). Time-varying fishery
...
JTC2014, JTC2015, JTC2016, JTC2017}. Year-specific recruitment deviations were estimated
from 1966-2017 as well as the years \Sexpr{assess.yr}, \Sexpr{assess.yr+1}, and
...
For this assessment document, we have slightly refined the Glossary
(Appendix~\ref{chap:glossary}), adding an explicit definition of
...
The addition of 2017 catch allowed the ending year of the model to be 
extended to 2018, but the estimates for 2018 remaining highly uncertain 
(Figure~\ref{fig:main-bridge-5-panel}) in the absence of additional information 
about recent recruitment.
The change with the biggest impact was the addition of the 2017 survey biomass 
...
reducing the uncertainty. The addition of the age compositions for the 
survey and fishery had relatively little additional impact on the biomass
...
\subsubsection{Fecundity (**new for 2019, need to reference this from elsewhere)}

**Andy: just want to get the weight-at-age part written up
today (it may be better in a separate section). 
Nees carefully reading later. Taking from my write-up in Issue \#469.
This assumes we cannot use the US weight-at-age data -- if we can then some of this will
need to be updated.

In 2018 the SRG-requested alternative model run used time-varying
fecundity. We have done this in the 2019 base model, and ran
three model runs with alternative assumptions about weights-at-age
for years without data. For a given year, fecundity is the
...
Figures~\ref{fig:main-empirical.weight.at.age.sens.a}-{fig:main-empirical.weight.at.age.sens.d}.
...
1975-2017) because it is more consistent with time-varying weight-at-age 
...
case (bridging model 43 shows no effect, **see section **).

\subsubsection{Assessment model results}\label{sec:assessment-model-results}
\subsubsub{Model Fit}

For the base model, the MCMC chain length was increased from the 12 million in the
\Sexpr{last.assess.yr} assessment \citep{JTC2017} to a chain length of 24 million. 
...
\citet{Xu2018} differs from the status-quo approach in that the deviations 
