...
assessments used to inform management decisions without rigourous simulation experiments
(e.g., decision analysis) because the truth is never known with 100\% certainty.
Hindsight comparisons have been conducted since 2021 \citep{JTC2021} to evaluate
how projections provided in decision tables
(e.g., Tables~\ref{tab:main-risk-year-1} and~\ref{tab:main-risk-year-2})
at the time perform relative to more updated assessments.
...
Full methodological details can be found in \citet{JTC2022}. As an example,
...
decrease the following year (except for 2018 and 2021), and
estimates from assessments further back in time are closer to 50\% compared to
those from the current base model (Figure~\ref{fig:main-historical-1}).
...
decline the following year (given the catch that subsequently occurred), whereas
...
categorical outcomes of an 'increase' or 'decrease' in biomass, despite the
overall change in biomass not being very substantial.
In 2021, the assessment \citep{JTC2021} had no information on the 2020 cohort and predicted the
...
a realized large deviation from average recruitment (rather than assuming average) can be on
forecasted outcomes.
...
A range of catch alternatives are shown for this (\Sexpr{assess_yr}) assessment because
...
i.e.,~P$(B_{t+1} < \Bforty)$.
The 2012 assessment was the only one evaluated that gave a $>50\%$ chance of the biomass falling below
$\Bforty$ in the subsequent year, but later assessments determined that the 2010 year class was indeed
substantial and in hindsight the probablity of going below $\Bforty$ was much lower.
...
incoming 2020 cohort. The biomass has been relatively high during the time period shown,
...
Using Bayesian results for the retrospective analyses,
restrospective versions of Figures~\ref{fig:main-historical-1}
and~\ref{fig:main-historical-2} can be calculated using the
current base model formulations but data only up to a certain year (Figures~\ref{fig:main-historical-retro-1}--\ref{fig:main-historical-retro-bforty-all}).
...
data (blue triangles). The largest discrepency is due to the retrospective uncertainty
with the 2014 cohort (i.e., rapid increase in the expected cohort size followed by a downward
adjustment; see Figure~\ref{fig:main-retrospective-recruitment})
