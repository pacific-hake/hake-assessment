...
\emph{Response -- **write something short here referencing the new Section \ref{sec:assessment-weights}}
...
% From Issue \#468:
\emph{Response -- We thought of three avenues:
  (a) Try estimating $\sigma_R$ in MCMC runs while testing semi-parametric selectivity.
  (b) Try estimating recruitment autocorellation to see what impact that has on the model
  and whether it reduces sensitivity to choice of $\sigma_R$.
  (c) Use the Template Model Builder (TMB)
  code developed by Nis Jacobsen for the Management Strategy Evaluation to explore
  random effects treatment of recruitment variability.
  However, due to time constraints (because of the US Government Shutdown), the JTC has
  been unable to investigate these.}
...
catch-weighted average weight-at-age was used (however see Section~\ref{sec:assessment-weights}
...
\subsubsection{Weights-at-age and fecundity (**new for 2019, need to reference this from elsewhere)}\label{sec:assessment-weights}

**Andy:
Needs carefully reading later. Taking from my write-up in Issue \#469.
This text mostly assumes we cannot use the US weight-at-age data (also need
to mention that we used 2015-2017 for 2018 but not to fit it).
If we can use US data then some of this will
need to be updated....
...
The eight runs are now explained in the context of the 2018 assessment.
