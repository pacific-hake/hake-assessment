...
algorithm with a similar number of posterior samples as the base model. A comparison of the
parameter estimates between the main sensitivity models and those from the base model are shown
...
\Sexpr{assess_yr} base model. Here we focus on the main sensitivities which include
the following:
