...
at age two for the acoustic survey of age-2$+$ biomass
and age one for the fishery until a maximum age of six, after which
...
\emph{Also, Appendix~\ref{chap:rec-plot} presents some new ideas
  for visualising recruitment relative to large recruitment years,
  to avoid a misunderstanding of how large some recruitment
  events might be. It also includes figures that further
  enhance understanding of
  the current method for simulating recruitments in the projection years.}
...
\emph{Response -- The JTC conducted the standard suite of catch projections using recruitment fixed
  ...
  **the new tables are not done yet.
...
\emph{Response -- The JTC has extended this analysis in
  ...
  presented for the age-1 sensitivity run in
