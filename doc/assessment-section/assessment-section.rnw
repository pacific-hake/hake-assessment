...
\citep{HelserMartell2007}. **Maybe add something about last year's assessment
\citep{JTC2017}.
...
%% capricious: subject to whims.
Available data have changed over the years, and there
...
**From 2017 assessment: may want to use the same answers for 2018 assessment,
...
\begin{itemize}
  \item Update the Stock Synthesis modeling software from version 3.24 to 
    version 3.30.
  \item Update data from years prior to 2017 (mostly just additional ages
    from the end of 2016 that weren't available in time for last year's 
    assessment)
...
In general, these changes mimic the steps routinely applied, with the except of 
the change in software version. Stock Synthesis version 3.30 includes a large
...
release and used for at least six production stock assessments for species
managed under the Pacific Fishery Management Council (including four 
rockfish species as well as Lingcod, and California Scorpionfish).

% Ian: paragraph below could be shortened if I'm able to complete an Appendix
% related to selectivity parameterization
One change between Stock Synthesis versions with significance for this 
...
adaption to account for this option no longer being available. In particular, 
...
parameter space.
Through trial and error, a value of $\phi=1.50$, applied in the standard 
parameter space, was found to be a close approximation to the $\phi=0.20$ 
applied in the logistic parameters space in the 2017 assessment. This
approximation was found to be reasonably close as measured both by both 
the resulting pattern of fishery selectivity over time and the 
overall estimated spawning biomass and other model outputs.

**This change is discussed in more detail in Appendix XX on parameterization of
selectivity.
% Ian is working on writing this appendix, expanding on Appendix C in the
% 2014 assessment
...
(Figures~\ref{fig:main-biomass-base-alternative-5} and
\ref{fig:main-status-base-alternative-5}; \Sexpr{end.yr} estimates are
\Sexpr{f(100*sens.models.1.for.table[[1]]$derived_quants["Bratio_2017","Value"],1)}\%
of unfished biomass for the base model and
\Sexpr{f(100*sens.models.1.for.table[[4]]$derived_quants["Bratio_2017","Value"],1)}\%
...
(Figure~\ref{fig:main-biomass-base-alternative-5}).
...
\ref{fig:main-status-base-tv-increase-6}).  Models with more flexible
...
\Sexpr{round(sens.models.4[[which(sens.model.names.4=="Natural mortality SD 0.3")]]$parameters$Value[1],3)} 
...
(Figures~\ref{fig:main-biomass-base-alternative-4} 
and~\ref{fig:main-status-base-alternative-4}).
...
\Sexpr{round(sens.models.4[[which(sens.model.names.4=="Steepness prior mean 0.5")]]$parameters["SR_BH_steep","Value"],3)}. 
However, neither steepness
sensitivity had a strong impact on the overall model results
(Figures~\ref{fig:main-biomass-base-alternative-4} and
\ref{fig:main-status-base-alternative-4}). The small influence of steepness on
...
high (2.0) and low (1.0) states. An additional value of 1.51 was added this
