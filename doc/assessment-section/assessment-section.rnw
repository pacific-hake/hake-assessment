...
different or new insights brought up during the peer review process.
...
recent data. **[re-write and mention age-1 used in 2022] Several recruitment indices have been considered
but ultimately identified as adding no appreciable contribution to model results
\citep{HelserDornEtAl2002, HelserFleischerEtAl2005, StewartHamel2010}.
...
** An age-1 index was included in the base model. It spans the same years as the acoustic survey, as it was
part of the same survey, on the same vessels (but no value is available for 2001). Selectivity was set to one for age-1 fish for this index
...
as well as the years \Sexpr{assess_yr}--\Sexpr{assess_yr+3}
...
year's assessment \citep{JTC2021}. The number of samples for the sensitivity analyses
remained the same as last year's assessment at 8,000 samples.
...
\citep{CuddingtonEdwardsEtAl2021} to encourage wider uptake of such tools among biologists.
...
\emph{The JTC has requested DFO funding to collaborate with Australian, US, and Canadian
  researchers on developing an index of age-0 \fishname.
  \citet{SuthersWhiteEtAl2022} proposed and simulation-tested a novel method for estimating growth and mortality of fish larvae, based
on size distributions. Dr. Suthers (from Australia) subsequently visited NOAA's Southwest Fisheries Science Center in 2022 to
start developing the method for age-0 \fishname\ using CalCOFI data
(which include 1.3 million hake larvae), and to compare results to estimated
recruitments from the last hake assessment \citep{JTC2022}.
\citet{HinchliffePepinEtAl2019} proposed a novel method relating the mortality/growth ratio of larval fish to the slope of the zooplankton
size spectrum (which characterises the size distribution of zooplankton), to quantify the recruitment potential of larval fish.}

\emph{The proposed work is a collaboration with authors of the above two
  papers and the JTC. It will (i)~continue the hake-larvae analyses, and
  (ii)~analyse the extensive CalCOFI zooplankton data to try and predict variations in age-0
  hake recruitment from variations in the size spectrum. This
  gives two potential approaches to providing new information on hake
  recruitment and will utilise JTC expertise in developing likelihood methods for fitting size
  spectra to data \citep{EdwardsRobinsonEtAl2020}.}

\emph{**Kelli's suggestion: The JTC has requested DFO funding to collaborate with Australian, U.S.,
  and Canadian researchers on developing two potential approaches for
  developing an index of age-0 \fishname. The first method will expand upon
  \citet{SuthersWhiteEtAl2022}, which proposed and simulation tested a novel
  method for estimating growth and mortality of fish larvae based on size
  distributions. In 2022, Suthers applied the method to CalCOFI data (which
  includes 1.3 million hake larvae) and compared the estimates to estimates of
  recruitment from the last hake assessment \citep{JTC2022}. Funding will
  support work to include the index of age-0 fish in the assessment. Second, a
  ...
  \citep{EdwardsRobinsonEtAl2020}, will be used to quantify the recruitment
  potential of larval \fishname. Funding will support work to predict
  variations in recruitment from variations in the size spectrum.}
...
\begin{itemize}
  ...
  \item Add weight-at-age data for \Sexpr{last_data_yr} as well as update data for years prior; and
...
how files are saved for MCMC samples and
the timing of the calculation of key population processes such as
...
** The addition of \Sexpr{last_data_yr} catch and fishery age compositions extends the model
to the start of \Sexpr{assess_yr} but the estimates for
\Sexpr{assess_yr} are highly uncertain
...
trajectory were relatively unchanged with the addition of \Sexpr{last_data_yr}
weight-at-age data.
...
%the \Sexpr{end_yr-1} assessment model would have otherwise predicted
%given mortality.
was the first time
...
** The final bridging was adding the age-1 index time series (1995 to
\Sexpr{last.survey.year}) as an additional source of information to inform recent
recruitment (Section~\ref{sec:data-acoustic-survey}).
The age-1 index has been evaluated in many previous stock
assessments as a sensitivity analysis and in recent years a close examination
of the full suite of assessment results were produced to examine this key sensitivity.
The age-1 index is fit
in the assessment model through a lognormal contribution to the likelihood function.
The addition of this data source resulted
...
remains similar to the \Sexpr{last_assess_yr} base model.
...
The addition of \Sexpr{last_assess_yr} fishery data
was the main reason for this change in fit to the 2019 data point (Figure~\ref{fig:main-bridge-5-panel-1}).
The \Sexpr{last_assess_yr} survey estimate was lower than in 2019 (second lowest since 2013),
...
** The median posterior density estimates were less than the 2015 survey index,
...
were estimated slightly under the index value and, being so young, had the largest amount of uncertainty in the time
...
structure, i.e., \Sexpr{top.coh(base_model, survey_end_yr, 3, cap = FALSE, fleet = 2)}.
...
it
was not extremely large (\Sexpr{survey.a2.prop}\% of the \Sexpr{survey_end_yr} survey catch).
Residual
...
age. This level
of variation led to results that were consistent with the \Sexpr{survey_end_yr} acoustic survey
...
pattern, where selectivity increased smoothly from age-2
