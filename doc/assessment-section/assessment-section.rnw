...
\emph{The JTC plans to participate in collaborative research to investigate the concurrent
  ...
  In this assessment, the first two parameters are input as fixed values rather than estimated, though we have explored their estimation within Stock Synthesis in the past and did so again this year.
...
As the JTC works towards documenting which fish are included in historical age-composition samples (response 4), investigating disaggregated forms of the compositional data will become a doable task because currently the data are unavailable. 
Thus, below we note our progress on investigating historical methods used to collect data beyond those noted before.}

\emph{**2020 KFJ: Changes in sampling protocol.}
...
base model and thus no effect on resulting time series either (Figure~\ref{fig:main-bridge-5-panel-1}).
...
weight-at-age values as databases continually are updated.
...
a reduction in the 2015 and 2018 year classes with the addition of \Sexpr{last.data.yr}
...
efficient MCMC sampler (No-U-Turn Sampler, NUTS; \citealt{HoffmanGelman2014}) as implemented using the
{\tt adnuts} R package
\citep{Monnahan2018, Monnahan2019}. NUTS is considered by many to be a straightforward improvement
in efficiency with high dimensional models relative to classic Hamiltonian
approaches (via adaptive sampling steps), as well as improved (more effective
and consistent) parameter space coverage over classic random walk approaches
...
MCMC samples from the posterior distributions for model parameters and derived
quantities.  This is near a three-fold increase in samples from 
...
\href{https://mc-stan.org/users/interfaces/shinystan}).
...
,~\ref{fig:main-mcmc-diag-h-extra-sd}, and~\ref{fig:main-mcmc-diag-dm}). 
...
convergence was adequately achieved (examined via {\tt ShinySTAN} web application).
...
The prior for $\theta_{\text{fish}}$ and $\theta_{\text{surv}}$ results
...
The prior results in the survey age-composition parameter being well-sampled
with
...
Overall, the 2001 data point continues to be well below any model predictions that were evaluated,
...
a couple of above-average cohorts that are now mature resulting in a population
...
The MLEs (**or median posterior density) for numbers, biomass,
...
the 2014 cohort was only just under half of the surviving biomass of the 2010
cohort.
...
distributions, including $\theta_{\text{surv}}$ (as outlined in
...
declined since 2017 as the 2014 year class moves through through the growth-mortality
...
However, estimated recruitments for some recent years have slighlty changed with
the addition of new data. For example, this assessment's median estimate of the 2014 recruitment changed by
\Sexpr{f((recruitment.med.to.compare - prev.assess.recruitment.med)["2014"], 1)}~billion
fish (a \Sexpr{f(((((recruitment.med.to.compare - prev.assess.recruitment.med)["2014"]) /
(prev.assess.recruitment.med)["2014"]) * 100), 0)}\% change) compared to last
year's assessment. Similarly, estimates for 2016 and 2018 have changed by
...
notion that year class strength is above average for 2016 and well below average for
2018 remains.
...
in 2015, and then it leveled off around 75\% from 2016 to 2019 before dropping to
