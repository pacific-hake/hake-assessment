...
\emph{For the U.S. sectors, the average number of fish sampled per haul in the at-sea sector in recent years is three fish, whereas the average number of fish sampled per trip in the shore-based sector is twenty. We assign the input sample size of an at-sea haul or a shore-based trip equally but account for differences in sample weights in by weighting composition by landings.
The JTC explored reducing the input sample size of the U.S. (or U.S. and Canadian) at-sea sectors without changing the method of weighting by landings and found that it led to a similar estimate of equilibrium and current spawning stock biomasses compared to what was estimated for this assessment (results not shown).
Additionally, removing all age-composition data through 1990 led to similar stock trajectories from 1990 forward (results not shown). Therefore, the JTC believes it is most important to investigate input sample sizes for years that include fish still in the population rather than focusing on historical samples that can be removed from the model without affecting current estimated biomasses.}
...
catch in Figure~\ref{fig:main-forecast-depletion-comparison}, the median in 2021
essentially equals the median in 2022 (i.e., zero difference in the medians),
which might be expected to imply a 50\% probability of a decline (not the 65\%
just mentioned). However, the median change between 2021 and 2022
is a decline of 0.028 (from calculating the difference for each MCMC sample and
then taking the median, namely the median of the differences). About 15\% of the
...
\citep{Monnahan2018, Monnahan2019} to implement a NO-U-Turn Sampler (NUTS)
...
base-model value of 1.4. Future work will assess similar metrics
...
of unfished biomass for the base model (95\% credible interval
...
in 2014 (age-1 in 2015) and 2018 (age-1 in 2019) then what the base model
otherwise estimates (Figures~\ref{fig:main-age-one-index}-\ref{fig:main-index-base-alternative-2}).
These changes are subtle because the base model generally tracks the trends in the age-1 index well.
...
offered more flexibility than the choice of age-5. All three alternative runs had reduced
...
years' data and estimating the posterior distribution of parameters under the assumptions of the base
model. Models with 5 years of data removed had information
...
Cohort strength is not well estimated until several (usually 3 or more) years of fishery
...
are above the 2016 and 2017 cohorts, but the estimated size of the 2014 cohort didn't
