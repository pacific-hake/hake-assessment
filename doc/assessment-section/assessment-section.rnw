...
\emph{The Management Strategy Evaluation (MSE) framework for \fishname\ creates
  ...
  model. Insights gained through the treatment of recruitment
  variability as random effects \citep{Thorson2019} continue to be explored.
...
\emph{Stock synthesis currently does not have capacity to draw from past observations or use
  ...
  Note that some of the figures in Appendix~\ref{chap:rec-plot} give an
  improved understanding of the current method for simulating recruitment
  in the projections.}
...
\emph{Response -- The JTC conducted the standard suite of catch projections using recruitment fixed
  ...
  **wrong appendix, the new tables are not included yet.
...
\emph{Response -- The JTC has conducted all of the requested sensitivities (and many
  ...
  tabular (Tables beginning with~\ref{tab:main-parameter-estimates-sens-1}), and
  graphical (Figures beginning with~\ref{fig:main-biomass-base-alternative-1}) formats in
...
\emph{Response -- The age-1 index is incorporated into the 2022 base assessment model.
  As a result, the JTC has produced executive summary style tables and figures for
...
\emph{Response -- The JTC produced Appendix ~\ref{chap:rec-plot} in response to this
  request, which includes a refinement of the analysis presented last year.  Related, the
...
\emph{Response -- **Kelli to add more or edit pending analysis** In 2021, the JTC explored the absence of 2020 Canadian freezer-
  ...
  age data available from the shoreside fleet, and the Canadian shoreside and freezer-
  trawler fleets tend to provide similar age-composition information.}

\emph{In 2021, there was almost no biological data available for the Canadian fishery.
  In summary, there was a single shoreside trip sampled, compared to a recent five year
  average of 55 trips, and no freezer-trawler age data. The consequence of
  this loss of data is a disproportionate (relative to areal catch) reliance on age data
  sampled from Unites States waters as being representative of the coastwide population for the
  2021 fishing year.  The fishery in the United States on average comprises of lower proportions of older fish
  relative to Canada}

\emph{The JTC is  concerned about multi-year changes in sampling and, in particular,
  the loss of complete data series.  As a result, the JTC conducted analyses to evaluate potential
  medium to long-term impacts on the stock assessment with the loss of Canadian age data.  **Method
  and results of Kelli's analysis here...}
...
\emph{Response -- It is important to capture key, mechanistic and/or stochastic, population processes
  ...
  management decisions. Likewise, it is as important to evaluate the basic population dynamic
  assumptions used in such projections. Recent, current, and future recruitment is perhaps the most
  influential source of uncertainty in stock assessment projections. As a result, the JTC has
...
\emph{Empirical weight-at-age data indicate inter-annual variation in \fishname growth, and this variation could
  be related to individual cohort effects (e.g., large cohorts have a negative growth relationship with
  ...
  a recent 5-year average in the projection period, because it is consistent with recent data (** this figure
  reference needs changing to the wt-at-age across time figure that we show in presentations, once added
  to the document;** see Figure~\ref{fig:main-bridge-5-panel-1}).}
