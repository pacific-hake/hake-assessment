...
\item In the current assessment, the change from $\phi = 0.03$ to $\phi = 0.2$
  ...
  \emph{Response -- The JTC has conducted several analyses in response to this 
    ...
    approach to setting the time-varying, all described in 
    ...
    improvement over the status-quo parameterization with with the larger $\phi$ 
...
was not being sampled efficiently due to many samples occurring in a part of
