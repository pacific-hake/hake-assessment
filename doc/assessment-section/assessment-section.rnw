...
\emph{Response -- Developing best practices for modeling equilibrium recruitment ($R_0$)
  ...
  collaborate on and monitor ongoing research projects into
  approaches for advancing recruitment estimation,
  as it applies to \fishname and in general. We briefly discuss several of these research
  endeavors next.}
...
\emph{The JTC is conducting and collaborating on simulation projects looking into the concurrent
  ...
  associated with time-varying selectivity, $\sigma_R$, extra standard deviation parameters
  on survey index data, and Dirichlet-Multinomial parameters $\theta_{\text{fish}}$ and $\theta_{\text{surv}}$.
  This is particularly
  important for $\sigma_R$ because without an index of recruitment to directly inform the
  estimation of $\sigma_R$, variance parameters in stock assessment models can be interrelated
  when attempting to capture other sources of variance attributed to model misspecification.
  This work is related with other time-varying selelctivity research, as discussed in response 2
  below.}
...
\emph{The near completion of the Management Strategy Evaluation framework for \fishname creates
  ...
  framework can also be used to evaluted the robustness of recruitment modeling assumptions on
...
\emph{The JTC conducted an analysis looking into autocorrelation of recruitment deviations over
  ...
  (see Section~\ref{sec:assessment-retrospective-analyses}), this result suggest that the
  assessment model is not overly misspecified. Given the lack of autocorrelation, the
  assessmnet model was not sensitive to the addition of an autocorrelation parameter.}
...
genetic analyses. That team provided the follwoing update.}
...
these boxes. RADseq (\citep{Baird2008, Ali2016}) has been utilized to generate 8,763 genome
...
existing boxes to boost sample size. This approach will provide the best picture to date of \fishname
genetic population structure. The team expects to complete the sequencing and analyses for these samples
...
updated maturity analysies planned for the upcoming year. However, logistical considerations
...
and storage lipids (triglycerides) are indicative of female maturation status (immature vs. mature)
...
\emph{Response -- The JTC evaluated the No U-turn sampler (NUTS) method for conducting efficient
...
MCMC sampling, are shown in Appendix~\ref{chap:chap:adnuts}. In addition, the JTC explored
...
used for running NUTS is displayed at the end of Appendix~\ref{chap:chap:adnuts}.}
...
In addition, the JTC communicated with State and Provincial partners to ensure the
...
tribal catch were included in the stock assessment, depsite not yet appearing in
...
zero. Doing so removes a known coding error that has implications when 
using zero-sum constrained vectors in MCMC algorithms run through AD Model Builder
software. The issue with the AD Model Builder sampling procedure is that the zero-sum
constraint isn't actually being applied in the mcmc phase where the samples are being
drawn, just the mceval phase where the results get calculated. So there's a mismatch
between the samples that get reported and the sampling process that draws them. This is
...
$R_0$ and the main recdev period could, in theory, lead to a better fit to the indices of abundance or
...
Thus, using non-zero centered constrained recruitment deviations will increase the
...
resulted in a median recruitment deviation distribtuion of 0.086 in the \Sexpr{assess.yr}
...
Including the fishery-independent data led to a slightly more optimistic
...
The addition of age-composition data from the survey led to reduced
uncertainty of the 2010 and 2014 year classes,
...
The new priors for $\theta_{\text{fish}}$ and $\theta_{\text{surv}}$ resulted ....
**Andy to come back to this when Chris finishes the posterior tables (as he'll have some values to
use). Should change the following ranges to the usual credible intervals.**
the resulting multiplier applied to the survey
...
levels, despite the total 2016 catch being one of the highest of the precedeing years.
...
Figure~\ref{fig:main-forecast-depletion-comparison} shows the predicted
...
\Sexpr{end.yr} catch level similar to that for \Sexpr{end.yr-1} (catch alternative~d).
...
\Sexpr{f( 100 - base.model$risks[[1]][8,"Bratio_2021<0.10"], 0)}$\%,
and in the healthy zone with a probability of
P$(B_{\Sexpr{assess.yr+1}} > \Bforty)=
\Sexpr{f( 100 - base.model$risks[[1]][8,"Bratio_2021<0.40"], 0)}$\%.
...
P$(B_{\Sexpr{assess.yr+1}} > \Bmsy) = \Sexpr{f(min(dfo.probs.fore[,"SSB_2021>SSB_MSY"]))}$\%.
...
That probabality decreases to \Sexpr{f(base.model$risks[[1]][,3][4])}\% and \Sexpr{f(base.model$risks[[1]][,4][4])}\%, respectively, if the catch level stays about the same in \Sexpr{assess.yr + 1} as in \Sexpr{assess.yr}.
%Manual done 2020
...
\item Removal of ageing error from the model; and
...
underfitting the age-1 index estimates of the size of the 2014 cohort
