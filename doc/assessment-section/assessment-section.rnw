...
\emph{Response -- The JTC has begun exploring approaches to make informed decisions about
  ...
  Sections~\ref{sec:data-acoustic-survey} and~\ref{sec:assessment-base-model}). Additionally, the JTC
...
\emph{Response -- Genetic work headed by Gary Longo at the Northwest Fisheries Science
  ...
  as British Columbia, including the Strait of Georgia and Puget Sound.
...
\emph{Response -- The age-1 index is incorporated into the 2022 base assessment model.
  ...
  of the Executive Summary tables and figures for the results
...
\emph{Response -- The JTC has extended this analysis in
  ...
  all retrospective model runs are performed using MCMC.
  Related, the
...
\emph{In 2021, there were almost no biological data available for the Canadian fishery.
  ...
  sampled from US waters as being representative of the coastwide population for the
  2021 fishing year.  The fishery in the US on average comprises lower proportions of older fish
  relative to the fishery in Canada.}
...
The historic probability of \Sexpr{prob.decline.from.2012.to.2013.historic}\%
...
Table~\ref{fig:main-age-one-index}).
...
Interestingly, in Figure~B.3.1 of \citet{JTC2021} we found that the 2018 probability from the
2021 base model was above
the 50\% line (unlike for this year's base model), agreeing with the 2018 assessment calculation. And it was the only
...
a much higher probability of 43\%. This difference is presumably due to changes in model structure
since 2012 (Table~\ref{tab:main-assessment-changes}).
...
the 2011 age-1 index that shows the large 2010 cohort), gives
P$(B_{2013} < B_{2012}) >20\%$
(Figure~\ref{fig:historical-retro-all-age1}). This suggest that the age-1 index
accounts for some of the improvement of the estimate of
of P$(B_{2013} < B_{2012})$ between the retrospective calculation using the
current base model (grey point in top panel of Figure~\ref{fig:main-historical-retro-1}) and
the 2012 assessment's calculation (red point in top panel of Figure~\ref{fig:main-historical-retro-1}).

Note that P$(B_{2013} < B_{2012})$ in Figure~\ref{fig:historical-retro-all-age1}
(excluding the age-1 index)
is still somewhat below the value estimated in the 2012 assessment, unlike
Figure~\ref{fig:main-historical-retro-1} that includes the age-1 index. Hence,
it is not solely the addition of the age-1 index (but also other model changes)
that leads to the improved estimation of P$(B_{2013} < B_{2012})$ when compared
to the latest base model with all information. **Though maybe that's a bit
circular - comparing new base model with data up to 2011 with new base model
with all data, and saying it does better than 2012 model assumptions. Think about.

**Andy got to here for now...

 Final retro point should in theory
  be close to the red point, difference is due to model changes (and
  maybe slightly updated data).
