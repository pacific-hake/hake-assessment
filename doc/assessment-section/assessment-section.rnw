...
2020 \citep{JTC2020} and earlier assessments. The number of samples used for bridging models, sensitivity models, retrospective models, and forecasting scenarios was also set to \Sexpr{f(num.mcmc.samples)}.
...
The server setup allowed all JTC members to work together in a way not possible before. This integrated setup will ensure an ideal workflow, with a centralized system housing all model runs and document software.
...
recruitment in 2019 and 2015 decreased (39/% and 11/%, respectively) from that
...
and througout the current forecast period, because there is considerable uncertainty
associated with the size of the 2020 year class, now fully mature, that propogates into
...
Figure~\ref{fig:main-recruit-base-alternative-2}).  However, further work is needed
...
are P$(B_{\Sexpr{assess_yr}} > \Bten) = approximately \Sexpr{probs.curr.bten}$\% and
...
are P$(B_{\Sexpr{assess_yr}} > 0.4\Bmsy) = approximately \Sexpr{f(dfo.probs.curr[1, 2], 0)}$\% and
...
\Sexpr{end_yr+1} is above 72\% for all catch levels examined
...
biomass than the larger cohorts. At age-3 during 2023, the 2020 cohort will likely continue to gain
biomass through growth at a faster pace then declines from mortality.  The probability
...
P$(B_{\Sexpr{assess_yr+1}} > \Bten) = approximately
...
probability of
P$(B_{\Sexpr{assess_yr+1}} > 0.4\Bmsy) = approximately \Sexpr{dfo.prob.next.over.40bmsy}$\%,
