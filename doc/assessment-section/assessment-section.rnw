...
**think some numbers are wrong ($R_0$, $B_0$ and exploitation fraction); checking with Chris.
...
of the age-1 survey index (****Appendix~\ref{chap:age1-survey}), and the use of
the Metropolis Hastings MCMC algorithm for the base model as used in past
assessments (Appendix~\ref{chap:rwmh}). The inclusion of the age-1
...
median of the female spawning biomass at $\Fforty$ (namely $\BSPRforty$) and
...
**mention exploitation fraction also once fixed?
% Manual 2021 done, except exploitation fraction.
