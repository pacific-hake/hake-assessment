...
quantities. The Dirichlet-Multinomial (D-M) weighting parameters (theta) were fixed at the MLE
estimates during MCMC evaluations, because they were occasionally hitting 
the upper bound of 1.0 for the sample size multiplier.  This results in the theta 
parameters having almost no impact on the model results, such that the MCMC algorithm has little
information about that dimension. Stationarity of the posterior distribution for model parameters was
re-assessed via a suite of standard single-chain and multi-chain diagnostic tests. The objective function, as
