...
Since 1989, Stock Synthesis models fit to
...
\begin{itemize}
  ...
  \item Update catch, age-composition, and weight-at-age data from years prior to \Sexpr{last_data_yr};
  \item Add new catch data for \Sexpr{last_data_yr};
  ...
  %\item Add the \Sexpr{last_data_yr} acoustic survey biomass and age-composition estimates; and
  \item Add weight-at-age data for \Sexpr{last_data_yr}.
  \item Add fishery age-composition data for \Sexpr{last_data_yr}
  %\item Add the age-1 index time series.
  %\item Add \Sexpr{last_data_yr} survey biomass estimate;
  %\item Add \Sexpr{last_data_yr} survey age-composition data; and
\end{itemize}

**Kelli to add to this paragraph if needed:

Stock Synthesis version \Sexpr{ss_version} includes
a number of changes since the version used by \citet{JTC2022},
mostly related to options not explicitly used in this assessment.
Adaptations within the Stock Synthesis modeling framework itself had little effect
on parameter estimates compared to the \Sexpr{last_assess_yr}
base model and thus no effect on resulting time series (Figure~\ref{fig:main-bridge-5-panel-1}).
...
little impact on the model results (not shown).
...
though slightly increases the estimated spawning biomass
(Figure~\ref{fig:main-bridge-5-panel-1}).
...
in spawning biomass by the start of \Sexpr{assess_yr}, as these fish are
considered mostly mature at the start of \Sexpr{assess_yr}.
Despite both fishery age compositions and the age-1 index pointing towards
a strong 2020 cohort, estimates of \Sexpr{assess_yr} spawning biomass remain highly uncertain,
...
class in 2012).
%The median estimate of the 2010 year class is
...
\item Consideration of an alternative prior distribution (mean and standard deviation) for natural
  mortality based on the \citet{Hamel2015} life history meta-analytic method;
...
The Hamel prior model estimated natural mortality at \Sexpr{f(nat_m_hamel[2], 3)}
...
The level of uncertainty associated with each assessment's estimate of that year's current spawning biomass (used to convey current stock status and inform management advice)
...
given the stock size (i.e., similar to a coefficient of variation) is not inconsistent with previous
assessments (Figure~\ref{fig:main-historical-dispersion}).
