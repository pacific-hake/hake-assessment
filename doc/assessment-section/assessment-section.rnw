...
data, and a new approach for parameterizing time-varying selectivity in the base
model.
...
extended to 2018, but the estimates for 2018 remained highly uncertain 
...
with several important model parameters including: the magnitude of the stock
(via the $R_0$ parameter for equilibrium
...
previous assessments, whereby the input sample sizes are iteratively 
...
resulted in potentially implausibly high estimates of recent recruitments. The
...
However, the model with $\sigma_s = 0.695$ had higher estimates of 2016 recruitment
...
individuals to predict the size of the 2014 year class.  The base model now has three
...
of data in this year's assessment the 2014 cohort appears to be more similar
...
spawning biomass for recent years have declined in the \Sexpr{end.yr} asssessment relative
