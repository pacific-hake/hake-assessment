...
\emph{The Management Strategy Evaluation (MSE) framework for \fishname\ creates
  ...
  on management performance and uncertainty.  Research in publication review by Dr. Cathleen Vestfals and
...
\emph{Response -- The JTC continues to explore approaches to make informed decisions about
  ...
  Ongoing research includes expanding variables to years beyond 2010 to create a single
  time series from the two data periods and investigating relationships leading to better informed
...
\emph{Available options in Stock Synthesis
  ...
  A member of the JTC is on the next generation NOAA stock assessment modeling
  platform (FIMS) development team, which ensures features such as recruitment forecast options
...
\emph{The JTC has requested DFO funding to collaborate with Australian, U.S.,
  ...
  spectrum to data \citep{EdwardsRobinsonEtAl2020}, and aims to provide new information
...
\emph{Response -- \fishname\ ovary samples were not collected over the past year, but plans include more
  ...
  West Coast.  The project aims to evaluate how interannual changes effect size and age at maturity across a latitudinal
...
\emph{Response -- The JTC has included this analysis in
  Figures~\ref{fig:main-historical-1}--\ref{fig:main-historical-retro-bforty-all}
  again this year to continue evaluating current base model versus previous forecast
  relationships. Related, the JTC is continuing to explore options for improving recruitment
...
\emph{Response -- The JTC has conducted analyses previously showing that small changes to
  ...
  stock assessment results used for management decisions. The JTC has not yet had the opportunity to
...
\emph{Response -- It is important to capture key, mechanistic and/or stochastic population processes
  ...
  mentioned in this section. Over the next biennium, the JTC plans to work with the MSE working group
...
\emph{Response -- Resources were made available for the JTC to use a new high-performance computing
   ...
   forecasts) and build all assessment document dependencies.
...
\emph{Response -- The JTC continues to explore the implementation of a hake-like assessment in WHAM
  Woods Hole Assessment Model) and SAM (state-space Stock Assessment Model), in addition to the
  custom TMB (Template Model Builder) estimation model used in the MSE, to evaluate the impact of
...
\emph{Response -- Teamwork and collaboration is an attribute of great interest
  ...
  the science objectives as outlined in the Treaty.}
...
in 2014 (age-1 in 2015) and 2016 (age-1 in 2017) and moderately higher
recruitment in 2018 (age-1 in 2019)
and 2020 (age-1 in 2021) than the model without the index
...
over the last few years of the model. The sensitivity run with
maximum selectivity at age-5 resulted in the lowest estimate of the 2016 year class compared
