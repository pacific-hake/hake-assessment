...
recent data. Various recruitment indices have also been considered and subsequently
...
and zero for all other ages. For more information about the index and reasons for its
inclusion in the assessment model, see Section~\ref{sec:data-acoustic-survey}.
...
\emph{Response -- Specifying projected recruitment as the median of a given year range
  ...
  Results will be presented during the SRG in 2022 during the review.}
