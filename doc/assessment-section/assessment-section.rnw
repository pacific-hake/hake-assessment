...
\item In the current assessment, the change from $\phi = 0.03$ to $\phi = 0.2$
  ...
  \emph{Response -- The JTC has conducted several analyses in response to this 
    ...
    semi-parametric option shows promise, no alternative considered was a clear
    improvement over the status-quo parameterization with with the larger $\phi$ 
    value (previously $\phi = 0.2$ but converted to $\phi = 1.4$ after
    removing a logistic transformation as described in 
    Section~\ref{sec:assessment-changes}). 
    The data from 2017 suggests that the 2014 cohort was more 
    in line with the estimates of from the model with more flexible fishery 
    selectivity, providing support for maintaining that flexibility for this 
    year.}
...
\item The next priority for 2018 is processing the archived ovary collections of
  ...
  \emph{Response -- The JTC worked with the Northwest Fisheries Science Center
    ...
    between fecundity, age, and weight would depend on collecting whole ovaries
