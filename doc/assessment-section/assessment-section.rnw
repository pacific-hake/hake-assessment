...
\citep{MethotWetzel2013} version \Sexpr{ss.version} was used, which is a significant update to
...
series where there were more old individuals than in the more recent years with
...
\Sexpr{last.assess.yr} assessment \citep{JTC2017} to a chain length of 24 million. This
resulted in the first 4,000,000 discarded to eliminate `burn-in' effects and
...
that the 2014 year-class (53\% of the catch for that year) was above average. The
...
these two cohorts, with 38\% and 31\%, respectively.
...
the base model MCMC values reported elsewhere (see
Tables~\ref{tab:main-parameter-estimates-sens-1} and
\ref{tab:main-parameter-estimates-sens-2} for a set of comparisons of the base
