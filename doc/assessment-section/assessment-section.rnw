...
years assessment \citep{JTC2021}. The {\tt adnuts} software goes through several steps including
a pilot run which is short parallel-chain Random Walk Metropolis (RWM) run to ensure the model MCMC is
...
samples from the posterior were generated. The $adapt\_delta$ value, which reduces
divergence and forces sampling from a wider part of the posterior, was set to 0.95 for
the main NUTS run. The choice an $adapt\_delta$ of 0.95 was made based on less divergence
of the posterior and higher effective sample size compared to 0.9, which was used for last
...
can be found in \citealp{Monnahan2019}.
