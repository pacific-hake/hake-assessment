...
(No-U-Turn Sampler; NUTS) was used to create parameter posterior distributions,
...
Appendix~\ref{chap:rwmh}). The list of changes discussed above is for illustrative
...
(No-U-Turn Sampler; NUTS) for constructing posterior densities. The latter lead to
the explicit use of Bayesian inference in the stock assessment (i.e., base model,
sensitivity models, and retrospective models).
% Manual 2020 KFJ
...
Uncertainty of estimated quantities was calculated around the median results
(50\% quantile) via Markov chain Monte Carlo
(MCMC) simulations using the No-U-Turn Sampler (NUTS) set achieve a minimum of 8,000
posterior samples. The bounds of 95\% credibility intervals were calculated as
