...
Results from initial liver lipid analyses indicate that levels of important structural (phospholipids)
...
zero. This should increase the
compatibility between MCMC and MLE results, and also increase the stability of model results,
because penalties have implications for calculating derivatives. Relative
to the \Sexpr{last.assess.yr} assessment,
relative spawning biomass increased for all years because the estimate of
...
The new priors led to increased correlation between
estimates of $M$ and $\log R_0$ (Appendix~\ref{chap:mcmc-diagnostics}), but very little differences in the
...
** AMB can you add more interpretation here? [Andy removed the tuning sentence,
maybe we're good?].
...
[**ANDY table 27 isn't right but I think Kelli fixed it; check when can rebuild]
...
**ANDY: that doesn't look very true to me, but wait until table is fixed.
% Manual 2020 done.
