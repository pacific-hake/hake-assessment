...
parameters and derived quantities. This is a four-fold increase in samples from the
2020 \citep{JTC2020} and earlier assessments. The number of samples used for bridging models, sensitivity models, and retrospective models were also set to \Sexpr{f(num.mcmc.samples)}.
...
MCMC samples, to give equal-tailed intervals.

The {\tt adnuts} software goes through several steps including
a pilot run using rwMH sampling with a few short chains in parallel to ensure the MCMC algorithm is
converging. Once the pilot had been checked for convergence, low effective sample size,
and for slow mixing parameters by comparing pairwise correlation plots, MLE was run to
obtain the covariance matrix. The covariance matrix is then used as the mass matrix for
a short run producing 500 samples using a warmup of 100 samples.
The covariance matrix from this short run is used as the mass matrix for the main run, providing
\Sexpr{f(num.mcmc.samples)} samples from the posterior for inference. The {\tt adapt\_delta} value
sets the target average acceptance probability and was set to 0.95 for all
models presented in this assessment. The choice of {\tt adapt\_delta} of 0.95
(same as for the 2022 assessment) was made based on less divergence of the posterior and higher effective sample size. A full explanation of the NUTS algorithm and the {\tt adnuts} package,
including an analysis with the \fishname\ stock
can be found in \citet{Monnahan2019}.

A new server was purchased and used to run all the models and for running the software that builds the document. The server has 80 Intel Xeon Gold CPUs and 128 GB of RAM and is running Ubuntu Linux version 22.04.1 LTS which was the newest stable version at the time of writing.

The server was set up with user account for each JTC member, with a remote desktop server (X-Remote Desktop Protocol; XRDP) running for easy access. All connections were through a 4096-bit passwordless Secure SHell (SSH) and the server was otherwise locked down from a security standpoint.

The entire server is automatically backed up on a mirrored RAID device once per day, with model input files also backed up to a Google drive. If everything were lost the models could be re-run very quickly and the document regenerated in full.

Scripts were written in the Bash shell scripting language to allow operating system level control and monitoring over parallel model runs. Each run was also parallelized internally with 16 chains per model; that parallelism was controlled from within R using the {\tt future} and {\tt furrr} packages. With each model needing 16 CPUs, the JTC were able to run 4 models at a time, taking up 64 CPUs and leaving 16 available for use in writing and building the document. The total runtime for these models is now on the order of 1-3 hours, depending on the CPU and memory load on the server at the time.

The server setup allowed all JTC members to work together in a way not possible before. This integrated setup will ensure an ideal workflow in the coming years, with a centralized system housing all model runs and document software.

Calculations and figures from Stock Synthesis output were performed using
\Sexpr{R.Version()$version.string} and many R \citep{R2021} packages (in
particular {\tt r4ss}, {\tt adnuts}, and {\tt xtable}). The use of R, {\tt knitr}, \LaTeX, and GitHub immensely facilitated the collaborative writing of this document.
In particular, having all of the code under {\tt git} version control
since the 2016 assessment \citep{JTC2016} has ensured delivery of the assessment document within a contracted timeframe.
...
\emph{Over the past year, the JTC has continued to engage in research activities and outreach related to
  ...
  conducted a learning session at the summer JMC meting to promote internal awareness
...
\emph{Response -- \fishname\ ovary samples were not collected over the past year, but plans include more
  ...
  remains a goal, but barriers (lab training, capacity, and moving bio-samples across the broader) still
...
\emph{Response -- Resources were made available for the JTC to use a new high-performance computing
   ...
   forecasts).
   Another high-preformance server unit and two high-performance laptops have also been aquired for
   completing the \fishname\ assessment.  These resources are used for data preperation, diagnostic examination,
...
In the \Sexpr{last_data_yr} fishery, \Sexpr{top.coh(base_model, last_data_yr, 3, cap = FALSE)}.
Age compositions from the \Sexpr{survey_end_yr} acoustic survey suggest a similar age
structure
%i.e., \Sexpr{top.coh(base_model, survey_end_yr, 3, cap = FALSE, fleet = 2)}
...
transition during a period of high catches. The increase in spawning biomass since 2021 is
...
of the 2020 cohort (e.g., not yet sampled by the acoustic survey).

The median estimate of
the \Sexpr{end_yr} spawning biomass is \Sexpr{curr.bio.median}~million~t (with a
...
\Sexpr{top.coh(base_model, 2016, spec.yr=2014)}\% of the 2016 catch,
\Sexpr{top.coh(base_model, 2017, spec.yr=2014)}\% of the 2017 catch,
\Sexpr{top.coh(base_model, 2018, spec.yr=2014)}\% of the 2018 catch,
\Sexpr{top.coh(base_model, 2019, spec.yr=2014)}\% of the 2019 catch,
\Sexpr{top.coh(base_model, 2020, spec.yr=2014)}\% of the 2020 catch
\Sexpr{top.coh(base_model, 2021, spec.yr=2014)}\% of the 2021 catch, and
\Sexpr{top.coh(base_model, 2022, spec.yr=2014)}\% of the 2022 catch.
The 2016 cohort also appears to be above average, comprising
\Sexpr{top.coh(base_model, 2018, spec.yr=2016)}\% of the 2018 catch,
\Sexpr{top.coh(base_model, 2019, spec.yr=2016)}\% of the 2019 catch,
\Sexpr{top.coh(base_model, 2020, spec.yr=2016)}\% of the 2020 catch, and
\Sexpr{top.coh(base_model, 2021, spec.yr=2016)}\% of the 2021 catch, and
\Sexpr{top.coh(base_model, 2022, spec.yr=2016)}\% of the 2022 catch.
The absolute size of the 2014 year
class has now stabalized after observations across eight years of fishery observations
...
but consistent with, the base model value of \Sexpr{base_model$sigma_R_in}.  % $
...
Tables~\ref{tab:main-median-posterior} and \ref{tab:main-ci-posterior}).
It was close
to the target in 2008 and 2010, but harvest in those years did not exceed
...
was below the SPR management target from 2012 through 2016 and again from 2019
to 2022 (Figure~\ref{fig:main-fishing-intensity}). The median estimate
for 2017 and 2018 is below the management target though the 95\% posterior
...
(\Sexpr{can.last.5.years.attainment}\%).
...
and close to the average over the last decade (\Sexpr{tot.last.10.years.attainment}\%),
it decreased in 2022 to \Sexpr{tot.last.year.attainment}\%. This is primarily due
...
As noted above, the median relative fishing intensity was below target in all years.
...
intensity has remained well below target, and total catch has been declining since the
...
trajectory. This is particularly true for spawning biomass estimates in \Sexpr{assess_yr}
and throughout the current forecast period, because there is considerable uncertainty
associated with the size of the 2020 year class, now mostly mature, that propogates into
...
P$(B_{\Sexpr{assess_yr-1}} > \Bforty) = 90.5\%$], % hardwired**
such that the stock is estimated to be in the `healthy zone' (above the upper
stock reference point of $\Bforty$). Note that a probability of `100\%' (or
`0\%`) is based
...
that the spawning biomass is well above $\Bforty$ and $\Btwentyfive$.
The probability that spawning biomass at the beginning of \Sexpr{assess_yr}
is above $\Bforty$ is
...
\Sexpr{catch.limit.quantiles["median"]}~t and
...
% possible future use)
The probability of the spawning biomass decreasing from \Sexpr{end_yr} to
...
influence the exact change in spawning biomass, Figure~\ref{fig:main-maturity}).
% Already said above:
% The probability
% that the \Sexpr{forecast_yrs[2]} spawning biomass will be less than the
% \Sexpr{forecast_yrs[1]} spawning biomass ranges from \Sexpr{f(min(base_model$risks[[1]][,2]))}\% to \Sexpr{f(max(base_model$risks[[1]][,2]))}\%
...
\Sexpr{assess_yr - 1} (\Sexpr{same.catch.as.last.year}~t, catch scenario~\Sexpr{letters[catch.actual.ind]})
...
\item Consideration of an alternative prior distribution (mean and standard deviation) for natural
  mortality based on the \citet{Hamel2015} and \citet{HamelCope2015} life history meta-analytic method;
...
\item Downweighting the fishery age-composition data;
...
to $\sigma_r$, whereas absolute estimates of spawning biomass are relatively insensitive.
...
$\sigma_r$ of 1.68, which was more similar to the
base-model value of 1.4 than the 1.77 estimated in the \Sexpr{assess_yr-1} assessment.
...
This differene is due to the age-1 index providing additonal information on recruitment for cohorts
...
In this assessment, there is very little retrospective bias. Present is
slight year-specific positive and negative bias in spawning biomass, some minor adjustments to
...
dynamics. While uncertainty around the absolute amount of 2023 spawning biomass is the
...
Without rigourous simulation experiments
...
%was expecting that the spawning biomass would probably decline.
...
likely decline in spawning biomass given current information.
...
substantial and so in hindsight the probablity of going below $\Bforty$ was 0\%
...
Restrospective versions of Figures~\ref{fig:main-historical-1}
...
%Exceptions include during the relatively flat spawning biomass trajectory from
