...
\emph{Response -- Developing best practices for modeling equilibrium recruitment ($R_0$)
  ...
  remain broad topics of contemporary research. Recent calls have been that the next
...
\emph{The JTC plans to reinvigorate collaborative research plans to investigate the concurrent
  estimation of multiple variance parameters. This includes the estimation of variability
  associated with time-varying selectivity, $\sigma_R$, the extra standard deviation parameters
  on survey index data, and the Dirichlet-Multinomial parameters $\theta_{\text{fish}}$ and $\theta_{\text{surv}}$.
  ...
  This work is related with time-varying selectivity research, as discussed in response 2
  below.}
...
\emph{The completion of the Management Strategy Evaluation framework for \fishname\ continues to create
  ...
  colleagues at the Northwest Fisheries Science Center is looking at identifying climate drivers of
  ...
  Further, the MSE could be an additional option for
...
\emph{Response -- Members of the JTC are also part of the \fishname MSE technical work group.
  ...
  dependence and ecosystem change and a Ocean Frontier Institute workshop on management reference
...
\emph{Preliminary investigations based on estimated no-fishing ("unfished") time series seems to suggest
  ...
  indicate. If this was the case, estimates of relative fishing intensity would be higher than currently
...
\emph{The JTC is collaborating with regional oceanographers and ecosystem scientists
  ...
  dynamics for use as an annual reporting tool. These metrics could be evaluated as a set
  "stoplight" indicators, collectively support ecosystem-based fisheries management, and
...
\emph{Response -- Results from using the random walk Metropolis MCMC
  algorithm are presented in Appendix~\ref{chap:rwmh}, and full results from including the age-1
...
\emph{Response -- An updated ageing error analysis will be conducted after the an updated
  CARE exchange is completed among ageing labs. The JTC plans to utilize an updated ageing error
  software package being conducted jointly by the University of Washington and the
  Commonwealth Scientific and Industrial Research Organisation (CSIRO) upon completion.}
...
of unfished biomass for the base model with a 95\% credible interval
of \Sexpr{f(100*bratio_curr[1], 1)}--\Sexpr{f(100*bratio_curr[3], 1)}\% and
\Sexpr{f(100*bratio_age1[2], 1)}\%
for the age-1 index model) with a 95\% credible interval
of \Sexpr{f(100*bratio_age1[1], 1)}--\Sexpr{f(100*bratio_age1[3], 1)}\%.
This change is likely due to the base model
...
to the other model runs. The choice of age-6 as the maximum was retained in the base model as it
was more flexible than the choice of age-5. While runs with higher maximum age produced similar
overall stock dynamics, but at the cost of a considerable increase in the number of total model
parameters. All three alternative runs had reduced performance in terms of MCMC diagnostics when
