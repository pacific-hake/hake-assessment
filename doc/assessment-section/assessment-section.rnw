...
\emph{Response -- **Due to time constraints the JTC has been unable to do this.}
   % (no need to keep mentioning the Shutdown, just for the first one)
...
\emph{Response -- **(Okay?:) These were rectified during the 2018 meeting (for the alternative catch stream
sensitivity run), and have been permanently fixed.}
...
\emph{Regarding including the Canadian samples in the maturity analysis
  **... See Issue \#465}
