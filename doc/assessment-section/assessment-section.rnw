...
part of the same survey, on the same vessels. Selectivity was set to one for age-1 for this index
...
year's assessment \citep{JTC2021}. The number of samples in sensitivities to the base model
was not increased relative to the last assessment, i.e., 8,000 samples.
...
\emph{Response -- Specifying projected recruitment as the median of a given year range
  ...
  Results will be presented during the 2022 SRG review meeting.}
...
The addition of \Sexpr{last.data.yr} catch and fishery age compositions extend the model
