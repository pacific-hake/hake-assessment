...
** Not sure how to make it apparent that this is not a forced change due to
changing to the latest version of SS, but a choice we made.
The choice to constrain recruitment deviations during the main period to sum to
zero was removed in this assessment. This change should increase the
compatibility between MCMC and MLE results and the stability of model results
because penalties have implications for calculating derivatives. Relative
spawning biomass increased for the entire time series because the estimate of
$\log R_0$ increased relative to the \Sexpr{last.assess.yr} assessment and
