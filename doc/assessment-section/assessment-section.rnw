...
1975-2017 **update if get 2018 ages) because it is more consistent with time-varying weight-at-age 
than the non-time varying fecundity used in the 2018 base model. That
fecundity multiplied the maturity ogive by an average weight-at-age
to give age-dependent fecundity that was constant in time. Sensitivity
runs 54, 55 and 58 also use time-varying fecundity.
Non-time-varying fecundity is examined in sensitivity runs 52, 53, 56 and~57.
...
(1975-2017 [**will be 2018 if we update wtatage data]) for the equilibrium and
the 1966-1974 fecundity, as for the 2018 base model. Using the fecundity based on
1975-1979 is examined in sensitivity runs 55, 56, 57 and 58.

We did not use the 1975-1979 fecundity because **Aaron to write something here.
Here are our notes based on
discussions at the JTC meeting:
...
(see the bold value for 1975 in XXFigure 13). The same holds for age-14. If we
...
over 2015-2017.'' **Aaron to update [and presumably the SS configuration issue
needs to be thought about again] -- this is what we have: 
Aaron's analysis did not find compelling evidence to use a
short-term average for the forecasts (see above quote). However, since the 2018
age data are unavailable for the assessment, we have used the 2015-2017 average
weight-at-age for the 2018 weight-at-age and therefore for the 2018 fecundity
(this serendipitously avoids the Stock Synthesis configuration issue). It
therefore seems sensible to use the same fecundities for the projections (rather
than the average over all years). 

Sensitivity runs 53, **was base so switch around, 55 and 57 tested the alternative assumption of setting
the 2018 weight-at-age (and fecundity for
2018 and forecast years when the fecundity is time-varying) to be the mean from
1975-2017. **Results from these runs are based on MLE only (we show MCMC results
...
Therefore, to acheive consistency in the 2019 base run we use the 2015-2017 mean
weight-at-age for the forecasts (but stick with the long-term average for the
early years, as per (b) above). 
