...
(\Sexpr{start_yr.age.comps}--\Sexpr{last.data.yr}; see Section~\ref{sec:data-weight-at-age}).
The D-M
...
to the start of \Sexpr{assess.yr} but the estimates for
\Sexpr{assess.yr} are highly uncertain
