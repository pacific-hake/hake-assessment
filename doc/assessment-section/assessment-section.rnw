...
\eb
S_ay = S_a \cdot \exp{ \epsilon_{ay}}
\ee
where the $S_a$ are as described in equations (1)-(3), the 
the $\epsilon_{ay}$ are the selectivity deviations, $\sigma_s$ is
the parameter which controls the variability in the deviations 
(equivalent to $\phi$ in the base model), and the likelihood
contribution is from treating the deviations as $N(0, \sigma_s)$.

This was expected to reduce correlation among the deviation 
