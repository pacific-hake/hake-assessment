...
\emph{The JTC is conducting and collaborating on simulation projects looking into the concurrent
  ...
  on survey index data, and dirichlet multinomial($\theta$) parameters. This is particularly
...
smaller. [**ANDY Fig needs fixing first] Figure~\ref{fig:main-mle-vs-mcmc} shows the MLE and Bayesian (from
...
are P$(B_{\Sexpr{assess.yr}} > 0.4\Bmsy) = \Sexpr{f(dfo.probs.curr[1, 2], 1)}$\% and
...
the five most recent years (\Sexpr{assess.yr-5}--\Sexpr{assess.yr-1}); and all other
...
\Sexpr{f(sens.models.1[[which(sens.model.names.1=="Sigma R 1.8")]]$sigma_R_info$SD_of_devs[1], 2)}.
