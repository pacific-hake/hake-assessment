...
Table~\@ref(tab:es-fishing-intensity) for recent years and
...
(Table~\@ref(tab:es-fishing-intensity). Median relative fishing intensity is
...
`r tot_last_10_yrs_attainment`% (Table~\@ref(tab:es-landings-tac). Over
...
```{r landings-tac-table, results = "asis", echo = FALSE}
...
(ref:es-phase-plot-cap) Estimated historical path of median relative spawning
...
```{r es-phase-plot, fig.cap = "(ref:es-phase-plot-cap)", fig.height = 5, fig.width = 8, is.fig = TRUE, alt.text = "This figure shows that we are currently not overfishing nor in an overfished state."}
...
years except 2007--2010 (Figures~\@ref(fig:es-relative-spawning-biomass)
...
population sizes (see Table~\@ref(tab:es-reference-points)), highlighting
...
```{r es-reference-points-table, results = "asis", echo = FALSE}
...
````{r es-decisions-biomass-table, results = "asis", echo = FALSE}
...
catch alternatives for the base model (Tables~\@ref(tab:es-decisions-biomass)
and~\@ref(tab:es-decisions-spr)). The tables are organized to show the
...
HR) in 2023 results in a median relative fishing intensity of 0.91 (Table~\@ref(tab:es-decisions-spr)).

```{r es-decisions-spr-table, results = "asis", echo = FALSE}
decision_table(base_model,
               xcaption = paste0(
                 ...
                 "Table~\\@ref(tab:es-decisions-biomass). Values greater ",
...
(Tables~\@ref(tab:es-risk-year-1), \@ref(tab:es-risk-year-2),
...
`r assess_yr` (Table~\@ref(tab:es-risk-year-1) and
...
(Tables~\@ref(tab:es-risk-year-2) and~\@ref(tab:es-risk-year-3)
