...
\item **Explore alternative approaches and related assumptions for parameterizing
  time-varying fishery selectivity in the assessment.
%\enlargethispage{\baselineskip}

\item **Continue investigations into \fishname\ fecundity and maturity, including
  trying to understand links between fecundity and size, age, weight, and batch
  spawning, as well as spatio-temporal variability in the timing of spawning,
  skip spawning, batch fecundity, and size and age at maturity.
  Additionally, a more spatially comprehensive maturity analyses that
  incorporates information from Canadian samples would be advantageous.

\item **Explore potential recruitment indices for juvenile or young (0 and/or
  1 year old) \fishname, including further investigations into survey options,
  refinements and analyses, as well as environmental linkages to recruitment.
  Also investigate alternative ways to model and forecast recruitment, given
  the uncertainty present.

\item **Update ageing error calculations using information from recent double
  reads and inter-laboratory ageing comparisons. This would include updated
  information about ageing imprecision and the effects of
  ...
  exchanges), so another exchange program is needed to obtain the best
  current available information on ageing error.
...
\item **Continue to collect and analyze life-history data, including weight,
  ...
  growth and population density. Continue to explore the possibility of using
