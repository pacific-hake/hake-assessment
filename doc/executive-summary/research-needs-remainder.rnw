...
\item Continue to investigate maturity observations of \fishname\ and explore 
  additional sampling sources to determine fecundity and when spawning occurs. 
  Continue to explore ways to include new maturity estimates in the assessment. 
  This would involve:
\begin{enumerate}[label=(\alph*)]
  \item Read ages for samples that do not currently have an age.
  \item Further investigation of the smaller maturity-at-length south of Point 
    Conception.
  \item Determining the significance of batch spawning and viability of 
    spawning events throughout the year. 
  \item Studying fecundity as a function of size, age, weight, and batch 
    spawning. 
\end{enumerate}
...
\item Improve the characterization and accounting of research catch that is 
  reported to standard databases to improve data tracking and avoid double 
  counting.
...
\item Maintain the flexibility to undertake annual acoustic surveys for 
  \fishname\ under pressing circumstances in which uncertainty in the hake 
