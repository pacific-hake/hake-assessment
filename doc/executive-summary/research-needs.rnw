...
example of this has been the annual production of \fishname\ distribution forecasts
according to 9-month lead time subsurface (100 m) temperature predictions from J-SCOPE
...
tool should be used, or further developed, to examine how information on recruitment
...
The existing
\ifnum\value{counter_research-needs}>0 {MSE }\else{management strategy evaluation (MSE) }\fi
tool should be used, or further developed, to examine how changes in survey methods can be used
...
and provide other non-assessment indicators of system-state to management.
