...
with 2017--2020 having the highest catches on record.
%and below it
...
respectively.
In the \Sexpr{end.yr-1} catch,
\Sexpr{top.coh(base.model, last.data.yr, 3, cap = FALSE)}.
% Can check that manually from final row of Table 10 showing the proportions at age
Due to staffing issues, age data were unavailable from the Canadian freezer-trawler
fleet and minimally available for the Shoreside fleet in \Sexpr{end.yr-1}. 
...
\begin{figure}[tbp]
\begin{center}
<<survey-age1, fig.height=5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the survey estimate has decreased slighlty from 2019 to 2021">>=
#make.survey.biomass.plot(base.model)
make.survey.age1.plot(age.1.index)
@
\end{center}
%\vspace{0mm}
\caption{Age-1 index (numbers of fish) with approximate 95\% confidence intervals are
  based on sampling variability. See Table~\ref{tab:main-survey-history} for values
...
(\Sexpr{survey.start.yr}--\Sexpr{survey.end.yr}), an age-1 index (Figure~\ref{fig:es-survey-age1}),
as well as fishery age compositions (\Sexpr{start.yr.age.comps}--\Sexpr{last.data.yr}).
The 2011 survey index value was the lowest in the time series and was followed by the index increasing
...
of pre-spawn fish in 2011, 2009, 2015, and 2021 (2010, 2008, 2014, and 2020 year-
