...
**ALL TEXT from here onwards is likely just copied from 2015 assessment and needs to be modified/updated/automated.
...
The median catch for \Sexpr{min(forecast.yrs)} based on the default harvest policy (\Ffortyten) is \textbf{FIX..}~t, but has a
...
indicates fishing in excess of the $\Fforty$ default harvest rate catch limit. The default harvest rate catch limit results in a
median fishing intensity above 100\% in 2015, 2016, and 2017 because the $\Fforty$ default harvest rate catch limit is calculated
