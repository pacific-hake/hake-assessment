...
In particular, while the model expects the 2020 cohort to be above average, its absolute size remains
...
size of the 2020 year-class, which will lessen uncertainty of estimates of female spawning biomass.
...
(relative spawning biomass; Figure~\ref{fig:es-forecast-depletion-comparison})
...
of stock status and fishing intensity. Population dynamics and governing
...
be slightly different that the target because the $\Fforty$ default
...
\Sexpr{end_yr} catch level similar to that for \Sexpr{end_yr-1} (scenario f: \Spexr{end_yr} catch).
