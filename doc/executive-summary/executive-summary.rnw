...
(\Sexpr{round(100*mean(ct$us_catch / ct$tot_catch),1)}\% of the
...
\Sexpr{f(mean(ct[ct$year %in%
((max(ct$year)-9):max(ct$year)),]$tot_catch))}~t with U.S. and Canadian
catches averaging \Sexpr{f(mean(ct[ct$year %in%
((max(ct$year)-9):max(ct$year)),]$us_catch))}~t and
\Sexpr{f(mean(ct[ct$year %in%
((max(ct$year)-9):max(ct$year)),]$can_catch))}~t, respectively. The
