...
\caption{Acoustic survey biomass index (millions of tons).
  ...
  variability; black bar for 2009 does not include additional uncertainty due to
...
the long-term. The results of past (and ongoing) closed-loop simulations influenced
...
some developed for response to \Sexpr{last.assess.yr} SRG requests, although many sensitivity
...
relatively constant biomass since 2017.
% Manual 2019 done.
...
% Manual done 2019. Just check the sentence reads okay (a/an).
...
A relative fishing intensity above 1 (or 100\% when shown as a percentage) indicates fishing greater than the
...
years, whereas the forecasted catches are removed using selectivity
%
<<decisions.spr.table, results='asis', echo=FALSE>>=
make.decision.table(base.model,
                    xcaption = paste0("Forecast quantiles of \\fishname\\
                     relative fishing intensity (1-SPR)/(1-SPR\\subscr{40\\%}),
                     expressed as a percentage, for the ",min(forecast.yrs),"--",max(forecast.yrs)," catch
