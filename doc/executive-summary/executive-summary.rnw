...
and catch in U.S. waters averaged \Sexpr{f(mean(catches$Ustotal))}~t,
...
assessment for which the sensitivity analyses use Bayesian estimation (rather than just maximum
likelihood estimation).
Retrospective analyses
identify possible poor performance of the assessment model with respect to
future predictions**[mention MCMC if done for retros]. Past assessments have conducted closed-loop simulations
...
median and narrower credibility interval than the 1.312~(0.471-3.601)~million~t estimated in the \Sexpr{end.yr-1}
assessment.
