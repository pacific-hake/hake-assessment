...
**[should update automatically, check it reads okay] Coast-wide fishery \fishname\
landings averaged \Sexpr{f(mean(ct$tot_catch))}~t from
...
catch in U.S. waters averaged \Sexpr{f(mean(ct$us_catch))}~t,
(\Sexpr{f(mean(ct$us_prop_tot_catch), 1)}\% of the
...
\Sexpr{catch_can_mean_10yrs}~t, respectively. The
...
with the most recent four years having the highest catches on record.
%and below it
...
The 2019 estimate is the fourth highest of the series. **Mention 2021 survey
...
efficient No-U-Turn
Sampler (NUTS) that was successfully tested in 2020 and used for the 2021 assessment.
Sensitivity analyses are used to identify alternative model
assumptions that may also be consistent with the data. **This is the second
assessment for which the sensitivity and retrospective analyses also use Bayesian
...
approaches. Time-varying fecundity, which was introduced in 2019, is retained.
The weight-at-age information for the forecast period
is a representation of the last five years, as for the 2020 assessment.
...
\begin{figure}[tbp]
\begin{center}
<<relative-spawning-biomass, fig.height=5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the relative biomass has been decreasing since 2017">>=
...
Median spawning biomass is estimated to have peaked again in 2013 and 2014 due
...
\Sexpr{curr.bio.upper}~million~t). The current estimate of the \Sexpr{end.yr-1} female
...
**The new data available and implementation of NUTS for this assessment do not significantly change the
...
class because the \Sexpr{end.yr-2} acoustic survey did not sample age-0 fish and the \Sexpr{end.yr-1} fishery
...
an agreement on a coast-wide 2020 TAC. The U.S. catch target was
\Sexpr{us.allotment.percent.last.year}\% of the total coast-wide catch target, and
the Canada catch target was \Sexpr{can.allotment.percent.last.year}\%. These
percentages are different to the usual
\Sexpr{us.allotment.percent}\% and
\Sexpr{can.allotment.percent}\% as specified in the Joint U.S.-Canada
Agreement for \fishname.
...
The median relative female spawning biomass was above the $\Bforty$ reference point in
...
(Figures~\ref{fig:es-relative-spawning-biomass} and~\ref{fig:es-phase}).
...
The target fishing mortality ($\Fforty$) and
...
**The medians of sustainable yields and biomass reference points are almost
9\% lower than in the \Sexpr{last.assess.yr} assessment.
This is a result of increasing the effective sample size used to describe the
posterior distributions of model parameters, leading to more accurate point estimates.
...
at the end of \Sexpr{end.yr-1} is \Sexpr{probs.curr.rel.fish.intens.above.one}\%.
...
**We also present detailed results
for a model that includes the age-1 survey index
and for the base model with Bayesian estimation performed using the
random walk Metropolis Hastings algorithm (as used in previous assessments).
...
they are (i.e., cohort strength is not well known until it is observed
...
make.decision.table(base.model,
                    xcaption = paste0("**Forecast quantiles of \\fishname\\
                     ...
                     "), to catch levels reduced by 10\\% each year (rows ",
                     catch.reduction.str,
...
**Mention switch to new format**  Decision tables give the projected population status (relative spawning biomass)
...
\begin{figure}[tbp]
\begin{center}
<<forecast-depletion-comparison-plot, fig.height=5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the median relative spawning biomass is projected to decline for all non-zero catch levels evaluated">>=
...
\Sexpr{assess.yr+1} because catch alternatives~h and~i
have catches that are $>$\Sexpr{f(catch_levels[[5]][[1]][1])}~t (the constant
catch for alternative~\Sexpr{letters[5]}) in \Sexpr{assess.yr} but
$<$\Sexpr{f(catch_levels[[5]][[1]][1])}~t in \Sexpr{assess.yr+1}.
%
...
% are conditional and can be non-linear.
...
\begin{figure}[tbp]
\begin{center}
<<forecast-risk-comparison-plot-year-3, fig.height=4.5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that there is a high probability that the stock will decline in the next three years regardless of catch level">>=
make.forecast.risk.comparison.plot(base.model,
                                   forecast_yrs = forecast_yrs,
...
its overall biomass will also decrease as it continues to age.
