...
\end{center}
%\vspace{0mm}
\caption{Age-1 index (numbers of fish) and approximate 95\% confidence intervals
...
biomass indices, acoustic survey age-composition data, age-1 indices, and fishery
...
assumptions that may also be consistent with the data. This is the first time
the entire assessment, including model bridging, sensitivity, and retrospective
analyses, has used Bayesian
...
This \Sexpr{assess.yr} assessment retains the same general population dynamics structure
as the base assessment model from \Sexpr{last.assess.yr} as well as many of the previous
elements as configured in Stock Synthesis. The main change from \Sexpr{last.assess.yr} is
the addition of the age-1 index to help inform recent and current recruitment expectations
(see Section~\ref{sec:data-acoustic-survey} for more details).
Analyses conducted in 2014
...
is a representation of the last five years, as for the \Sexpr{last.assess.yr} assessment.
...
The new data available and addition of the age-1 index for this assessment do not significantly change the
pattern of recruitment estimated in recent assessments.
However, estimates of absolute recruitment for some recent years have slightly increased.
...
Similarly, estimates for 2016 and 2018 have increased by
...
indicate that the \Sexpr{end.yr-2} year class is above average.
...
fraction was the highest in 2017 followed closely by 2020 (Table~\ref{tab:es-fishing-intensity}).
Note that in earlier assessments the
...
target (of 1.0) for 2017--2018 (Figure~\ref{fig:es-fishing-intensity}).
