...
\Sexpr{last.5.years.above.avge[length(last.5.years.above.avge)]}
(these last three years being the highest catches on record),
and below it
...
total catch of the 2010, 2014 and 2016 year classes is estimated to be about
...
in 2012, 2013, and 2015; the 2019 survey is the fourth highest of the series.
% rank(-survey.comparison$with.extrap)
% Not worth automating as have to think
%  about what is worth saying each time anyway.
...
This \Sexpr{assess.yr} assessment retains most of the structural form of the base
...
selectivity in this assessment, continuing with the new parameterization and
increase variability introduced
in the 2017 assessment.
We apply the Dirichlet-Multinomial approach to weighting
composition data again this year, though introduce a normal prior for the
parameters [**maybe say more here once main text is written].
We again provide sensitivities to alternative data-weighting
approaches. We retain the time-varying fecundity introduced in 2019 **mention
assumptions (once they are summarised in main text).
% Hardwire (rather than use assess.yr) years in these final sentences, else they
%  become incorrect the following year.
...
assessment.
% Manual 2020 done (updated numbers in final sentence and revised text).
...
The model currently estimates smaller-than-average 2011, 2012, 2013,
2015, 2017 and 2018 year classes (median recruitment below the mean of all median recruitments).
% Done for for 2020 assessment
%  years.since.2010.recruitment.med.below.mean  # ignore assess.yr-1
...
above) peaked in 1999, and then reached slightly higher values in 2006 and 2008
...
in 2015 before leveling off to 75-78\% since 2016.
...
slightly decreasing to \Sexpr{exploitation.med.2018} in 2018 and 2019. % Happen
                                        % to be the same for 2020 assessment.
There is a
...
% Manual 2020 done. For last sentence just check
...
all years except 1999 and 2007-2010
...
\caption{Estimated historical path followed by medians of relative fishing
         ...
         relative spawning biomass (horizontal). **Andy to check figure, see
         Issue 443.}
\label{fig:es-phase}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
% Manual done 2020. Years chosen are based on median.relative.bio.below.target,
%  2011 no longer below ref point in 2020 assessment.
...
% Manual done 2020. Just check the sentence reads okay (a/an).
...
The medians of sustainable yields and biomass reference points are
almost 10\% lower than in the \Sexpr{last.assess.yr-1} assessment.
% Manual done 2020.
...
make.decision.table(base.model,
                    xcaption = paste0("Forecast quantiles of \\fishname\\
                     ...
                     expressed as a percentage, for the ",min(forecast_yrs),"--",max(forecast_yrs)," catch
...
**TO UPDATE based on main text. The addition of time-varying fecundity into the 2019 assessment resulted
...
\Sexpr{forecast_yrs[length(forecast_yrs)]} (Tables~\ref{tab:es-risk-year-1}
...
values
in \Sexpr{assess.yr} (Table~\ref{tab:es-risk-year-1} and
Figure~\ref{fig:es-forecast-risk-comparison-year-1}). However, interpolation is
...
% \Sexpr{catch_levels[[5]][[1]][1]}~t
in \Sexpr{assess.yr+1} (Table~\ref{tab:es-decisions-spr});
this explains why a few probabilities decline
(rather than rise) with increased \Sexpr{assess.yr+1} catch levels in Table~\ref{tab:es-risk-year-2} and
and Figure~\ref{fig:es-forecast-risk-comparison-year-2}).

Figure~\ref{fig:es-forecast-depletion-comparison} shows the predicted
relative spawning biomass trajectory through
\Sexpr{forecast_yrs[length(forecast_yrs)]} for several of the management
...
\Sexpr{forecast_yrs[1]} to \Sexpr{forecast_yrs[2]} (Table~\ref{tab:es-risk-year-1}), and a
\Sexpr{zero.catch.prob.bio.down.2}\% probability of decreasing from
\Sexpr{forecast_yrs[2]} to \Sexpr{forecast_yrs[3]} (Table~\ref{tab:es-risk-year-2}).
%Manual 2020 done, though all automated.
...
Figure~\ref{fig:es-forecast-risk-comparison-year-1}). It is 95\% for the
\Sexpr{end.yr} catch equaling that for \Sexpr{end.yr-1} (catch alternative~d).
...
The only years since 2010 with estimated above-average recruitment are 2014 and
2016, which may result in increases to
the spawning biomass as these cohorts increase in weight. **Last sentence okay? Could
just omit (we had something last year).
