...
\end{center}
%\vspace{0mm}
\caption{Acoustic survey biomass indices of age-2+ fish (millions of tons).
...
\end{center}
%\vspace{0mm}
\caption{Relative age-1 indices (numbers of fish) and approximate 95\% confidence intervals
  based on sampling variability. See Table~\ref{tab:main-survey-history} for values
  used in the base model. The index is relative because
  the survey does not attempt to get catch all available
  age-1 fish and the index does not include kriging
  like the age-2+ biomass estimate.}
