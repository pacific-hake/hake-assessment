...
fishing intensity and relative spawning biomass, the model estimates a less than
6\% joint probability of being both above the target relative fishing intensity
...
alternative model in Appendix~\ref{chap:srg-requests}. 

An inconsistency in this alternative model is that the the mean weight-at-age 
