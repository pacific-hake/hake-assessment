...
an agreement on a coast-wide 2020 TAC. The U.S. specified catch target was
...
percentages are different then the usual
...
(Figure~\ref{fig:es-relative-spawning-biomass} and~\ref{fig:es-phase}).
