...
the coast-wide catch has been in steady decline to \Sexpr{last.year.landings}~t in
\Sexpr{end_yr - 1}, out of a
...
efficient No-U-Turn
Sampler (NUTS) that was successfully tested in 2020 and used in assessments since that
time. Sensitivity analyses are used to identify alternative model
assumptions that may also be consistent with the data. The
entire assessment, including model bridging, sensitivity, and retrospective
analyses, uses a Bayesian framework for
estimation.
...
elements as configured in Stock Synthesis. The main change from the \Sexpr{last_assess_yr} assessment is
...
Weight-at-age and selectivity used for the forecast period
is a representation of conditions during the last five years, as for the \Sexpr{last_assess_yr} assessment.

\subsection*{Stock biomass} \label{sec:es-stock-biomass} \phantomsection
\addcontentsline{toc}{subsection}{Stock biomass}
Results from the base stock assessment model
...
The addition of 2022 data for this assessment does not significantly change the
...
before dropping nearly by half in 2022 (Table~\ref{tab:es-fishing-intensity}).
...
% Happen to be the same for 2020 assessment.
There is a
...
target (of 1.0) in 2017 and 2018 (Figure~\ref{fig:es-fishing-intensity}).
...
The medians of sustainable yields and biomass reference points are very similar
to these in the \Sexpr{last_assess_yr} assessment.
...
document. 
%A more extensive examination of sensitivity results are provided for a model that excludes
...
In particular, while the model expects the 2020 cohort to be above average, the absolulte size remains
highly uncertain.  This uncertainty propogates directly into current and forecasted estimates of
spawning stock biomass. The upcoming 2023 acoustic survey will provide additional information on the
size of the 2020 year-class, which will lessen uncertainty with spawning stock biomass estimates.
...
distribution. Tables show results for up to three year's of future catch levels on subsequent estimates
...
A relative fishing intensity of 1 (or 100\% when shown as a percentage) indicates fishing at the
$\Fforty$ default harvest rate catch target. Slight differences in median relative fishing intensity can
arise in projected years because the $\Fforty$ default
...
(scenario m: default HR) in 2023 results in a median relative fishing intensity of
0.91.  
%Alternative catch levels where median relative fishing intensity is
...
\Sexpr{end_yr+1} is above 72\% for all catch levels
(Table~\ref{tab:es-risk-year-1} and
Figure~\ref{fig:es-forecast-risk-comparison-year-1}). It is 81\% for the
...
in determining spawning stock biomass during the forecast years presented here.
