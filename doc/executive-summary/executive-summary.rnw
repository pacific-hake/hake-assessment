...
and 2014 (30.90\%) year classes. Due to the coronavirus disease 2019 (COVID-19)
pandemic, no biological samples
were available from the Canadian freezer-trawler sector in 2020 because observers
...
The medians of sustainable yields and biomass reference points have changed by
nearly 9\% compared to the \Sexpr{last.assess.yr} assessment. This is a result
of increasing the effective sample size used to describe the posterior distributions
of model parameters, leading to more precise point estimates.
...
by the fishery and survey, typically at minimum age-3).  Further, the interaction
among variance parameters that govern variability in fishery selectivity and recruitment
parameters through time, as well as those used in relative data weighting, is not well
...
%%** Remove "(unilaterally summed)" text from this caption (and main-table caption; table 28) after 2021
...
make.decision.table(base.model,
                    xcaption = paste0("Forecast quantiles of \\fishname\\
                     ...
                     ") and the (unilaterally summed) TAC from ",
                     min(forecast_yrs)-1," (row ",
...
\Sexpr{assess.yr+1} because catch alternatives~h and~i apply a
\Sexpr{assess.yr+1} catch level that is dependent on the
\Sexpr{assess.yr} catch (i.e., it is not constant).  Therefore, the probabilities for
% \Sexpr{catch_levels[[5]][[1]][1]}~t
% \Sexpr{catch_levels[[5]][[1]][1]}~t
\Sexpr{assess.yr+1} shown in Figure~\ref{fig:es-forecast-risk-comparison-year-2}
are conditional and can be non-linear. This explains why a few probabilities decline
...
\Sexpr{end.yr+1} is over 65\% for all catch levels
(Table~\ref{tab:es-risk-year-1} and
Figure~\ref{fig:es-forecast-risk-comparison-year-1}). It is 86\% for the
...
as losses from mortality outweigh increases from growth.  The smaller but
above-average 2016 cohort is entering this growth-mortality transition period, suggesting that
their overall biomass is also expected to decrease as they continue to age.
...
decision-making (see Section~\ref{sec:research}). The top three are:
