...
parameters; this is done via Markov chain Monte Carlo sampling using the No-U-Turn
...
composition data and use a new prior for the corresponding parameters.
...
all years to match the years used for other time-varying processes.
...
almost 10\% lower than in the \Sexpr{last.assess.yr} assessment.
...
and present the key ones in the main document. We also present full Bayesian
