...
\caption{Total \fishname\ catch used in the assessment by sector,
  \Sexpr{start_yr}--\Sexpr{last.data.yr}.
...
landings (\Sexpr{start_yr}--\Sexpr{last.data.yr}), an acoustic survey biomass
index (Figure~\ref{fig:es-survey-biomass}) and age compositions
(\Sexpr{survey.start_yr}--\Sexpr{survey.end.yr}), an acoustic survey index of
age-1 fish (Figure~\ref{fig:es-survey-age1}),
and fishery age compositions (\Sexpr{start_yr_age_comps}--\Sexpr{last.data.yr}).
...
estimated above-average (yet still highly uncertain) 2020 cohort will play a large role in determining spawning stock biomass
