...
without adequate basis (i.e., based upon quite limited data). **Probably here
mention new technical updates also.
...
This is **slightly
higher than the 2.129~(0.763-7.445)~million~t estimated in the \Sexpr{end.yr-1}
assessment.
% Manual 2018: updated numbers in final sentence, just have to check if
%  `slightly higher' still true.
...
all years except 1968, 1998-2000 and 2007-2011
...
fishing intensity shows that only in 1999 was the median relative fishing
intensity above the target of 1.0 and the female spawning biomass below the
...
%%%%%%%%%%%%%%%%%%%%%%%
%
Between 2007 and 2011, however, median relative fishing intensity ranged from
...
fishing intensity and relative spawning biomass, the model predicts a less than
4\% joint probability of being both above the target relative fishing intensity
...
slightly lower sustainable yields and reference biomasses estimated in this
assessment.
% Manual done 2017.
...
and the scientific basis for prior probability distributions.  To address
structural uncertainties, the JTC investigated a range of alternative models,
and we present a subset of key sensitivity analyses in the main document.
...
age-3). **This is particularly apparent for this assessment, because the 2014
cohort is potentially very large but is still highly uncertain
(Figure~\ref{fig:es-recruitment}).

The JTC presented results from closed-loop simulations evaluating the effect of
including potential age-1 indices on management outcomes at a 2015 Joint
Management Committee (JMC) meeting in Victoria, B.C. We found that fitting to an
...
age-1 index is not included in the base model but included in a sensitivity run.
% Manual 2017 done.
...
wide range of uncertainty, the 2.5\% to 97.5\% range being
...
make.decision.table(base.model,
                    xcaption <- paste0("Forecast quantiles of \\fishname\\
                     ...
                     catch levels (rows a, b, c, d, e), including the TAC from  ",
                     min(forecast.yrs)-1," (row d), the catch values that result
                     ...
                     " and ",min(forecast.yrs)+1," (row h). Row~e uses 600,000~t
                     rather than the 500,000~t from last year's assessment,
                     because 500,000~t is essentially row~d. Catch in ",
...
\Sexpr{end.yr+1} is less than 50\% for only the 0~t and 180,000~t catch levels
(Table~\ref{tab:es-risk-year-1} and
Figure~\ref{fig:es-forecast-risk-comparison-year-1}). The highest probability of
decrease is 82\%, which is for the default harvest policy.
...
dropping below $B_{40\%}$ is 18\% for all catches explored
(Table~\ref{tab:es-risk-year-1} and
Figure~\ref{fig:es-forecast-risk-comparison-year-1}).  It should be noted that
...
The model estimated below-average recruitment for the 2011, 2012 and 2013
cohorts, but a potentially large 2014 cohort that will result in increases to
the spawning biomass as it continues to mature.
% Still true for 2017 assessment - see
%  years.since.2010.recruitment.med.below.mean
