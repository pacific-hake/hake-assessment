...
coast-wide catch in \Sexpr{end_yr - 1} was \Sexpr{last.year.landings}~t, out of a
...
Due to staffing issues, age data from \Sexpr{end_yr-1} were unavailable from the Canadian
...
%% in \Sexpr{survey_end_yr} have been added to the survey time series
...
%% Further new data for this \Sexpr{assess_yr}
%% assessment, that were not in the \Sexpr{last_assess_yr} assessment,
%% are the
%% \Sexpr{last_data_yr} fishery catch and fishery age compositions, and the mean
%% weight-at-age for \Sexpr{last_data_yr}.
%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[tbp]
\begin{center}
...
(\Sexpr{survey_start_yr}--\Sexpr{survey_end_yr}), an acoustic survey index of
age-1 fish (Figure~\ref{fig:es-survey-age1}),
and fishery age compositions (\Sexpr{start_yr_age_comps}--\Sexpr{last_data_yr}).
...
This \Sexpr{assess_yr} assessment retains the same general population dynamics structure
as the base assessment model from \Sexpr{last_assess_yr} as well as many of the previous
elements as configured in Stock Synthesis. The main change from \Sexpr{last_assess_yr} is
...
is a representation of the last five years, as for the \Sexpr{last_assess_yr} assessment.
...
\caption{Median of the posterior distribution for beginning of the year female
         spawning biomass ($B_t$ in year $t$) through \Sexpr{end_yr} (solid line) with 95\% posterior
...
make.depletion.plot(base_model,
                    start_yr = start_yr,
                    end_yr = end_yr,
                    color = "blue")
@
...
make.recruitment.plot(base_model,
                      equil.yr = unfished.eq.yr,
                      start_yr = start_yr,
                      end_yr = end_yr,
                      color = "blue",
...
indicate that the \Sexpr{end_yr-2} year class is likely above average, while the 2019 year class is
...
Estimates of the \Sexpr{assess_yr} base model reference points with posterior
...
10\% higher than in the \Sexpr{last_assess_yr} assessment.
...
make.reference.points.table.old(base_model,
...
%catch for alternative~\Sexpr{letters[5]}) in \Sexpr{assess_yr} but
%$<$\Sexpr{f(catch_levels[[5]][[1]][1])}~t in \Sexpr{assess_yr+1}.
%
% \Sexpr{assess_yr+1} because catch alternatives~h and~i apply a
%\Sexpr{assess_yr+1} catch level that is dependent on the
%\Sexpr{assess_yr} catch (i.e., it is not constant).  Therefore, the probabilities for
%% \Sexpr{catch_levels[[5]][[1]][1]}~t
%% \Sexpr{catch_levels[[5]][[1]][1]}~t
%\Sexpr{assess_yr+1} shown in Figure~\ref{fig:es-forecast-risk-comparison-year-2}
...
% The spawning biomass is predicted to decrease from \Sexpr{end_yr} to
%  \Sexpr{end_yr+1} with a less than 50\% probability only for the 0~t and
...
the start of \Sexpr{end_yr+1} is 0\%, and of
...
make.forecast.depletion.comparison.plot(base_model,
                                        models.inds,
                                        models.names,
                                        start_yr = 2009,
                                        model.end_yr = end_yr,
