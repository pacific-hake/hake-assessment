...
the coast-wide catch has been declining annually to \Sexpr{last.year.landings}~t in
...
the magnitude of estimates of the strength of extreme cohorts. In closed-loop simulations,
...
We retained the Dirichlet-multinomial estimation approach to weighting
composition data and
investigated sensitivity to an alternative data-weighting
approach. Time-varying fecundity, which was introduced in 2019, is retained.
Assumptions for the forecast period for weight at age and selectivity are based on 
conditions during the last five years, as for the \Sexpr{last_assess_yr} assessment.
...
\caption{Median (solid line) of the posterior distribution for relative spawning
         ...
         credibility intervals (shaded area). Dashed, horizontal lines are displayed for
         10\%, 40\%, and 100\% $B_t / B_0$.}
...
% The female spawning biomass is predicted to decrease from \Sexpr{end_yr} to
...
the maximum probability of the female spawning biomass dropping below $B_{10\%}$ at
