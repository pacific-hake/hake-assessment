...
beginning in 2018 the definition age was lowered to age-2 because these
...
in 2015 before increasing again to \Sexpr{median.intensity.penult.yr}\%
in \Sexpr{end.yr-1}. The exploitation fraction has increased from a recent low of
\Sexpr{exploitation.med.2012} in 2012 to \Sexpr{exploitation.med.penult.yr} in
...
is below the
reference point of $\Bforty$ (Figure~\ref{fig:es-phase})  This highlights that
the target fishing mortality ($\Fforty$) and the target spawning biomass  ($\Bforty$)
result in different population sizes (see Table~\ref{tab:es-reference.points} ).
