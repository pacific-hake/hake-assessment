...
range of alternative assumptions using maximum likelihood estimation, 
and present the key ones in the main document. We also conducted full Bayesian MCMC
analyses for the model that included the age-1 survey index 
(see Appendix~\ref{chap:age1-survey}), and the model that used the 
Auto-Differentiation No-U-Turn-Sampling algorithm (AD-NUTS, 
\citet{Monnahan2018}, \citet{Monnahan2019}) for efficient MCMC sampling 
(see Appendix~\ref{chap:adnuts}).
