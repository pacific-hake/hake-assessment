...
The U.S. government shutdown affected the data used in the \Sexpr{assess.yr}
assessment. U.S. age data were not available, and Canadian age data could not
be used as they are not necessarily representative of the coastwide stock. Usually U.S.
and Canadian age data are combined as there is only one fleet in the model. Only total \Sexpr{last.data.yr} catch
was added to the model this year. U.S. catch was extracted before the government shutdown took place,
after hake fishing had finished for the year.
...
selectivity in this assessment, albeit with a new parameterization due to a
change in Stock Synthesis. The assumed variability of the annual deviations in
selectivity was increased in the 2017 assessment because the settings used in previous
assessments resulted in an extremely large estimate of the 2014 year class
without adequate basis (i.e., based upon quite limited data). We retained the equivalent
assumed variability here. We also included a new approach for automatically weighting composition
data.
...
cohorts, has resulted in a relatively constant biomass since 2013.
% Manual 2018 done.
...
(\Sexpr{prev.bio.lower}--\Sexpr{prev.bio.upper})~million~t. This is a lower
median than the 2.129~(0.763-7.445)~million~t estimated in the \Sexpr{end.yr-1}
assessment, though the credibility interval lies within that from the \Sexpr{end.yr-1}
assessment.
% Manual 2018: updated numbers in final sentence and revised text.
...
% Done for for 2018 assessment - see
...
(Table~\ref{tab:es-fishing-intensity} and
Figure~\ref{fig:es-exploitation-fraction}). Note that in previous assessments
exploitation fraction was defined in terms of fish age-3 and above, but we
revised the definition this year because age-2 fish are often caught by the
fishery.
% Manual 2018 done.
%
%%%%%%%%%%%%%%%%%%%%%%%
...
in \Sexpr{end.yr-1}, while the exploitation fraction has increased from
...
rates differed between the United States (\Sexpr{usa.last.5.years.attainment}\%)
and Canada (\Sexpr{can.last.5.years.attainment}\%). Total landings last exceeded
the coast-wide quota in 2002 when utilization was 112\%.
% Manual 2018 done. For last sentence just check
...
The joint history of the medians of relative spawning biomass and relative
...
is below the
reference point of $\Bforty$ (Figure~\ref{fig:es-phase}).
...
% Manual done 2018. Based on median.relative.bio.below.target.
...
5.7\% joint probability of being both above the target relative fishing intensity
in \Sexpr{end.yr-1} and below the $\Bforty$ relative spawning biomass level at
the start of \Sexpr{end.yr}.
% sum(base.model$mcmc$Bratio_2018<0.4 & base.model$mcmc$SPRratio_2017>1) /nrow(base.model$mcmc)
...
assessment (except for SPR at $\Bforty$ and SPR at MSY).
% Manual done 2018.
...
cohort is (i.e., cohort strength is not well known until it is at least
age-3).
% **This is particularly apparent for this assessment, because the 2014
...
models.names <- sapply(catch.levels, "[[", 2)[models.inds]
...
\caption{Time series of estimated relative spawning biomass to \Sexpr{end.yr}
         ...
         defined in Table~\ref{tab:es-decisions-biomass} (grey region), with 95\%
...
%Manual 2018 done, though all automated.
...
\Sexpr{end.yr+1} is less than 50\% for only the 0~t catch level
...
the start of \Sexpr{end.yr+1} is less than 1\% and the maximum probability of
dropping below $B_{40\%}$ is 24\% for all catches explored
(Table~\ref{tab:es-risk-year-1} and
Figure~\ref{fig:es-forecast-risk-comparison-year-1}).
...
%  Figure~\ref{fig:es-forecast-risk-comparison-year-1}).
% Manual 2018 done above.
During the 2018 Scientific Review Group (SRG) meeting, the SRG requested an alternative
run. This alternative run includes the following changes from the base model,
related to fecundity:

\begin{itemize}

   \item add time-varying fecundity by multiplying the weight-at-age matrix (rather than
...
Decision tables, reference points, probabilities associated with alternative
forecast catch levels, and comparisons to the base model are presented for the
\altshort\ in Appendix~\ref{chap:srg-requests}. The difference between the results
is predominantly due to the assumptions about equilibrium weight-at-age.

An inconsistency in this \altshort\ is that the the mean weight-at-age
across all years is still used for the calculation of
stock biomass in the years outside the range with empirical data (1975-2017),
