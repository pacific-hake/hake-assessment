...
\begin{figure}[!b]
\begin{center}
<<total-catches, fig.height=4, fig.width=8, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that catches in the last 5 years have been the largest in the time series">>=
...
catch in U.S. waters averaged \Sexpr{f(mean(catches$Ustotal))}~t,
(\Sexpr{round(100*mean(catches$Ustotal/catches$TOTAL),1)}\% of the
...
coast-wide catch in \Sexpr{end.yr-1} was \Sexpr{last.year.landings}~t, out of a
...
, with the most recent four years being the highest catches on record).
%and below it
...
respectively. Landings in 2020 were most represented by the 2016 (35.23\%)
and 2014 (30.90\%) year classes. Due to the Covid-19 pandemic, no biological samples
were available from the Canadian Freezer Trawler sector in 2020. 
...
\begin{figure}[tbp]
\begin{center}
<<survey-biomass, fig.height=5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the survey estimate has increased from 2017 to 2019">>=
#make.survey.biomass.plot(base.model)
make.survey.biomass.extrap.plot(survey.comparison)
@
\end{center}
%\vspace{0mm}
\caption{Acoustic survey biomass indices (millions of tons).
...
age compositions (\Sexpr{start.yr.age.comps}--\Sexpr{last.data.yr}). The 2011 survey index value
...
The 2019 estimate is the fourth highest of the series.
...
aggregated fisheries and the acoustic survey provide data that
facilitates estimating relative cohort strength, i.e., strong and weak
cohorts.
...
efficient No-U-Turn
Sampler (NUTS) that was successfully tested in the \Sexpr{last.assess.yr} assessment.
Sensitivity analyses are used to identify alternative model
assumptions that may also be consistent with the data. This is the first
assessment for which the sensitivity and [**TRUE?] retrospective analyses also use Bayesian
...
management based upon assessment models parameterized with time-varying fishery
...
inter-annual variability in catch compared to assessment models parameterized with
...
selectivity in this assessment.
We retain the Dirichlet-Multinomial estimation approach to weighting
composition data.
We again provide sensitivities to alternative data-weighting
approaches. Time-varying fecundity, which was introduced in 2019, was retained.
The weight-at-age information for the forecast period
was a representation of the last five years, as for the 2020 assessment.
...
\begin{figure}[tbp]
\begin{center}
<<spawning-biomass, fig.height=5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the stock has been decreasing in size since 2017">>=
...
\begin{figure}[tbp]
\begin{center}
<<relative-spawning-biomass, fig.height=5, fig.width=8, is.fig = TRUE>>=
...
the \Sexpr{end.yr} female spawning biomass is \Sexpr{curr.bio.median}~million~t (with a
...
(\Sexpr{prev.bio.lower.last.assess}--\Sexpr{prev.bio.upper.last.assess})~million~t estimated in the \Sexpr{end.yr-1}
assessment.
...
However, estimates of absolute recruitment for some recent years have slightly changed
with the addition of new data. 
For example, this assessment's median estimate of the 2014 recruitment changed by 
...
(prev.assess.recruitment.med)["2014"]) * 100), 0)}\% change) than last
...
\begin{figure}[tbp]
\begin{center}
<<recruitment, fig.height=5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows the last three large cohorts' recruitment: 2010, 2014, and 2016">>=
...
The model currently estimates small 2011, 2013, 2015,
and 2018 year classes (median recruitment well below the mean of all median recruitments).
...
The 2014 and 2016 year classes are likely both larger than average, however there is a
very high chance (\Sexpr{prob.percent.2014.rec.gt.2016.rec}\%) that 2014 is larger than 2016. 
There is very little information in the data to estimate the size of the \Sexpr{end.yr-2} year
class, because the \Sexpr{end.yr-2} acoustic survey does not sample age-0 fish and the \Sexpr{end.yr-1} fishery
largely did not encounter this year class.
...
be unreliable prior to at least model age-3 (observed at age-2).
% Manual 2020 done.
...
above) peaked in 1999 and then reached similar levels in 2006 and 2008
...
since the 2018 assessment the definition age was lowered to age-2 because these
...
\begin{figure}[tbp]
\begin{center}
<<exploitation-fraction, fig.height=5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that the current exploitation fraction of the stock biomass is approximately 0.1">>=
...
\Sexpr{median.intensity.2010}\% of the target in 2010 to \Sexpr{median.intensity.2015}\%
in 2015, and then it leveled off around 75\% from 2016 to 2019 before dropping to
\Sexpr{median.intensity.2020}\% in \Sexpr{end.yr-1}.
...
\Sexpr{exploitation.med.2018}). % Happen
...
target (of 1.0) for 2016--\Sexpr{end.yr-2} (Figure~\ref{fig:es-fishing-intensity}).
...
and \Sexpr{can.last.5.years.attainment}\% for Canada. However, country-specific
quotas (or catch targets) in 2020 were specified unilaterally, due to the lack of
an agreement on a coast-wide 2020 TAC.  Total landings last exceeded
...
make.landings.tac.table(landings.vs.tac,
                        ...
                        xcaption = "Recent trends in \\fishname\\ landings and
                                     management decisions. Catch targets in 2020 were
                                     specified unilaterally, due to the lack of an
                                     agreement on a coast-wide 2020 TAC.",
...
all years except 2009 and 2010
...
As such, the median relative fishing
...
While the target fishing mortality ($\Fforty$) and
...
the 2008, 2010, 2014, and 2016 recruitments, and
...
% Manual done 2021. Just check the sentence reads okay (a/an).
...
almost 10\% [**that was last year's, worrying if they've dropped again] lower than in the \Sexpr{last.assess.yr} assessment.
The probability that spawning biomass at the beginning of \Sexpr{assess.yr}
is below $\Bforty$ is
P$(B_{\Sexpr{assess.yr}} < \Bforty) = \Sexpr{probs.curr.below.bforty}$\%,
and of being below $\Btwentyfive$ is
P$(B_{\Sexpr{assess.yr}} < \Btwentyfive) = \Sexpr{probs.curr.below.btwentyfive}$\%.
The probability that the relative fishing intensity was above its target of 1.0
at the end of \Sexpr{end.yr-1} was \Sexpr{probs.curr.rel.fish.intens.above.one}\%.
...
make.reference.points.table(base.model,
                            xcaption = paste0("Summary of median and 95\\%
                             ...
                             weight-at-age and 1966--",
                             end.yr-1," averages for selectivity-at-age."),
...
range of alternative assumptions using Bayesian estimation,
and present the key ones in the main document.
%We also present full Bayesian
%analyses for a model that includes the age-1 survey index
%and a model that uses the new efficient
%No-U-Turn-Sampler (NUTS) within the R package
%{\tt adnuts}.

**Presumably can delete this paragraph (dealt with last year)? In a 2015 Joint Management Committee (JMC) meeting, the JTC presented results
...
biomass falls below $\Bten$, and a lower average annual variability in
...
intensive sampling, which may not be the case for the existing age-1 index. This is
why the estimates are included in a sensitivity run rather than the base model.
% Manual 2020 done.
...
they are (i.e., cohort strength is not well known until it is has been observed
...
make.decision.table(base.model,
                    xcaption = paste0("**UPDATE ORDERINGForecast quantiles of \\fishname\\
...
distribution. Figure~\ref{fig:es-forecast-depletion-comparison} shows
the projected biomass for several catch alternatives. Population dynamics and governing
...
100\% for three years of projections are provided for comparison (scenario g:
FI=100\%).
% Manual 2020 done.
%
...
\begin{figure}[tbp]
\begin{center}
<<forecast-depletion-comparison-plot, fig.height=5, fig.width=8, is.fig = TRUE>>=
...
\begin{figure}[tbp]
\begin{center}
<<forecast-risk-comparison-plot-year-1, fig.height=5, fig.width=8, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that there is a high probability that the stock will decline next year regardless of catch level">>=
...
Panel (AP) in 2012 are presented for
\Sexpr{forecast_yrs[2:(length(forecast_yrs)-1)]} and
\Sexpr{forecast_yrs[length(forecast_yrs)]} projections
...
Figure~\ref{fig:es-forecast-risk-comparison-year-1}). **[May not be needed
depending on results; if not then comment out rather than delete so we have for
future] However, interpolation is
...
(rather than rise) with increased \Sexpr{assess.yr+1} catch levels in Table~\ref{tab:es-risk-year-2}
...
\begin{figure}[tbp]
\begin{center}
<<forecast-risk-comparison-plot-year-2, fig.height=5, fig.width=8, is.fig = TRUE, alt.text = "This figure shows that there is a high probability that the stock will decline next year regardless of catch level">>=
make.forecast.risk.comparison.plot(base.model,
                                   forecast_yrs = forecast_yrs,
...
% \Sexpr{f(min(base.model$risks[[1]][,2]))}\% (see assesment-section).
...
above-average 2016 cohort will add to overall spawning biomass as it matures.
