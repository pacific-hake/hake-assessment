...
(Figure~\ref{fig:es-survey-biomass}). A new age-baed maturity ogive was
...
change in Stock Synthesis. The constraint on the annual deviation in
selectivity was loosened in the 2017 assessment because the settings used in previous
...
deviation here. We also included a new approach for automatically weighting composition
data.
