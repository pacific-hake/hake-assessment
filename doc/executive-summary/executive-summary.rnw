...
\end{center}
%\vspace{0mm}
\caption{Relative acoustic survey index of
...
(\Sexpr{survey.start.yr}--\Sexpr{survey.end.yr}), an acoustic survey index of
age-1 fish (Figure~\ref{fig:es-survey-age1}),
...
analyses, has used only Bayesian
estimation (rather than some maximum likelihood estimation).
...
assessment. **Flesh out an explanation, see Issue.
...
catch levels in Tables~\ref{tab:es-risk-year-2} and~\ref{tab:es-risk-year-3}
