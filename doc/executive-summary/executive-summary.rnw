...
again investigate sensitivity to an alternative data-weighting
approach. Time-varying fecundity, which was introduced in 2019, is retained.
...
Since 2010, the model currently estimates small 2011, 2012, 2013, 2015, 2017, 2018,
and 2019 year classes (median recruitment well below the mean of all median recruitments).
% Done for for 2022 assessment
...
The 2014 and 2016 year classes are likely both larger than average, with a
very high chance (\Sexpr{prob.percent.2014.rec.gt.2016.rec}\%) that 2014 is
larger than 2016, but zero chance that it is larger than 2010. % prob.percent.2014.rec.gt.2010.rec
...
indicate that the \Sexpr{end.yr-2} year class is likely above average, while the 2019 year class is
...
target (of 1.0) in 2017 (Figure~\ref{fig:es-fishing-intensity}).
...
an agreement on coast-wide 2020 and 2021 TACs. The 2021 U.S. catch target was
...
intensity has never been above the target of 1.0
%when the female spawning biomass
%is below the
%reference point of $\Bforty$
(Figure~\ref{fig:es-phase}). This highlights the
...
the age-1 index (see Appendix~\ref{chap:age1-survey}).
...
address current or future recruitment expectations. Further, the interactions
...
estimated above-average 2020 (yet still highly uncertain) cohort will play a large role in determining spawning stock biomass
