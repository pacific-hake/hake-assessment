...
# Data Figures {#sec:app-figures}

This appendix contains empirical data figures only. There are no model output figures included. The `r end_yr` assessment is a simple projection model which includes only the total `r last_data_yr` catch as new data. The figures presented here contain historical data as well as the data for `r last_data_yr`, even though only catch was updated in the modelling for this year. All the `r last_data_yr` data presented here will be used in future assessment years' model runs.
...
```{r main-can-depths-fig, fig.cap = "(ref:main-can-depths-cap)", fig.height = 5, out.height = "5in"}
...
(ref:main-age-proportions-cap) Recent age-proportion data for U.S. and Canadian fleets. Proportions are calculated from numbers of individuals in each age group. Age 15$+$ is an accumulator group.

(ref:main-age-proportions-alt) The U.S. fleets observe higher age-proportions of younger fish compared to the Canadian fleets.

```{r main-age-proportions-fig, fig.cap = "(ref:main-age-proportions-cap)", fig.width = 6, fig.height = 6, out.height = "6in"}
