...
#' values to plot
#' @param bar_alpha The transparency of the bars
#'
...
plot_fore_age_comps <- function(model,
  ...
  natsel_prop <- model$extra.mcmc$natsel.prop
  natselwt_prop <- model$extra.mcmc$natselwt.prop

  reformat <- function(df){
    df |>
      apply(2, quantile, probs = c(0.025, 0.5, 0.975)) |>
      as_tibble(rownames = "quant") |>
      mutate(quant = ifelse(quant == "2.5%",
                            "lower",
                            ifelse(quant == "97.5%",
                                   "upper",
                                   "med"))) |>
