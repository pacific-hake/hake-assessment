...
#' @param my_ct_levels The table of catch levels with same structure as
...
fetch_forecasts <- function(forecasts_path = NULL,
  ...
  if(is.null(my_ct_levels[1])){
    stop("`my_ct_levels` cannot be `NULL`",
         call. = FALSE)
  }
  ...
  ct_levels_catch <- map(my_ct_levels, ~{.x[[1]]})
  ct_levels_desc <- map_chr(my_ct_levels, ~{.x[[2]]})
  ct_levels_names <- map_chr(my_ct_levels, ~{.x[[3]]})
  ...
  lst <- map(forecast_yrs, function(fore_yr){
    ...
    if(!identical(ct_levels_names, dir_listing)){
      stop("There is a discrepancy between what you have set ",
           "for the ct_levels_names \n and what appears in the forecasts ",
           "directory '", fore_path,"'. \n Check the names in both and try ",
           "again.\n\n",
           call. = FALSE)
    }

    lvls_lst <- imap(my_ct_levels, function(catch_level, catch_level_ind){
