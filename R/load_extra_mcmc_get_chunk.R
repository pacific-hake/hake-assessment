...
load_extra_mcmc_get_chunk <- function(lst,
  ...
  if(!length(headmark_ind)){
    stop("Could not find a line in the input matching the regular ",
         "expression `", beg_pat, "`",
         call. = FALSE)
  }
  ...
  if(header_ind == length(x)){
    stop("The beginning marker `beg_pat` = `", beg_pat, "` had only blank ",
  ...
  if(tail_ind == length(x)){
    stop("The ending marker `end_pat` = `", end_pat, "` had only blank ",
         "lines after it, all the way to the end of the input. Check your ",
         "regular expression for errors",
         call. = FALSE)
  }
...
load_extra_mcmc_get_chunk <- function(lst,
  ...
  if(tailmark_ind < headmark_ind){
    stop("The ending regular expression matched a line before the beginning ",
