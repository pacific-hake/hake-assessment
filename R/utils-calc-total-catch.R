...
table_catch_tac_summary <- function(yr = NULL,
  ...
  lst <- list(
    c("Total", "", "Total", ""),
    c("Catch (t)", f(pull(ct_df, `U.S. Total`)), "Catch (t)", f(pull(ct_df, `Canada Total`))),
    c("TAC", f(pull(ct_df, `U.S. TAC`)), "TAC", f(pull(ct_df, `Canada TAC`))),
    c("Attainment", us_attain, "Attainment", can_attain),
    c("Shore-based", "", "Shoreside", ""),
    c("Catch (t)", f(pull(ct_df, `U.S. Shore-based`)), "Catch (t)", f(pull(ct_df, `Canada Shoreside`))),
    c("TAC", f(us_ss_tac) , "\\% of Total Canadian Catch", can_ss_perc_catch),
    c("Attainment", us_ss_attain, "--", "--"),
    c("Mothership", "", "Freezer-trawler", ""),
    c("Catch (t)", f(pull(ct_df, `U.S. Mothership`)), "Catch (t)", f(pull(ct_df, `Canada Freezer-trawler`))),
    c("TAC", f(us_ms_tac), "\\% of Total Canadian Catch", can_ft_perc_catch),
