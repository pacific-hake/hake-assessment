# Projections

## Projection methods

* Single projection year to the start of `r end_yr + 1`, with alternative
  catch levels for `r end_yr` explored.

* A single projection was done because it was deemed pragmatic to not
  overextend the forecasts too far ahead without using all the most recent data.

* The projections estimates the biomass at start of `r end_yr + 1`
  - Mean of fishery selectivity from `r end_yr - 6`--`r end_yr - 2` used in
    projections
  - Mean of weight-at-age and maturity-at-age (and thus fecundity) from
    `r end_yr - 6`--`r end_yr - 2` used in projections
  - Recruitment from stock--recruitment relationship

## Default harvest policy for `r assess_yr`
:::::: {.columns}
::: {.column width="38%"}
* Using the defined *F~SPR=40%~* harvest rate with a 40:10 adjustment, the
  median projected `r end_yr` TAC is \textbf{`r ct_limit_quantiles["median"]` t}
\item[]
* 2.5% and 97.5% quantiles: `r ct_limit_quantiles["lower"]` and
  `r ct_limit_quantiles["upper"]` t
:::

::: {.column width="62%"}
```{r main-projected-catch-density, out.height = "80%"}

plot_param_density(base_model,
                   param_nm = paste0("ForeCatch_", assess_yr),
                   y_lim = c(0, 0.0017),
                   expand_vec = c(100, 100),
                   show_y_labels = FALSE,
                   x_lab = paste0("Projected ", assess_yr, " catch (kt) ",
                                  "based on the default harvesy policy"))
```
:::
::::::

## Projection outcome
:::::: {.columns}
::: {.column width="38%"}
- All catch options result in decline of the stock to the beginning of `r end_yr + 1`
\item[] <!-- Blank line between bullets -->
- Fixing recruitment deviations from `r end_yr - 3` onward underrepresents
  uncertainty
\item[] <!-- Blank line between bullets -->
- There is no chance for large recruitments from `r end_yr - 3` onwards,
  which will result in an underestimate of potential future recruitment
:::

::: {.column width="62%"}
```{r main-forecast-depletion-comparison-plot, out.height = "80%"}

# See the file `forecast-catch-levels.R` at the list called `ct_levels`. The values in
# `fore_inds` below are the indices of those forecast values in that list.
plot_depl_fore_comparison(base_model,
                          fore_inds = c(1, 2, 6, 7, 10),
                          forecast_yrs = forecast_yrs[1:2],
                          is_catch_proj = TRUE,
                          leg_pos = c(0.15, 0.85))
```
:::
::::::

## Projected age proportions for `r end_yr` fishery catch

\*\* **\text{\small 2025 age data is not included in the model projection}**

:::::: {.columns}
::: {.column width="45%"}
**Median proportions (2026):**
```{r fore-age-props, results = "asis"}

table_fore_age_props(fore_catch_prop,
                     fore_catch_prop_wt,
                     ages_filt = 1:15,
                     font_size = 8) |>
    # Highlight two largest cohorts
    row_spec(10:11,
           bold = table_highlight_bold,
           background = table_highlight_color)
```
:::

::: {.column width="55%"}
```{r main-age-comp-forecast, fig.height = 3.5, fig.width = 5.5, out.height = "80%"}
plot_fore_age_comps(base_model,
                    y_lim = c(0, 0.7),
                    is_catch_proj = TRUE)
```
:::
::::::
