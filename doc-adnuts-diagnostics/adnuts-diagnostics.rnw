...
fig_align <- "center"
fig_pos <- "htb"
library(knitr)   # Otherwise next line gives hidden error for the first run
...
<<setup, echo=FALSE>>=
# this.sens.model.ind = 0       # start with base model
# this.sens.model = base.model

# diagnostic.model.names <- c("Base model")
#                            sens.model.names.1)  # then add .2 etc.
...
# This duplicates the ..one-model.rnw code with {{this.sens.model}} replaced
#  with each element of diagnostic.model in turn:
