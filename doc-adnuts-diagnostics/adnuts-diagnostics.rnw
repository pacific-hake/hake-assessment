...
%%  knit_expand doesn't create a giant knitr file. Set cache=FALSE if in trouble.
...
\title{Diagnostics plots of ADNUTS results for sensitivity analyses for the \Sexpr{assess_yr} \fishname\ (whiting) stock assessment
%% Uncomment for DRAFT:
...
\lfoot{ADNUTS diagnostics -- hake \Sexpr{assess_yr}}
...
of the \Sexpr{length(key_posteriors_titles)} key posterior parameters (if they
...
if(!(length(sens_models_1) == 6 &
     length(sens_models_2) == 2 &
     # length(sens_models_3) == ?? &
     length(sens_models_4) == 3 &
     length(sens_models_5) > 100 & # it currently just has one model
     length(sens_models_6) == 3 &
     length(sens_models_7) > 100 # it currently just has one model
     )
...
diagnostic.model <- c("base_model",
                      ...
                      "sens_models_4[[2]]",
                      "sens_models_4[[3]]",
                      "sens_models_5",     # only one element
                      "sens_models_6[[1]]",
                      "sens_models_6[[2]]",
                      "sens_models_6[[3]]",
                      "sens_models_7"       # only one element
                      )
