%% retro-diagnostics.Rnw. Diagnostics for the retrospective models.
%% Pacific Hake Joint technical committee.
%% knitr::knit("retro-diagnostics.rnw")

%% Note that ../appendix-mcmc-diagnostics/adnuts-diagnostics-one-model.rnw gets
%%  repeatedly called from here,
...
%% Set cache=FALSE if in trouble, as often the models might have changed (and
%%  just deleting the knitr-cache-retro/ folder doesn't always seem to fix it,
%%  maybe because of the chunk numbering).
...
opts_chunk$set(
             ...
             comment = "#>",
             results = 'hide',
             fig.path = 'knitr-cache-retro/',
             cache.path = 'knitr-cache-retro/',
             cache = TRUE)
...
\title{Diagnostics plots of ADNUTS results for retrospective models for the \Sexpr{assess_yr} \fishname\ (whiting) stock assessment
%% Uncomment for DRAFT:
...
\lfoot{Retro diagnostics -- hake \Sexpr{assess_yr}}

\rfoot{Introduction}
\chapter{Introduction}
\label{chap:introduction}

This document automatically generates the MCMC diagnostic plots for the retrospective
model runs that are defined in {\tt model-setup.R}; estimates from some of these
...
of the \Sexpr{length(key_posteriors.titles)} key posterior parameters (if they
exist for that run), plus histograms of diagnostics for all estimated
parameters, and pairs plots for key parameters and further standard ones of
interest. See captions for details.

Captions are identical from chapter to chapter, and the current base model is
not duplicated here (see adnuts-diagnostics.rnw or the assesessment Appendix).
**[Not tweaked for bridging runs:]
**Note that for the sensitivity
...
if(!(length(retro.list) == 6 # &
     #length(bridge_models_2) == 2
     )
   ){ stop("Need to update diagnostic.model in retro-diagnostics.rnw")
  }
...
diagnostic.model <- c("retro.list[[1]]",
                      "retro.list[[2]]",
                      "retro.list[[3]]",
                      "retro.list[[4]]",
                      "retro.list[[5]]",
...
# diagnostic.model <- rep(diagnostic.model[1],
#                         2)# for debugging
@
