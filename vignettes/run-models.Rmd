...
**A note about parallelism in the scripts**
...
<!-- The general method in the order in which it typically should take place -->
<!-- is as follows: -->
...
steps concurrently (for example 3 and 4 can be done at the same time):

1. `run-base-model.sh`
1. Wait for `run-base-model.sh` to finish
1. `create-rds-base.sh`
1. `run-forecasts.sh`
1. Wait for `run-forecasts.sh` to finish
...
1. `run-retrospectives.sh`
   ...
   - Wait for `run-forecasts.sh 8 9 10` to almost finish
