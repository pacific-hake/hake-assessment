...
1. You must link the directory on your computer from the download with the
   ...
   this example was `c:\work\hake\srv`. If you commit this change
   ...
   as long as you always download the `srv` folder from the Google drive to
...
1. Once the Docker Bash terminal starts, it will update the hake repository
   ...
   - Rstudio will open in your browser. You will start in the `doc` directory
     (this is the directory in which the main document is built) and the
     files `_bookdown.yml` and `000-launcher.rmd` will be open.
   - To build the document in full, type `render()`. For more information
...
1. Once the Docker container's bash prompt appears, type `/init`

1. Go to a Web browser, open a new tab, and type `loclhost:8787` in the
   URL bar (you can bookmark this).

1. Login to Rstudio with user `rstudio` and password `a`
