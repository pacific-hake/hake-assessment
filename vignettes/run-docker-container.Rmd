...
1. If the directory where your `models` are located is not `/srv`, edit the
   `create-volume.sh` file and replace `device=/srv` with
   `device=/path/to/models/dir` where `/path/to/models/dir` is the path to
   the models directory on your machine. Save the file and close.
...
1. Once the Docker Bash terminal starts, you can connect to the Rstudio server:
   ...
   - In the Bash terminal, type: `/init`. This will run some code and leave
   ...
   - Rstudio will open in your browser. You will start in the hake directory
     (this is the hake-assessment root directory).
   - To build the document, see the README.md for the hake project [GitHub
     repository](https://github.com/pacific-hake/hake-assessment).
   - To exit, click the button on the top right of the RStudio server page
     ...
     the right off a white square.
   - Go back to the Docker Bash terminal and press Ctrl-c to return to the
     Bash prompt. 
