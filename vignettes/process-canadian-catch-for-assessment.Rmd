...
just skip to #4. Check the timestamps of the files to verify.** \
  ...
  1. Open R and load the hake package: \
   ...
   1. Run the following to create (overwrite) the Canadian catch-by-year file
      ...
      ```{r echo = TRUE}
      fleet_catch <- canada_load_catch_data(dr = "/srv/hake/other/landings") |>
   ...
   1. Manually edit this file: \
      ```{r, echo = FALSE}
      here(data_tables_path, can_landings_tac_fn)
      ```
      by copying the Canadian values from this file:
      ```{r, echo = FALSE}
      here(data_tables_path, can_catch_by_year_fn)
      ```

   1. Manually add the new year's Canadian carryover value and JV TAC (if any)
      to this file:
      ```{r, echo = FALSE}
