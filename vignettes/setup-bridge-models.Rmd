...
fixed_weight_at_age <- r4ss::SS_readwtatage(
  fs::path(here::here(), data_tables_path, "length-weight-age", "wtatage_fix.ss"),
  verbose = FALSE
)
inputs[["wtatage"]] <- fixed_weight_at_age
r4ss::SS_write(
  inputs,
  dir = fs::path(bridge_dir, "02-fix-weight-at-age"),
  overwrite = TRUE
...
inputs[["dat"]][["agecomp"]] <- dplyr::bind_rows(
  ...
  fishery_age_composition |>
    dplyr::filter(Yr < current_yr - 1, Yr >= 2008)
) |>
  dplyr::arrange(-FltSvy, Yr)
...
new_weight_at_age <- r4ss::SS_readwtatage(
  fs::path(here::here(), data_tables_path, "length-weight-age", "wtatage.ss"),
  verbose = FALSE
)
inputs[["wtatage"]] <- new_weight_at_age
r4ss::SS_write(
  inputs,
  dir = fs::path(bridge_dir, "20-add-weight-at-age"),
  overwrite = TRUE
...
```{r survey-1-t-distribution, eval = FALSE}
r4ss::copy_SS_inputs(
  dir.old = fs::path(bridge_dir, "50-add-fishery-ages"),
  dir.new = fs::path(bridge_dir, "60-survey-1-t-distribution"),
  overwrite = TRUE,
  use_ss_new = FALSE,
  verbose = FALSE,
  recursive = TRUE
...
inputs <- r4ss::SS_read(
  dir = fs::path(bridge_dir, "60-survey-1-t-distribution"),
  ss_new = FALSE,
  verbose = FALSE
)

# DOF of the student-t distribution is = n for the index - 1
inputs[["dat"]][["CPUEinfo"]][
  inputs[["dat"]][["CPUEinfo"]][["Fleet"]] == 3,
  "Errtype"
] <- inputs[["dat"]][["CPUE"]] |> dplyr::filter(index == 3) |> NROW() - 1
r4ss::SS_write(
  inputs,
  dir = fs::path(bridge_dir, "60-survey-1-t-distribution"),
  overwrite = TRUE
...
```{r tv-weight-at-age, eval = FALSE}
r4ss::copy_SS_inputs(
  dir.old = fs::path(bridge_dir, "60-survey-1-t-distribution"),
  dir.new = fs::path(bridge_dir, "70-tv-weight-at-age"),
  overwrite = TRUE,
  use_ss_new = FALSE,
  verbose = FALSE,
  recursive = TRUE
...
# Read in the three csv weight-at-age files
# TODO: make object names for us and survey csv files
# model
m1 <- sdmTMB::sdmTMB(
...
pred_grid <- expand.grid(
  year = unique(weight_at_age_df$year),
  sex = unique(weight_at_age_df$sex),
  age = 0:15
) |>
  dplyr::mutate(cohort = year - age) |>
  dplyr::filter(cohort %in% weight_at_age_df[["cohort"]]) |>
  dplyr::mutate(
    fyear = as.factor(year),
    fcohort = as.factor(cohort)
  )
...
saveRDS(
  preds,
  file = fs::path("/srv/hake/other/tv", "weight-at-age-preds.rds")
)
...
utils::write.csv(
  ...
  fs::path(
    data_tables_path,
    "length-weight-age",
    "EWAA_fishery_survey_nospatial_1975-2023_long.csv"
  ),
  row.names = FALSE
)

ewaa_complete <- pad_weight_at_age(ewaa)
...
x <- fs::path(bridge_dir, "70-tv-weight-at-age")
system(glue::glue("cd {x} && clean_ss3"))
```

```{r redo-fishery-comp, eval = FALSE}
r4ss::copy_SS_inputs(
  dir.old = fs::path(bridge_dir, "70-tv-weight-at-age"),
  dir.new = fs::path(bridge_dir, "71-redo-fishery-comp"),
  overwrite = TRUE,
  use_ss_new = FALSE,
  verbose = FALSE,
  recursive = TRUE
...
inputs <- r4ss::SS_read(
  dir = fs::path(bridge_dir, "71-redo-fishery-comp"),
  ss_new = FALSE,
  verbose = FALSE
)
...
r4ss::SS_write(
  inputs,
  dir = fs::path(bridge_dir, "72-tv-fecundity"),
  overwrite = TRUE,
  verbose = FALSE
)
# The following code is for sensitivity analyses not bridging
test <- update_ss3_maturity(
  maturity = data.frame(
    age = seq_along(hakedataUSA::maturity_at_age) - 1,
    year = 2023,
...
test <- update_ss3_maturity(
  maturity = maturity_estimates_df |>
    dplyr::filter(model == "Null", !is.na(p_mature)),
  weight_at_age = inputs[["wtatage"]]
)
test <- update_ss3_maturity(
  maturity = maturity_estimates_df |>
    dplyr::filter(model == "Spatial + AR(1) year effects", !is.na(p_mature)),
  weight_at_age = inputs[["wtatage"]]
)
```
...
```{r base, eval = FALSE}
r4ss::copy_SS_inputs(
  dir.old = fs::path(bridge_dir, "72-tv-fecundity"),
...
i_runs <- list(1:7, 7:15, c(7, 8, 14, 15))
