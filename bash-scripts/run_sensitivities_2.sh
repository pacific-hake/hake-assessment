...
run_adnuts('$MODELS_DIR/2022.01.22_M_hamel_prior', adapt_delta = 0.95, extra_mcmc = FALSE)" \
> /dev/null 2>&1; echo "2022.01.22_M_hamel_prior MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.23_age1Survey', adapt_delta = 0.95, extra_mcmc = FALSE)" \
> /dev/null 2>&1; echo "2022.01.23_age1Survey MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.24_compWeight_HarmonicMean', adapt_delta = 0.95, extra_mcmc = FALSE)" \
> /dev/null 2>&1; echo "2022.01.24_compWeight_HarmonicMean MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.27_tvSelect_phi_extralow', adapt_delta = 0.95, extra_mcmc = FALSE)" \
> /dev/null 2>&1; echo "2022.01.27_tvSelect_phi_extralow MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.28_tvSelect_phi_low', adapt_delta = 0.95, extra_mcmc = FALSE)" \
> /dev/null 2>&1; echo "2022.01.28_tvSelect_phi_low MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.29_tvSelect_phi_high', adapt_delta = 0.95, extra_mcmc = FALSE)" \
