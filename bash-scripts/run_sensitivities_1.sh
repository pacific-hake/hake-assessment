...
run_adnuts('$MODELS_DIR/2022.01.15_h_prior_mean_low', adapt_delta = 0.95, n_cores = 16)" \
> /dev/null 2>&1; echo "2022.01.15_h_prior_mean_low MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.16_h_fix_high', adapt_delta = 0.95, n_cores = 16)" \
> /dev/null 2>&1; echo "2022.01.16_h_fix_high MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.17_sigmR_fix_low', adapt_delta = 0.95, n_cores = 16)" \
> /dev/null 2>&1; echo "2022.01.17_sigmR_fix_low MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.18_sigmR_fix_high', adapt_delta = 0.95, n_cores = 16)" \
> /dev/null 2>&1; echo "2022.01.18_sigmR_fix_high MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.20_M_0.2SD', adapt_delta = 0.95, n_cores = 16)" \
> /dev/null 2>&1; echo "2022.01.20_M_0.2SD MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.21_M_0.3SD', adapt_delta = 0.95, n_cores = 16)" \
