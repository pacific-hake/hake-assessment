...
run_adnuts('$MODELS_DIR/2022.01.43_maxSel_Age5', adapt_delta = 0.95)" \
> /dev/null 2>&1; echo "2022.01.43_maxSel_Age5 MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.44_maxSel_Age7', adapt_delta = 0.95)" \
> /dev/null 2>&1; echo "2022.01.44_maxSel_Age7 MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.45_maxSel_Age8', adapt_delta = 0.95)" \
> /dev/null 2>&1; echo "2022.01.45_maxSel_Age8 MCMC complete") &

(trap 'kill 0' SIGINT; Rscript -e "setwd(here::here()); source('R/all.R'); \
run_adnuts('$MODELS_DIR/2022.01.100_zerosumcontraint', adapt_delta = 0.95)" \
