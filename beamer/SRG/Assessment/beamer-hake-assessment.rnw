...
\frametitle{Changes from \Sexpr{last.assess.yr} assessment}
  \bi
    \item Update Stock Synthesis to version \Sexpr{ss.version}
      compiled with ADMB \Sexpr{admb.version}
...
\frame{\frametitle{Bridging: Summary}
  \bi
    ...
    \item Updating fishery catch and age composition data to \Sexpr{last.assess.yr} has little effect
      on the stock trajectory.
    \item Adding the \Sexpr{last.data.yr} survey biomass estimate and age compositions leads to
      slightly more optimistic estimate of biomass for the last few years and reduces uncertainty.
    \item Small changes in recruitment with the addition of \Sexpr{last.assess.yr} fishery age composition
      data were largely offset with the addition of \Sexpr{last.assess.yr} survey age composition data.
...
\frame{\frametitle{Catchability (\emph{$q_b$}) posterior and values for recent assessments}
 \bc
...
% \bc
%  <<histQ.age1, , fig.height=4, fig.width=8, out.width='0.95\\columnwidth'>>=
...
\frame{\frametitle{\Sexpr{last.assess.yr} SRG assessment recommendations\\
  ...
  \bi
    \item Exploring approaches to make informed decisions about current and
      future recruitment, instead of default long-term averages or \emph{adhoc}
...
\frame{\frametitle{\Sexpr{last.assess.yr} SRG assessment recommendations\\
  ...
  \bi
    ...
    \item To work around this, the JTC calculated the median of estimates
      ...
      recruitment forecasts in terms of virgin recruitment. The standard suite of
      catch projections were then conducted using mean recruitment from the main time
      period and scaled virgin recruitment based on the median estimates of recruitment
      relative to $R_0$. Brief example of results will be shown.
  \ei
}
