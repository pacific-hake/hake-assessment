...
\frametitle{Changes from \Sexpr{last.assess.yr} assessment}
  \bi
    ...
    \item Change from zero-sum recruitment deviations to
...
\frame{\frametitle{Bridging to the base model: update SS / simple Rec. Devs.}
  \bc
  \vspace{-5pt}
...
%----------------------------------------------------------
\frame{\frametitle{Bridging to the base model: update Pre-\Sexpr{last.data.yr}
...
%----------------------------------------------------------
\frame{\frametitle{Bridging to the base model: add \Sexpr{last.data.yr} data / DM Priors}
  \bc
  \vspace{-5pt}
...
\frame{\frametitle{Bridging to the base model: Summary}
  \bi
    ...
    \item Adding fishery age compositions and weight-at-age data increases recruitment
      estimates of the 2016 and 2017 cohorts and decreases the 2018 cohort estimate.
    \item Adding age compositions has an effect on the recent recruitment deviations.
    ...
    \item The 2010 and 2014 cohorts were estimated higher than in the 2019 assessment. 
   \ei
}
...
\bi
  \item Used in the assessment since 2018. Prior to this, an iteritave weighting scheme was used
    (McAllister-Ianelli).
  ...
  \item A Normal (almost Uniform) prior is used in  \Sexpr{assess.yr} to improve MCMC convergence of these parameters.
    In the \Sexpr{last.assess.yr} assessment, they were stuck between 0.99--1.00 (at the upper bound).
\ei
\end{frame}
  ...
  \begin{table}
    \begin{tabular}{cccc} \hline
      MLE (\Sexpr{assess.yr}) & Median (\Sexpr{assess.yr}) &
        Median (\Sexpr{assess.yr - 1}) & Median (\Sexpr{assess.yr - 2}) \\
      \hline
...
b.curr.prop.2016 = baa.curr[assess.yr - 2016 + 1]/baa.curr.tot
b.curr.prop.2014 = baa.curr[assess.yr - 2014 + 1]/baa.curr.tot
b.curr.prop.2010 = baa.curr[assess.yr - 2010 + 1]/baa.curr.tot

tmp.last.2014 <- quantile(last.yr.base.model$mcmc$Recr_2014, probs = c(0.025, 0.5, 0.975) / 1000)
tmp.curr.2014 <- quantile(base.model$mcmc$Recr_2014, probs = c(0.025, 0.5, 0.975) / 1000)
last.yr.recr.2014 <- f(c(tmp.last.2014, tmp.last.2014[3] - tmp.last.2014[1]), 0)
recr.2014 <- f(c(tmp.curr.2014, tmp.curr.2014[3] - tmp.curr.2014[1]), 0)
recr.ci.diff.prop.2014 <- f((tmp.curr.2014[3] - tmp.curr.2014[1]) / (tmp.last.2014[3] - tmp.last.2014[1]) * 100, 1)
tmp.last.2016 <- quantile(last.yr.base.model$mcmc$Recr_2016, probs = c(0.025, 0.5, 0.975) / 1000)
tmp.curr.2016 <- quantile(base.model$mcmc$Recr_2016, probs = c(0.025, 0.5, 0.975) / 1000)
last.yr.recr.2016 <- f(c(tmp.last.2016, tmp.last.2016[3] - tmp.last.2016[1]), 0)
recr.2016 <- f(c(tmp.curr.2016, tmp.curr.2016[3] - tmp.curr.2016[1]), 0)
recr.ci.diff.prop.2016 <- f((tmp.curr.2016[3] - tmp.curr.2016[1]) / (tmp.last.2016[3] - tmp.last.2016[1]) * 100, 1)
@
\bi
...
Addition of new data for this year's model increases 2014 cohort uncertainty to \Sexpr{recr.ci.diff.prop.2014}\%
and 2016 cohort uncertainty to \Sexpr{recr.ci.diff.prop.2016}\% of what it was in the \Sexpr{assess.yr - 1} assessment:

  \begin{table}
    \begin{tabular}{ccrcc} \hline
      Calculation (millions) & Low (2.5\%) & Median & High (97.5\%) & Interval size\\
      \hline
      \Sexpr{assess.yr-1} assessment recruitment in 2014 & \Sexpr{last.yr.recr.2014[1]} & \Sexpr{last.yr.recr.2014[2]} & \Sexpr{last.yr.recr.2014[3]} & \Sexpr{last.yr.recr.2014[4]}\\
      \Sexpr{assess.yr} assessment recruitment in 2014 & \Sexpr{recr.2014[1]} & \Sexpr{recr.2014[2]} & \Sexpr{recr.2014[3]} & \Sexpr{recr.2014[4]}\\
      \hline
      \Sexpr{assess.yr-1} assessment recruitment in 2016 & \Sexpr{last.yr.recr.2016[1]} & \Sexpr{last.yr.recr.2016[2]} & \Sexpr{last.yr.recr.2016[3]} & \Sexpr{last.yr.recr.2016[4]}\\
      \Sexpr{assess.yr} assessment recruitment in 2016 & \Sexpr{recr.2016[1]} & \Sexpr{recr.2016[2]} & \Sexpr{recr.2016[3]} & \Sexpr{recr.2016[4]}\\
      \hline
    \end{tabular}
  \end{table}
  ~\\
...
\bi
\item 2010 cohort is estimated to make up \Sexpr{f(b.curr.prop.2010 * 100)}\%
  of the biomass
\item 2014 cohort is estimated to make up \Sexpr{f(b.curr.prop.2014 * 100)}\%
  of the biomass
\item 2016 cohort is estimated to make up \Sexpr{f(b.curr.prop.2016 * 100)}\%
  of the biomass
\ei
}
...
%       \item Estimated slope parameters were slightly sensitive to excluding data from 1975--1980.
%       \ei
...
%   \bi
%   \item Forecast weights-at-age were chosen to be the mean of the 1975--2018 weights-at-age, as in
%     the \Sexpr{last.assess.yr} base model.
%     \bi
...
%     \item The configuration of Stock Synthesis version \Sexpr{ss.version} precludes properly testing other
...
%       \ei
%       \item In sensitivity models, the JTC were unable to set future weight-at-age values to be the mean of
...
%   \item Issues remain regarding the availability of U.S. tribal catch by JTC data deadlines,
%     including the tracking of best available estimates and the incorporation of those estimates into
%     standard databases (PacFIN).
%   \ei
% }
% 
% %----------------------------------------------------------
...
%     (Baird et al., 2008; Ali et al., 2016).
...
%   \item The 2016 recruitment is estimated to be large, but is highly uncertain.
