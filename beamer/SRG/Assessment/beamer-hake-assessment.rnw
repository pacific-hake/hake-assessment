...
\frametitle{Changes from \Sexpr{last.assess.yr} assessment}
  \bi
    ...
    \item Change the phase of the data weighting parameters
...
\frame{\frametitle{Diagnostics: Summary}
  \bi
    ...
    \item Additional diagnostic evaluations where performed using the ShinySTAN application, an
...
\frame{\frametitle{\Sexpr{last.assess.yr} SRG assessment recommendations\\
  ...
  \bi
    ...
    \pause
    \item Explore how early life history survival and factors affecting recruitment can be linked
...
\frame{\frametitle{\Sexpr{last.assess.yr} SRG assessment recommendations\\
  ...
  \bi
    ...
    \item Next steps include extracting historical age compositions and rectifying differences,
      then conduct simulations to determine how to best assign sample sizes to disparate sampling
...
\frame{\frametitle{Summary -- Key Points}
  ...
  \item Very low catch of age-2 and age-1 fish in 2020 fishery
  \item Large 2010 and 2014 cohort biomass: mortality outpacing growth (28\% and 33\%
    decline, respectively, from beginning of 2020)
  \item Rate of decline in spawning stock biomass has increased over last 3 years
  \ei
}
