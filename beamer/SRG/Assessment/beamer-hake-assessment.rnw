...
\bi
  \item Concern about broad-reaching impact of increasing the flexibility of
    ...
    \bi
      \item It was changed from $\phi=0.03$ to $\phi=0.20$, to reduce an
        anomalously high estimate of 2014 recruitment based solely on fishery
        catch-at-age data.
      \item Recommend continued work on evaluating impact, and more objective way
        of determining appropriate value of $\phi$ for future assessments.
      \item {\bf Response:} {\em ***}
    \ei
