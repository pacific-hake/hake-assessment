...
\bi
  ...
  \item **Show some MCMC diagnostics (uncomment what's already below)
  \item Fits to data
...
\frametitle{Changes from \Sexpr{last.assess.yr} assessment}
  \bi
    ...
    \item Modified parameterization for time-varying selectivity
...
\bi
  ...
  \item Therefore requires a {\em modified parameterization for time-varying 
      selectivity}.
\ei
\end{frame}
...
\bi
  ...
  \item In 2017 we used $\phi=0.20$, an increase from previous $\phi=0.03$; 
    ...
    \bi
      \item The large proportion of 2014 cohort in 2017 age data (age-2 fish)
...
\bi
  ...
  \bi
    ...
    \item Removes need for manual iterative reweighting (and subjective choice of
