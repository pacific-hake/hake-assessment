...
\frametitle{Changes from \Sexpr{last.assess.yr} assessment}
  \bi
    ...
    \item Updated pre-\Sexpr{last.assess.yr} data:
      \bi
      \item Fishery catch
    ...
    \ei
    \item Incorporates time-varying fecundity
...
\frame{\frametitle{Bridging to the base model: Summary}
  \bi
    ...
    \item Adding time-varying fecundity has the most effect, with a large increase
      in late 1970s biomass due to the inflated weight-at-age of older fish in that time period.
   \ei
}
...
\frame{\frametitle{MCMC -- MLE Biomass trajectory comparison}
<<biomass.mcmc.mle, fig.height=4, fig.width=8, out.width='0.95\\columnwidth'>>=
...
\frame{\frametitle{MCMC -- MLE recruitment comparison}
<<recr.mcmc.mle, fig.height=4, fig.width=8, out.width='0.95\\columnwidth'>>=
make.mcmc.vs.mle.plot(base.model,
                      end.yr = end.yr,
                      type = "o",
                      subplot = 8)
