...
\frametitle{Changes from \Sexpr{last.assess.yr} assessment}
  \bi
    ...
    \item Incorporates time-varying fecundity, similar to alternative base case from
      ...
      Fecundity\subscr{year} = Weight-at-age\subscr{year} * Maturity vector
  \ei
\end{frame}
...
\frame{\frametitle{Bridging to the base model: Summary}
  \bi
    \item Updating pre-\Sexpr{last.assess.yr} has almost no effect on model outcome.
...
last.yr.recr.2014 <- f(quantile(last.yr.base.model$mcmc$Recr_2014, probs = c(0.025, 0.5, 0.975) / 1000))
recr.2014 <- f(quantile(base.model$mcmc$Recr_2014, probs = c(0.025, 0.5, 0.975) / 1000))
@
...
Extra data halves 2014 cohort uncertainty from that in the \Sexpr{assess.yr - 1} assessment:

  \begin{table}
    \begin{tabular}{ccrc} \hline
      Calculation (millions) & Low (2.5\%) & Median & High (97.5\%) \\
      \hline
      \Sexpr{assess.yr-1} assessment recruitment in 2014 &
         \Sexpr{last.yr.recr.2014[1]} & \Sexpr{last.yr.recr.2014[2]} & \Sexpr{last.yr.recr.2014[3]} \\
      \Sexpr{assess.yr} assessment recruitment in 2014 &
         \Sexpr{recr.2014[1]} & \Sexpr{recr.2014[2]} & \Sexpr{recr.2014[3]} \\
      \hline
    \end{tabular}
  \end{table}
  ~\\
...
% \setbeamercovered{dynamic}   % need for \pause
  ...
  \item Spawning biomass is calculated as the product of
    ...
    \item SRG requested a sensitivity model to explore this incoporating:
      \bi
...
\frame{\frametitle{\Sexpr{last.assess.yr} SRG assessment recommendations\\
    {\bf Response to Weights-at-age and fecundity issue}}
  \bi
  \item Included time-varying fecundity
