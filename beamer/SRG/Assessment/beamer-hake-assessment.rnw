...
\frametitle{Changes from \Sexpr{last.assess.yr} assessment}
  \bi
    ...
    \bi
      \item Necessitates modified parameterisation for time-varying selectivity
        (same as  \Sexpr{last.assess.yr})
      \item Time-varying parameters cannot have a logistic
  ...
  \item Requires a {\em modified} parameterisation for time-varying
      selectivity.
    \ei
