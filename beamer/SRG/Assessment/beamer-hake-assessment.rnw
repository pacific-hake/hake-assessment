...
**Explain updated maturity ogive - first check if in data section
...
\frame{\frametitle{Bridging to the base model: fishery and survey data in
    different order}
  \bc
  \includegraphics[height=3in]{compare2_spawnbio_uncertainty_landscape.eps}
  \ec
}
...
\frame{\frametitle{Bridging to the base model: Summary}
  \bi
    ...
    \item Age data make little further difference, however (**new fig)
    ...
    \item New maturity ogive adds variability when large cohorts reach ages 3 and
...
\end{document}   % Just while updating slides in order
