...
\frame{\frametitle{Bridging to the base model: Summary}
  \bi
    ...
    \item Age data make little further difference, however adding the fishery
      age data before adding the survey does partly pull down biomass in 2016 and
      2017.
...
year's assessment, the credible interval remains within the range of last year's.
}
