...
\bi
  ...
  \item Therefore requires a {\em new parameterization for time-varying 
      selectivity}.
\ei
\end{frame}
...
\begin{frame}
\frametitle{Time-varying fishery selectivity}
...
\bi
  ...
  ~\\
  \item In 2017 $\phi=0.20$ used, an increase from previously-used $\phi=0.03$; 
    i.e.~in 2017 greater deviation (more flexibility) was allowed, because:
    \bi
      \item such a large proportion of 2014 cohort in 2017 age data (age-2 fish)
        that $\phi=0.03$ produced implausible estimates of 2014 recruitment
      ...
      \item good performance of higher $\phi$ in past MSE.
    \ei
\ei
\end{frame}
