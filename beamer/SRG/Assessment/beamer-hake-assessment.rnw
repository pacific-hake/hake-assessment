...
\bi
  ...
  \item Updating fishery catch and age composition data to \Sexpr{last_assess_yr} has little effect on the stock trajectory.
  ...
  \item Small changes in recruitment with the addition of \Sexpr{last_assess_yr} fishery age composition data were largely offset with the addition of \Sexpr{last_assess_yr} survey age composition data.
  \item Adding \Sexpr{last_data_yr} weight-at-age data resulted in a slight upward shift in current biomass, because the mean weights in \Sexpr{last_data_yr} were mostly above the recent 5-year average mean weights (which is the assumption before the addition of these data).
