...
\bi
  \item Updating Stock Synthesis software has no effect on results. Changes include:
    \bi
      \item Weight-at-age specified in first year fo forecast, rather than carried forward
      \item Report files for empirical growth models do not have output that uses growth parameters
      \item 
    \ei
...
%% BRING EACH SLIDE IN ONE BY ONE AND COMPILE
