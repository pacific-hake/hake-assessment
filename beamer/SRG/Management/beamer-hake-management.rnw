...
\frame{\frametitle{Forecast methods}
  \bi
    \item Forecasts estimate the biomass at start
...
\frame{\frametitle{Three-year forecasts}
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
...
%      \item \textcolor{gray}{m. Constant catch (\Sexpr{catch_levels[[7]][[2]]})}
      \item \textcolor{gray}{m. Fishing Intensity = 100\% in each year
      ...
      \item \textcolor{gray}{o. Fishing intensity giving 50\% prob.~that the
