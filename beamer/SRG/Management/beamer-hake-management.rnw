...
\frame{\frametitle{Update on web accessibility}
  \bi
	  \item Now using a leading-edge LaTeX package called \textbf{tagpdf}
	    to automatically tag the document for accessibility
	  \item Alternate text is part of the \textbf{tagpdf} routine and was set up to
	    inserted by code during the document building routine
	  \item This was done during and after the draft submission this year
	\ei
}
