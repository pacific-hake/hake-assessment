...
\frame{\frametitle{Forecast methods}
  \bi
    \item Forecasts are for \Sexpr{end.yr}, \Sexpr{end.yr+1}, and \Sexpr{end.yr+2} (explore \Sexpr{end.yr+3})
    \bi
...
\frame{\frametitle{Decision Tables}
  \bi
    ...
    \bi
      \item a. \Sexpr{catch.levels[[1]][[2]]} (zero catch)
      \item b. Constant catch of \Sexpr{catch.levels[[2]][[2]]}
      \item c. Constant catch of \Sexpr{catch.levels[[3]][[2]]}
      \item \textcolor{gray}{d. Constant catch (\Sexpr{catch.levels[[4]][[2]]})}
      \item e. Constant catch (\Sexpr{catch.levels[[5]][[2]]})
      ...
      \item \textcolor{gray}{h. Median catch in 2019 is the same as
        fixed catch in 2018}
    \ei
...
\begin{columns}
  \begin{column}{0.33\textwidth}
    \bi
      ...
      \item Lower 5\% quantile below $B_{40\%}$ in \Sexpr{end.yr} (31\%)
      ...
      \bi
        ...
        \bi
          \item similarly high correlation with both 2010 and 2014 recruitment
            ...
            respectively)
          \item increasing correlation with 2016 recruitment
            (\Sexpr{round(cor(base.model$mcmc[paste0("ForeCatch_",end.yr)],
              base.model$mcmc$Recr_2016),2)}, not shown)
        \ei
...
\frame{\frametitle{Summary of forecasting}
  \bi
    ...
    \item There is a 50\% chance that median spawning biomass will decline from 2019 to 2020 at a catch level between 180,000~t and 350,000~t (around 280,000~t), partly due to mortality on ageing 2010 cohort 
