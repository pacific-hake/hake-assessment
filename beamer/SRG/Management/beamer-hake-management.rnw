...
\frame{\frametitle{Methods}
  \bi
    ...
    \bi
      \item Average fishery selectivity from \Sexpr{end.yr-6}--\Sexpr{end.yr-1} 
        used in forecasts
...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      \item Catch stream 180,000~t results in stable spawning biomass
...
\frame{\frametitle{Risk table definitions}
  ...
  \bi
    \item Fixed catch in all years previous, and calculate default catch in 
      forecast year
