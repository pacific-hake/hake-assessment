...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      ...
      \smallskip
      \item 2.5\% and 97.5\% quantiles:\\ \Sexpr{catch.limit.quantiles["lower"]} and \Sexpr{catch.limit.quantiles["upper"]} t\thanks{Note: quantile values include correction relative to pre-review draft document}
    \ei
  \end{column}
...
%% %%%%%%%%%%%%%%%%%%%%%%% 
%% \frame{\frametitle{Forecast age compositions in numbers and in weight for \Sexpr{end.yr} fishery catch}
  
%%   \begin{columns}
%%     \begin{column}{0.3\textwidth}
%%       \bi
%%     \item Age-3 fish from 2014 cohort expected to dominate catch in \Sexpr{end.yr}
%%       \bi
%%     %% \item \Sexpr{round(100*quantile(forecast.age.info$natsel.prop$X3, c(0.025, 0.5, 0.975)))}\% by numbers 
%%     %% \item \Sexpr{round(100*quantile(forecast.age.info$natselwt.prop$X3, c(0.025, 0.5, 0.975)))}\% by weight
%%       \ei
...
%%       <<main.age.comp.forecast, fig.height=3.5, fig.width=6>>=
      ...
      \bi
        ...
        \bi 
          \item correlated even more with 2014 recruitment 
              (\Sexpr{round(cor(base.model$mcmc$ForeCatch_2018,
                base.model$mcmc$Recr_2014),2)})
          \item Correlation with 2010 and 2008 is less than 0.5 
        \ei
      \ei
    \end{column}
