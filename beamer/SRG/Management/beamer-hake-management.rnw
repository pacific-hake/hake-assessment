...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Historical calculations}
% \subsection{Risk}
\frame{\frametitle{Compare probabilities in historical assessments to current estimates}
  ...
  \item 2019 assessment Table~i we had:
    %Example: in 2018 assessment we calculated probability that spawning biomass
    %would decline between 2018 and  2019, for a number of different values of the
  % 2018 catch (Table~i).
  \ei
\begin{table}[h]
...
\end{table}
\bi
  \item Now we \emph{know} the 2019 catch was 411,574~t, so row~d was close enough.
  \item How do the probabilities in row~d compare to estimating them using the
    current base model?
\ei
}
...
\bc
<<historical.prob.plot.1, fig.height=3.5, fig.width=7, out.width='0.9\\columnwidth'>>=
...
\frame{\frametitle{Summary of forecasting}
  \bi
    ...
    \item Uncertainty is large, but stock has \Sexpr{probs.curr.below.bforty}\%
    ...
    \item There is $>78\%$ chance that spawning biomass will decline from
      \Sexpr{assess.yr} to \Sexpr{assess.yr+1} for any catch $>180,000$~t.
    \item Maintaining a constant catch of 529,290~t (i.e., 2020 TAC) gives a 43\%
