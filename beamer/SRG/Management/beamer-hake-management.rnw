...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      \item No fishing results in a slight decline in spawning biomass from
        \Sexpr{assess.yr} to \Sexpr{assess.yr+1} before leveling off
      \item All other catch streams result in decreases in spawning biomass relative to \Sexpr{assess.yr}
...
\frame{\frametitle{Decision Tables}
  \bi
    ...
    \bi
      ...
      \item \textcolor{gray}{j. Median catch in 2020 is the same as
        fixed catch in 2019}
    \ei
...
\frame{\frametitle{Risk table definitions}
  ...
  (executive summary: Tables i and j; Figures j and k)
  \bi
...
\frame{\frametitle{Summary of forecasting}
  \bi
    ...
    \item Uncertainty is large, but stock is likely above $B_{40\%}$ and relative fishing intensity likely to have been below 100\%
    \item Forecasts strongly influenced by size of above average, but uncertain, 2014 and 2016 cohorts and
      very uncertain 2017 cohort
    \item There is more than a 80\% chance that spawning biomass will decline from 2020 to 2021 at any catch level,
      partly due to mortality on ageing 2010 and 2014 cohorts
    \item Maintaining a constant catch of 597,500~t (i.e., 2017 through 2019 TACs) results in a 37\%
      ...
      compared to last year's assessment (26\% and 40\%, respectively)
  \ei
}
