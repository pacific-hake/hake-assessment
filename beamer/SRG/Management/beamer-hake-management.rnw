...
% knitr::knit("beamer-hake-management.rnw")
% shell("latex beamer-hake-management"); shell("dvips beamer-hake-management");  shell("ps2pdf beamer-hake-management.ps")
...
%    \bi
%      % \item Lower 5\% quantile below $B_{40\%}$ in \Sexpr{end_yr} (35\%)
...
%\frame{\frametitle{Proportions of biomass for cohorts in \Sexpr{assess_yr} for default harvest rule catch}
...
%      labs(y = paste0("Proportion of Biomass in ", assess_yr,  " catch")) +
...
%%         \item \Sexpr{end_yr} default catch limit (bottom row)
%%         \bi
%%           \item similarly high correlation for 2010, 2014, and 2016 recruitment
%%             ( Sexpr{round(cor(base_model$mcmc[paste0("ForeCatch_",end_yr)],
%%                 base_model$mcmc$Recr_2010),2)},
%%              Sexpr{round(cor(base_model$mcmc[paste0("ForeCatch_",end_yr)],
%%                 base_model$mcmc$Recr_2014),2)},
%%             and
%%              Sexpr{round(cor(base_model$mcmc[paste0("ForeCatch_",end_yr)],
%%                 base_model$mcmc$Recr_2016),2)},
%%             respectively)
%%           \item increasing correlation with 2017 recruitment
%%             ( Sexpr{round(cor(base_model$mcmc[paste0("ForeCatch_",end_yr)],
%%               base_model$mcmc$Recr_2017),2)}, not shown)
%%         \ei
%%         \item Sexpr{end_yr+1} default catch limit (not shown) is more dependent on
...
%%     make.comparison.plot.mcmc(c(list(base_model), sens_models_5),
%%                               subplots = 2,
%%                               model.names = c(paste0("Base model ", end_yr), sens_model_names_5),
%%                               end_yr = end_yr+2,
%%                               xlims = c(2008,end_yr+2),
%%                               legendloc = "topleft")
%%     make.comparison.plot.mcmc(c(list(base_model), sens_models_5),
%%                               subplots = 8,
%%                               model.names = c(paste0("Base model ", end_yr), sens_model_names_5),
...
%%       \item Estimated numbers of age-1 hake in \Sexpr{assess_yr-1} (\Sexpr{assess_yr-2} year class) lower than **2016
%%       and **2014 year classes
%%       \bi
%%         %\item Survey and model estimates suggest decline in numbers of age-1 hake in 2019
%%          %  relative to 2017
%%         \item Numbers of age-1 hake caught in the \Sexpr{assess_yr-1} fishery **1--2\% of
...
%%       make.survey.age1.plot(base_model, age.1.index)
%%     @
%%   \end{column}
%% \end{columns}
%% }
...
# models.names <- map_chr(base_model$catch.levels[models.inds], ~{.x[[2]]})
make.forecast.depletion.comparison.plot(base_model,
                                        models.inds,
                                        models.names,
                                        start_yr = 2009,
                                        ...
                                        end_yr = forecast_yrs[length(forecast_yrs)],
                                        legend.loc = "topleft",
                                        ylimAdj = 1.018) # you can only scale ylim,
                                          # not set it unfortunately, in
                                          # SSplotComparisons, so change here
                                          # so this fig has same y-axis as next
...
par(mar = c(20, 4.1, 0, 1))
# models.inds <- c(1, 2, 3, catch.tac.ind, catch.default.policy.ind)
# models.names <- map_chr(sens_models_2[[1]]$catch.levels[models.inds], ~{.x[[2]]})
make.forecast.depletion.comparison.plot(sens_models_2[[1]],
                                        models.inds,
                                        models.names,
                                        start_yr = 2009,
                                        model.end_yr = end_yr,
...
% -----------------------------------------------------------------------------%
\subsection{Probabilities change with exclusion of age-1 index}
\begin{frame}
\frametitle{Probabilities change with exclusion of age-1 index}
\begin{table}[tbp]
\centering
\begin{tabular}{lrr}
\hline
Probability & Base model & Without age-1 index \\
\hline
With zero catch scenario: & & \\
P$(B_{\Sexpr{assess_yr+1}} < \Bforty)$ &
  \Sexpr{f( base_model$risks[[1]][1, paste0("Bratio_", end_yr + 1, "<0.40")])}\% &
  \Sexpr{f( sens_models_2[[1]]$risks[[1]][1,paste0("Bratio_", end_yr + 1, "<0.40")])}\% \\
P$(B_{\Sexpr{assess_yr+2}} < \Bforty)$ &
  \Sexpr{f( base_model$risks[[2]][1, paste0("Bratio_", end_yr + 2, "<0.40")])}\% &
  \Sexpr{f( sens_models_2[[1]]$risks[[2]][1, paste0("Bratio_", end_yr + 2, "<0.40")])}\% \\
With \Sexpr{f(catch_levels[[catch.actual.ind]][[1]][1])}~t catch scenario: & & \\
P$(B_{\Sexpr{assess_yr+1}} < \Bforty)$ &
  \Sexpr{f( base_model$risks[[1]][catch.actual.ind, paste0("Bratio_", end_yr + 1, "<0.40")])}\% &
  \Sexpr{f( sens_models_2[[1]]$risks[[1]][catch.actual.ind, paste0("Bratio_", end_yr + 1, "<0.40")])}\% \\
P$(B_{\Sexpr{assess_yr+2}} < \Bforty)$ &
  \Sexpr{f( base_model$risks[[2]][4, paste0("Bratio_", end_yr + 2, "<0.40")])}\% &
  \Sexpr{f( sens_models_2[[1]]$risks[[2]][catch.actual.ind, paste0("Bratio_", end_yr + 2, "<0.40")])}\% \\
\hline
\end{tabular}
\end{table}
...
make.historical.probs.retro.plot(sens_models_2[[1]],
                                 type = "bforty",
                                 select.retros = 1,
                                 make.one.figure = FALSE,
...
%%       make.comparison.plot(c(list(base_model), list(sens_models_2[[1]])),
%%                      subplots = 2,
%%                      model.names = c(base_model_name, sens_model_names_2[1]),
%%                      end_yr = end_yr,
%%                      legendloc = "topright")
%%     @
%%   \end{column}
...
%%   \begin{column}{0.25\textwidth}
%%     \bi
%%       \item median $\log(R_0)$ = \Sexpr{round(median(base_model$mcmc$SR_LN),2)}
...
%% %   \begin{column}{0.25\textwidth}
%% %     \bi
%% %       \item median $\log(R_0)$ = \Sexpr{round(median(sens_models_6$mcmc$SR_LN),2)}
...
%% %    make.mcmc.diag.plot(sens_models_6, key_posteriors[2], key_posteriors_titles[2])
%% %    @
%% %   \end{column}
%% % \end{columns}
%% % }
...
%%     make.decision.table.pres(base_model,
%%                              models.inds,
%%                              xcaption = NULL,
%%                              xlabel = "tab:es-decisions-spr",
%%                              font.size = 8,
...
%%     make.decision.table.pres(sens_models_2[[1]],
%%                              models.inds,
%%                              xcaption = NULL,
%%                              xlabel = "tab:es-decisions-spr",
%%                              font.size = 8,
...
%% %     make.decision.table.pres(sens_models_6,
%% %                              models.inds,
%% %                              xcaption = NULL,
%% %                              xlabel = "tab:es-decisions-spr",
%% %                              font.size = 8,
...
%% %<<decisions.biomass.base_model.with.4.yrs.forecasts, results = 'asis', echo = FALSE>>=
%% %  base_model.4 <-  load.models(model.dir, "2019.03.00_base_forecast4")
%% %  make.decision.table.pres(base_model.4,
%% %                           model.inds = 1:8,
%% %                           xcaption = NULL,
%% %                           font.size = 4,
%% %                           space.size = 5,
