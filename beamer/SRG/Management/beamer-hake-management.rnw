...
\begin{columns}
  \begin{column}{0.45\textwidth}
      ...
      \bi
        \item Default median HR TAC = \Sexpr{f((catch.targets %>% filter(Year == last.assess.yr))$`Default HCR TAC`)}~t
...
%--------------------------------------------------------------
\frame{\frametitle{Probabilities based on \Sexpr{end.yr} and \Sexpr{end.yr+1} catches}
...
\begin{columns}
  \begin{column}{0.35\textwidth}
    ...
    \item \Sexpr{fore.catch.prop$Age12}\% age-12 fish from the large \Sexpr{forecast_yrs[1] - 12} cohort
...
\bc
<<historical.prob.plot.retro, fig.height=3.2, fig.width=6, out.width='0.6\\columnwidth'>>=
make.historical.probs.retro.plot(base.model,
                                 select.retros = 1,
                                 make.one.figure = FALSE)
@
\ec
Current base model with data up to 2011 would have said low chance of decline
from 2012 to 2013 ...
}

\frame{\frametitle{Can also do retrospective calculations -- remove age-1 index}
\bc
<<historical.prob.plot.retro.age1, fig.height=3.2, fig.width=6, out.width='0.6\\columnwidth'>>=
...
@
\ec
... and even omitting age-1 index gives a lower prob of decline
from 2012 to 2013 than the 2012 assessment. Suggests other modelling
changes over the years have helped performance.
}
...
\section{Summary}
\subsection{}
\frame{\frametitle{Summary of projections}
  \bi
    ...
    \item But $>50\%$ chance that spawning biomass will decline from
      \Sexpr{assess.yr+1} to \Sexpr{assess.yr+2} for any catch $>0$~t.
