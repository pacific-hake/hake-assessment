...
\frame{\frametitle{Past management: Performance}
  \bc
  <<main.phase, out.width='0.7\\columnwidth'>>=
...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
    	\item Forecasted 2016 TAC is \Sexpr{fmt0(median(base.model$mcmc$ForeCatch_2016))} t
      \bi
        \item 2.5\% and 97.5\% quantiles: \Sexpr{fmt0(quantile(base.model$mcmc$ForeCatch_2016,probs=0.025))} and \Sexpr{fmt0(quantile(base.model$mcmc$ForeCatch_2016,probs=0.975))}
        \item $F_{SPR=40\%}$ harvest reate with no 40:10 adjustment
      \ei
    \ei
  \end{column}
