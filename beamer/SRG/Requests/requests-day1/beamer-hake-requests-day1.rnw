...
\frametitle{Sum of Recruitment Deviates}
  ...
  base_R0 <- f((median(base_model$mcmc[,"Recr_Virgin"])) / 1e3, 0)
  @
...
make.comparison.plot(c(list(base_model), list(mod200), list(mod201), list(mod202)),
                     subplots = 2,
                     model.names = c(base_model_name, req3.model.names),
                     end_yr = end_yr)
@
\end{frame}
% -----------------------------------------------------------------------------
