...
\section[Request 1]{Request 1 -- age-1 index inputs}
\subsection{}
\begin{frame}
\frametitle{Request 1 -- age-1 index inputs}
...
\frame{\frametitle{Request 1 -- age-1 index inputs (this is base model)}
\bc
...
\frame{\frametitle{Request 1 -- age-1 index inputs (this adds in 2001)}
\bc
<<mcmc.age1.2001.fit, fig.height=4, fig.width=8, out.width='0.95\\columnwidth'>>=
make.mcmc.survey.fit.plot(base_model,   # request_models[[1]][[2]]
                          start_yr = survey_start_yr,
                          end_yr = survey_end_yr,
                          probs = c(0.025, 0.975),
                          y.max = 10e6,
...
\frame{\frametitle{Request 1 -- age-1 index inputs (removes 1995, 1998, and 2001)}
\bc
<<mcmc.age1.start.2003.fit, fig.height=4, fig.width=8, out.width='0.95\\columnwidth'>>=
make.mcmc.survey.fit.plot(base_model,   # request_models[[1]][[3]]
                          start_yr = survey_start_yr,
                          end_yr = survey_end_yr,
                          probs = c(0.025, 0.975),
                          y.max = 10e6,
...
\begin{center}
\vspace{5pt}
<<age1.inputs.biomass.fig, fig.height=3.0, fig.width = 7.5>>=
plot_biomass(model_lst = list(base_model,
                              base_model,   # Delete this and next line,
                                        # uncomment next two
                              base_model),
#                              request_models[[1]][[2]],
#                              request_models[[1]][[3]]),
             model_names = c(base_model_name,
                             "dummy 1",   # As above, remove these and
                                        # uncomment next two
                             "dummy 2"),
#                                request_models_desc[[1]][1],
#                                request_models_desc[[1]][2]),
             x_expansion = 4,
             ylim = c(0, 5),
             y_breaks = seq(0, 20, 2),
             leg_font_size = 5,
...
\begin{center}
\vspace{-5pt}
<<age1.sensitivity.table, results='asis', echo=FALSE>>=
param_est_table(models = list(base_model,
                              base_model,    # Remove this and next and
                                             # uncomment next two when model is run
                              base_model),
#                              request_models[[1]][[2]],
#                              request_models[[1]][[3]]),
                model.names = c(base_model_name,
                                base_model_name,   # As above, remove these and
                                        # uncomment next two
                                base_model_name),
#                                request_models_desc[[1]][1],
#                                request_models_desc[[1]][2]),
