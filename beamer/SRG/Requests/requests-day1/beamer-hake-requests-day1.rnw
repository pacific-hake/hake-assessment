...
\bi
\item Run 1: Fix survey theta at median of the posterior and multiply the input survey sample size by 2.0
\item Run 2: Fix survey theta at median of the posterior and multiply the input survey sample size by 0.20
\item Run 3: Estimate survey theta when input survey sample size is multiplied by 2.0
\ei
%<<fig1, fig.height = 3.5>>=
...
\subsection{}
\begin{frame}
\frametitle{Closer Look at Survey DM Posterior}
<<fig1, fig.height = 3.5>>=
test<-exp(base.model$mcmc[,"ln(DM_theta)_2"])
test <- test / (1 + test)
hist(test,breaks=200,main="Posterior Distribution of Survey Theta",xlab="")
