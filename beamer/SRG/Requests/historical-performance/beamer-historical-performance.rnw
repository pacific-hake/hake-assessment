...
\frame{\frametitle{Probabilities of being below $\Bforty$ in subsequent year}
  \bi
  \item{Previous assessments have 'correctly' projected being above $\Bforty$ in
      all years, except 2012 (presumably due to unknown huge 2010 recruitment)}
  \item{Given large uncertainties, assessments always give some small probability of
      falling below $\Bforty$; current model says that's (in hindsight) even more unlikely}
  \item{Analysis gives some confidence in the current expectation of remainig
