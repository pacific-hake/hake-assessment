...
2.	Colour code weight at age matrix by column rather than global colouring
3.	Investigate changes in weights-at-age for 1987, when 1977 was expected to change but did not occur (compare 2019 to 2020).
4.	Calculate average of recruitment deviations over the main period (1970-2018); include both MLE and MCMC (current algorithm and NUTS). Attempting understand equilibrium recruitment assumption relative R0
5.	Redo ADNUTs plots thinning down to about 2,000 samples from samples available, and redo convergence diagnostic plots. Chris do function, Andy do figure.
6.
7.	Produce a time plot of effective N and input N to look for temporal patterns; Are recent samples upweighted relative to historical samples?  Aaron.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[Request 1 - squid age-1]{Squid plots for age-1 index}
\begin{frame}
\frametitle{Request 1}
Request 1 -- squid plot for base-case model with and without Age-1 index.
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[Request 6 - retros age-1]{Retrospectives using age-1 index}
\begin{frame}
\frametitle{Request 6}
Request 1 -- Redo recruitment retrospective analysis including Age-1 index, focusing on the 2014 and other strong year-classes.
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{}
\begin{frame}
\frametitle{Retrospectives -- base model}
\begin{center}
\vspace{-5pt}
...
\subsection{}
\begin{frame}
\frametitle{Retrospectives -- age-1 index}
\begin{center}
\vspace{-5pt}
<<retro.1.age1>>=
oldpar <- par()
...
%%%%%%%%%%%%%%%%%%%%%%%
\section[Request 4 - rec devs]{Mean recruitment deviations}
\subsection{}
\begin{frame}
\frametitle{Recruitment deviations over the main period (1970-2018) -- MLE vs MCMCs}
\bi
...
%%%%%%%%%%%%%%%%%%%%%%%
\section[Request 7 - effN]{Temporal sampling changes}
\frame{\frametitle{Request 7 - temporal patterns input N and effective N}
\bc
  \definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}
  \includegraphics[height=2.9in]{Figures/samplesizes.eps}
 \ec
}
%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%
\section[Request 5 - thin ADNUTS]{Convergence diagnostics for thinned ADNUTS run}
\subsection{}
\begin{frame}
...
\subsection{}
\begin{frame}
\frametitle{NUTS all 7,850 samples}
\begin{center}
\vspace{5pt}
<<adnuts.hists, fig.height=3.5>>=
oldpar <- par()
par(mar=c(4.1, 4.1, 3.1, 0.1))
...
\subsection{}
\begin{frame}
\frametitle{NUTS thinned to 2,000 samples - every 3rd and then the first 2,000}
\begin{center}
\vspace{5pt}
<<adnuts.hists.thin.2000, fig.height=3.5>>=
oldpar <- par()
par(mar=c(4.1, 4.1, 3.1, 0.1))
...
\subsection{}
\begin{frame}
\frametitle{NUTS thinned to 1,000 samples - every 7th and then the first 1,000}
\begin{center}
\vspace{5pt}
<<adnuts.hists.thin.1000, fig.height=3.5>>=
oldpar <- par()
par(mar=c(4.1, 4.1, 3.1, 0.1))
...
\begin{center}
\vspace{5pt}
<<adnuts.traces.2000, fig.height=3.5>>=
oldpar <- par()
par(mar=c(4.1, 4.1, 12, 0.1))
...
\begin{center}
\vspace{5pt}
<<adnuts.traces.1000, fig.height=3.5>>=
oldpar <- par()
par(mar=c(4.1, 4.1, 12, 0.1))
...
\section[Request 2]{Request 2 - change coloration in weight-at-age heatmap}
\subsection{}
\begin{frame}
\frametitle{Color code weight-at-age matrix by weight}
<<day1-2a, fig.height=3.5>>=
...
\begin{frame}
\frametitle{Color code weight-at-age matrix by age}
<<day1-2b, fig.height=3.5>>=
weight.at.age.heatmap(base.model,
                      proj.line.yr = base.model$endyr,
                      extrap.mask = weight_age_extrapolation_mask,
                      longterm.mean.ages = NULL,
...
\begin{frame}
\frametitle{Color code weight-at-age matrix by weight and age}
<<day1-2c, fig.height=3.5>>=
weight.at.age.heatmap(base.model,
                      proj.line.yr = base.model$endyr,
                      extrap.mask = weight_age_extrapolation_mask,
                      longterm.mean.ages = NULL,
...
\section[Request 3]{1998 Weight-at-age data}
\subsection{}
\begin{frame}
Investigate changes in weights-at-age for 1987, when 1977 was expected to change but did not occur (compare 2019 to 2020).

Canadian Joint-Venture information for that year was removed. Running the 2020 base model with the 2019 vs. 2020 weight-at-age makes almost no difference.
\end{frame}
