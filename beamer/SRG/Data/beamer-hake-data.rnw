  ...
  \bi
    ...
    \item Highest proportion-at-age by sector
      \bi
        ...
        \item Age-4 fish in U.S. Mothership and U.S. Shoreside sectors; and
...
\frame{\frametitle{U.S. at-sea catch depth distribution}
  ...
  usemeherea <- read.csv("../../../data/depth-us-atsea-fishing.csv")
  usemehereb <- read.csv("../../../data/depth-us-atsea-bottom.csv")
  makebox(
    usemeherea[usemeherea$year >= max(usemeherea$year) - 4, ], 
    main = "Fishing depth", col = "colour",
    labels = c("", ""))
  makebox(
    usemehereb[usemehereb$year >= max(usemehereb$year) - 4, ], 
...
\frame{\frametitle{Canada bottom depth distribution}
  <<depthca>>=
  par(mfrow = c(1, 2), mar=c(2.2, 3.2, 1.2, 0.2), oma = c(2, 1, 0, 0))
  usemeherea <- read.csv("../../../data/depth-can-freezer-trawlers.csv")
  usemehereb <- read.csv("../../../data/depth-can-shoreside.csv")
  localim <- c(0, max(c(usemeherea$upper95, usemehereb$upper95)) * 1.05)
  makebox(
    usemeherea[usemeherea$year >= max(usemeherea$year) - 4, ], 
