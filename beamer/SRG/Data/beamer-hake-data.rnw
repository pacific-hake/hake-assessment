...
\frame{\frametitle{Fleets and allocations}
  The \Sexpr{end.yr-1} TAC of \Sexpr{last.year.tac} was split between the U.S. and Canada as follows:
  \bi
    \item Canada -- \Sexpr{last.year.can.tac} (\Sexpr{paste0(can.allotment.percent, "\\%")})
      \bi
        \item Freezer trawlers
        \item Shoreside
        \item Joint venture
      \ei
    \item United States -- \Sexpr{last.year.us.tac} (\Sexpr{paste0(us.allotment.percent, "\\%")})
      \bi
        ...
        \item Tribal               -- \Sexpr{fmt0(last.year.us.tribal)} t
...
\section{Canadian quota and catch}
\frame{\frametitle{Canadian quota and catch}
  The \Sexpr{end.yr-1} Canadian TAC was \Sexpr{last.year.can.tac}
  \bi
    \item Catch by fleet
      \bi
        \item Freezer trawlers -- \Sexpr{last.year.can.freezer} t
        \item Shoreside        -- \Sexpr{last.year.can.shore} t
        \item Joint venture    -- 0 t (No allocation this year)
      \ei
    \item Total Canadian catch -- \Sexpr{last.year.can.landings} t
    \item \Sexpr{paste0(last.year.can.attained, "\\%")} of Canada's quota was caught.
  \ei
}
