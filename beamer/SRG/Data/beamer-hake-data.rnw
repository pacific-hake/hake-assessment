...
\frame{\frametitle{Year in review}
  ...
  @
  \bi
    \item \Sexpr{last_assess_yr} TAC, adjusted for carryovers, was \Sexpr{last.year.tac} t.
    \item Canadian and U.S. fisheries predominantly started in April and May, respectively.
    \item \Sexpr{scales::ordinal(tail(rank(base.model$catch$Obs*-1),1))} highest coast-wide catch.
    \item \Sexpr{pchangecatch}\% decrease in catch from last year.
    \item No Canadian Joint-Venture Fishery and no ages from Freezer-Trawlers in \Sexpr{last_assess_yr}.
...
%        \item Age-\Sexpr{max.freezer.trawler.age.prop.age} fish in Canadian freezer-trawler sector, and
      ...
      \ei
    \item Canadians fished shallower than normal to avoid rockfish species.
    \item Stakeholders reported good fishing in the U.S.A. and difficulty finding fish in Canada.
%    \item No new information from the survey in \Sexpr{last_assess_yr}.
    \item \Sexpr{last.survey.year} acoustic survey biomass estimate was \Sexpr{last.survey.year.biomass} million tons (\Sexpr{(type.convert(as.is = TRUE, last.factor.penult))*100}\% of \Sexpr{penult.survey.year}).
  \ei
}
...
\begin{columns}[t]
  ...
  \bi
      ...
      \ei
      \item Acoustic survey (\Sexpr{survey.start_yr}--\Sexpr{survey.end_yr})
      \bi
...
\frame{\frametitle{Allocations of \Sexpr{end_yr-1} TAC of \Sexpr{last.year.tac} t}
  \begin{columns}
...
\frame{\frametitle{Catches of \Sexpr{end_yr-1} TAC of \Sexpr{last.year.tac} t}
  \begin{columns}
    \begin{column}[t]{0.5\linewidth}
      \center{\textbf{\textcolor{blue}{Canada}}} \\
    \center{\textbf{TAC was \Sexpr{last.year.can.tac} t}} \\
\Sexpr{last.year.can.landings} t, \Sexpr{paste0(last.year.can.attained, "\\%")} of the Canadian TAC
        \bi
          \item Freezer Trawlers -- \Sexpr{last.year.can.freezer} t, \Sexpr{paste0(last.year.can.freezer.percent, "\\%")} of Canadian catch
          \item Shoreside        -- \Sexpr{last.year.can.shore} t, \Sexpr{paste0(last.year.can.shore.percent, "\\%")} of Canadian catch
          \item Joint Venture    -- \Sexpr{last.year.can.jv} t, \Sexpr{paste0(last.year.can.jv.percent, "\\%")} of Canadian catch
        \ei
    \end{column}
    \begin{column}[t]{0.5\linewidth}
      \center{\textbf{\textcolor{blue}{United States}}} \\
    ...
    \Sexpr{last.year.us.landings} t, \Sexpr{paste0(last.year.us.attained, "\\%")} of the U.S. TAC
          \bi
            ...
            \item Shore-Based         -- \Sexpr{f(last.year.us.shore.catch)} t, \Sexpr{paste0(last.year.us.shore.catch.percent, "\\%")} of shore-based allocation, includes \Sexpr{f(round(sum(subset(us.ti.catch.by.month,year == last_assess_yr)$catch), 0))} t of tribal catches
          \ei
    \end{column}
  \end{columns}
}
...
\begin{center}
  <<survey.age.comps, out.height='85%', out.width='85%'>>=
    make_age_comp_bubble_plot(base.model,
                              ...
                              subplot = 2,
                              xlim = c(survey.start_yr, survey.end_yr))
  @
\end{center}
}
...
\begin{center}
  <<age.comp.comparison.fishery.survey, out.height='90%', out.width='90%'>>=
    ...
    make.age.comp.compare.bubble.plot(base.model,
                                      start_yr = survey.start_yr,
                                      end_yr = survey.end_yr,
                                      show.key = TRUE,
...
\centering
  <<survey.age1, fig.height=4.5, fig.width=8, out.width='0.9\\columnwidth'>>=
    make.survey.age1.plot.data(age.1.index, log.scale = FALSE, yLim = c(0, 16))
  @
\end{figure}
}
...
<<main-weight-at-age-numbers, out.height='90%', out.width='90%'>>=
  midyear <- (base.model$endyr - start_yr_age_comps)/2 + start_yr_age_comps
