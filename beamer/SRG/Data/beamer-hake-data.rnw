...
\frame{\frametitle{Year in review}
  ...
  \bi
    ...
    \item Canadian and U.S. fisheries predominantly started in February and May, respectively.
    \item Third-highest coast-wide catch, \Sexpr{pchangecatch}\% decrease from last year.
    \item No Canadian Joint-Venture Fishery in \Sexpr{last.assess.yr}.
    \item Highest proportion-at-age by sector
      ...
      \ei
    \item Avoidance of bycatch was problematic for all sectors.
    \item Stakeholders reported generally good fishing in \Sexpr{last.assess.yr}.
    \item \Sexpr{last.survey.year} acoustic survey biomass estimate was \Sexpr{last.survey.year.biomass} million tons (\Sexpr{(type.convert(last.factor.penult)-1)*100}\% \Sexpr{ifelse(last.factor.penult > 1, "increase", "decline")} from \Sexpr{penult.survey.year}).
  \ei
}
...
\frame{\frametitle{Allocations of \Sexpr{end.yr-1} TAC of \Sexpr{last.year.tac} t}
  ...
  \end{column}
    ...
    \bi
      ...
      \item Tribal               --
        \bi
        \item \Sexpr{f(last.year.us.tribal)} t
        \item \Sexpr{f(last.year.us.tribal.quota.reallocated)} t reallocated on \Sexpr{last.year.us.tribal.reallocate.dates}
        \ei
      ...
      \bi
        \item Total U.S. catch -- \Sexpr{last.year.us.landings} t
          \bi
            ...
            \item Tribal (included in shore-based) -- 4,192 t
          \ei
...
\bi
  \item No weight-at-age data for the Freezer Trawlers in Canada due to lack of
    scales.
  \item U.S.~data collected prior to 1975 are irrelevant because they were collected from the Puget Sound stock.
  \item Newly included data
    \begin{itemize}
      \item 1977 -- 2092 samples from Poland Acoustic Survey.
      \item 2017 --  300 samples from Canadian Acoustic Survey.
...
\frame{\frametitle{Sample sizes of weight-at-age by year cont.}
\begin{center}
<<main-weight-at-age-numbers-cont, out.height='90%', out.width='90%'>>=
  weight.at.age.heatmap(base.model,
                        proj.line.yr = base.model$endyr,
                        extrap.mask = weight_age_extrapolation_mask,
                        longterm.mean.ages = NULL,
                        samplesize = TRUE,
                        print.years = c(start.yr.age.comps-0.5, midyear+0.5))
@
\end{center}
}
...
%    \begin{column}[t]{0.5\linewidth}
      \bi
        ...
        \item Data from 2018 and 2019 are expected to be analyzed in 2020.
      \ei
      \vspace{-3mm}
%    \end{column}
    %\begin{column}[t]{0.5\linewidth}
