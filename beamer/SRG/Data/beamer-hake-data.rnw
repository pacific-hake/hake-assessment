...
\frame{\frametitle{Year in review}
  \bi
    ...
    \item U.S. and Canadian fisheries predominantly started in May (some Shoreside catch in April).
    ...
    \item Highest coastwide catch in time-series, 33\% increase from last year.
...
\frame{\frametitle{Fecundity}
  \bi
    \item Fecundity-at-age is set to maturity-at-age $*$ mean weight-at-age in SS3.
