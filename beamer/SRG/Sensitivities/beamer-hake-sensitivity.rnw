...
\begin{enumerate}
  \item Sensitivities
        ...
        \item Alternative maximum age for fishery and survey selectivity
        \item Include age-1 index
        \item Removal of cohort-based error from the model
        \item Use of random walk Metropolis Hastings (rwMH) MCMC sampling
        \ei
  \item Retrospectives
        \bi
...
% -----------------------------------------------------------------------------%
\section{Weighting}
\subsection{Alternative methods for data weighting}
\begin{frame}
...
\begin{frame}
\frametitle{Including the age-1 index -- Age-1 index fit}
\begin{center}
\vspace{5pt}
...
% -----------------------------------------------------------------------------%
\subsection{Age-1 estimates comparisons}
\begin{frame}
...
% -----------------------------------------------------------------------------%
\begin{frame}
\frametitle{Include age-1 index -- age-1 estimates compared to age-1 index}
\begin{center}
\vspace{5pt}
...
\begin{frame}
\frametitle{Including age-1 index -- Spawning biomass}
\begin{center}
\vspace{5pt}
...
\begin{frame}
\frametitle{Including age-1 index -- Recruitment}
\begin{center}
...
% -----------------------------------------------------------------------------%
\subsection{$R_0$ and $B_0$ for age-1 index}
\begin{frame}
...
yrs <- c(2010, 2014, 2016, 2017, 2018)
...
% -----------------------------------------------------------------------------%
\section{rwMH}
\subsection{random walk Metropolis Hastings (rwMH) MCMC}
\begin{frame}
\frametitle{Run base model using rwMH algorithm}
\fontsize{11pt}{2}\selectfont
\bi
  \item Base model uses NUTS MCMC algorithm
  \item Compare NUTS with previous rwMH algorithm (last year's base model)
  \item NUTS:
    \bi
      \item took $\sim$4.5~hours to run
      \item resulted in 8,250 posterior samples
      \item uses a parallel algorithm, on multiple cores
      \item better explores the parameter space
      \item has the same base model structure, just a different numerical algorithm
  ...
  \item rwMH:
    \bi
      \item took $\sim$38~hours to run
      \item resulted in 2,041 posterior samples from a 24,000,000 chain length, after thinning and burning-in
      \item We also ran one to a chain length of 110,320,000 which resulted in 10,632 posterior samples, and took $\sim$8~days to complete. Results were essentially identical to the 24,000,000 run.
    \ei
\ei
\end{frame}
% -----------------------------------------------------------------------------%
...
\subsection{Spawning biomass}
\begin{frame}
\frametitle{rwMH algorithm -- Spawning biomass}
\begin{center}
\vspace{5pt}
<<biomass-base-alt-rwmh-sens-1>>=
oldpar <- par()
par(mar = c(7.1, 4.1, 0, 1))
...
\begin{frame}
\frametitle{rwMH algorithm -- Relative Spawning biomass}
\begin{center}
\vspace{5pt}
<<biomass-base-alt-rwmh-sens-2>>=
oldpar <- par()
par(mar = c(7.1, 4.1, 0, 1))
make.comparison.plot(c(list(base.model), list(sens.models.7)),
                     subplots = 3,
...
plot_mcmc_param_stats(base.model, effn_labels = TRUE, ylim_mult = 1.2)
par <- oldpar
@
\end{center}
\end{frame}
...
% ----------------------------------------------------------------------------%
\subsection{rwMH MCMC diagnostics}
\begin{frame}
\frametitle{rwMH MCMC diagnostics}
\begin{center}
\vspace{5pt}
<<age1.hists, fig.height=3.5>>=
oldpar <- par()
par(mar = c(4.1, 4.1, 3.1, 0.1))
plot_mcmc_param_stats(sens.models.7, effn_labels = TRUE, ylim_mult = 1.2)
par <- oldpar
@
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{Include age-1 index retrospective -- squid plot}
\begin{center}
\vspace{-5pt}
%, out.width='0.9\\columnwidth'>>=   Keep following values else
%   years get too close to the tendrils
...
\bi
  ...
  \item Addition of Age-1 index increases all recruitment estimates
  \item Retrospective squid plots show that:
  \bi
    \item 2010 cohort is the largest in the population
    \item 2014 cohort is the second largest
    \item 2016 cohort is the third largest
    \item No large cohorts since 2016
  \ei
\ei
\end{frame}
% -----------------------------------------------------------------------------%
...
\bi
  ...
  \bi
    ...
    \item Sensitivities: 21 * $\sim$4.5~hours = $\sim$94.5 hours
    \item Retrospectives: 10, total = $\sim$96 hours
    \item rwMH long chain run: $\sim$8 days
    \item Total with rwMH long chain: $\sim$16 days
    \item \textbf{Total without rwMH long chain run: 8 days}
  \ei
  \item \textbf{Total time to run all MCMC models last year: 44.5 hours}
\ei
\end{frame}
% -----------------------------------------------------------------------------%
