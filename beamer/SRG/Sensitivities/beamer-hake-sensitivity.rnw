...
\begin{enumerate}
  \item Sensitivities
        ...
        \item Using time-invariant ageing error instead of cohort-based ageing error
...
% -----------------------------------------------------------------------------%
\begin{frame}
\frametitle{Alternative deviations for selectivity -- Acoustic survey fit}
\begin{center}
\vspace{5pt}
...
\subsection{Spawning biomass}
\begin{frame}
\frametitle{Age Error -- Spawning biomass}
\begin{center}
\vspace{5pt}
<<age-error, fig.height=3.5>>=
oldpar <- par()
par(mar = c(7.1, 4.1, 0, 1))
...
\bi
  ...
  \bi
    ...
    \item Total without rwMH long chain run: 8 days
  \ei
  \item \textbf{Total time to run all MCMC models last year: 44.5 hours}
  \item \textbf{Total time to run all MCMC models this year: 16 days}
\ei
\end{frame}
% -----------------------------------------------------------------------------%
