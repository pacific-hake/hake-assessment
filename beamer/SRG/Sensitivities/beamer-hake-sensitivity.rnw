...
\fontsize{11pt}{2}\selectfont
\bi
  \item Base model uses NUTS MCMC algorithm (in SS using ADMB)
  \item Compare with previous (slower) rwMH algorithm
  \item Same base model structure, just different numerical algorithm
\ei
~\\
\bi
  \item NUTS resulted in 8,250 posterior samples
  \item 74,000,000 chain length, with 2,041 posterior samples
  \item Took $\sim$8~days to run, compared to 4.5~h for NUTS algorithm
\ei
\end{frame}
% -----------------------------------------------------------------------------%
...
\item rwMH appears to have no significant differences compared to NUTS approach
