...
\begin{enumerate}
  ...
  \item Sensitivities
        ...
        \item No reduction in ageing error for 2014
        \item Adjustment to last year for full bias correction in recruitment
        \item Standard deviations ($\phi$) for time-varying selectivity
...
\begin{center}
\vspace{-5pt}
<<baby.squid, fig.height=4.5, fig.width=8, out.width='0.9\\columnwidth'>>=
oldpar <- par(mar=c(5,4,1,1),no.readonly = TRUE)
#par(mar = c(4.5, 4.1, 1.5, 1.1))
...
\item All show large estimated increases in recent spawning biomass (driven by 2010 and 2014 cohorts)
\item ***
\end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%
