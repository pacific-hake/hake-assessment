...
\bi
  ...
  \item Each model and retrospective has \Sexpr{f(nrow(sens.models.1[[1]]$mcmc))} samples except base model with \Sexpr{f(num.mcmc.samples)}
...
oldpar <- par()
par(mar = c(7.1, 4.1, 0, 1))
make.comparison.plot(list(base_model, sens.models.2[[1]]),
                     subplots = 2,
