...
and m)
* Fix the catch in `r assess_yr` to estimate *B*~`r assess_yr + 1`~
...
* Fix the catches in `r assess_yr`, `r assess_yr + 1`, and `r assess_yr + 2`
  to estimate *B*~`assess_yr + 3`~
* P(*B*~`r assess_yr + 1`~ < *B*~y~): Probability that biomass in
  `r assess_yr + 1` is less a specified biomass such as *B*~`r assess_yr`~
   or `r b_40`
...
## Probabilities based on `r end_yr` catch
```{r forecast-risk-comparison-plot-year-1, fig.height = 3.8, fig.width = 7}
...
## Probabilities based on `r end_yr` and `r end_yr + 1` catches
```{r forecast-risk-comparison-plot-year-2, fig.height = 3.8, fig.width = 7}
...
```
## Probabilities based on `r end_yr`, `r end_yr + 1`, and `r end_yr + 2` catches
```{r forecast-risk-comparison-plot-year-3, fig.height = 3.8, fig.width = 7}
