...
<<loading, echo = FALSE, results = "asis">>=
...
cat(res, sep = '\n')
res <- knitr::knit_child("../../../doc/003-load-project-variables.rmd")
cat(res, sep = '\n')
@
...
\begin{columns}[t]
  ...
  \begin{column}{0.65\linewidth}
    <<data-overview-map, out.height = "3in", out.width = "1\\columnwidth">>=
...
%----------------------------------------------------------
%out.width = "0.9\\columnwidth">>=\section[Catches]{Catches}
\subsection{Catch history}
\frame[t]{\frametitle{Catches}
 \vspace{-4mm}
 \begin{figure}
 \centering
  <<catches, fig.height = 3.9>>=
...
\frame{\frametitle{Allocations of \Sexpr{end_yr-1} TAC of \Sexpr{last_yr_tac} t}
  \begin{columns}
    ...
    \bi
      ...
      \item Joint Venture                  -- \Sexpr{last_yr_can_tac_jv} t
    \ei
  \end{column}
    \begin{column}[t]{0.5\linewidth}
...
\Sexpr{last_yr_can_landings} t, \Sexpr{paste0(last_yr_can_attained, "\\%")} of the Canadian TAC
        \bi
          \item Freezer Trawlers -- \Sexpr{last_yr_can_freezer} t, \Sexpr{paste0(last_yr_can_freezer_percent, "\\%")} of Canadian catch
          \item Shoreside        -- \Sexpr{last_yr_can_shore} t, \Sexpr{paste0(last_yr_can_shore_percent, "\\%")} of Canadian catch
          \item Joint Venture    -- \Sexpr{last_yr_can_jv} t, \Sexpr{paste0(last_yr_can_jv_percent, "\\%")} of Canadian catch
        \ei
    \end{column}
    \begin{column}[t]{0.5\linewidth}
      \center{\textbf{\textcolor{blue}{United States}}} \\
...
\frame{\frametitle{Cumulative catch by month}
  <<cumulative-catches, fig.height = 3.5>>=
...
<<age-comp-comparison, fig.height = 3.9>>=
...
\begin{center}
<<survey-age-comps, fig.height = 3.5>>=
...
\begin{center}
<<age-comp-comparison-fishery-survey, fig.height = 3.5>>=
...
%----------------------------------------------------------
 ...
 \frame{\frametitle{Acoustic survey age-2$^+$ biomass estimates}
   \begin{figure}
\centering
<<survey-compare-extrap-noextrap, fig.height = 3.5>>=
...
\begin{center}
<<main-weight-at-age-numbers, fig.height = 3.9>>=
...
\frame{\frametitle{Maturity}
\begin{figure}
\centering
<<maturity.ogive.figure, fig.height = 3.5>>=
...
\begin{figure}
\centering
<<fecundity.figure, fig.height = 3.5>>=
