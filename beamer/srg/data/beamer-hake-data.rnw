...
\Sexpr{last.year.can.landings} t, \Sexpr{paste0(last.year.can.attained, "\\%")} of the Canadian TAC
    ...
    \Sexpr{last.year.us.landings} t, \Sexpr{paste0(last.year.us.attained, "\\%")} of the U.S. TAC
          \bi
            ...
            \item Shore-Based         -- \Sexpr{f(last.year.us.shore.catch)} t, \Sexpr{paste0(last.year.us.shore.catch.percent, "\\%")} of shore-based allocation, includes \Sexpr{f(round(sum(subset(us_ti_catch_by_month_df,year == last_assess_yr)$catch), 0))} t of tribal catches
          \ei
    \end{column}
  \end{columns}
}
