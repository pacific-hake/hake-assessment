...
\begin{frame}
  ...
  \bi
    \item Same general structure with minor changes to data weighting,
      selectivity parameterization and maturity and fecundity assumptions
    ...
    \item New 2017 age data from fishery and survey fit well by the model
    ...
    \item Assumptions about unfished equilibrium and reference points identified
      as focus of uncertainty and debate during review meeting
  \ei
\end{frame}
...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      \item 1999 cohort overpredicted when it was younger
...
\begin{center}
\vspace{-5pt}
<<retro.3, fig.height=5, fig.width=8, out.width='0.9\\columnwidth'>>=
...
\frame{\frametitle{Time-varying selectivity}
  \begin{columns}
    \begin{column}{0.3\textwidth}
      \bi
        \item More flexible time-varying selectivity retained from 2017 assessment
        \item Age compositions from 2017 fishery support the flexible added last year 
      \ei
    \end{column}
...
\subsection{}
\frame{\frametitle{Add section explaining equilibrium assumptions, alternative model, and additional risk tables}
}

\section{Cohort impacts}
\subsection{}
...
\begin{columns}
  ...
  \item Age-4 fish from 2014 cohort expected be largest proportion
    ...
    \ei
  \item Age-2 fish from 2016 cohort expected above average (still highly uncertain)
    \ei
  \end{column}
  \begin{column}{0.65\textwidth}
...
\frame{\frametitle{Estimated cumulative catch for large cohorts}
  \begin{columns}
    ...
    \begin{column}{0.48\textwidth}
      ...
      lines(ages2014, cohortCumSum2014/1e3, lwd=3, col=4)
