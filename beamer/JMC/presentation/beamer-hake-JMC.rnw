...
\frametitle{Overview of stock assessment (1/2)}
  \bi
    ...
    \item  Update fishery catch and age-composition data:
      ...
      \ei
    \item Retain survey biomass estimate and age data
    %\bi
    %  \item Years prior to \Sexpr{last_assess_yr}
    %  \item Add new data for \Sexpr{last_assess_yr}
    %\ei
    \item Update and add new \Sexpr{last_assess_yr} weight-at-age data
    \item Retain the relative age-1 index time series
...
\begin{frame}
  ...
  \bi
    ...
    \item The 2014, 2016, and 2020 cohorts make up a large fraction of catches in 2022
    \bi
      \item 2017 cohort estimated to be average
      \item 2018, 2019, and 2021 cohort estimated to be below average
    ...
    \item New \Sexpr{last_data_yr} age data from fishery are fit well by the model
...
%----------------------------------------------------------
\frame{\frametitle{Female spawning biomass}
  \bc
...
%----------------------------------------------------------
\frame{\frametitle{Relative fishing intensity and exploitation rate}
  \begin{columns}
...
\frame{\frametitle{Estimated cumulative catch for select cohorts}
  \begin{columns}
    ...
    \begin{column}{0.48\textwidth}
      ...
      plot_cohort_catch(base_model, arrow_size = 0.02)
      @
    \end{column}
  \end{columns}
}
...
\begin{columns}
  \begin{column}{0.4\textwidth}
      ...
      \bi
        ...
        \item JMC TAC = \Sexpr{last.year.tac} t
      \ei
  ...
  \begin{column}{0.38\textwidth}
    \bi
      \item No fishing results in flat \emph{median} relative
        ...
        leveling off
      \item 180,000 t catch shows similar biomass in \Sexpr{end_yr + 1} followed by slow decline
      \item All others shown result in declines in the median, but remain above $\BfortyB$ by the start of \Sexpr{assess_yr + 3}
...
<<decisions-biomass-table-zero, results='asis', echo=FALSE>>=
...
%----------------------------------------------------------
\frame{\frametitle{Relative spawning biomass, 2022 catch and TAC}
...
%----------------------------------------------------------
\frame{\frametitle{Relative spawning biomass, 10\% reduction}
...
%----------------------------------------------------------
\frame{\frametitle{Fishing intensity (selected streams)}
...
\frame{\frametitle{Probability table definitions}
  ...
  \bi
    ...
    \item P($B_{\Sexpr{assess_yr + 1}} < B_y$): Probability that biomass in \Sexpr{assess_yr + 1}
      ...
      such as $B_{\Sexpr{assess_yr}}$ or $\BfortyB$
    \item P(relative FI > 100\%): Probability that relative fishing intensity in the catch
      year is greater than $\Fforty$
  \ei
}
...
\frame{\frametitle{Probabilities based on \Sexpr{end_yr} catch}
\bc
<<forecast-risk-comparison-plot-year-1, fig.height=3.5, fig.width=7, out.width='0.9\\columnwidth'>>=
...
\frame{\frametitle{Probabilities based on \Sexpr{end_yr} and \Sexpr{end_yr+1} catch}
\bc
<<forecast-risk-comparison-plot-year-2, fig.height=3.5, fig.width=7, out.width='0.9\\columnwidth'>>=
plot_fore_compare(base_model,
                  forecast_yrs = forecast_yrs,
...
\frame{\frametitle{Probabilities based on \Sexpr{end_yr}, \Sexpr{end_yr+1},
    and \Sexpr{end_yr+2} catches}
\bc
<<forecast-risk-comparison-plot-year-3, fig.height=3.5, fig.width=7, out.width='0.9\\columnwidth'>>=
plot_fore_compare(base_model,
                  forecast_yrs = forecast_yrs,
...
\frame{\frametitle{Key metrics}
  \bi
    ...
    \item Projections influenced by size of above-average 2014 and 2016
      cohorts, and strongly influenced by the large but still highly
      uncertain 2020 cohort.
    \pause
    \item There is \Sexpr{prob_decl_yr1_zero_catch}\% chance that spawning biomass will decline from
      ...
      $>=$\Sexpr{prob_decl_yr1_other_catch}\% chance of decline for all other catch streams evaluated.
    \item There is \Sexpr{prob_decl_yr2_zero_catch}\% chance that spawning biomass will decline from
      ...
      $>=$\Sexpr{prob_decl_yr2_other_catch}\% chance of decline for all other catch streams evaluated.
...
\frame{\frametitle{Key points}
  \bi
       ...
       \item The median estimate of \Sexpr{end_yr} relative spawning biomass is
         ...
         \bi
       \item Large contributions expected from the highly uncertain 2020 cohort.
       \item Fishery age-composition data from 2022 season suggests very large 2020 cohort.
       \item Relative age-1 index in 2021 suggests above average 2020 cohort but not as large as fishery data alone indicate.
       \item Additional considerations include low catches in Canada, differences in U.S. fleet catch-at-age data,
             cohort not yet seen by acoustic survey, and ecosystem conditions.
         \ei
