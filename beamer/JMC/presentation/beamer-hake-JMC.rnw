...
\frame{\frametitle{Decision Tables}
  \bi
    ...
    \item Subset of catch streams included in this talk
    \bi
      ...
      \item Fishing Intensity = 100\% in each year conditioned on 
        fixed catch in previous year
      \item Median default Harvest Rule in each year conditioned on 
        fixed catch in previous year
