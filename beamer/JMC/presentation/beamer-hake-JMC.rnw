...
history.info <- read.csv(file.path(rootd.data, "catch-targets-biomass.csv")) # manual update to file in 2017
...
\frametitle{Overview of stock assessment}
  \bi
    ...
    \item No new acoustic survey biomass (but reanalysis of 1995 now complete)
    \item New 2016 fishery data
    ...
    \item 2014 cohort appears large as well
    \item Stock estimated to be at its highest biomass level since the 1980s
...
\frame{\frametitle{Survey index}
  ...
  make.survey.biomass.extrap.plot(survey.comparison, show=1:2)
  @
  \ec
}
...
\frame{\frametitle{Time-varying selectivity}
  \begin{columns}
    \begin{column}{0.3\textwidth}
      \bi
        \item More flexible time-varying selectivity chosen for 2017 assessment
        \item Age compositions of recent catches were inconsistent with assumptions about selectivity used in previous assessment
        \item Old assumptions produced implausible estimates of 2014 recruitment
      \ei
    \end{column}
    ...
    title(main="2017 assessment")
    @
    \end{column}
    \begin{column}{0.35\textwidth}
    <<mountains.phi0.03, fig.height=8, fig.width=5,  out.width='0.9\\columnwidth'>>=
    make.tv.selex.plot(sens.models.8[[1]], oma=rep(0,4), mcmc=FALSE) # model59, phi = 0.03 for all years
    title(main="Less flexible pattern used in 2016")
    @
    \end{column}
  \end{columns}
}
...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      \item Age-1 index appears to qualitatively follow estimates of recruitment
...
\begin{columns}
  ...
  \end{column}
  \begin{column}{0.59\textwidth}
    <<main.projected.catch.density, out.width='.95\\columnwidth'>>=
...
\begin{columns}
  \begin{column}{0.35\textwidth}
    \bi
  \item Dependent on assumptions about selectivity and weight
  \item Age-3 fish from 2014 cohort expected to be large proportion of \Sexpr{end.yr} catch
    \bi
...
\frame{\frametitle{Summary}
  \bi
    ...
    \item Uncertainty is large, but stock is likely well above $B_{40\%}$
    ...
    \medskip
    \item Last year we wrote ``The 2014 year class does not affect the 2016 catch decision much, but will be extremely influential next year''. The first part may have been less accurate but the second one still likely true with respect to the 2017 catch decision.
  \ei
}
