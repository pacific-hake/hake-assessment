...
\begin{frame}
  ...
  \bi
    ...
    \item New \Sexpr{last.data.yr} age data from fishery and survey is fit well by the model
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \bi
        ...
        \item Pre-1975 period (\Sexpr{start.yr}--\Sexpr{start.yr.age.comps - 1}) defined as average of
          \Sexpr{start.yr.age.comps}--\Sexpr{assess.yr - 1}
        \item Forecast period (\Sexpr{forecast_yrs[1]}--\Sexpr{forecast_yrs[length(forecast_yrs)]})
          defined as average of \Sexpr{assess.yr - 5}--\Sexpr{assess.yr - 1}
      \ei
  \vspace{5pt}
    \end{column}
    \begin{column}{0.5\textwidth}
...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      ...
      \item The 2010, 2014, and 2016 cohorts fit well
    \ei
  \end{column}
  \begin{column}{0.59\textwidth}
    \vspace{-5pt}
...
\begin{columns}
  \begin{column}{0.35\textwidth}
    \bi
      ...
      \item Estimated numbers of age-1 hake in 2019 (2018 year class) lower than 2016
      and 2014 year classes
      \bi
        \item Survey and model estimates suggest decline in numbers of age-1 hake in 2019
          relative to 2017
        \item Numbers of age-1 hake caught in the 2019 fishery 1--2\% of
...
\frame{\frametitle{Biomass and relative fishing intentisy}
  \bi
    \item Biomass has mostly remained above target ($\frac{B_{\Sexpr{assess.yr}}}{B_0} = \Sexpr{f(base.model$mcmccalcs$dmed[names(base.model$mcmccalcs$dmed) %in% end.yr], 2)}$)
    ...
    intensity is above its target at the end of \Sexpr{end.yr-1} is
    \Sexpr{probs.curr.rel.fish.intens.above.one}\%
    \item The joint probability of both of these things occuring is \Sexpr{joint.percent.prob.above.below}\%
  \ei
}
...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      ...
      \item Fishing intensity reaches fishing target with a catch of 410,000 t within three years time
      \item Using the default harvest rate, fishing intensity is slightly above the target (107--108\%; not shown)
    \ei
  \end{column}
  \begin{column}{0.59\textwidth}
...
\begin{columns}
  ...
  \item Age-6 (2014 cohort): \Sexpr{round(100*median(base.model$extra.mcmc$natsel.prop$`6`),0)}\% by numbers,
    \Sexpr{round(100*median(base.model$extra.mcmc$natselwt.prop$`6`),0)}\% by weight
  \item Age-4 (2016 cohort): \Sexpr{round(100*median(base.model$extra.mcmc$natsel.prop$`4`),0)}\% by numbers,
    \Sexpr{round(100*median(base.model$extra.mcmc$natselwt.prop$`4`),0)}\% by weight
  \item Age-10 (2010 cohort): \Sexpr{round(100*median(base.model$extra.mcmc$natsel.prop$`10`),0)}\% by numbers,
    \Sexpr{round(100*median(base.model$extra.mcmc$natselwt.prop$`10`),0)}\% by weight
  \item Age-3 (2017 cohort): \Sexpr{round(100*median(base.model$extra.mcmc$natsel.prop$`3`),0)}\% by numbers,
    \Sexpr{round(100*median(base.model$extra.mcmc$natselwt.prop$`3`),0)}\% by weight
    \ei
  \end{column}
  \begin{column}{0.65\textwidth}
...
%----------------------------------------------------------
\frame{\frametitle{Estimated changes in biomass at age for recent large cohorts (1)}
<<main.cohort.effects.table.1, results='asis', echo=FALSE>>=
cohort_table(base.model,
             end_yr = last.data.yr + 1,
...
%----------------------------------------------------------
\frame{\frametitle{Estimated changes in biomass at age for recent large cohorts (2)}
<<main.cohort.effects.table.2, results='asis', echo=FALSE>>=
cohort_table(base.model,
             end_yr = last.data.yr + 1,
             cohorts = c(2014, 2016, 2017),
             xcaption = NULL,
             font_size = 6,
             space_size = 7)
@
...
\frame{\frametitle{Summary}
  \bi
    ...
    \item Forecasts strongly influenced by size of above average, but uncertain, 2014 and 2016 cohorts and very uncertain 2017 cohort
    ...
    \item Maintaining a constant catch of 597,500~t (i.e., 2017 through 2019 TACs) results in a 37\%
      chance of falling below $B_{40\%}$ in 1 year (60\% in 2 years time), which is a considerable increase
      compared to last year's assessment (26\% and 40\%, respectively) (Table j)
  \ei
}
...
\frame{\frametitle{Looking Forward}
  \bi
    \item Highest catch levels on record over last three years (benefit of 2010 and 2014 year classes)
    \item Stock forcasted to decline by \Sexpr{assess.yr + 1} and \Sexpr{assess.yr + 2} under all future catch scenarios
    ...
    \item Stock moving towards 'target' biomass levels; however too rapid of a decline can result in 'overshooting'
       $\BfortyB$ and other benchmark reference points
  \ei
}
