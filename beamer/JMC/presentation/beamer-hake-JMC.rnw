...
\frame{\frametitle{Estimated cumulative catch for select cohorts}
  \begin{columns}
    ...
    %\item 1999 cohort (ages 1--\Sexpr{last_data_yr - 2010}): \Sexpr{f(max(cohortCumSum1999[ages1999 <= (last_data_yr - 2010 + 1)]))} t
    ...
    \item 2017 cohort (ages 1--\Sexpr{last_data_yr - 2017}): \Sexpr{f(max(cohortCumSum2017))} t
      \ei
    \end{column}
    \begin{column}{0.48\textwidth}
...
%      make.phase.plot(last_yr_base_model,
%                      start_yr = start_yr,
%                      end_yr = end_yr - 1,
%                      cex.lab = 0.5)
%  @
...
\frame{\frametitle{Total allowable catch}
    <<tac.vs.realized.catch.ts, fig.height=3.5>>==
      ...
      connect_vars_alpha = 0.2, curr_assess_biomass = base_model$catch.default.policy[1])
    @
}
...
%               (\Sexpr{round(cor(base_model$mcmc$ForeCatch_2017,base_model$mcmc$Recr_2014),2)})
