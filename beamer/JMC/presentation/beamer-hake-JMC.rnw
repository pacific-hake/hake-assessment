...
%\usepackage{tcolorbox} %% allows boxed text
...
models.inds <- c(1, 2, 3, 4, catch.tac.ind)
...
\begin{frame}
  ...
  \bi
    \item Stock assessment has same general structure as recent years,
      ...
      \bi
      \item data weighting,
      \item selectivity parameterization, and
    ...
    \item New 2017 age data from fishery and survey is fit well by the model.
...
\begin{frame}
  ...
  \bi
    ...
    \item Large 2010 cohort continues to make up large fraction of catches.
    \item 2014 cohort also estimated large, but not as big as expected last year.
    ...
    \item Assumptions about unfished equilibrium and reference points identified \\
      as a source of uncertainty and focus of debate during the review.
  \ei
\end{frame}
...
\begin{columns}
  \begin{column}{0.45\textwidth}
    \bi
      \item New maturity-at-age replaces length-based relationship
      ...
      \item Big thank-you to Melissa Head for countless hours in the lab
    \ei
  \end{column}
  \begin{column}{0.53\textwidth}
    \bc
    \vspace{-10pt}
    <<maturity.ogive.figure, fig.height=6.5, fig.width=6.5, out.width='0.95\\columnwidth'>>=
  ...
  \begin{columns}
    \begin{column}{0.3\textwidth}
      \bi
        ...
        \item Age compositions observed in 2017 support the flexibility added last year
    ...
    make.tv.selex.plot(base.model, oma=rep(0,4), mcmc=FALSE)
    @
    \end{column}
  \end{columns}
}
...
\begin{columns}
  \begin{column}{0.35\textwidth}
    \bi
      \item Age-1 index continues to track model predictions
        in spite of not being directly included in the model
      \item Age-1 hake in 2017 is above average but lower than 2015,
        supporting model estimate of 2016 as above average but lower than
        2014 and 2010 cohorts.
    \ei
  \end{column}
  \begin{column}{0.65\textwidth}
...
%%%%%%%%%%%%%%%%%%%%%%%
\frame{\frametitle{Background on reference points}
%\begin{tcolorbox}[colback=green!5,colframe=green!40!black,title=Treaty definitions]
\begin{block}{Treaty definitions}
\bi
\item ``F-40 percent'' means a fishing mortality rate that would reduce the
  biomass, calculated on a per recruit basis, to 40 percent of what it would
  have been in the absence of fishing mortality;
\item ``40/10 adjustment'' means an adjustment to the overall total allowable
  catch (TAC) that is triggered when the biomass falls below 40 percent of its
  unfished level. This adjustment reduces the TAC on a straight-line basis from
  the 40 percent level such that the TAC would equal zero when the stock is at
  10 percent of its unfished level;
\ei
\end{block}

\begin{block}{Interpretation by the JTC}
\bi
\item JTC has been calculating $\Bforty$ and $\Bten$ reference points in terms of
  ``relative spawning biomass'':
  the ratio of the beginning-of-the-year female spawning biomass to
  the unfished equilibrium female spawning biomass ($B_0$).
\ei
\end{block}
}
...
\frame{\frametitle{Calculation of $B_0$}
\begin{columns}
  \begin{column}{0.5\textwidth}
    \bi
      \item $B_0$ = $R_0  \times$ spawning biomass per recruit
      \item $R_0$ is an estimated quantity linked to average recruitment
      \item spawning biomass per recruit = per-recruit-numbers-at-age $\times$
        fecundity, summed across all ages
      \item fecundity = weight-at-age $\times$ maturity-at-age
    \ei
  \end{column}
  \begin{column}{0.5\textwidth}
  <<SPR.illustration.1, fig.height=7, fig.width=6, out.width='0.85\\columnwidth'>>=
    make.SPR.illustration(base.model, show.fished=FALSE, show.legend=FALSE)
  @
  \end{column}
\end{columns}
}
...
\frame{\frametitle{Calculation of Spawning Potential Ratio (SPR)}
\begin{columns}
  \begin{column}{0.5\textwidth}
    \bi
      \item $B_0$ = $R_0  \times$ spawning biomass per recruit
  ...
  \end{column}
  \begin{column}{0.5\textwidth}
  <<SPR.illustration.2, fig.height=7, fig.width=6, out.width='0.85\\columnwidth'>>=
    make.SPR.illustration(base.model, show.fished=TRUE, show.legend=TRUE)
  @
  \end{column}
\end{columns}
}
...
\frame{\frametitle{Weight-at-age}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \bi
  \vspace{-15pt}
    \item Matrix of weight-at-age is based on 205,248 samples
    \item Earliest samples (available at this time) \\ are from 1975
      \ei
  \vspace{5pt}
  <<weight.samples, fig.height=2.8, fig.width=6, out.width='0.95\\columnwidth'>>=
    # figure showing number of weight-at-age samples per year
    par(mar=c(3,3,.1,.1))
    plot(aggregate(dat$Age_yrs, by=list(dat$Year), FUN=function(x){length(x)}),
    type='h', lend=3, lwd=10, col='grey50', ylim=c(0,11000), yaxs='i',
    main="", ylab="", xlab="", axes=FALSE)
    mtext("Number of samples of weight-at-age (thousands)", side=3, line=-1,
    font=2)
    axis(2, at=seq(0,10000,2000), labels=seq(0,10,2), las=1)
  ...
  @
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{figure}
        \centering
        \includegraphics[width=2.4in, height=2.9in]{../../../doc/main-figures/EWAforDoc.eps}
      \end{figure}
    \end{column}
  \end{columns}
}
...
\subsection{}
\frame{\frametitle{Alternative model developed during the SRG meeting}
\bi
\item Spawning biomass in the base model uses
  new maturity-at-age $\times$ weight-at-age averaged across all years
\item Alternative assumption is that spawning biomass is
  related to time-varying weight-at-age
\item Additional assumption is needed for years outside the range of
  empirical observations: unfished equilibrium and forecast
\item SRG suggested alternative approach to base unfished equilibrium on average
  weight-at-age over 1975-1979 and forecast based on average over 2015-2017.
\ei
...
\frame{\frametitle{Spawning biomass comparison}
  \bc
  <<compare.spawning.biomass, fig.height=4.5, fig.width=8, out.width='0.9\\columnwidth'>>=
...
\frame{\frametitle{Relative spawning biomass comparison}
  \bc
  <<compare.relative.spawning.biomass, fig.height=4.5, fig.width=8, out.width='0.9\\columnwidth'>>=
  par(mar=c(4.1,4.1,.2,.2))
...
\frame{\frametitle{Recruitment comparison}
  \bc
  <<compare.recruitment, fig.height=4.5, fig.width=8, out.width='0.9\\columnwidth'>>=
  par(mar=c(4.1,4.1,.2,.2))
...
\frame{\frametitle{Fishing intensity comparison}
  \bc
  <<compare.fishing.intensity, fig.height=4.5, fig.width=8, out.width='0.9\\columnwidth'>>=
  par(mar=c(4.1,4.1,.2,.2))
...
\begin{frame}
\frametitle{Default harvest control rule catch from base and alternative models}
\begin{center}
Estimated median catch limit is \Sexpr{catch.limit.quantiles["median"]}~t in base model and
 \Sexpr{f(median(alt.base.model$mcmc$ForeCatch_2018))}~t in alternative model.

<<app.sens.alt.status.3, fig.height=3.5, fig.width=8, out.width='0.8\\columnwidth'>>=
  par(mar=c(4.1,3,.5,.5))
  base.and.alt.names <- c("Base model","Alternative time-varying fecundity run")
  make.comparison.plot.mcmc(list(base.model, alt.base.model),
                            model.names=base.and.alt.names, subplot=14,
                            densitynames="ForeCatch_2018",
                            densityxlab="Default harvest control rule catch for 2018")
@
\end{center}
\end{frame}
...
\frame{\frametitle{Relative Spawning Biomass from base and alternative}
\begin{columns}
  \begin{column}{0.5\textwidth}
    \bc
    Base model
  \vspace{-15pt}
    \ec
    <<decisions.biomass.table.copy, results='asis', echo=FALSE>>=
    make.decision.table.pres(base.model,
                             models.inds,
                             xcaption = NULL,
                             xlabel = "tab:copy.es-decisions-spr",
                             font.size = 8,
                             space.size = 10,
                             type = "biomass",
                             placement = "h")
  ...
  \vspace{-15pt}
    \ec
    <<alt.decisions.biomass.table, results='asis', echo=FALSE>>=
    make.decision.table.pres(alt.base.model,
                             models.inds,
                             xcaption = NULL,
                             xlabel = "tab:alt.es-decisions-spr",
                             font.size = 8,
                             space.size = 10,
                             type = "biomass",
                             placement = "h")
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{\frametitle{Risk based on fixed \Sexpr{end.yr} catch using alternative model}
...
\frame{\frametitle{Summary of alternative model}
  \bi
    \item Differences between models are primarily in assumption about
      how to calculate unfished equilibrium biomass ($B_0$)
    \item Both models estimate similar time series of
      recruitment, biomass, and fishing intensity
    \item Alternative model has much higher risk of falling below $\Bforty$
  \ei
}
...
\begin{columns}
  \begin{column}{0.35\textwidth}
    \bi
  \item Forecasts depend on assumptions about selectivity and weight-at-age
  \item Age-8 fish from 2010 cohort expected
    \bi
    ...
    \ei
  \item Age-4 fish from 2014 cohort expected as majority
    \bi
    ...
    \ei
  \item Age-2 fish from 2016 cohort could be significant
    \ei
  \end{column}
  \begin{column}{0.65\textwidth}
...
\frame{\frametitle{Estimated cumulative catch for large cohorts}
  \begin{columns}
    \begin{column}{0.48\textwidth}
      ...
      \bi
    \item 1999 cohort (ages 1--18): \Sexpr{f(max(cohortCumSum1999))} t
    \item 1999 cohort (ages 1--7): \Sexpr{f(max(cohortCumSum1999[ages1999 <= 7]))} t
    \item 2010 cohort (ages 1--7): \Sexpr{f(max(cohortCumSum2010))} t
    \item 2014 cohort (ages 1--3): \Sexpr{f(max(cohortCumSum2014))} t
      \ei
    \end{column}
    \begin{column}{0.48\textwidth}
...
\frame{\frametitle{Summary}
  \bi
    ...
    \item Under alternative model, the estimated median catch limit for \Sexpr{min(forecast.yrs)} \\
      is \Sexpr{f(median(alt.base.model$mcmc$ForeCatch_2018))}~t
      (with 95\% interval from \Sexpr{f(quantile(alt.base.model$mcmc$ForeCatch_2018, 0.025))}
      to \Sexpr{f(quantile(alt.base.model$mcmc$ForeCatch_2018, 0.975))}~t).
