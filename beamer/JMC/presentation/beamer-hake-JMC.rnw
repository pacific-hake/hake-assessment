...
%  \ei
  ...
  \item The JTC is looking into a 32-core processor (64-thread) server to run these in the future
...
\begin{enumerate}
  ...
  \item JTC looking for JMC support of computing request
  \item Would also hugely benefit the MSE process
\end{enumerate}
