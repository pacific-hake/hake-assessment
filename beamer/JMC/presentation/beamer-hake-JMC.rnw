...
%----------------------------------------------------------
\frame{\frametitle{Age-1 index: Base model}
\begin{columns}
...
%----------------------------------------------------------
\frame{\frametitle{Age-1 index: Model including Age-1 index}
\begin{columns}
  \begin{column}{0.35\textwidth}
    \bi
...
\frame{\frametitle{Biomass and relative fishing intentisy}
  \bi
    ...
    \item The joint probability of both of these things occurring is \Sexpr{joint.percent.prob.above.below}\%
  \ei
}
...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      \item No fishing results in a no change in spawning biomass from 2021 to 2022 before increasing slightly
      \item All other catch streams result in decreases in spawning biomass relative to 2021
...
\frame{\frametitle{Decision Tables}
  \bi
    ...
    \bi
      ...
      \item d. \Sexpr{catch_levels[[4]][[2]]}
      \item e. Constant catch (\Sexpr{catch_levels[[5]][[2]]})
      \item f. \Sexpr{catch_levels[[6]][[2]]}
      \item g. \Sexpr{catch_levels[[7]][[2]]}
      \item h. Fishing Intensity = 100\% in each year conditioned on fixed catch in previous year
      \item i. Median default Harvest Rule in each year conditioned on fixed catch in previous year
      \item j. Median catch in \Sexpr{assess.yr} is the same as fixed catch in \Sexpr{assess.yr - 1}
    \ei
...
\begin{columns}
  \begin{column}{0.33\textwidth}
    \bi
      ...
      \item Lower 5\% quantile below $B_{40\%}$ in \Sexpr{end.yr} (28\%), last year this was 35\%.
      \item Median (50\%) decreases from \Sexpr{end.yr} to \Sexpr{end.yr+1} and \Sexpr{end.yr+2}
        ...
        \item Median goes below $B_{40\%}$ in 2023 between d and e
        \ei
    \ei
  \end{column}
  \begin{column}{0.65\textwidth}
...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      ...
      \item Using the default harvest rate, fishing intensity is slightly above the target (105, 104, 103\%; not shown)
    \ei
  \end{column}
  \begin{column}{0.59\textwidth}
...
\frame{\frametitle{Summary}
  \bi
    \item Uncertainty is large, but stock is likely above $B_{40\%}$,
      and fishing intensity is likely to have been below 100\%
    ...
    \item Forecasts strongly influenced by size of above average 2010, 2014, and 2016 cohorts
    \item There is a \Sexpr{f(base.model$risks[[1]][4, 2])}\% chance that spawning biomass will decline from \Sexpr{assess.yr} to \Sexpr{assess.yr + 1} at a catch level of \Sexpr{f(base.model$risks[[1]][4, 1])}~t, partly due to mortality on ageing large cohorts (Figure j)
