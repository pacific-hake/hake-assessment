...
\usepackage{xcolor}            %% Used in Historical performance section
...
\frametitle{Overview of stock assessment (1/2)}
  \bi
    ...
    \item  Update fishery catch and age composition data:
      \bi
      \item Years prior to \Sexpr{last.assess.yr}
    ...
    \bi
      ...
      \item Add new data for \Sexpr{last.assess.yr}
    \ei
    \item Update and add new \Sexpr{last.assess.yr} weight-at-age data
...
\begin{frame}
  ...
  \bi
    ...
    \item New \Sexpr{last.data.yr} age data from fishery and survey are fit well by the model
...
\frame{\frametitle{Biomass and relative fishing intensity}
  \bi
    \item Biomass has remained above $\BfortyB$ since 2010; current estimate is $\frac{B_{\Sexpr{assess.yr}}}{B_0} = \Sexpr{f(base.model$mcmccalcs$dmed[names(base.model$mcmccalcs$dmed) %in% end.yr], 2)}$
...
\frame{\frametitle{Estimates of above average cohorts}
  ...
  \begin{table}
    \begin{tabular}{ccrcc} \hline
      ...
      \hline
      \Sexpr{assess.yr-1} assessment recruitment in 2020 & \Sexpr{rec_2020[[1]][1]} & \Sexpr{rec_2020[[1]][2]} & \Sexpr{rec_2020[[1]][3]} & \Sexpr{rec_2020[[1]][4]}\\
      \Sexpr{assess.yr} assessment recruitment in 2020 & \Sexpr{rec_2020[[2]][1]} & \Sexpr{rec_2020[[2]][2]} & \Sexpr{rec_2020[[2]][3]} & \Sexpr{rec_2020[[2]][4]}\\
      \hline
    \end{tabular}
  \end{table}
  ~\\
...
\begin{columns}
  \begin{column}{0.38\textwidth}
    \bi
      \item Using the defined \emph{F}\subscr{SPR=40\%} harvest rate with a 40:10 adjustment, the median projected \Sexpr{end.yr} TAC is
      \smallskip
  ...
  \begin{column}{0.65\textwidth}
    <<main.projected.catch.density, out.width='.99\\columnwidth'>>=
      make.forecast.catch.posterior.plot(base.model, fore.yr = end.yr, xmax=2500)
      @
  \end{column}
\end{columns}
}
...
\frame{\frametitle{Projection methods}
  \bi
    \item Projections estimate the biomass at start
      of \Sexpr{end.yr+1}, \Sexpr{end.yr+2}, and \Sexpr{end.yr+3}
    \bi
      \item Mean of fishery selectivity from \Sexpr{end.yr-5}--\Sexpr{end.yr-1} used in projections
      \item Mean of weight-at-age from \Sexpr{end.yr-5}--\Sexpr{end.yr-1} used in projections
...
%        slight increase
      \item No fishing results in increase in \emph{median} relative
        spawning biomass from \Sexpr{assess.yr} to \Sexpr{assess.yr+2} then
        levelling off
      \item All others shown result in eventual declines in the median
      \item Uncertainty is large and increases from \Sexpr{end.yr} to \Sexpr{end.yr+3}
    \ei
  \end{column}
...
\frame{\frametitle{Decision tables}
  \bi
    ...
    \ei
    \item Catch alternatives % text manually updated 2022,
                             %  used to use grey for ones not
                             %  explicitly showing in next table
                             %  but just showing first five now
    \bi
...
decision_table(base.model,
               ...
               space.size = 10,
               type = "biomass",
               forecast_inds = c(5, 8, 10))
@
}

\frame{\frametitle{Relative spawning biomass, 525,000 t catch}
\begin{table}[H]
\centering
\label{tab:**}
...
\frame{\frametitle{Probability table definitions}
  ...
  \bi
    \item Fix the catch in \Sexpr{assess.yr} to estimate $B_{\Sexpr{assess.yr + 1}}$
    \item Fix the catches in \Sexpr{assess.yr} and \Sexpr{assess.yr + 1} to estimate $B_{\Sexpr{assess.yr + 2}}$
    \item Fix the catches in \Sexpr{assess.yr}, \Sexpr{assess.yr + 1},
      and \Sexpr{assess.yr + 2} to estimate $B_{\Sexpr{assess.yr + 3}}$
    \item P($B_{\Sexpr{assess.yr + 1}} < B_y$): Probability that biomass in \Sexpr{assess.yr + 1}
      is less a specified biomass\\
      such as $B_{\Sexpr{assess.yr}}$ or $\Bforty$
    ...
    %\item P($C_{y+1} < C_y$): Probability that the calculated catch in projection
...
\frame{\frametitle{Probabilities based on \Sexpr{end.yr}, \Sexpr{end.yr+1},
    and \Sexpr{end.yr+2} catches}
...
%----------------------------------------------------------
\frame{\frametitle{Projected age compositions for \Sexpr{end.yr} fishery catch}
\begin{columns}
  \begin{column}{0.35\textwidth}
    Median proportions (by numbers) are:
    \bi
    % From assessment-section.rnw using more automated code; check
    %  that text to see which cohorts we highlighted
...
\begin{columns}
  ...
  \begin{column}{0.5\textwidth}
    ...
    yrs <- seq(2017,2022,1)
    Day <- c(17,26,23,25,23,18)
    maxD <- round((Day/max(Day))*100,0)
    plot(Day~yrs,ylim=c(0,30),xlim=c(2016.8,2022.2),ylab="Business Days to Complete Draft Assessment",xlab="Year",type="b")
...
%    Models
%    \bi
%    \item Freezer trawler ages 2008--present
%    \item All Canadian ages 2008--present
%    \item All Canadian ages 2018--present
%    \item All Canadian ages 2019--present
%    \item All Canadian ages 2020--present
%    \ei
%  \end{column}
%  \begin{column}{0.75\textwidth}
%<<nocanagesfigure,fig.height=4.5, fig.width=8, out.width='0.85\\columnwidth'>>=
%ghostmodelname <- file.path("..", "..", "..",
%  "models", "2022.01.50_ghostfleets"
%)
%newpaths <- dir("../../../models", pattern = "no[FC]", full.names = TRUE)[c(2, 16, 26:28) - 1]
%modlist <- mapply(r4ss::SS_output, c(ghostmodelname, newpaths),
%  MoreArgs = list(verbose = FALSE, printstats = FALSE),
%  SIMPLIFY = FALSE
%)
%make.comparison.plot(modlist, subplot = 1,
%  model.names = gsub("[0-9\\.]+_", "", c("base", basename(newpaths))),
%  plot_mcmc = FALSE, legend_pos = c(0.5, 0.9),
%  end.yr = 2021
%)
%@
  \includegraphics[width=\maxwidth, height=2.8in]{../../images/nocanagesfigure-1.eps}
%  \end{column}
%\end{columns}
}
