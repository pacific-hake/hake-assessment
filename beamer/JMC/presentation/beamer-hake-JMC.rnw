...
\newcommand<>{\rawalert}{\textcolor#1{beameralert}} % To avoid extra line in
                                        % tabular:
...
fi.names <- sapply(base.model$catch.levels, "[[", 2)[fi.inds]
@
...
%        slight increase
      \item All non-zero catch streams result in decreases in the median
...
\begin{columns}
  \begin{column}{0.35\textwidth}
    \bi
      ...
      \item Median (50\% column) gets to roughly 100\% fishing intensity in
        \bi
           \item 2023 for 380,000~t
           \item 2022 for 430,000~t
           \item 2021 for 529,290~t
         \ei
    \ei
  But large uncertainty, e.g., 25\% chance that 380,000~t is $>$100\% in 2021.
  \end{column}
  \begin{column}{0.65\textwidth}
...
%----------------------------------------------------------
\frame{\frametitle{Current decision table format}
\begin{columns}
  \begin{column}{0.33\textwidth}
    \bi
      \item Compare to alternative format (next slide) to hopefully improve understanding
      %\item Median (50\%) decreases from \Sexpr{end.yr} to \Sexpr{end.yr+1} and \Sexpr{end.yr+2}
      %  for all non-zero catch levels (continued decline of large 2010 and 2014 cohorts)
    \ei
  \end{column}
  \begin{column}{0.65\textwidth}
  <<decisions.biomass.table.2, results='asis', echo=FALSE>>=
    make.decision.table.pres(base.model,
                             models.inds,
                             xcaption = NULL,
                             xlabel = "tab:es-decisions-spr",
...
%----------------------------------------------------------
\frame{\frametitle{Alternative decision table format}
\begin{columns}
  \begin{column}{0.3\textwidth}
    \bi
      \item Alternative format to hopefully improve understanding
      \item \rawalert<1>{2021 biomass only once}
      \item \rawalert<2>{Consequence of a catch shown on that row}
      \item \rawalert<3>{To show three years requires \emph{three} years of projections}
      \item \rawalert<4>{Less cluttered -- removal of \% signs and 25\% and 75\% quantiles}
    \ei
  \end{column}
  \begin{column}{0.7\textwidth}
% Replacing the knitr with .tex to manually cut out some of the rows. Can adapt
% the function in 2022 if people like the new one.
...
% Thu Mar 04 10:18:16 2021
\begin{table}[h]
\centering
\label{tab:decisions-new}
\begingroup\fontsize{8}{10}\selectfont
\begin{tabularx}{\textwidth}{|ccc|cYYY|}
  % had to take out "Resulting" before "Relative spanwing biomass" to fit on slide
   \hline  &  &  & Model quantile & 5\% & 50\% & 95\% \\
   \cline{1-3}\multicolumn{3}{|c|}{Management Action} & Biomass year &
   \multicolumn{3}{c|}{\textbf{Relative spawning biomass}} \\ \cline{1-3} &
   Catch year & Catch (t) & \rawalert<1>{Start 2021} & \rawalert<1>{0.27} & \rawalert<1>{0.58} & \rawalert<1>{1.20} \\  \hline  a: & 2021 &       0 & Start 2022 & 0.28 & 0.58 & 1.23 \\
   & 2022 &       0 & Start 2023 & 0.29 & 0.61 & 1.46 \\
   & 2023 &       0 & Start 2024 & 0.33 & 0.69 & 1.67 \\
    \hline b: & \rawalert<2>{2021} & \rawalert<2>{180,000} & \rawalert<2>{Start 2022} & \rawalert<2>{0.24} & \rawalert<2>{0.53} & \rawalert<2>{1.17} \\
   & 2022 & 180,000 & Start 2023 & 0.20 & 0.51 & 1.36 \\
   & \rawalert<3>{2023} & \rawalert<3>{180,000} & \rawalert<3>{Start 2024} & \rawalert<3>{0.18} & \rawalert<3>{0.54} & \rawalert<3>{1.52} \\
    \hline d: & 2021 & 380,000 & Start 2022 & 0.19 & 0.47 & 1.12 \\
  2020 & 2022 & 380,000 & Start 2023 & 0.09 & 0.41 & 1.25 \\
  catch & 2023 & 380,000 & Start 2024 & 0.05 & 0.37 & 1.36 \\
    \hline e: & 2021 & 430,000 & Start 2022 & 0.17 & 0.46 & 1.10 \\
   & 2022 & 430,000 & Start 2023 & 0.08 & 0.39 & 1.22 \\
   & 2023 & 430,000 & Start 2024 & 0.05 & 0.33 & 1.32 \\
    \hline i: & 2021 & 554,678 & Start 2022 & 0.14 & 0.43 & 1.06 \\
  default & 2022 & 414,557 & Start 2023 & 0.07 & 0.36 & 1.19 \\
  HR & 2023 & 352,540 & Start 2024 & 0.04 & 0.33 & 1.32 \\
    \hline \end{tabularx}
\endgroup
\end{table}
...
\bi
  ...
  \item{An assessment's projection is almost always less definitive than from
      the current base model because the current model has more information}
...
\frame{\frametitle{Probabilities of being below $\BfortyB$ in subsequent year}
  ...
  \item{Given uncertainty in the most recent recruitments, assessments always give some small probability of
...
\begin{frame}
\frametitle{Computational times}
%\vspace{-2cm}
\bi
%  \item Total time to run all MCMC models this year:
%  \bi
%    \item Base model: $\sim$4.5 hours
%    \item Sensitivities: 21 $\times \sim$4.5~hours = $\sim$4 days
...
%  \ei
  \item \textbf{Total time to run all MCMC models last year: \textcolor{red}{45 hours} (1 processor)}
  \item \textbf{Total time to run all MCMC models this year: \textcolor{red}{12+ days}}
  % \item \textbf{12 days $\times$ 7 processor threads in parallel = 84 days}
  \item \textbf{Total time JTC has to conduct assessment after receiving all
      data = \textcolor{red}{31 days}}
  \item The JTC is looking into a 32-core processor (64-thread) server
  \item Estimated cost about CA\$30,000 (roughly one day of ship survey time)
%  \item Also looking at cloud computing options
\ei
\begin{enumerate}
  \item Timeline was very tight this year, though there no major hiccups and no new survey data
  \item Deadline for draft assessment was 8th February, really cannot go any earlier
  \item JTC looking for JMC support of computing power for assessment and MSE
\end{enumerate}
...
\frame{\frametitle{Key metrics}
  \bi
    ...
    \item With a \Sexpr{assess.yr} catch of \Sexpr{f(base.model$risks[[1]][4, 1])}~t (i.e., 2020 catch), there is a \Sexpr{f(base.model$risks[[1]][4, 2])}\% chance that spawning biomass will decline from \Sexpr{assess.yr} to \Sexpr{assess.yr + 1}, partly due to mortality on ageing large cohorts
    \item Maintaining a constant catch of 529,290~t (i.e., 2020 TAC) results in a 43\%
      chance of falling below $B_{40\%}$ in 1 year and 58\% in 2 years' time
    \item Maintaining a constant catch of 180,000~t results in a 26\%
      chance of falling below $B_{40\%}$ in 1 year and 31\% in 2 years' time
...
\frame{\frametitle{Key points}
  \bi
    ...
    \item While relative spawning biomass is currently above $\BfortyB$, it is approaching it at an increasing pace,
...
\begin{frame}
\frametitle{Questions/requests to the JMC from the JTC}
%\vspace{-2cm}
\begin{enumerate}
  \item Replace decision tables with new format next year?
  \item For consistency, then also add extra probability tables for the third
    projection year.
  \item We call a relative fishing intensity of 100\% a `target', but it is not
    really a target (something to be aimed for). Suggest in future calling it a
    `reference level'.
  \item Requesting support for computing request (high-level support may be
    helpful due to government computing rules).
\end{enumerate}
