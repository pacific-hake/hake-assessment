...
\frame{\frametitle{Biomass and relative fishing intentisy}
  \bi
    \item Biomass has remained above $\BfortyB$ since 2011; current estimate is $\frac{B_{\Sexpr{assess_yr}}}{B_0} = \Sexpr{f(base.model$mcmccalcs$dmed[names(base.model$mcmccalcs$dmed) %in% end_yr], 2)}$
...
\begin{columns}
  ...
  \begin{column}{0.65\textwidth}
    <<main.projected.catch.density, out.width='.99\\columnwidth'>>=
      make.forecast.catch.posterior.plot(base.model, fore.yr = end_yr, xmax=2000)
      @
  \end{column}
\end{columns}
}
...
\begin{columns}
  ...
  decision_table(srg_day2_req2_model,
                 xcaption = NULL,
...
<<compare.3.yr.models.biomass, fig.height=4, fig.width=8, out.width='0.95\\columnwidth'>>=
  ...
  make.comparison.plot(c(list(base.model),
                         ...
                         list(model_2019)),
                     subplots = 2,
                     model.names = c(base_model_name, last.yr.base_model_name, "2019 Base model"),
                     end_yr = c(end_yr, end_yr - 1, end_yr - 2),
                     lwd = 4,
                     legendloc = "topright",
                     plot_mcmc = TRUE)
@
...
\frame{\frametitle{Summary}
  \bi
    ...
    \item There is a \Sexpr{f(base.model$risks[[1]][4, 2])}\% chance that spawning biomass will decline from \Sexpr{assess_yr} to \Sexpr{assess_yr + 1} at a catch level of \Sexpr{f(base.model$risks[[1]][4, 1])}~t (i.e., 2020 catch), partly due to mortality on ageing large cohorts (Figure j)
